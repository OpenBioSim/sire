...
def fix_MolView(c, molview, props):
    ...
    for property in props.properties():
        p = property[0]
        prop = (
            property[1].replace("::", "_").replace("<", "_").replace(">", "_")
        )

        c.add_registration_code(
            'def( "_get_property_%s", &%s::property< %s >, %s)'
            % (prop, molview, p, return_const)
        )
        c.add_registration_code(
            'def( "_get_metadata_%s", get_Metadata_%s_function1, %s)'
            % (prop, prop, return_const)
        )
        c.add_registration_code(
            'def( "_get_metadata_%s", &get_Metadata_%s_function2, %s)'
            % (prop, prop, return_const)
        )

        c.add_declaration_code(
            """ const %s& get_Metadata_%s_function1(const %s &atom,
                                   const QString &metakey){ return atom.metadata< %s >(metakey); }"""
            % (p, prop, molview, p)
        )

        c.add_declaration_code(
            """ const %s& get_Metadata_%s_function2(const %s &atom,
...
def fix_MolEditorBase(c):
    ...
    c.add_registration_code(
        'def( "setMetadata", &set_Metadata_function1, %s)' % (return_self)
    )

    c.add_registration_code(
        'def( "setMetadata", &set_Metadata_function2, %s)' % (return_self)
    )
...
def fix_MolViewEditorBase(c, molview, props):
    ...
    for property in props.properties():
        ...
        c.add_registration_code(
            """def( \"_set_property_%s\",
                                   &%s::setProperty< %s >, %s )"""
            % (p_rep, molview, p, return_self)
        )

        c.add_registration_code(
            'def( "_set_metadata_%s", &set_Metadata_%s_function1, %s)'
            % (p_rep, prop, return_self)
        )

        c.add_registration_code(
            'def( "_set_metadata_%s", &set_Metadata_%s_function2, %s)'
            % (p_rep, prop, return_self)
        )

        c.add_declaration_code(
            """%s& set_Metadata_%s_function1(
                                  %s &molview,
...
def fix_Mover(c):
    ...
    c.decls("makeWhole").call_policies = call_policies.return_self()
