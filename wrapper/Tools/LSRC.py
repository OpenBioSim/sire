...
coul_power = Parameter(
    "coulomb power", 0, """The soft-core coulomb power parameter (integer)"""
)
...
def createStage(
    ...
    ligand_mol1 = ligand_mol1.move().align(
        ligand_mol0, AtomMatchInverter(mapper)
    )
                ...
                if getMinimumDistance(ligand_mol1, water_mol) > 1.5:
                    for j in range(0, water_mol.nResidues()):
                        water_mol = (
                            ...
                            .edit()
                            .setProperty(
                                PDB.parameters().pdbResidueName(), "FWT"
                            )
                            .commit()
                            .molecule()
                        )
    ...
    if use_oldff.val:
        ligand0_bound_nrg = (
            bound_ligand0_mobile.components().total(0)
            + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_f = (
            bound_ligand0_mobile.components().total(1)
            + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_b = (
            bound_ligand0_mobile.components().total(2)
            + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_next = (
            bound_ligand0_mobile.components().total(3)
            + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_prev = (
            bound_ligand0_mobile.components().total(4)
            + bound_ligand0_fixed_nrg
        )

        ligand1_bound_nrg = (
            bound_ligand1_mobile.components().total(0)
            + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_f = (
            bound_ligand1_mobile.components().total(1)
            + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_b = (
            bound_ligand1_mobile.components().total(2)
            + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_next = (
            bound_ligand1_mobile.components().total(3)
            + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_prev = (
            bound_ligand1_mobile.components().total(4)
            + bound_ligand1_fixed_nrg
        )

        ligand0_free_nrg = (
            free_ligand0_mobile.components().total(0) + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_f = (
            free_ligand0_mobile.components().total(1) + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_b = (
            free_ligand0_mobile.components().total(2) + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_next = (
            free_ligand0_mobile.components().total(3) + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_prev = (
            free_ligand0_mobile.components().total(4) + free_ligand0_fixed_nrg
        )

        ligand1_free_nrg = (
            free_ligand1_mobile.components().total(0) + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_f = (
            free_ligand1_mobile.components().total(1) + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_b = (
            free_ligand1_mobile.components().total(2) + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_next = (
            free_ligand1_mobile.components().total(3) + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_prev = (
            free_ligand1_mobile.components().total(4) + free_ligand1_fixed_nrg
        )
    else:
        ligand0_bound_nrg = (
            bound_ligand0_mobile.components().total() + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_f = (
            bound_ligand0_mobile.components().total("f")
            + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_b = (
            bound_ligand0_mobile.components().total("b")
            + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_next = (
            bound_ligand0_mobile.components().total("next")
            + bound_ligand0_fixed_nrg
        )

        ligand0_bound_nrg_prev = (
            bound_ligand0_mobile.components().total("prev")
            + bound_ligand0_fixed_nrg
        )

        ligand1_bound_nrg = (
            bound_ligand1_mobile.components().total() + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_f = (
            bound_ligand1_mobile.components().total("f")
            + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_b = (
            bound_ligand1_mobile.components().total("b")
            + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_next = (
            bound_ligand1_mobile.components().total("next")
            + bound_ligand1_fixed_nrg
        )

        ligand1_bound_nrg_prev = (
            bound_ligand1_mobile.components().total("prev")
            + bound_ligand1_fixed_nrg
        )

        ligand0_free_nrg = (
            free_ligand0_mobile.components().total() + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_f = (
            free_ligand0_mobile.components().total("f")
            + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_b = (
            free_ligand0_mobile.components().total("b")
            + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_next = (
            free_ligand0_mobile.components().total("next")
            + free_ligand0_fixed_nrg
        )

        ligand0_free_nrg_prev = (
            free_ligand0_mobile.components().total("prev")
            + free_ligand0_fixed_nrg
        )

        ligand1_free_nrg = (
            free_ligand1_mobile.components().total() + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_f = (
            free_ligand1_mobile.components().total("f")
            + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_b = (
            free_ligand1_mobile.components().total("b")
            + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_next = (
            free_ligand1_mobile.components().total("next")
            + free_ligand1_fixed_nrg
        )

        ligand1_free_nrg_prev = (
            free_ligand1_mobile.components().total("prev")
            + free_ligand1_fixed_nrg
        )
    ...
    bound_nrg = ((1 - lam) * ligand0_bound_nrg_sym) + (
        lam * ligand1_bound_nrg_sym
    )

    bound_nrg_f_sym = Symbol("E_{bound_{f}}")
    bound_nrg_f = ((1 - lam_f) * ligand0_bound_nrg_f_sym) + (
        lam_f * ligand1_bound_nrg_f_sym
    )

    bound_nrg_b_sym = Symbol("E_{bound_{b}}")
    bound_nrg_b = ((1 - lam_b) * ligand0_bound_nrg_b_sym) + (
        lam_b * ligand1_bound_nrg_b_sym
    )

    bound_nrg_next_sym = Symbol("E_{bound_{next}}")
    bound_nrg_next = ((1 - lam_next) * ligand0_bound_nrg_next_sym) + (
        lam_next * ligand1_bound_nrg_next_sym
    )

    bound_nrg_prev_sym = Symbol("E_{bound_{prev}}")
    bound_nrg_prev = ((1 - lam_prev) * ligand0_bound_nrg_prev_sym) + (
        lam_prev * ligand1_bound_nrg_prev_sym
    )

    free_nrg_sym = Symbol("E_{free}")
    free_nrg = (lam * ligand0_free_nrg_sym) + (
        (1 - lam) * ligand1_free_nrg_sym
    )

    free_nrg_f_sym = Symbol("E_{free_{f}}")
    free_nrg_f = (lam_f * ligand0_free_nrg_f_sym) + (
        (1 - lam_f) * ligand1_free_nrg_f_sym
    )

    free_nrg_b_sym = Symbol("E_{free_{b}}")
    free_nrg_b = (lam_b * ligand0_free_nrg_b_sym) + (
        (1 - lam_b) * ligand1_free_nrg_b_sym
    )

    free_nrg_next_sym = Symbol("E_{free_{next}}")
    free_nrg_next = (lam_next * ligand0_free_nrg_next_sym) + (
        (1 - lam_next) * ligand1_free_nrg_next_sym
    )

    free_nrg_prev_sym = Symbol("E_{free_{prev}}")
    free_nrg_prev = (lam_prev * ligand0_free_nrg_prev_sym) + (
        (1 - lam_prev) * ligand1_free_nrg_prev_sym
    )
    ...
    if use_oldff.val:
        ...
        system.add(
            PropertyConstraint(
                "alpha0", FFName("free:ligand1-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
                "alpha0",
                FFName("bound:ligand1-mobile"),
        ...
        system.add(
            PropertyConstraint(
                "alpha0", FFName("bound:ligand0-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
                "alpha0",
                FFName("free:ligand0-mobile"),
                alpha_scale.val * (1 - lam),
            )
        )
        ...
        system.add(
            PropertyConstraint(
                "alpha", FFName("free:ligand1-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha", FFName("bound:ligand0-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
    ...
    residue_nrgmon = FreeEnergyMonitor(
        monitor_prosol, ligand_group0, ligand_group1
    )
    ...
    if water_monitor_distance.val:
        ...
        boundwater_nrgmon = FreeEnergyMonitor(
            boundwater_assigner, ligand_group0, ligand_group1
        )
        freewater_nrgmon = FreeEnergyMonitor(
            freewater_assigner, ligand_group1, ligand_group0
        )
    ...
    if nequilmoves.val:
        ...
        if len(broken_keys) > 0:
            print(
                "ERROR - PROGRAM BUG : SOME OF THE FORCEFIELDS ARE LEAKING ENERGY"
            )
            for key in broken_keys:
                print(
                    "%s : %s versus %s"
                    % (key, broken_nrgs[key][0], broken_nrgs[key][1])
                )
            print(
                "ERROR: Please send 'broken_system.s3' in for debugging, along with everything printed above!"
            )
...
def mergeLSRC(sys0, ligand0_mol, sys1, ligand1_mol, watersys):
    if watersys:
        ...
        print(
            "Merging the two ligand complexes with a vacuum box to create the ligandswap system..."
        )
    ...
    if mcs_prematch.val:
        mapping = AtomMCSMatcher(
            AtomIDMatcher(mcs_prematch.val), mcs_timeout.val, False
        ).match(ligand0_mol, PropertyMap(), ligand1_mol, PropertyMap())
    else:
        mapping = AtomMCSMatcher(mcs_timeout.val, False).match(
            ligand0_mol, PropertyMap(), ligand1_mol, PropertyMap()
        )
    ...
    for key in mapping.keys():
        lines.append(
            "%s <=> %s"
            % (
                ligand0_mol.atom(key).name().value(),
    ...
    lsrc_sys.removeAllSharedProperties()
    ...
    if sys0.containsProperty("reflection center"):
        ...
        if watersys:
            if not watersys.containsProperty("reflection center"):
                print(
                    "Lack of reflection sphere in the water system when it exists in sys0 and sys1!"
                )
                sys.exit(-1)

        reflection_center0 = sys0.property("reflection center").toVector()[0]
        reflection_radius0 = float(
            str(sys0.property("reflection sphere radius"))
        )

        reflection_center1 = sys1.property("reflection center").toVector()[0]
        reflection_radius1 = float(
            str(sys1.property("reflection sphere radius"))
        )

        if watersys:
            reflection_center_wat = watersys.property(
                "reflection center"
            ).toVector()[0]
            reflection_radius_wat = float(
                str(watersys.property("reflection sphere radius"))
            )
        else:
        ...
        if (
            reflection_center0 != reflection_center1
            or reflection_center0 != reflection_center_wat
            or reflection_radius0 != reflection_radius1
            or reflection_radius0 != reflection_radius_wat
        ):
            print("Disagreement of the reflection sphere in the boxes!")
            print(
                "sys0: %s and %s    sys1: %s and %s   water: %s and %s"
                % (
                    reflection_center0,
                    reflection_radius0,
                    reflection_center1,
                    reflection_radius1,
                    reflection_center_wat,
                    reflection_radius_wat,
                )
            )

            sys.exit(-1)

        lsrc_sys.setProperty(
            "reflection center", AtomCoords(CoordGroup(1, reflection_center0))
        )
        lsrc_sys.setProperty(
            "reflection sphere radius", VariantProperty(reflection_radius0)
        )
    ...
    (lsrc_sys, lsrc_moves) = createStage(
        lsrc_sys,
        sys0,
        ligand0_mol,
        ligand1_mol,
        watersys,
        AtomResultMatcher(mapping),
        "lsrc",
    )
...
def loadWater():
    ...
    if vacuum_calc.val:
        print(
            "Using a vacuum box, so calculating a relative hydration free energy."
        )
        return None

    if os.path.exists(water_s3file.val):
        print(
            "Restoring from Sire Streamed Save file %s..." % water_s3file.val
        )
...
def loadSystem(topfile, crdfile, s3file, ligand_name):
    ...
    if os.path.exists(s3file):
        ...
        system = addFlexibility(
            system, Vector(0, 0, 0), reflection_radius.val, scheme
        )
...
def loadInput():
    ...
    if not have_sys:
        ...
        (sys0, ligand0_mol) = loadSystem(
            topfile0.val, crdfile0.val, s3file0.val, ligand_name0.val
        )
        (sys1, ligand1_mol) = loadSystem(
            topfile1.val, crdfile1.val, s3file1.val, ligand_name1.val
        )
        watersys = loadWater()

        (lsrc_system, lsrc_moves) = mergeLSRC(
            sys0, ligand0_mol, sys1, ligand1_mol, watersys
        )
...
def analyseLSRC(
    ...
    print("\n=============", file=FILE)
    print(
        "Relative free energy for iteration %d equals %s"
