...
dobonds = Parameter(
    "move bonds", True, """Whether or not to move the ligands bonds"""
)
doangles = Parameter(
    "move angles", True, """Whether or not to move the ligands angles"""
)
dodihedrals = Parameter(
    "move dihedrals", True, """Whether or not to move the ligands dihedrals"""
)
...
BASE_DIHEDRALH_FLEX = Parameter(
    "h dihedral flex", 30 * degrees, "Base dihedral rotation for H"
)
BASE_DIHEDRAL_FLEX = Parameter(
    "dihedral flex", 20 * degrees, "Base dihedral rotation"
)
BASE_ANGLE_FLEX = Parameter(
    "angle flex", 0.25 * degrees, "Base angle rotation"
)
BASE_BOND_FLEX = Parameter(
    "bond flex", 0.025 * angstroms, "Base bond stretch amount"
)
BASE_TRANSLATION = Parameter(
    "translation", 0.75 * angstroms, "Base translation delta amount"
)
BASE_ROTATION = Parameter("rotation", 30 * degrees, "Base rigid body rotation")
BASE_MAXVAR = Parameter(
    "maxvar", 10, "Maximum number of degrees of freedom to move at once"
)
BASE_MAXVAR_B = Parameter(
    "maxvar bonds", 2, "Maximum number of bonds to move at once"
)
BASE_MAXVAR_A = Parameter(
    "maxvar angles", 4, "Maximum number of angles to move at once"
)
BASE_MAXVAR_D = Parameter(
    "maxvar dihedrals", 4, "Maximum number of dihedrals to move at once"
)
...
def createSystemFrom(
    molecules, space, system_name, naming_scheme=NamingScheme()
):
    ...
    if water_model.val == "tip4p":
        ...
        print(
            "Forcing all water molecules to use the %s water model..."
            % water_model.val
        )
...
def createSystem(top_file, crd_file, naming_scheme=NamingScheme()):
    ...
    print(
        'Loading the molecules from the files "%s" and "%s"...'
        % (crd_file, top_file)
    )

    return createSystemFrom(
        system[MGIdx(0)], system.property("space"), top_file, naming_scheme
    )


def centerSystem(system, molecule):
    print(
        "Setting the origin of the system to the center of molecule %s (%s)..."
...
def addFlexibility(
    ...
    fixed_group = MoleculeGroup(
        naming_scheme.fixedMoleculesGroupName().value()
    )

    # create a group for the fixed residues that are bonded to the mobile residues
    boundary_group = MoleculeGroup(
        naming_scheme.boundaryMoleculesGroupName().value()
    )

    if reflection_center is None or reflection_radius is None:
        print(
            "No reflection radius or reflection molecule specified, so moving all "
        ...
        print(
            (
                "Only moving molecules/residues that are within a distance %s A "
                "of the point %s."
            )
            % (reflection_radius.value(), reflection_center)
        )

        system.setProperty(
            "reflection center", AtomCoords(CoordGroup(1, reflection_center))
        )
        system.setProperty(
            "reflection sphere radius",
            VariantProperty(reflection_radius.to(angstroms)),
        )
    ...
    if naming_scheme.proteinsGroupName() in system.mgNames():
        ...
        zmat_maker.loadTemplates(
            os.path.join(parameter_directory, "amber.zmatrices")
        )

        # now create the molecule groups that hold the flexible side chains and flexible backbone groups
        mobile_sc_group = MoleculeGroup(
            naming_scheme.mobileProteinSidechainsGroupName().value()
        )
        mobile_bb_group = MoleculeGroup(
            naming_scheme.mobileProteinBackbonesGroupName().value()
        )

        # the extra atoms moved as part of a backbone move
        hn_atoms = (
            AtomName("N", CaseInsensitive)
            * AtomName("H", CaseInsensitive)
            * AtomName("HN", CaseInsensitive)
            * AtomName("HN1", CaseInsensitive)
            * AtomName("HN2", CaseInsensitive)
            * AtomName("HN3", CaseInsensitive)
        )
        ...
        for molnum in protein_group.molNums():
            ...
            if reflection_radius:
                ...
                print(
                    "Looking for which residues are within the reflection sphere..."
                )
                for i in range(0, protein_mol.nResidues()):
                    res = protein_mol.residue(ResIdx(i))
                    distance = space.minimumDistance(
                        CoordGroup(1, reflection_center),
                        getCoordGroup(res.atoms()),
                    )
                    ...
                    if distance < reflection_radius.value():
                        ...
                        if has_backbone:
                            ...
                            mobile_bb_group.add(
                                PartialMolecule(protein_mol, atoms)
                            )
                        else:
                            print(
                                "Not moving backbone of %s as it doesn't contain atoms N or CA"
                                % protein_mol.residue(ResIdx(i))
                            )
                ...
                for i in range(0, protein_mol.nResidues()):
                    ...
                    if not res.number() in mobile_resnums:
                        ...
                        for bonded_res in connectivity.connectionsTo(
                            res.number()
                        ):
                            bonded_resnum = protein_mol.residue(
                                bonded_res
                            ).number()
        ...
        print(
            "The number of residues with flexible sidechains equals %s"
            % mobile_sc_group.nViews()
        )
        print(
            "The number of residues with flexible backbones equals %s"
            % mobile_bb_group.nViews()
        )
        print(
            "The number of boundary residues equals %s"
            % boundary_group.nViews()
        )
    ...
    if naming_scheme.solutesGroupName() in system.mgNames():
        ...
        mobile_solute_group = MoleculeGroup(
            naming_scheme.mobileSolutesGroupName().value()
        )
        ...
        for molnum in solute_group.molNums():
            ...
            if reflection_radius:
                move_solute = (
                    Vector.distance(
                        reflection_center, solute_mol.evaluate().centerOfMass()
                    )
                    < reflection_radius.value()
                )

            if move_solute:
                print(
                    "\nAuto-detecting the flexible degrees of freedom for solute %s"
                    % molnum
                )
                ...
                print(
                    "Not moving solute %s as it is outside the spherical solvent cutoff of the ligand."
                    % solute_mol
                )
        ...
        if mobile_solute_group.nMolecules() > 0:
            ...
            system.setProperty(
                ...
                VariantProperty(
                    avg_trans_delta / mobile_solute_group.nMolecules()
                ),
            )
            system.setProperty(
                "average solute rotation delta",
                VariantProperty(
                    avg_rot_delta / mobile_solute_group.nMolecules()
                ),
            )

        print(
            "\nNumber of mobile solute molecules equals %s"
            % mobile_solute_group.nMolecules()
        )
    ...
    if naming_scheme.solventsGroupName() in system.mgNames():
        ...
        mobile_solvent_group = MoleculeGroup(
            naming_scheme.mobileSolventsGroupName().value()
        )
        ...
        if reflection_radius:
            for molnum in solvent_group.molNums():
                ...
                if (
                    Vector.distance(
                        reflection_center,
                        solvent_mol.evaluate().centerOfMass(),
                    )
                    < reflection_radius.value()
                ):
        ...
        print(
            "\nNumber of mobile solvent molecules equals %s"
            % mobile_solvent_group.nMolecules()
        )
    ...
    print(
        "\nNumber of fixed (or partially fixed) molecules equals %s"
        % fixed_group.nMolecules()
    )
