...
use_oldff = Parameter(
    "use old forcefields",
    False,
    """For debugging, use the old forcefields rather than the
                                                       new forcefields""",
)
...
water_monitor_distance = Parameter(
...
ligand_reflection_radius = Parameter(
    "ligand reflection radius",
    1 * angstrom,
...
water_topfile = Parameter(
    "water topfile",
    "%s/waterbox.top" % wsrc_tools_dir,
    """Name of the topology file containing the water box.""",
)

water_crdfile = Parameter(
    "water crdfile",
    "%s/waterbox.crd" % wsrc_tools_dir,
    """Name of the coordinate file containing the coordinates of the water box.""",
)

water_s3file = Parameter(
    "water s3file",
    "waterbox.s3",
    """Name to use for the intermediate s3 file that will contain the
...
move_backbone = Parameter(
    "move backbone", True, """Whether or not to move the protein backbone."""
)

coul_power = Parameter(
    "coulomb power", 0, """The soft-core coulomb power parameter"""
)

shift_delta = Parameter(
    "shift delta", 1.2, """The soft-core shift delta parameter"""
)

soften_water = Parameter(
    "soften water",
    1.1,
...
uncharge_lambda_values = Parameter(
    "uncharge lambda values",
    [0.0, 0.1, 0.25, 0.45, 0.55, 0.75, 0.9, 1.0],
...
def getOverlapWaters(ligand, waters, radius=2 * angstrom):
    ...
    for molnum in waters.molNums():
        ...
        mindist = space.minimumDistance(
            CoordGroup([oxygen.property("coordinates")]), coords
        )
    ...
    for atom in atoms:
        ...
        try:
            ...
            try:
                ...
                print(
                    "Atom %s has neither a mass or element. Cannot add an identity point."
                    % str(atom)
                )
                have_point[str(atom.name().value())] = False
    ...
    for key in list(have_point_keys):
        if have_point[key]:
            ...
            if len(bonded) >= 3:
                ...
                if n >= 3:
                    print(
                        "Skipping %s as it is bonded to 3 or more points..."
                        % atom.name()
                    )
    ...
    if fast_sim.val:
        forcefield.setSwitchingFunction(
            HarmonicSwitchingFunction(
                7.5 * angstrom, 7.5 * angstrom, 7.5 * angstrom, 7.5 * angstrom
            )
        )
    else:
        forcefield.setSwitchingFunction(
...
def createWSRCMoves(system):
    ...
    if fast_sim.val:
        ...
        if mobile_swap.nViews() > 0:
            ...
            if use_reflect_volume.val:
                rb_moves.setReflectionVolume(
                    mobile_ligand[MolIdx(0)], reflect_volume_radius.val
                )
    ...
    if mobile_backbones.nViews() > 0 and move_backbone.val:
        ...
        bb_moves.setCenterOfRotation(
            GetCOGPoint(
                AtomName("CA", CaseInsensitive), AtomName("N", CaseInsensitive)
            )
        )
    ...
    if not use_fixed_ligand.val:
        if mobile_ligand.nViews() > 0:
            ...
            if use_rot_trans_ligand.val:
                if (
                    flex.translation().value() != 0
                    or flex.rotation().value() != 0
                ):
...
def mergeSystems(protein_system, water_system, ligand_mol):
    ...
    if protein_system.containsProperty("reflection center"):
        ...
        prot_reflection_radius = float(
            str(protein_system.property("reflection sphere radius"))
        )

        wat_reflection_center = water_system.property(
            "reflection center"
        ).toVector()[0]
        wat_reflection_radius = float(
            str(water_system.property("reflection sphere radius"))
        )

        if (
            prot_reflection_center != wat_reflection_center
            or prot_reflection_radius != wat_reflection_radius
        ):
            print(
                "Disagreement of the reflection sphere in the protein and water boxes!"
            )
            print(
                "Protein: %s and %s    Water: %s and %s"
                % (
                    prot_reflection_center,
                    prot_reflection_radius,
                    wat_reflection_center,
                    wat_reflection_radius,
                )
            )
        ...
        system.setProperty(
            "reflection center",
            AtomCoords(CoordGroup(1, prot_reflection_center)),
        )
        system.setProperty(
            "reflection sphere radius", VariantProperty(prot_reflection_radius)
        )
    ...
    if MGName("fixed_molecules") in water_system.mgNames():
        ...
        for molnum in mols.molNums():
            fixed_free_water_group.add(
                mols[molnum][0].molecule().edit().renumber().commit()
            )
    ...
    if MGName("mobile_solutes") in protein_system.mgNames():
        mobile_bound_solutes_group.add(
            protein_system[MGName("mobile_solutes")]
        )
    ...
    if MGName("mobile_solvents") in protein_system.mgNames():
        ...
        for molnum in mols.molNums():
            ...
            try:
                ...
                water_mol.edit().setProperty(
                    "coordinates", solvent_mol.property("coordinates")
                )

                for j in range(0, solvent_mol.nResidues()):
                    solvent_mol = (
                        solvent_mol.residue(ResIdx(j))
                        .edit()
        ...
        print(
            "The number of bound leg mobile solvent molecules is %d."
            % mobile_bound_solvents_group.nMolecules()
        )
        print(
            "The number of these which are compatible water molecules is %d."
            % mobile_bound_water_group.nMolecules()
        )
    ...
    mobile_bound_protein_sidechains_group = MoleculeGroup(
        "mobile_bound_protein_sidechains"
    )
    mobile_bound_protein_backbones_group = MoleculeGroup(
        "mobile_bound_protein_backbones"
    )

    if (
        MGName("protein_sidechains") in protein_system.mgNames()
        or MGName("protein_backbones") in protein_system.mgNames()
    ):
        ...
        for molnum in all_proteins.molNums():
            ...
            if protein_mol.selectedAll():
                ...
                if len(mobile_protein) > 0:
                    mobile_bound_proteins_group.add(
                        Molecule.join(mobile_protein)
                    )
                ...
                if protein_sidechains.contains(molnum):
                    ...
                    for sidechain in sidechains:
                        ...
                        if view.nSelected() > 0:
                            mobile_bound_protein_sidechains_group.add(
                ...
                if protein_backbones.contains(molnum):
                    ...
                    for backbone in backbones:
                        ...
                        if view.nSelected() > 0:
                            mobile_bound_protein_backbones_group.add(
                                PartialMolecule(new_protein_mol, view)
                            )

                print(
                    "Number of moved protein sidechain residues = %s"
                    % mobile_bound_protein_sidechains_group.nViews()
                )
                print(
                    "Number of moved protein backbone residues = %s"
                    % mobile_bound_protein_backbones_group.nViews()
                )
    ...
    if use_identity_constraint:
        ...
        if identity_atoms.val is None:
            ...
            for identity_atom in identity_atoms.val:
                identity_points.append(
                    ligand_mol.atom(AtomName(identity_atom))
                )
        ...
        mobile_free_water_group = IdentityConstraint.constrain(
            mobile_free_water_group, identity_points
        )
        ...
        for i in range(0, len(identity_points)):
            ...
            for j in range(0, swap_water_mol.nResidues()):
                swap_water_mol = (
                    swap_water_mol.residue(ResIdx(j))
                    .edit()
        ...
        print(
            "found %d molecules that are now part of the swap water cluster"
            % swap_water_group.nMolecules()
        )
        ...
        swap_waters = move.extract(
            mobile_free_water_group.molecules(), reflect_volume_buffer.val
        )

        print(
            "Swap water cluster based on the %d water molecules overlapping with the ligand."
            % swap_waters.nMolecules()
        )
        ...
        if n_equil_reflect.val:
            ...
            if n > 10:
                for i in range(1, 11):
                    move.move(equil_system, int(n / 10), False)
                    PDB().write(
                        equil_system.molecules(), "swapcluster%02d.pdb" % i
                    )
            else:
                move.move(equil_system, n, False)
    ...
    if use_oldff.val:
        ...
        if disable_grid:
            ...
            bound_swap_fixed = setGridProperties(
                bound_swap_fixed, 6 * angstrom
            )
    ...
    if use_oldff.val:
        ligand_bound_coul_nrg = (
            bound_ligand_mobile.components().coulomb(0)
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg = (
            bound_ligand_mobile.components().lj(0) + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_f = (
            bound_ligand_mobile.components().coulomb(1)
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_f = (
            bound_ligand_mobile.components().lj(1) + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_b = (
            bound_ligand_mobile.components().coulomb(2)
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_b = (
            bound_ligand_mobile.components().lj(2) + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_next = (
            bound_ligand_mobile.components().coulomb(3)
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_next = (
            bound_ligand_mobile.components().lj(3) + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_prev = (
            bound_ligand_mobile.components().coulomb(4)
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_prev = (
            bound_ligand_mobile.components().lj(4) + bound_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg = (
            free_ligand_mobile.components().coulomb(0)
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg = (
            free_ligand_mobile.components().lj(0) + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_f = (
            free_ligand_mobile.components().coulomb(1)
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_f = (
            free_ligand_mobile.components().lj(1) + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_b = (
            free_ligand_mobile.components().coulomb(2)
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_b = (
            free_ligand_mobile.components().lj(2) + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_next = (
            free_ligand_mobile.components().coulomb(3)
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_next = (
            free_ligand_mobile.components().lj(3) + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_prev = (
            free_ligand_mobile.components().coulomb(4)
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_prev = (
            free_ligand_mobile.components().lj(4) + free_ligand_fixed_lj_nrg
        )
    else:
        ligand_bound_coul_nrg = (
            bound_ligand_mobile.components().coulomb()
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg = (
            bound_ligand_mobile.components().lj() + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_f = (
            bound_ligand_mobile.components().coulomb("f")
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_f = (
            bound_ligand_mobile.components().lj("f")
            + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_b = (
            bound_ligand_mobile.components().coulomb("b")
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_b = (
            bound_ligand_mobile.components().lj("b")
            + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_next = (
            bound_ligand_mobile.components().coulomb("next")
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_next = (
            bound_ligand_mobile.components().lj("next")
            + bound_ligand_fixed_lj_nrg
        )

        ligand_bound_coul_nrg_prev = (
            bound_ligand_mobile.components().coulomb("prev")
            + bound_ligand_fixed_coul_nrg
        )

        ligand_bound_lj_nrg_prev = (
            bound_ligand_mobile.components().lj("prev")
            + bound_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg = (
            free_ligand_mobile.components().coulomb()
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg = (
            free_ligand_mobile.components().lj() + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_f = (
            free_ligand_mobile.components().coulomb("f")
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_f = (
            free_ligand_mobile.components().lj("f") + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_b = (
            free_ligand_mobile.components().coulomb("b")
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_b = (
            free_ligand_mobile.components().lj("b") + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_next = (
            free_ligand_mobile.components().coulomb("next")
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_next = (
            free_ligand_mobile.components().lj("next")
            + free_ligand_fixed_lj_nrg
        )

        ligand_free_coul_nrg_prev = (
            free_ligand_mobile.components().coulomb("prev")
            + free_ligand_fixed_coul_nrg
        )

        ligand_free_lj_nrg_prev = (
            free_ligand_mobile.components().lj("prev")
            + free_ligand_fixed_lj_nrg
        )
    ...
    if use_oldff.val:
        swap_int_nrg = (
            lam_coul_swap * S_scl * swap_interclj.components().coulomb(0)
        ) + (lam_lj_swap * swap_interclj.components().lj(0))

        swap_int_nrg_f = (
            lam_coul_swap_f * S_scl_f * swap_interclj.components().coulomb(1)
        ) + (lam_lj_swap_f * swap_interclj.components().lj(1))

        swap_int_nrg_b = (
            lam_coul_swap_b * S_scl_b * swap_interclj.components().coulomb(2)
        ) + (lam_lj_swap_b * swap_interclj.components().lj(2))

        swap_int_nrg_next = (
            lam_coul_swap_next
            * S_scl_next
            * swap_interclj.components().coulomb(3)
        ) + (lam_lj_swap_next * swap_interclj.components().lj(3))

        swap_int_nrg_prev = (
            lam_coul_swap_prev
            * S_scl_prev
            * swap_interclj.components().coulomb(4)
        ) + (lam_lj_swap_prev * swap_interclj.components().lj(4))
    else:
        swap_int_nrg = (
            lam_coul_swap * S_scl * swap_interclj.components().coulomb()
        ) + (lam_lj_swap * swap_interclj.components().lj())

        swap_int_nrg_f = (
            lam_coul_swap_f * S_scl_f * swap_interclj.components().coulomb("f")
        ) + (lam_lj_swap_f * swap_interclj.components().lj("f"))

        swap_int_nrg_b = (
            lam_coul_swap_b * S_scl_b * swap_interclj.components().coulomb("b")
        ) + (lam_lj_swap_b * swap_interclj.components().lj("b"))

        swap_int_nrg_next = (
            lam_coul_swap_next
            * S_scl_next
            * swap_interclj.components().coulomb("next")
        ) + (lam_lj_swap_next * swap_interclj.components().lj("next"))

        swap_int_nrg_prev = (
            lam_coul_swap_prev
            * S_scl_prev
            * swap_interclj.components().coulomb("prev")
    ...
    if use_oldff.val:
        swap_bound_coul_nrg = (
            bound_swap_mobile.components().coulomb(0)
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg = (
            bound_swap_mobile.components().lj(0) + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_f = (
            bound_swap_mobile.components().coulomb(1)
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_f = (
            bound_swap_mobile.components().lj(1) + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_b = (
            bound_swap_mobile.components().coulomb(2)
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_b = (
            bound_swap_mobile.components().lj(2) + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_next = (
            bound_swap_mobile.components().coulomb(3)
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_next = (
            bound_swap_mobile.components().lj(3) + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_prev = (
            bound_swap_mobile.components().coulomb(4)
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_prev = (
            bound_swap_mobile.components().lj(4) + bound_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg = (
            free_swap_mobile.components().coulomb(0) + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg = (
            free_swap_mobile.components().lj(0) + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_f = (
            free_swap_mobile.components().coulomb(1) + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_f = (
            free_swap_mobile.components().lj(1) + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_b = (
            free_swap_mobile.components().coulomb(2) + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_b = (
            free_swap_mobile.components().lj(2) + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_next = (
            free_swap_mobile.components().coulomb(3) + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_next = (
            free_swap_mobile.components().lj(3) + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_prev = (
            free_swap_mobile.components().coulomb(4) + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_prev = (
            free_swap_mobile.components().lj(4) + free_swap_fixed_lj_nrg
        )
    else:
        swap_bound_coul_nrg = (
            bound_swap_mobile.components().coulomb()
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg = (
            bound_swap_mobile.components().lj() + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_f = (
            bound_swap_mobile.components().coulomb("f")
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_f = (
            bound_swap_mobile.components().lj("f") + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_b = (
            bound_swap_mobile.components().coulomb("b")
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_b = (
            bound_swap_mobile.components().lj("b") + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_next = (
            bound_swap_mobile.components().coulomb("next")
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_next = (
            bound_swap_mobile.components().lj("next") + bound_swap_fixed_lj_nrg
        )

        swap_bound_coul_nrg_prev = (
            bound_swap_mobile.components().coulomb("prev")
            + bound_swap_fixed_coul_nrg
        )

        swap_bound_lj_nrg_prev = (
            bound_swap_mobile.components().lj("prev") + bound_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg = (
            free_swap_mobile.components().coulomb() + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg = (
            free_swap_mobile.components().lj() + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_f = (
            free_swap_mobile.components().coulomb("f")
            + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_f = (
            free_swap_mobile.components().lj("f") + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_b = (
            free_swap_mobile.components().coulomb("b")
            + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_b = (
            free_swap_mobile.components().lj("b") + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_next = (
            free_swap_mobile.components().coulomb("next")
            + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_next = (
            free_swap_mobile.components().lj("next") + free_swap_fixed_lj_nrg
        )

        swap_free_coul_nrg_prev = (
            free_swap_mobile.components().coulomb("prev")
            + free_swap_fixed_coul_nrg
        )

        swap_free_lj_nrg_prev = (
            free_swap_mobile.components().lj("prev") + free_swap_fixed_lj_nrg
        )
    ...
    system.setComponent(
        ligand_bound_coul_nrg_next_sym, ligand_bound_coul_nrg_next
    )
    system.setComponent(
        ligand_bound_coul_nrg_prev_sym, ligand_bound_coul_nrg_prev
    )
    ...
    system.setComponent(
        ligand_free_coul_nrg_next_sym, ligand_free_coul_nrg_next
    )
    system.setComponent(
        ligand_free_coul_nrg_prev_sym, ligand_free_coul_nrg_prev
    )
    ...
    bound_nrg = (
        (lam_coul_on * ligand_bound_coul_nrg_sym)
        + (lam_coul_off * swap_bound_coul_nrg_sym)
        + (lam_lj_on * ligand_bound_lj_nrg_sym)
        + (lam_lj_off * swap_bound_lj_nrg_sym)
    )

    bound_nrg_f_sym = Symbol("E_{bound_{f}}")
    bound_nrg_f = (
        (lam_coul_on_f * ligand_bound_coul_nrg_f_sym)
        + (lam_coul_off_f * swap_bound_coul_nrg_f_sym)
        + (lam_lj_on_f * ligand_bound_lj_nrg_f_sym)
        + (lam_lj_off_f * swap_bound_lj_nrg_f_sym)
    )

    bound_nrg_b_sym = Symbol("E_{bound_{b}}")
    bound_nrg_b = (
        (lam_coul_on_b * ligand_bound_coul_nrg_b_sym)
        + (lam_coul_off_b * swap_bound_coul_nrg_b_sym)
        + (lam_lj_on_b * ligand_bound_lj_nrg_b_sym)
        + (lam_lj_off_b * swap_bound_lj_nrg_b_sym)
    )

    bound_nrg_next_sym = Symbol("E_{bound_{next}}")
    bound_nrg_next = (
        (lam_coul_on_next * ligand_bound_coul_nrg_next_sym)
        + (lam_coul_off_next * swap_bound_coul_nrg_next_sym)
        + (lam_lj_on_next * ligand_bound_lj_nrg_next_sym)
        + (lam_lj_off_next * swap_bound_lj_nrg_next_sym)
    )

    bound_nrg_prev_sym = Symbol("E_{bound_{prev}}")
    bound_nrg_prev = (
        (lam_coul_on_prev * ligand_bound_coul_nrg_prev_sym)
        + (lam_coul_off_prev * swap_bound_coul_nrg_prev_sym)
        + (lam_lj_on_prev * ligand_bound_lj_nrg_prev_sym)
        + (lam_lj_off_prev * swap_bound_lj_nrg_prev_sym)
    )

    free_nrg_sym = Symbol("E_{free}")
    free_nrg = (
        (lam_coul_off * ligand_free_coul_nrg_sym)
        + (lam_coul_on * swap_free_coul_nrg_sym)
        + (lam_lj_off * ligand_free_lj_nrg_sym)
        + (lam_lj_on * swap_free_lj_nrg_sym)
    )

    free_nrg_f_sym = Symbol("E_{free_{f}}")
    free_nrg_f = (
        (lam_coul_off_f * ligand_free_coul_nrg_f_sym)
        + (lam_coul_on_f * swap_free_coul_nrg_f_sym)
        + (lam_lj_off_f * ligand_free_lj_nrg_f_sym)
        + (lam_lj_on_f * swap_free_lj_nrg_f_sym)
    )

    free_nrg_b_sym = Symbol("E_{free_{b}}")
    free_nrg_b = (
        (lam_coul_off_b * ligand_free_coul_nrg_b_sym)
        + (lam_coul_on_b * swap_free_coul_nrg_b_sym)
        + (lam_lj_off_b * ligand_free_lj_nrg_b_sym)
        + (lam_lj_on_b * swap_free_lj_nrg_b_sym)
    )

    free_nrg_next_sym = Symbol("E_{free_{next}}")
    free_nrg_next = (
        (lam_coul_off_next * ligand_free_coul_nrg_next_sym)
        + (lam_coul_on_next * swap_free_coul_nrg_next_sym)
        + (lam_lj_off_next * ligand_free_lj_nrg_next_sym)
        + (lam_lj_on_next * swap_free_lj_nrg_next_sym)
    )

    free_nrg_prev_sym = Symbol("E_{free_{prev}}")
    free_nrg_prev = (
        (lam_coul_off_prev * ligand_free_coul_nrg_prev_sym)
        + (lam_coul_on_prev * swap_free_coul_nrg_prev_sym)
        + (lam_lj_off_prev * ligand_free_lj_nrg_prev_sym)
        + (lam_lj_on_prev * swap_free_lj_nrg_prev_sym)
    )
    ...
    box_nrg_next = (
        bound_bound_nrg_sym
        + free_free_nrg_sym
        + ligand_int_nrg_next_sym
        + swap_int_nrg_next_sym
    )
    ...
    box_nrg_prev = (
        bound_bound_nrg_sym
        + free_free_nrg_sym
        + ligand_int_nrg_prev_sym
        + swap_int_nrg_prev_sym
    )
    ...
    system.setComponent(
        Symbol("delta_nrg^{F}"), (total_nrg_f_sym - total_nrg_sym)
    )
    system.setComponent(
        Symbol("delta_nrg^{B}"), (total_nrg_b_sym - total_nrg_sym)
    )
    system.setComponent(
        Symbol("delta_nrg^{next}"), (total_nrg_next_sym - total_nrg_sym)
    )
    system.setComponent(
        Symbol("delta_nrg^{prev}"), (total_nrg_prev_sym - total_nrg_sym)
    )

    system.setComponent(
        Symbol("delta_bound_nrg^{F}"), (bound_nrg_f_sym - bound_nrg_sym)
    )
    system.setComponent(
        Symbol("delta_bound_nrg^{B}"), (bound_nrg_b_sym - bound_nrg_sym)
    )
    system.setComponent(
        Symbol("delta_free_nrg^{F}"), (free_nrg_f_sym - free_nrg_sym)
    )
    system.setComponent(
        Symbol("delta_free_nrg^{B}"), (free_nrg_b_sym - free_nrg_sym)
    )
    ...
    if dlam > 1 or dlam < 0.0000001:
        print(
            "WARNING: Weird value of delta_lambda (%s). Setting it to 0.001"
            % dlam
        )
        dlam = 0.001
    ...
    if uncharge_ligand.val:
        ...
        system.add(
            ComponentConstraint(
                lam_coul_on,
                vfunc(1 + (4 * lam * (v - 1.0)), (v / 0.75) * (1.0 - lam)),
            )
        )
        system.add(
            ComponentConstraint(
        ...
        system.add(
            ComponentConstraint(
                lam_coul_on_f,
                vfunc(1 + (4 * lam_f * (v - 1.0)), (v / 0.75) * (1.0 - lam_f)),
            )
        )
        system.add(
            ComponentConstraint(
        ...
        system.add(
            ComponentConstraint(
                lam_coul_on_b,
                vfunc(1 + (4 * lam_b * (v - 1.0)), (v / 0.75) * (1.0 - lam_b)),
            )
        )
        system.add(
            ComponentConstraint(
        ...
        system.add(
            ComponentConstraint(
                lam_coul_off_next,
                vfunc(
                    1 + (4 * (1 - lam_next) * (v - 1.0)),
                    (v / 0.75) * (1.0 - (1 - lam_next)),
                ),
            )
        )
        system.add(
        ...
        system.add(
            ComponentConstraint(
                lam_coul_off_prev,
                vfunc(
                    1 + (4 * (1 - lam_prev) * (v - 1.0)),
                    (v / 0.75) * (1.0 - (1 - lam_prev)),
                ),
            )
        )
        ...
        system.add(
            ComponentConstraint(
                lam_lj_on,
                Max(Min(2 * ((1 - lam) - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 1 to 0 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_off, Max(Min(2 * (lam - 0.25), lam_lj_max), lam_lj_min)
            )
        )  # scale from 0 to 1 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_on_f,
                Max(Min(2 * ((1 - lam_f) - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 1 to 0 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_off_f,
                Max(Min(2 * (lam_f - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 0 to 1 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_on_b,
                Max(Min(2 * ((1 - lam_b) - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 1 to 0 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_off_b,
                Max(Min(2 * (lam_b - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 0 to 1 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_on_next,
                Max(Min(2 * ((1 - lam_next) - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 1 to 0 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_off_next,
                Max(Min(2 * (lam_next - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 0 to 1 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_on_prev,
                Max(Min(2 * ((1 - lam_prev) - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 1 to 0 from lam=0.25 to 0.75
        system.add(
            ComponentConstraint(
                lam_lj_off_prev,
                Max(Min(2 * (lam_prev - 0.25), lam_lj_max), lam_lj_min),
            )
        )  # scale from 0 to 1 from lam=0.25 to 0.75
        ...
        system.add(
            ComponentConstraint(
                ...
                Max(
                    v,
                    Max(
                        4.0 * (v - 1.0) * lam_next + 1,
                        4.0 * (v - 1.0) * (1 - lam_next) + 1,
                    ),
                ),
            )
        )
        system.add(
            ComponentConstraint(
                ...
                Max(
                    v,
                    Max(
                        4.0 * (v - 1.0) * lam_prev + 1,
                        4.0 * (v - 1.0) * (1 - lam_prev) + 1,
                    ),
                ),
            )
        )
        ...
        system.add(
            ComponentConstraint(
                lam_lj_on, Max(Min(1 - lam, lam_lj_max), lam_lj_min)
            )
        )
        system.add(
            ComponentConstraint(
                lam_lj_off, Max(Min(lam, lam_lj_max), lam_lj_min)
            )
        )
        ...
        system.add(
            ComponentConstraint(
                lam_lj_on_f, Max(Min(1 - lam_f, lam_lj_max), lam_lj_min)
            )
        )
        system.add(
            ComponentConstraint(
                lam_lj_off_f, Max(Min(lam_f, lam_lj_max), lam_lj_min)
            )
        )
        ...
        system.add(
            ComponentConstraint(
                lam_lj_on_b, Max(Min(1 - lam_b, lam_lj_max), lam_lj_min)
            )
        )
        system.add(
            ComponentConstraint(
                lam_lj_off_b, Max(Min(lam_b, lam_lj_max), lam_lj_min)
            )
        )
        ...
        system.add(
            ComponentConstraint(
                lam_lj_on_next, Max(Min(1 - lam_next, lam_lj_max), lam_lj_min)
            )
        )
        system.add(
            ComponentConstraint(
                lam_lj_off_next, Max(Min(lam_next, lam_lj_max), lam_lj_min)
            )
        )
        ...
        system.add(
            ComponentConstraint(
                lam_lj_on_prev, Max(Min(1 - lam_prev, lam_lj_max), lam_lj_min)
            )
        )
        system.add(
            ComponentConstraint(
                lam_lj_off_prev, Max(Min(lam_prev, lam_lj_max), lam_lj_min)
            )
        )
    ...
    if use_oldff.val:
        system.add(
            PropertyConstraint(
                "alpha0", FFName("free:swap-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
                "alpha0",
                FFName("bound:swap-mobile"),
                alpha_scale.val * (1 - lam),
            )
        )

        system.add(
            PropertyConstraint(
                "alpha0", FFName("bound:ligand-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
                "alpha0",
                FFName("free:ligand-mobile"),
                alpha_scale.val * (1 - lam),
            )
        )
        ...
        system.add(
            PropertyConstraint(
                "alpha1", FFName("free:swap-mobile"), alpha_scale.val * lam_f
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha1",
                FFName("bound:ligand-mobile"),
                alpha_scale.val * lam_f,
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha2", FFName("free:swap-mobile"), alpha_scale.val * lam_b
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha3",
                FFName("free:swap-mobile"),
                alpha_scale.val * lam_next,
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha", FFName("free:swap-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha", FFName("bound:ligand-mobile"), alpha_scale.val * lam
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha[f]", FFName("free:swap-mobile"), alpha_scale.val * lam_f
            )
        )
        system.add(
            PropertyConstraint(
        ...
        system.add(
            PropertyConstraint(
                "alpha[b]", FFName("free:swap-mobile"), alpha_scale.val * lam_b
            )
        )
        system.add(
            PropertyConstraint(
    ...
    if use_oldff.val:
        system.add(
            PropertyConstraint(
                "alpha0",
                FFName("swap:interclj"),
                s_scl * (1 - 2 * Abs(lam - 0.5)),
            )
        )
        system.add(
            PropertyConstraint(
    ...
    if use_identity_constraint:
        ...
        print(
            "The number of candidates for the swap water equals: %d"
            % mobile_water.nMolecules()
        )

        system.add(mobile_water)
        system.add(
            IdentityConstraint(
                identity_points, mobile_water, {"space": Cartesian()}
            )
        )
    ...
    system.add(
        ...
        MonitorComponent(
            Symbol("delta_nrg^{F}"),
            FreeEnergyAverage(temperature.val, dlam * binwidth.val),
        ),
    )
    ...
    system.add(
        ...
        MonitorComponent(
            Symbol("delta_nrg^{B}"),
            FreeEnergyAverage(temperature.val, dlam * binwidth.val, False),
        ),
    )
    ...
    system.add(
        "delta_g^{next}",
        MonitorComponent(
            Symbol("delta_nrg^{next}"),
            BennettsFreeEnergyAverage(
                0 * kcal_per_mol, temperature.val, 0.1 * binwidth.val
            ),
        ),
    )
    ...
    system.add(
        "delta_g^{prev}",
        MonitorComponent(
            Symbol("delta_nrg^{prev}"),
            BennettsFreeEnergyAverage(
                0 * kcal_per_mol, temperature.val, 0.1 * binwidth.val, False
            ),
        ),
    )
    ...
    residue_nrgmon = FreeEnergyMonitor(
        monitor_prosol, ligand_group, mobile_swap
    )
    ...
    if water_monitor_distance.val:
        ...
        for molnum in mobile_bound_water_group.molNums():
            ...
            if getMinimumDistance(ligand_mol, water_mol) < dist:
                ...
                boundwater_points.append(
                    VectorPoint(water_mol.evaluate().center())
                )
        ...
        boundwater_assigner = IDAssigner(
            boundwater_points, mobile_bound_water_group, {"space": Cartesian()}
        )

        boundwater_assigner.update(system)

        freewater_assigner = IDAssigner(
            freewater_points, mobile_free_water_group, {"space": Cartesian()}
        )

        freewater_assigner.update(system)

        boundwater_nrgmon = FreeEnergyMonitor(
            boundwater_assigner, ligand_group, mobile_swap
        )
        freewater_nrgmon = FreeEnergyMonitor(
            freewater_assigner, mobile_swap, ligand_group
        )
...
def loadWSRC():
    ...
    if os.path.exists(protein_s3file.val):
        ...
        print(
            "Loading from Amber files %s / %s..."
            % (protein_topfile.val, protein_crdfile.val)
        )
        ...
        proteinsys = createSystem(
            protein_topfile.val, protein_crdfile.val, proteinsys_scheme
        )
    ...
    if os.path.exists(water_s3file.val):
        ...
        print(
            "Loading from Amber files %s / %s..."
            % (water_topfile.val, water_crdfile.val)
        )
        watersys = createSystem(water_topfile.val, water_crdfile.val)
        watersys = addFlexibility(
            watersys, Vector(0, 0, 0), reflection_radius.val
        )
    ...
    for i in range(0, comps.nComponents()):
        ...
        print(
            ...
            % (
                ...
                -comps.integrateLJ(i).values()[-1].y(),
            ),
            file=FILE,
        )
...
def printFreeEnergy(total, bound, free, FILE):
    ...
    print(
        "%s   %s   %s"
        % (
            -total.integrate().values()[-1].y(),
            -bound.integrate().values()[-1].y(),
            -free.integrate().values()[-1].y(),
        ),
        file=FILE,
    )
...
def analyseWSRC(
    ...
    for i in range(0, nreplicas):
        ...
        if write_pdbs:
            if save_all_pdbs.val or (i == 0) or (i == nreplicas - 1):
                ...
                PDB().write(
                    bound_leg,
                    "%s/bound_mobile_%000006d_%.5f.pdb"
                    % (outdir.val, iteration, lamval),
                )
                PDB().write(
                    free_leg,
                    "%s/free_mobile_%000006d_%.5f.pdb"
                    % (outdir.val, iteration, lamval),
                )

        dg_f[lamval] = monitors[MonitorName("delta_g^{F}")][-1].accumulator()
        dg_b[lamval] = monitors[MonitorName("delta_g^{B}")][-1].accumulator()
        dg_next[lamval] = monitors[MonitorName("delta_g^{next}")][
            -1
        ].accumulator()
        dg_prev[lamval] = monitors[MonitorName("delta_g^{prev}")][
            -1
        ].accumulator()
        dg_bound_f[lamval] = monitors[MonitorName("delta_bound_g^{F}")][
            -1
        ].accumulator()
        dg_bound_b[lamval] = monitors[MonitorName("delta_bound_g^{B}")][
            -1
        ].accumulator()
        dg_free_f[lamval] = monitors[MonitorName("delta_free_g^{F}")][
            -1
        ].accumulator()
        dg_free_b[lamval] = monitors[MonitorName("delta_free_g^{B}")][
            -1
        ].accumulator()
    ...
    printFreeEnergy(
        ti_freenrgs[iteration],
        bound_freenrgs[iteration],
        free_freenrgs[iteration],
        FILE,
    )
    ...
    print("\n=============", file=FILE)
    print(
        "Binding free energy for iteration %d equals %s"
...
def run():
    ...
    if os.path.exists(restart_file.val):
        ...
        if os.path.exists(sysmoves_file.val):
            ...
            if nequilmoves.val:
                print(
                    "Equilibrating the system (number of moves: %d)..."
                    % nequilmoves.val
                )
                wsrc_system = wsrc_moves.move(
                    wsrc_system, nequilmoves.val, False
                )
    ...
    if not os.path.exists(freenrgs_file):
        ...
        [bennetts_freenrgs, fep_freenrgs, ti_freenrgs] = Sire.Stream.load(
            freenrgs_file
        )
    ...
    if not os.path.exists(freenrg_components_file):
        ...
        [
            ...
            bound_water_freenrgs,
            free_water_freenrgs,
        ] = Sire.Stream.load(freenrg_components_file)
    ...
    for i in range(nattempted + 1, nmoves.val + 1):
        ...
        if i % restart_frequency.val == 0 or i == nmoves.val:
            ...
            try:
                shutil.copy(
                    freenrg_components_file, "%s.bak" % freenrg_components_file
                )
            except:
                pass
            ...
            Sire.Stream.save(
                [bennetts_freenrgs, fep_freenrgs, ti_freenrgs], freenrgs_file
            )
            Sire.Stream.save(
                [bound_freenrgs, free_freenrgs], freenrg_parts_file
            )
            Sire.Stream.save(
                [res_freenrgs, bound_water_freenrgs, free_water_freenrgs],
                freenrg_components_file,
            )
