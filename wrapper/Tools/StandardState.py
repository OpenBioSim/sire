...
trajfile = Parameter(
    "trajfile", "traj000000001.dcd", """File name of the trajectory to process."""
)
...
simfile = Parameter(
    "simfile", "sim.cfg", """ Configuration file with distance restraints dictionary"""
)
topfile = Parameter(
    "topfile",
    "SYSTEM.top",
...
delta_trans = Parameter(
    "dtrans", 0.20, """Step size for translational volume elements, in Angstrom"""
)
norient = Parameter(
    "norient", 4, """Number of orientations per [0,2pi] Euler Angles interval"""
)
temperature = Parameter(
    "temperature", 298 * Sire.Units.kelvin, """The temperature of the system"""
)
...
def run():
    ...
    print(
        ...
        % (Nx * Ny * Nz, norient.val * (norient.val / 2) * norient.val)
    )
    print("Evaluation...")
    count = 0
    free = 0
            ...
            for k in range(0, Nz):
                ...
                for orientation in guest_orientations:
                    ...
                    if deltarot != 0:
                        ...
                        if U * beta < 10:
                            loweight += 1 / (
                                norient.val * (norient.val / 2) * norient.val
                            )
                    else:
