...
BOOST_PYTHON_MODULE(_SireRDKit)
{
    ...
    bp::def("_set_bond_order_inference_callback",
            &set_bond_order_inference_callback,
            (bp::arg("callback")),
            "Internal function used to register a Python callback for RDKit "
            "bond-order/formal-charge inference, used as a fallback when "
            "RDKit's own determineBondOrders fails");
