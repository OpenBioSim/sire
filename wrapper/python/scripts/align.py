...
parser = argparse.ArgumentParser(
    description="Align ligands/small molecules/fragments against each other.",
    epilog="align is built using Sire and is distributed "
    "under the GPL. For more information please visit "
    "https://sire.openbiosim.org/align",
    prog="align",
)
...
if args.author:
    ...
    print(
        "It is based on the Molecule::move().align() function distributed in Sire."
    )
    must_exit = True
...
print(
    "Aligned coordinates of ligand %s will be written to file %s."
    % (ligname1, outfile)
)
...
if lig0 is None:
    print(
        "\nWARNING: Cannot find the first ligand (%s) in PDB file %s."
        % (ligname0, pdb0)
    )
    can_align = False
...
if lig1 is None:
    print(
        "\nWARNING: Cannot find the second ligand (%s) in PDB file %s."
        % (ligname1, pdb1)
    )
    can_align = False
...
if matcher:
    atommap = AtomMCSMatcher(
        AtomMatchInverter(matcher), timeout, match_light
    ).match(lig1, lig0)
else:
...
for key in keys:
    lines.append(
        "%s <=> %s" % (lig1.atom(key).name(), lig0.atom(atommap[key]).name())
    )
...
print(
    "\nRMSD before alignment == %s"
    % lig1.evaluate().rmsd(lig0, AtomResultMatcher(atommap))
)
...
print(
    "\nRMSD after alignment == %s"
    % lig2.evaluate().rmsd(lig0, AtomResultMatcher(atommap))
)
