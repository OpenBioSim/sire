...
{
    throw SireError::incompatible_error(QObject::tr("We can only perform a restart simulation "
...
WorkPacket createWorkPacket(const QString &filename, int nmoves, bool record_stats)
{
    ...
    // sanity check the header
    {
        ...
        if (header.dataTypes().count() != 2)
        {
            throw SireError::incompatible_error(
                QObject::tr("A RestartPacket can only process a restart file that contains "
    ...
    if (objects.count() != 2)
        throw SireError::file_error(QObject::tr("The restart file may be corrupted as despite the header claiming "
    ...
    if (p0->isA<System>())
    {
        ...
        if (p1->isA<Moves>())
        {
            return SimPacket(p0->asA<System>(), p1->asA<Moves>(), nmoves, record_stats);
        }
        else if (p1->isA<Move>())
        {
            return SimPacket(p0->asA<System>(), SameMoves(p1->asA<Move>()), nmoves, record_stats);
        }
        else
    ...
    else if (p0->isA<SupraSystem>())
    {
        ...
        {
            return SupraSimPacket(p0->asA<SupraSystem>(), p1->asA<SupraMoves>(), nmoves, record_stats);
        }
        else if (p1->isA<SupraMove>())
        {
            return SupraSimPacket(p0->asA<SupraSystem>(), SameSupraMoves(p1->asA<SupraMove>()), nmoves, record_stats);
        }
        else
            ::throwIncompatibleError(p0, p1);
    }
...
QString getNextName(QString old_name)
{
    ...
    {
        base_name = QString("%1_%2").arg(regexp.cap(1)).arg(regexp.cap(2).toInt() + 1);
    }
    else
    {
    ...
    {
        return QString("%1/%2").arg(filename.path(), base_name);
    }
    else
    {
        return QString("%1/%2.%3").arg(filename.path(), base_name, filename.completeSuffix());
    }
}
...
void saveRestartFile(const WorkPacket &workpacket, const QString &filename)
{
    try
    {
        if (workpacket.isA<SimPacket>())
        {
            ...
            SireStream::saveToFile(simpacket.system(), simpacket.moves().read(), filename);
        }
        else if (workpacket.isA<SupraSimPacket>())
        {
            SupraSimPacket simpacket = workpacket.asA<SupraSimPacket>();

            SireStream::saveToFile(simpacket.system(), simpacket.moves(), filename);
        }
        else
        {
            throw SireError::program_bug(QObject::tr("This code only works with SimPacket and SupraSimPacket "
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        if (not ok)
            printError(QObject::tr("Could not interpret the time from \"%1\". "
                                   ...
                                   "and 52 seconds would be 02:14:52.")
                           .arg(time));
    }
...
QString timeToString(int time)
{
    ...
    {
        return QObject::tr("%1, %2 and %3").arg(parts.at(0), parts.at(1), parts.at(2));
    }
}
...
bool readBool(const QString &flag, bool &ok)
{
    ...
    if (lower_flag == QLatin1String("true") or lower_flag == QLatin1String("on") or
        lower_flag == QLatin1String("yes") or lower_flag == QLatin1String("1"))
    {
        return true;
    }
    else if (lower_flag == QLatin1String("false") or lower_flag == QLatin1String("off") or
             lower_flag == QLatin1String("no") or lower_flag == QLatin1String("0"))
    {
        return false;
    }
    else
...
QList<RestartFile> parseCommandLine(int argc, char **argv, int &repeat, int &time, bool &ok, bool &test)
{
        ...
        if (arg.startsWith("--"))
        {
            ...
            else if (arg.startsWith("--time"))
            {
                ...
                if (times.count() != 2)
                {
                    ...
                    printError(QObject::tr("Cannot interpret the time %1. The time should be set "
            ...
            else if (arg.startsWith("--repeat"))
            {
                ...
                if (repeats.count() != 2)
                {
                    ...
                    printError(QObject::tr("Cannot interpret the number of repetitions from %1. "
            ...
            else if (arg.startsWith("--nmoves"))
            {
                ...
                if (moves.count() != 2)
                {
                    ...
                    printError(QObject::tr("Cannot interpret the number of moves from %1. "
                                           "This should be set using the argument "
            ...
            else if (arg.startsWith("--statistics"))
            {
                ...
                if (stats.count() != 2)
                {
                    ...
                    printError(QObject::tr("Cannot interpret whether or not to record statistics "
        ...
        else if (arg.startsWith("-"))
        {
            ...
            else if (arg == QLatin1String("-t"))
            {
                ...
                if (i == argc)
                {
                    ...
                    printError(QObject::tr("Cannot interpret the time. The time should be set using "
            ...
            else if (arg == QLatin1String("-r"))
            {
                ...
                if (i == argc)
                {
                    ...
                    printError(QObject::tr("Cannot interpret the number of repetitions. "
                                           "This should be set using the argument "
                                           "\"--repeat=n\" or \"-r n\" where \"n\" is the number "
                                           "of times you wish to repeat the simulation."));
                }
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (Cluster::getRank() == 0)
        {
            ...
            printBox(QObject::tr("Sire Simulation Runner, Copyright (C) 2009 Christopher Woods. \n\n"
...
// thread for the Python interpreter
            ...
            QList<RestartFile> restart_files = parseCommandLine(argc, argv, repeat, time, ok, test_load);
            ...
            else if (test_load and nrestarts > 0)
            {
                ...
                {
                    printOut(QObject::tr("Loading restart file %1...").arg(restart_files.at(i).filename));
            ...
            {
                printOut(QObject::tr("Number of repetitions equals %1.").arg(repeat));

                if (time > 0)
                    printOut(QObject::tr("Total allowed runtime equals %1.").arg(timeToString(time)));
                ...
                for (int i = 0; i < nrestarts; ++i)
                {
                    ...
                    printOut(QObject::tr("%1 : %2 ( nmoves = %3, recording statistics = %4 )")
                                 .arg(i + 1)
                                 .arg(r.filename)
                                 .arg(r.nmoves)
                ...
                for (int ir = 0; ir < repeat; ++ir)
                {
                    ...
                    if (repeat > 1)
                        printOut(QObject::tr("\nRunning repetition %1 of %2...").arg(ir + 1).arg(repeat));

                    printOut(QObject::tr("Running %1 simulation(s)...").arg(nrestarts));

                    Nodes nodes = Cluster::getNodes(nrestarts);
                    printOut(QObject::tr("Number of nodes in the cluster is %1").arg(nodes.count()));
                    ...
                    for (int i = 0; i < nrestarts; ++i)
                    {
                        ...
                        if (error_count[i] > 5)
                        {
                            printOut(QObject::tr("Skipping %1 as it has failed five times in a row!").arg(r.filename));
                        ...
                        try
                        {
                            ...
                            printOut(QObject::tr("Creating workpacket %1...").arg(i + 1));
                            WorkPacket workpacket = createWorkPacket(r.filename, r.nmoves, r.record_stats);
                        ...
                        {
                            printOut(QObject::tr("There was a problem when reading %1.").arg(r.filename));
                    ...
                    {
                        printBox(QObject::tr("All restart files have finished in error!"));
                        break;
                    }

                    printOut(QObject::tr("Waiting for all submitted jobs to finish..."));
                    ...
                    for (int i = 0; i < promises.count(); ++i)
                    {
                        ...
                        {
                            printOut(
                                QObject::tr("The simulation connected with %1 has finished.").arg(running_files[i]));
                    ...
                    for (int i = 0; i < promises.count(); ++i)
                    {
                        ...
                        {
                            printOut(QObject::tr("There was a problem when running %1.").arg(running_files[i]));
                    ...
                    {
                        printOut(QObject::tr("Remaining time: %1.").arg(timeToString(time - elapsed_time)));
        ...
        else
        {
            ...
            printBox(QObject::tr("%4@%5: Starting one of the compute nodes (%1 of %2): nThreads()=%3")
                         .arg(Cluster::getRank())
                         .arg(Cluster::getCount())
                         .arg(ppn)
                         .arg(username, hostname));
            ...
            printOut(QObject::tr("compute%1 waiting to start...").arg(Cluster::getRank()));
            ...
            printOut(QObject::tr("compute%1 waiting to wait...").arg(Cluster::getRank()));
    ...
    {
        SireError::printError(SireError::unknown_exception(QObject::tr("An unknown error occurred!"), CODELOC));
