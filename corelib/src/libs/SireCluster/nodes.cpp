...
Node Nodes::_pvt_getNode()
{
    ...
    if (uid.isNull())
        throw SireError::program_bug(QObject::tr("How have we managed to reserve a node, but there are no "
...
void Nodes::add(Node node)
{
    ...
    if (old_nodes.d.get() == d.get())
    {
        ...
        if (not d->frontends.contains(uid))
            throw SireError::program_bug(QObject::tr("The node %1 thinks it is part of this Nodes object, but "
                                                     "this Nodes object isn't so sure!")
                                             .arg(uid.toString()),
                                         CODELOC);
...
void Nodes::reclaimThisThread(const QUuid &uid)
{
    ...
    if (this_thread_uids.hasLocalData())
    {
        ...
        {
            throw SireError::program_bug(QObject::tr("We cannot reclaim the local thread (UID == %1) as "
