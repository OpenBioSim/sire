...
MultiQuaternion::MultiQuaternion(const MultiDouble &x, const MultiDouble &y, const MultiDouble &z, const MultiDouble &w)
{
    sc[0] = x;
    sc[1] = y;
    sc[2] = z;
...
MultiQuaternion SireMaths::operator*(const MultiQuaternion &q, const MultiVector &p)
{
...
MultiQuaternion SireMaths::operator*(const MultiVector &p, const MultiQuaternion &q)
{
    return q * p;
}
...
MultiVector MultiQuaternion::rotate(const MultiVector &p) const
{
    ...
    return MultiVector(two * ((half - sy2 - sz2) * p.x() + (sxy - swz) * p.y() + (sxz + swy) * p.z()),

                       two * ((sxy + swz) * p.x() + (half - sx2 - sz2) * p.y() + (syz - swx) * p.z()),

                       two * ((sxz - swy) * p.x() + (syz + swx) * p.y() + (half - sx2 - sy2) * p.z()));
}
...
QVector<MultiVector> MultiQuaternion::rotate(const QVector<MultiVector> &points) const
{
    ...
    for (int i = 0; i < ret.count(); ++i)
    {
        ...
        p = MultiVector(two * ((half - sy2 - sz2) * p.x() + (sxy - swz) * p.y() + (sxz + swy) * p.z()),
...
MultiQuaternion SireMaths::operator*(const MultiQuaternion &p1, const MultiQuaternion &p2)
{
    ...
    // calculate new 'w'
    MultiDouble nw = p1.sc[3] * p2.sc[3] - (p1.sc[0] * p2.sc[0] + p1.sc[1] * p2.sc[1] + p1.sc[2] * p2.sc[2]);
...
MultiQuaternion SireMaths::operator+(const MultiQuaternion &p1, const MultiQuaternion &p2)
{
    return MultiQuaternion(p1.sc[0] + p2.sc[0], p1.sc[1] + p2.sc[1], p1.sc[2] + p2.sc[2], p1.sc[3] + p2.sc[3]);
}
