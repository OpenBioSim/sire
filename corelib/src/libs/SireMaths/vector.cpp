...
Vector::Vector(const NVector &v)
{
    if (v.count() != 3)
        throw SireError::incompatible_error(
            QObject::tr("Cannot create a 3D vector from a NVector of size %1.").arg(v.count()), CODELOC);
...
{
    return pow_2(v2.sc[0] - v1.sc[0]) + pow_2(v2.sc[1] - v1.sc[1]) + pow_2(v2.sc[2] - v1.sc[2]);
}

/** Return the distance between two vectors */
double Vector::distance(const Vector &v1, const Vector &v2)
{
    return std::sqrt(pow_2(v2.sc[0] - v1.sc[0]) + pow_2(v2.sc[1] - v1.sc[1]) + pow_2(v2.sc[2] - v1.sc[2]));
}
...
{
    return double(1.0) /
           std::sqrt(pow_2(v1.sc[0] - v2.sc[0]) + pow_2(v1.sc[1] - v2.sc[1]) + pow_2(v1.sc[2] - v2.sc[2]));
}

/** Return 1 / distance2 between two vectors */
double Vector::invDistance2(const Vector &v1, const Vector &v2)
{
    return double(1.0) / (pow_2(v1.sc[0] - v2.sc[0]) + pow_2(v1.sc[1] - v2.sc[1]) + pow_2(v1.sc[2] - v2.sc[2]));
}
...
Vector Vector::normalise() const
{
    ...
    if (SireMaths::isZero(l))
        throw SireMaths::math_error(QObject::tr("Cannot normalise a zero vector, %1").arg(this->toString()), CODELOC);
...
Vector Vector::fromString(const QString &str)
{
    ...
    {
        throw SireError::invalid_arg(QObject::tr("Cannot find anything that looks like a vector in the string "
                                                 "\"%1\"")
                                         .arg(str),
                                     CODELOC);
    }

    return Vector(vecregexp.cap(1).toFloat(), vecregexp.cap(2).toFloat(), vecregexp.cap(3).toFloat());
}
...
Vector Vector::generate(Length dst, const Vector &v1, const Angle &ang, const Vector &v2, const Angle &dih,
                        const Vector &v3)
{
...
Vector Vector::generate(double dst, const Vector &v1, const Angle &ang, const Vector &v2, const Angle &dih,
                        const Vector &v3)
{
...
Matrix Vector::metricTensor() const
{
    ...
    return Matrix(y2 + z2, -xy, -xz, -xy, x2 + z2, -yz, -xz, -yz, x2 + y2);
}
...
const Vector SireMaths::operator/(const Vector &p1, double c)
{
    if (isZero(c))
        throw SireMaths::math_error(
            QObject::tr("Cannot divide a vector by zero! %1 / 0 is a error!").arg(p1.toString()), CODELOC);
