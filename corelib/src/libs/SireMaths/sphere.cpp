...
Sphere::Sphere(const Vector &position, const double &radius) : _center(position), _radius(radius)
{
    if (radius < 0.0)
        _radius = -radius;
}
...
{
    return QObject::tr("Sphere( center() == %1, radius == %2 )").arg(center().toString()).arg(radius());
}
...
double Sphere::intersectionVolume(const Sphere &sphere1, const Sphere &sphere2) const
{
    ...
    if (sphere0.radius() < sphere1.radius() or sphere0.radius() < sphere2.radius() or
        sphere1.radius() < sphere2.radius())
    {
        throw SireError::program_bug(QObject::tr("Spheres should be arranged in size order? %1 %2 %3")
                                         .arg(sphere0.toString())
                                         .arg(sphere1.toString())
                                         .arg(sphere2.toString()),
                                     CODELOC);
    }

    // all three spheres must be intersecting...
    if (not(sphere0.intersects(sphere1) and sphere0.intersects(sphere2) and sphere1.intersects(sphere2)))
    {
    ...
    else if (w2 > 0)
    {
        ...
        const double q1 = a * (b2 + c2 - a2 + beta2 + gamma2 - 2.0 * alpha2 + epsilon1 * (b2 - c2));

        // q2 = b [ c^2 + a^2 - b^2 + gamma^2 + alpha^2 - 2 beta^2 + epsilon2 (c^2 - a^2) ]
        const double q2 = b * (c2 + a2 - b2 + gamma2 + alpha2 - 2.0 * beta2 + epsilon2 * (c2 - a2));

        // q3 = c [ a^2 + b^2 - c^2 + alpha^2 + beta^2 - 2 gamma^2 + epsilon3 (a^2 - b^2) ]
        const double q3 = c * (a2 + b2 - c2 + alpha2 + beta2 - 2.0 * gamma2 + epsilon3 * (a2 - b2));
        ...
        const double v_abc =
            (w / 6.0) -
            ((a / 2.0) * (beta2 + gamma2 - a2 * (one_sixth - 0.5 * epsilon1 * epsilon1)) * my_arctan(two_w / q1)) -
            ((b / 2.0) * (gamma2 + alpha2 - b2 * (one_sixth - 0.5 * epsilon2 * epsilon2)) * my_arctan(two_w / q2)) -
            ((c / 2.0) * (alpha2 + beta2 - c2 * (one_sixth - 0.5 * epsilon3 * epsilon3)) * my_arctan(two_w / q3)) +
    ...
    else
    {
        ...
        const double p1 =
            (SireMaths::pow_2(b2 - c2 + beta2 - gamma2) + SireMaths::pow_2(std::sqrt(t2) - std::sqrt(t_abg2))) /
                (4.0 * a2) -
            alpha2;
        const double p2 =
            (SireMaths::pow_2(b2 - c2 + beta2 - gamma2) + SireMaths::pow_2(std::sqrt(t2) + std::sqrt(t_abg2))) /
        ...
        const double p3 =
            (SireMaths::pow_2(c2 - a2 + gamma2 - alpha2) + SireMaths::pow_2(std::sqrt(t2) - std::sqrt(t_bga2))) /
                (4.0 * b2) -
            beta2;
        const double p4 =
            (SireMaths::pow_2(c2 - a2 + gamma2 - alpha2) + SireMaths::pow_2(std::sqrt(t2) + std::sqrt(t_bga2))) /
        ...
        const double p5 =
            (SireMaths::pow_2(a2 - b2 + alpha2 - beta2) + SireMaths::pow_2(std::sqrt(t2) - std::sqrt(t_cab2))) /
                (4.0 * c2) -
            gamma2;
        const double p6 =
            (SireMaths::pow_2(a2 - b2 + alpha2 - beta2) + SireMaths::pow_2(std::sqrt(t2) + std::sqrt(t_cab2))) /
        ...
        else
        {
            ...
            qDebug() << "HOW DID WE GET HERE?" << p1 << p2 << p3 << p4 << p5 << p6;
...
double Sphere::combinedVolume(const QVector<SireMaths::Sphere> &spheres)
{
    ...
    else if (spheres.count() == 2)
    {
        ...
        return spheres.at(0).volume() + spheres.at(1).volume() - spheres.at(0).intersectionVolume(spheres.at(1));
    }
    else
    {
        QElapsedTimer timer;
        ...
        for (int i = 0; i < spheres.count(); ++i)
        {
            if (intersecting_pairs[i].count() > 1)
            {
                ...
                // spheres
                QList<int> intersects = intersecting_pairs[i].values();
                    ...
                    if (sphere1 > sphere0)
                    {
                        for (int k = j + 1; k < intersects.count(); ++k)
                        {
                            ...
                            if (intersecting_pairs[sphere1].contains(sphere2))
                            {
                                ...
                                total_volume += spheres[sphere0].intersectionVolume(spheres[sphere1], spheres[sphere2]);
                            }
                        }
                    }
                }
...
double Sphere::combinedVolumeMC(const QVector<Sphere> &spheres, qint64 nsamples)
{
    ...
    if (nsamples > 0)
    {
        ...
        {
            Vector test(rand.rand(minbox.x(), maxbox.x()), rand.rand(minbox.y(), maxbox.y()),
    ...
    else
    {
        ...
        while (true)
        {
            ...
            if (n_tests > 1000 and (n_tests % 1000 == 0))
            {
                ...
                {
                    last_timeout += 2000;
                    qDebug() << "Solving... SAMPLES:" << n_tests << "CONVERGENCE:" << std::abs(ratio - old_ratio)
    ...
    const double total_volume = double(n_in_sphere) * (maxbox.x() - minbox.x()) * (maxbox.y() - minbox.y()) *
