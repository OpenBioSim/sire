 ...
 */
PropertyMap PropertyMap::addPrefix(const QString &prefix,
                                   const QStringList &keys) const
{
    ...
    {
        const auto mod_key = prefix + key;

        if (this->specified(mod_key))
        {
            ret.set(key, this->operator[](mod_key));
        }
        else if (this->specified(key))
        {
            const auto value = this->operator[](key);

            if (value.hasSource())
            {
                auto new_key = prefix + value.source();

                if (this->specified(new_key))
                {
                    const auto new_value = this->operator[](new_key);

                    if (new_value.hasSource())
                    {
                        ret.set(key, PropertyName(prefix + new_value.source()));
                    }
                    else
                        ret.set(key, new_value);
                }
                else
                    ret.set(key, PropertyName(new_key));
            }
        }
        else
        {
            ret.set(key, PropertyName(prefix + key));
        }
    }
 ...
 */
PropertyMap PropertyMap::addSuffix(const QString &suffix,
                                   const QStringList &keys) const
{
    if (suffix.isEmpty())
        return *this;
    ...
    for (const auto &key : keys)
    {
        ...
        else if (this->specified(key))
        {
            ...
            if (value.hasSource())
            {
                auto new_key = value.source() + suffix;
                ...
                if (this->specified(new_key))
                {
                    ...
                    if (new_value.hasSource())
                    {
                        ret.set(key, PropertyName(new_value.source() + suffix));
                    }
                    else
                        ret.set(key, new_value);
                }
        ...
        {
            ret.set(key, PropertyName(key + suffix));
        }
    }
