...
namespace SireBase
{
    ...
    */
    ...
    class ChunkedHash
    {
        ...
        friend const void *SireBase::detail::get_shared_container_pointer<>(const ChunkedHash<Key, T, N> &);
    ...
    /** Empty constructor */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N>::iterator::iterator() : chunks(0), current_chunk(0)
    {
    }

    /** Copy constructor */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N>::iterator::iterator(
        const typename ChunkedHash<Key, T, N>::iterator &other)
        : chunks(other.chunks), current_chunk(other.current_chunk), current_it(other.current_it)
    {
    }
    ...
    /** Comparison operator */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedHash<Key, T, N>::iterator::operator==(
        const typename ChunkedHash<Key, T, N>::iterator &other) const
    {
        return chunks == other.chunks and current_it == other.current_it;
    }
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const Key &ChunkedHash<Key, T, N>::iterator::key() const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE T &ChunkedHash<Key, T, N>::iterator::value() const
    {
    ...
    /** Return the value of the current item */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE T &ChunkedHash<Key, T, N>::iterator::operator*() const
    {
        return *(current_it);
    }
    ...
    /** Empty constructor */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N>::const_iterator::const_iterator() : chunks(0), current_chunk(0)
    {
    }

    /** Copy constructor */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N>::const_iterator::const_iterator(
        const typename ChunkedHash<Key, T, N>::const_iterator &other)
        : chunks(other.chunks), current_chunk(other.current_chunk), current_it(other.current_it)
    {
    }
    ...
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator &ChunkedHash<
        Key, T, N>::const_iterator::operator=(const typename ChunkedHash<Key, T, N>::iterator &other)
    {
        chunks = other.chunks;
        current_chunk = other.current_chunk;
        current_it = other.current_it;
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator &ChunkedHash<
        Key, T, N>::const_iterator::operator=(const typename ChunkedHash<Key, T, N>::const_iterator &other)
    {
        chunks = other.chunks;
        current_chunk = other.current_chunk;
        current_it = other.current_it;
    ...
    /** Comparison operator */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedHash<Key, T, N>::const_iterator::operator==(
        const typename ChunkedHash<Key, T, N>::const_iterator &other) const
    {
        return chunks == other.chunks and current_it == other.current_it;
    }
    ...
    /** Return the key of the current item */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const Key &ChunkedHash<Key, T, N>::const_iterator::key() const
    {
        return current_it.key();
    }
    ...
    /** Return the value of the current item */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const T &ChunkedHash<Key, T, N>::const_iterator::value() const
    {
        return current_it.value();
    }
    ...
    /** Return the value of the current item */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const T &ChunkedHash<Key, T, N>::const_iterator::operator*() const
    {
        return *(current_it);
    }
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N>::ChunkedHash()
    {
    }

    /** Construct from a QHash */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N>::ChunkedHash(const QHash<Key, T> &hash)
    {
        this->reserve(hash.count());

        for (typename QHash<Key, T>::const_iterator it = hash.constBegin(); it != hash.constEnd(); ++it)
        {
    ...
    /** Copy constructor */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N>::ChunkedHash(const ChunkedHash<Key, T, N> &other)
        : _chunks(other._chunks), key_to_chunkidx(other.key_to_chunkidx)
    {
    }
    ...
    /** Copy assignment operator */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N> &ChunkedHash<Key, T, N>::operator=(const ChunkedHash<Key, T, N> &other)
    {
    ...
    /** Copy from a QHash */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N> &ChunkedHash<Key, T, N>::operator=(const QHash<Key, T> &hash)
    {
    ...
    /** Comparison operator */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedHash<Key, T, N>::operator==(const ChunkedHash<Key, T, N> &other) const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE T &ChunkedHash<Key, T, N>::operator[](const Key &key)
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const T ChunkedHash<Key, T, N>::operator[](const Key &key) const
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::iterator ChunkedHash<Key, T, N>::begin()
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator ChunkedHash<Key, T, N>::begin() const
    {
    ...
    /** Return a const_iterator pointing to the first item in the hash */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator ChunkedHash<Key, T, N>::constBegin() const
    {
        return this->begin();
    }
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::iterator ChunkedHash<Key, T, N>::find(const Key &key)
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    /** Return an iterator pointing to the most recently inserted value with key 'key',
        or end() if no such item exists */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator ChunkedHash<Key, T, N>::find(
        const Key &key) const
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator ChunkedHash<Key, T, N>::constFind(
        const Key &key) const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::iterator ChunkedHash<Key, T, N>::end()
    {
        typename ChunkedHash<Key, T, N>::iterator it;
    ...
    /** Return an iterator that points one beyond the last item in the hash */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator ChunkedHash<Key, T, N>::end() const
    {
        typename ChunkedHash<Key, T, N>::const_iterator it;
    ...
    /** Return an iterator that points one beyond the last item in the hash */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::const_iterator ChunkedHash<Key, T, N>::constEnd() const
    {
        return this->end();
    }
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedHash<Key, T, N>::capacity() const
    {
        int c = 0;
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedHash<Key, T, N>::clear()
    {
        _chunks.clear();
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedHash<Key, T, N>::contains(const Key &key) const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedHash<Key, T, N>::count(const Key &key) const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedHash<Key, T, N>::count() const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedHash<Key, T, N>::empty() const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::iterator ChunkedHash<Key, T, N>::insert(const Key &key,
                                                                                                     const T &value)
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE typename ChunkedHash<Key, T, N>::iterator ChunkedHash<Key, T, N>::insertMulti(const Key &key,
                                                                                                          const T &value)
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    /** Return whether or not this hash is empty */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedHash<Key, T, N>::isEmpty() const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const Key ChunkedHash<Key, T, N>::key(const T &value, const Key &defaultKey) const
    {
        int nchunks = _chunks.count();
        const QHash<Key, T> *chunks_array = _chunks.constData();
        ...
        for (int i = 0; i < nchunks; ++i)
        {
            for (typename QHash<Key, T>::const_iterator it = chunks_array[i].constBegin(); it != chunks_array[i].constEnd();
                 ++it)
            {
                if (it.value() == value)
                    return it.key();
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const Key ChunkedHash<Key, T, N>::key(const T &value) const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE QList<Key> ChunkedHash<Key, T, N>::keys() const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE QList<Key> ChunkedHash<Key, T, N>::keys(const T &value) const
    {
        QList<Key> ks;
    ...
    /** Remove all items associated with the key 'key' - return the number of items
        removed */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedHash<Key, T, N>::remove(const Key &key)
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedHash<Key, T, N>::reserve(int size)
    {
    ...
    /** Return the number of items in this hash */
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedHash<Key, T, N>::size() const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedHash<Key, T, N>::squeeze()
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE T ChunkedHash<Key, T, N>::take(const Key &key)
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE QList<Key> ChunkedHash<Key, T, N>::uniqueKeys() const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedHash<Key, T, N> &ChunkedHash<Key, T, N>::unite(const ChunkedHash<Key, T, N> &other)
    {
        for (typename ChunkedHash<Key, T, N>::const_iterator it = other.constBegin(); it != other.constEnd(); ++it)
        {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const T ChunkedHash<Key, T, N>::value(const Key &key) const
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const T ChunkedHash<Key, T, N>::value(const Key &key, const T &defaultValue) const
    {
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE QList<T> ChunkedHash<Key, T, N>::values() const
    {
        QList<T> vals;
    ...
    template <class Key, class T, int N>
    SIRE_OUTOFLINE_TEMPLATE QList<T> ChunkedHash<Key, T, N>::values(const Key &key) const
    {
        int idx = key_to_chunkidx.value(key, -1);
    ...
    namespace detail
    {
        ...
        template <class Key, class T, int N>
        SIRE_OUTOFLINE_TEMPLATE const void *get_shared_container_pointer(const SireBase::ChunkedHash<Key, T, N> &hash)
        {
            if (hash.empty())
                return 0;
            else
...
template <class Key, class T, int N>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireBase::ChunkedHash<Key, T, N> &hash)
{
    // this streams out the data using the same format as QHash
    ds << qint32(hash.count());

    for (typename SireBase::ChunkedHash<Key, T, N>::const_iterator it = hash.constBegin(); it != hash.constEnd(); ++it)
    {
        ds << it.key() << it.value();
    }
...
template <class Key, class T, int N>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireBase::ChunkedHash<Key, T, N> &hash)
{
...
/** Serialise to a binary datastream */
template <class Key, class T, int N>
SIRE_OUTOFLINE_TEMPLATE SireStream::SharedDataStream &operator<<(SireStream::SharedDataStream &sds,
                                                                 const SireBase::ChunkedHash<Key, T, N> &hash)
{
    sds.sharedSaveContainer<SireBase::ChunkedHash<Key, T, N>, SireBase::detail::GetChunkedHashPointer<Key, T, N>>(hash);
...
template <class Key, class T, int N>
SIRE_OUTOFLINE_TEMPLATE SireStream::SharedDataStream &operator>>(SireStream::SharedDataStream &sds,
                                                                 SireBase::ChunkedHash<Key, T, N> &hash)
{
    sds.sharedLoadContainer<SireBase::ChunkedHash<Key, T, N>, SireBase::detail::GetChunkedHashPointer<Key, T, N>>(hash);
