...
namespace SireBase
{
    ...
    namespace detail
    {
        ...
        public:
            ...
            MajorMinorVersionData(quint64 majv, quint64 minv) : last_major_version(majv), last_minor_version(minv)
            {
            }
    ...
    */
    ...
    /** Constructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE VersionRegistry<T>::VersionRegistry()
    {
    }
    ...
    /** Return whether or not an object with key 'key' has been
        registered */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE bool VersionRegistry<T>::registered(const T &key)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE MajorMinorVersion VersionRegistry<T>::registerObject(const T &key)
    {
        QMutexLocker lkr(&registry_mutex);
