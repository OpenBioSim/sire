...
#define SIRE_UNITTEST(X) static SireBase::UnitTest _SireBase_UnitTest_register_##X(#X, &X);
    ...
    void assert_equal(const T &a, const T &b, const QString &code_location)
    {
        if (a != b)
        {
            throw SireError::assertation_failed(
                QObject::tr("NOT EQUAL!\n\n%1\n\n!=\n\n%2").arg(Sire::toString(a)).arg(Sire::toString(b)), code_location);
        }
    }
    ...
    {
        if (not result)
            throw SireError::assertation_failed(QObject::tr("The expression is not true"), code_location);
    }
    ...
    {
        if (result)
            throw SireError::assertation_failed(QObject::tr("The expression is not false"), code_location);
    }
                                          ...
                                          const QString &code_location)
    {
        ...
        catch (const SireError::exception &thrown)
        {
            ...
            {
                throw SireError::assertation_failed(
                    QObject::tr("The expected exception %1 was not thrown. Instead the exception "
                                "%2 was thrown.")
                        .arg(e.what())
                        .arg(thrown.what()),
                    code_location);
        ...
        {
            throw SireError::assertation_failed(
                QObject::tr("The expected exception %1 was not thrown. Instead an unknown exception "
                            "was thrown.")
                    .arg(e.what()),
                code_location);
        }
        ...
        throw SireError::assertation_failed(
            QObject::tr("The expected exception %1 was not thrown, as no exception was thrown!").arg(e.what()),
            code_location);
    }
#endif
