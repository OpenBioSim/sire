...
template <class T>
QDataStream &operator<<(QDataStream &, const SireBase::detail::PackedArray2D_Array<T> &);
template <class T>
QDataStream &operator>>(QDataStream &, SireBase::detail::PackedArray2D_Array<T> &);
...
namespace SireBase
{
    ...
    */
    ...
    public:
        ...
        void updateAll(const QVarLengthArray<int> &idxs, const PackedArray2D<T> &arrays);
...
namespace SireBase
{
    ...
    namespace detail
    {
        ...
        */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE char *PackedArray2DMemory<T>::create(quint32 narrays, quint32 nvalues)
        {
            // create space for the data
            char *storage =
                PackedArray2DMemoryBase::create(narrays, nvalues, sizeof(PackedArray2DData<T>), sizeof(PackedArray2D_Array<T>),
                                                sizeof(PackedArray2D_ArrayData<T>), sizeof(T));

            try
            {
                quint32 sz = PackedArray2DMemoryBase::getSize(narrays, nvalues, sizeof(PackedArray2DData<T>),
                                                              sizeof(PackedArray2D_Array<T>),
                                                              sizeof(PackedArray2D_ArrayData<T>), sizeof(T));

                // now we need to construct each object in turn at its correct
                // location in this space
                PackedArray2DData<T> *arraydata = new (storage) PackedArray2DData<T>(narrays, nvalues);
                ...
                if (narrays > 0)
                {
                    ...
                    for (quint32 i = 0; i < narrays; ++i)
                    {
                        ...
                        PackedArray2D_ArrayData<T> *data = new (storage + dataidx) PackedArray2D_ArrayData<T>(dataidx);
                    ...
                    BOOST_ASSERT(idx == array0_idx + narrays * sizeof(PackedArray2D_Array<T>));
                ...
                else
                {
                    ...
                    PackedArray2D_ArrayData<T> *array_arraydata = new (storage + data_idx) PackedArray2D_ArrayData<T>(data_idx);
                    ...
                    idx += sizeof(PackedArray2D_Array<T>) + sizeof(PackedArray2D_ArrayData<T>);
                }
        ...
        template <class T>
        SIRE_INLINE_TEMPLATE void PackedArray2DMemory<T>::incref(char *this_ptr, quint32 this_idx)
        {
            ((PackedArray2DData<T> *)(PackedArray2DMemoryBase::getRoot(this_ptr, this_idx)))->incref();
        }

        /** Decrease the reference count */
        template <class T>
        SIRE_INLINE_TEMPLATE void PackedArray2DMemory<T>::decref(char *this_ptr, quint32 this_idx)
        {
            ((PackedArray2DData<T> *)(PackedArray2DMemoryBase::getRoot(this_ptr, this_idx)))->decref();
        }

        /** Destroy the object 'array' */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2DMemory<T>::destroy(PackedArray2DData<T> *array)
        {
            ...
            if (narrays > 0)
            {
                ...
                {
                    PackedArray2D_ArrayData<T> *arraydata = const_cast<PackedArray2D_ArrayData<T> *>(arrays[i].d.constData());
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE char *PackedArray2DMemory<T>::detach(char *this_ptr, quint32 this_idx)
        {
            ...
            if (not arraydata->ref.hasSingleReference())
            {
                ...
                int sz = getSize(arraydata->nArrays(), arraydata->nValues(), sizeof(PackedArray2DData<T>),
                                 sizeof(PackedArray2D_Array<T>), sizeof(PackedArray2D_ArrayData<T>), sizeof(T));
        ...
        /** Null constructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2DData<T>::PackedArray2DData() : PackedArray2DDataBase()
        {
        }

        /** Construct to hold the specified number of arrays and values */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2DData<T>::PackedArray2DData(quint32 narrays, quint32 nvalues)
        ...
        /** Copy constructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2DData<T>::PackedArray2DData(const PackedArray2DData &other)
        ...
        /** Destructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2DData<T>::~PackedArray2DData()
        {
        }

        /** Decrease the reference count of this object */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2DData<T>::decref()
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2DData<T> *PackedArray2DData<T>::detach()
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const PackedArray2D_ArrayData<T> *PackedArray2DData<T>::nullArray() const
        {
            BOOST_ASSERT(this->nArrays() == 0);
            BOOST_ASSERT(this->getArrayData0() != 0);

            return (const PackedArray2D_ArrayData<T> *)(this->memory() + this->getArrayData0());
        }

        /** Return a raw pointer to the array of PackedArray2D_ArrayData objects */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const PackedArray2D_ArrayData<T> *PackedArray2DData<T>::arrayDataData() const
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const PackedArray2D_Array<T> *PackedArray2DData<T>::arrayData() const
        {
            if (this->getArray0() == 0)
                return 0;
            else
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2DData<T>::valueData() const
        {
            if (this->getValue0() == 0)
                return 0;
            else
        ...
        /** Return a raw pointer to the array of PackedArray2D_ArrayData objects */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_ArrayData<T> *PackedArray2DData<T>::arrayDataData()
        {
            if (this->getArrayData0() == 0)
                return 0;
            else
        ...
        /** Return a raw pointer to the array of PackedArray2D_Array<T> objects */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T> *PackedArray2DData<T>::arrayData()
        {
            if (this->getArray0() == 0)
                return 0;
            else
        ...
        /** Return a raw pointer to the array of objects in this array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE T *PackedArray2DData<T>::valueData()
        {
            if (this->getValue0() == 0)
                return 0;
            else
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2DData<T>::setNValuesInArray(quint32 i, quint32 nvalues)
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2DData<T>::close()
        {
            ...
            for (quint32 i = 0; i < this->nArrays(); ++i)
            {
                ...
                if (array.nValues() > 0)
                    PackedArray2DMemoryBase::setValue0(&array, value0 + n_assigned_vals * sizeof(T));
                else
            ...
            if (n_assigned_vals != this->nValues())
                throw SireError::program_bug(QObject::tr("Serious program bug - n_assigned_vals (%1) does not equal "
                                                         "this->nValues() (%2).")
                                                 .arg(n_assigned_vals)
                                                 .arg(this->nValues()),
                                             CODELOC);
        ...
        /** Null constructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_ArrayData<T>::PackedArray2D_ArrayData() : PackedArray2D_ArrayDataBase()
        {
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_ArrayData<T>::PackedArray2D_ArrayData(quint32 this_idx)
        ...
        /** Copy constructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_ArrayData<T>::PackedArray2D_ArrayData(const PackedArray2D_ArrayData<T> &other)
        ...
        /** Destructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_ArrayData<T>::~PackedArray2D_ArrayData()
        {
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_ArrayData<T> *PackedArray2D_ArrayData<T>::detach()
        {
            return (PackedArray2D_ArrayData<T> *)(PackedArray2DMemory<T>::detach((char *)this, this->getThisArray()));
        }

        /** Return a pointer to a PackedArray2DData object that contains
            just the data of this PackedArray2D_Array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2DData<T> *PackedArray2D_ArrayData<T>::extract() const
        {
            ...
            try
            {
                ...
                // copy the objects
                quickCopy<T>(new_array->valueData(), this->valueData(), this->nValues());
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2D_ArrayData<T>::incref()
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2D_ArrayData<T>::decref()
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2D_ArrayData<T>::valueData() const
        {
            if (this->getValue0() == 0)
                return 0;
            else
        ...
        /** Return a pointer to the first object in this array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE T *PackedArray2D_ArrayData<T>::valueData()
        {
            if (this->getValue0() == 0)
                return 0;
            else
        ...
        {
            const SharedArray2DPtr<PackedArray2DData<T>> &array = SireBase::detail::getSharedNull<T>();
        ...
        /** Null constructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T>::PackedArray2D_Array() : d(SireBase::detail::createNullArray<T>())
        {
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T>::PackedArray2D_Array(quint32 sz)
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T>::PackedArray2D_Array(quint32 sz, const T &value)
            : d(SireBase::detail::createArray<T>(sz))
        {
            if (sz == 0)
                return;
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T>::PackedArray2D_Array(const QVector<T> &values)
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T>::PackedArray2D_Array(detail::PackedArray2D_ArrayData<T> *data)
        {
            d.weakAssign(data);
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T>::PackedArray2D_Array(const PackedArray2D_Array<T> &other) : d(other.d)
        {
        }

        /** Destructor */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T>::~PackedArray2D_Array()
        {
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE PackedArray2D_Array<T> &PackedArray2D_Array<T>::operator=(const PackedArray2D_Array<T> &other)
        {
            this->d = other.d;
            return *this;
        }
        ...
        /** Comparison operator */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE bool PackedArray2D_Array<T>::operator==(const PackedArray2D_Array<T> &other) const
        {
        ...
        */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2D_Array<T>::assertValidIndex(quint32 i) const
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2D_Array<T>::data() const
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE T *PackedArray2D_Array<T>::data()
        {
            return this->d->valueData();
        }
        ...
        /** Return a raw pointer to the array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2D_Array<T>::constData() const
        {
            return this->d->valueData();
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const T &PackedArray2D_Array<T>::operator[](quint32 i) const
        {
        ...
        /** Return a reference to the ith object in this array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE T &PackedArray2D_Array<T>::operator[](quint32 i)
        {
            this->assertValidIndex(i);
            return this->data()[i];
        }
        ...
        /** Return a reference to the ith object in this array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const T &PackedArray2D_Array<T>::at(quint32 i) const
        {
            this->assertValidIndex(i);
            return this->data()[i];
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE int PackedArray2D_Array<T>::count() const
        {
            return d->nValues();
        }
        ...
        /** Return the number of objects in this array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE int PackedArray2D_Array<T>::size() const
        {
        ...
        /** Return the number of objects in this array */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE int PackedArray2D_Array<T>::nValues() const
        {
            return this->d->nValues();
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE bool PackedArray2D_Array<T>::isEmpty() const
        {
            return this->count() == 0;
        }
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE QVector<T> PackedArray2D_Array<T>::toQVector() const
        {
            if (this->isEmpty())
                return QVector<T>();
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE QString PackedArray2D_Array<T>::toString() const
        {
        ...
        */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE void PackedArray2D_Array<T>::update(const PackedArray2D_Array<T> &other)
        {
    ...
    /** Null constructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::PackedArray2D() : d(SireBase::detail::getSharedNull<T>())
    {
    }

    /** Construct from a passed Array */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::PackedArray2D(const typename PackedArray2D<T>::Array &array)
        : d(array.d->extract())
    {
    }
    ...
    /** Copy assignment operator - fast as this class is implicitly shared */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T> &PackedArray2D<T>::operator=(const PackedArray2D<T> &other)
    {
        d = other.d;
        return *this;
    }
    ...
    namespace detail
    {
        ...
        template <class T>
        static SharedArray2DPtr<PackedArray2DData<T>> createArray(quint32 narrays, quint32 nvals)
        {
            if (narrays == 0)
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::PackedArray2D(const QVector<T> &values)
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::PackedArray2D(const QVector<typename PackedArray2D<T>::Array> &arrays)
        : d(SireBase::detail::getSharedNull<T>())
    {
        if (arrays.isEmpty())
            return;
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::PackedArray2D(const QVector<QVector<T>> &values)
        : d(SireBase::detail::getSharedNull<T>())
    {
        if (values.isEmpty())
            return;
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::PackedArray2D(const PackedArray2D<T> &array0, const PackedArray2D<T> &array1)
    {
        if (array0.isEmpty())
        {
            d = array1.d;
    ...
    /** Copy constructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::PackedArray2D(const PackedArray2D<T> &other) : d(other.d)
    {
    }

    /** Destructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T>::~PackedArray2D()
    {
    }

    /** Comparison operator */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE bool PackedArray2D<T>::operator==(const PackedArray2D<T> &other) const
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::assertValidIndex(quint32 i) const
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const typename PackedArray2D<T>::Array &PackedArray2D<T>::operator[](quint32 i) const
    {
        this->assertValidIndex(i);
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const typename PackedArray2D<T>::Array &PackedArray2D<T>::at(quint32 i) const
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T &PackedArray2D<T>::operator()(quint32 i, quint32 j) const
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE T &PackedArray2D<T>::operator()(quint32 i, quint32 j)
    {
        this->assertValidIndex(i);
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T &PackedArray2D<T>::at(quint32 i, quint32 j) const
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE int PackedArray2D<T>::count() const
    {
        return d->nArrays();
    }
    ...
    /** Return the number of arrays in this PackedArray2D */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE int PackedArray2D<T>::size() const
    {
        return this->count();
    }
    ...
    /** Return the number of arrays in this PackedArray2D */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE int PackedArray2D<T>::nArrays() const
    {
        return this->count();
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE int PackedArray2D<T>::nValues() const
    {
        return d->nValues();
    }
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE int PackedArray2D<T>::nValues(quint32 i) const
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE bool PackedArray2D<T>::isEmpty() const
    {
        return this->count() == 0;
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const typename PackedArray2D<T>::Array *PackedArray2D<T>::data() const
    {
        return d->arrayData();
    }
    ...
    /** Return a raw pointer to the array of arrays. */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const typename PackedArray2D<T>::Array *PackedArray2D<T>::constData() const
    {
        return d->arrayData();
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::detach()
    {
        d->detach();
    }
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2D<T>::data(quint32 i) const
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE T *PackedArray2D<T>::data(quint32 i)
    {
        this->assertValidIndex(i);
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2D<T>::constData(quint32 i) const
    {
        return this->data(i);
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2D<T>::valueData() const
    {
        return d->valueData();
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE T *PackedArray2D<T>::valueData()
    {
        return d->valueData();
    }
    ...
    /** Return a raw pointer to the array of all values in all arrays */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T *PackedArray2D<T>::constValueData() const
    {
        return d->valueData();
    }
    ...
    /** Return the QVector that has the same contents as this array */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE QVector<T> PackedArray2D<T>::toQVector() const
    {
        if (this->nValues() == 0)
            return QVector<T>();
    ...
    /** Return the QVector that has the same contents as this array */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE QVector<QVector<T>> PackedArray2D<T>::toQVectorVector() const
    {
        if (this->isEmpty())
    ...
    /** Return a string representation of this array */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE QString PackedArray2D<T>::toString() const
    {
    ...
    namespace detail
    {
        ...
        SIREBASE_EXPORT void throwCannotConvertVariantError(const char *this_type, const char *type_t, const QString &codeloc);
        ...
        struct VariantConverter
        {
            ...
            static PackedArray2D<T> input(const PackedArray2D<QVariant> &variant)
            {
                ...
                const typename PackedArray2D<QVariant>::Array *variant_array = variant.constData();

                for (int i = 0; i < narrays; ++i)
                {
                    const typename PackedArray2D<QVariant>::Array &this_variant = variant_array[i];
                    ...
                    for (int j = 0; j < nvalues; ++j)
                    {
                        ...
                        {
                            throwCannotConvertVariantError(value.typeName(), QMetaType::typeName(qMetaTypeId<T>()), CODELOC);
                        }
                        else
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<QVariant> PackedArray2D<T>::toVariant() const
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PackedArray2D<T> PackedArray2D<T>::fromVariant(const PackedArray2D<QVariant> &variant)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::update(quint32 i, const Array &array)
    {
        this->assertValidIndex(i);
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::update(quint32 i, const QVector<T> &array)
    {
        this->assertValidIndex(i);
        d->arrayData()[i].update(array);
    }
    ...
    */
    template <class T>
    template <class C>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::updateAll(const C &idxs, const PackedArray2D<T> &arrays)
    {
        if (idxs.count() != arrays.count())
            throw SireError::incompatible_error(
                QObject::tr("You cannot update the arrays because the number of indicies (%1) "
        ...
        for (typename C::const_iterator it = idxs.constBegin(); it != idxs.constEnd(); ++it)
        {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::updateAll(const QVarLengthArray<int> &idxs,
    ...
    */
    template <class T>
    template <class C>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::updateAll(const C &idxs, const QVector<QVector<T>> &arrays)
    {
        if (idxs.count() != arrays.count())
            throw SireError::incompatible_error(
                QObject::tr("You cannot update the arrays because the number of indicies (%1) "
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::append(const Array &array)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::append(const PackedArray2D<T> &arrays)
    {
    ...
    /** Append the passed array onto the end of the array of arrays */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::append(const QVector<T> &array)
    {
        this->operator=(PackedArray2D<T>(*this, array));
    }
    ...
    /** Append the passed arrays onto the end of the array of arrays */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::append(const QVector<QVector<T>> &arrays)
    {
        this->operator=(PackedArray2D<T>(*this, arrays));
    }
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::remove(quint32 idx)
    {
        ...
        detail::SharedArray2DPtr<detail::PackedArray2DData<T>> new_d = SireBase::detail::createArray<T>(narrays, nvals);
        ...
        for (int i = 0; i < this->nArrays(); ++i)
        {
            if (i != int_idx)
            {
                quickCopy(values_array, array_data[i].constData(), array_data[i].count());
    ...
    */
    template <class T>
    template <class C>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::removeAll(const C &idxs)
    {
        if (idxs.isEmpty())
            return;
        ...
        for (QVector<bool>::const_iterator it = to_keep.constBegin(); it != to_keep.constEnd(); ++it)
        {
            if (*it)
                narrays += 1;
        }
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void PackedArray2D<T>::removeAll(const QVarLengthArray<int> &idxs)
    {
        if (idxs.isEmpty())
            return;
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds,
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(
    QDataStream &ds, const SireBase::detail::SharedArray2DPtr<SireBase::detail::PackedArray2DData<T>> &d)
{
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(
    QDataStream &ds, SireBase::detail::SharedArray2DPtr<SireBase::detail::PackedArray2DData<T>> &d)
{
    SireStream::SharedDataStream sds(ds);
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE SireStream::SharedDataStream &operator>>(
    SireStream::SharedDataStream &sds, SireBase::detail::SharedArray2DPtr<SireBase::detail::PackedArray2DData<T>> &d)
{
    sds.sharedLoad<SireBase::detail::SharedArray2DPtr<SireBase::detail::PackedArray2DData<T>>,
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE SireStream::SharedDataStream &operator<<(
    SireStream::SharedDataStream &sds,
    const SireBase::detail::SharedArray2DPtr<SireBase::detail::PackedArray2DData<T>> &d)
{
    sds.sharedSave<SireBase::detail::SharedArray2DPtr<SireBase::detail::PackedArray2DData<T>>,
                   SireBase::detail::GetPackedArrayPointer<T>>(d);
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireBase::PackedArray2D<T> &arrays)
{
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireBase::PackedArray2D<T> &arrays)
{
