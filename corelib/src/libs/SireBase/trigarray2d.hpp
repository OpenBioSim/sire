...
namespace SireBase
{
    ...
    */
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE TrigArray2D<T>::TrigArray2D() : TrigArray2DBase()
    {
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE TrigArray2D<T>::TrigArray2D(int dimension) : TrigArray2DBase(dimension)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE TrigArray2D<T>::TrigArray2D(int dimension, const T &default_value) : TrigArray2DBase(dimension)
    {
        if (TrigArray2DBase::nRows() > 0)
        {
            const int dim = TrigArray2D::nRows();
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE TrigArray2D<T>::TrigArray2D(const TrigArray2D<T> &other)
    ...
    /** Destructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE TrigArray2D<T>::~TrigArray2D()
    {
    }
    ...
    /** Copy assignment operator - fast as this class is implicitly
        shared */
    template <class T>
    SIRE_INLINE_TEMPLATE TrigArray2D<T> &TrigArray2D<T>::operator=(const TrigArray2D<T> &other)
    {
    ...
    /** Comparison operator */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE bool TrigArray2D<T>::operator==(const TrigArray2D<T> &other) const
    {
    ...
    */
    template <class T>
    SIRE_INLINE_TEMPLATE const T &TrigArray2D<T>::operator()(int i, int j) const
    {
    ...
    */
    template <class T>
    SIRE_INLINE_TEMPLATE T &TrigArray2D<T>::operator()(int i, int j)
    {
    ...
    */
    template <class T>
    SIRE_INLINE_TEMPLATE const T &TrigArray2D<T>::at(int i, int j) const
    {
        return TrigArray2D<T>::operator()(i, j);
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void TrigArray2D<T>::set(int i, int j, const T &value)
    {
        this->operator()(i, j) = value;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T &TrigArray2D<T>::get(int i, int j) const
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void TrigArray2D<T>::redimension(int dimension)
    {
    ...
    template <class T>
    SIRE_INLINE_TEMPLATE const T *TrigArray2D<T>::data() const
    {
        return array.data();
    }
    ...
    /** Return a raw pointer into the data of the array. Remember
        that the data is in row-major order */
    template <class T>
    SIRE_INLINE_TEMPLATE T *TrigArray2D<T>::data()
    {
        return array.data();
    }
    ...
    /** Return a raw pointer into the data of the array. Remember
        that the data is in row-major order */
    template <class T>
    SIRE_INLINE_TEMPLATE const T *TrigArray2D<T>::constData() const
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE TrigArray2D<T> TrigArray2D<T>::transpose() const
    {
        return *this;
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void TrigArray2D<T>::setAll(const T &value)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE QString TrigArray2D<T>::toString() const
    {
        if (this->nRows() == 0)
            return "( )";
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireBase::TrigArray2D<T> &array)
{
    SireStream::SharedDataStream sds(ds);

    sds << static_cast<const SireBase::TrigArray2DBase &>(array) << array.array;
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireBase::TrigArray2D<T> &array)
{
    SireStream::SharedDataStream sds(ds);

    sds >> static_cast<SireBase::TrigArray2DBase &>(array) >> array.array;
