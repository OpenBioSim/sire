...
namespace SireBase
{
    ...
    */
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PairMatrix<T>::PairMatrix() : array(0), n_outer(0), n_inner(0), n_elements(0)
    {
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PairMatrix<T>::PairMatrix(unsigned int i, unsigned int j)
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PairMatrix<T>::PairMatrix(const PairMatrix<T> &other)
        : n_outer(other.n_outer), n_inner(other.n_inner), n_elements(other.n_elements)
    {
        if (n_elements > 0)
        {
            array = new T[n_elements];
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE PairMatrix<T>::~PairMatrix()
    {
        delete[] array;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE unsigned int PairMatrix<T>::nOuter() const
    {
        return n_outer;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE unsigned int PairMatrix<T>::nInner() const
    {
        return n_inner;
    }

    template <class T>
    SIRE_INLINE_TEMPLATE const T &PairMatrix<T>::operator()(unsigned int i, unsigned int j) const
    {
        return array[i * n_inner + j];
    }

    template <class T>
    SIRE_INLINE_TEMPLATE T &PairMatrix<T>::operator()(unsigned int i, unsigned int j)
    {
        return array[i * n_inner + j];
    }

    template <class T>
    SIRE_INLINE_TEMPLATE const T &PairMatrix<T>::operator()(unsigned int j) const
    {
        return array[outer_index + j];
    }

    template <class T>
    SIRE_INLINE_TEMPLATE T &PairMatrix<T>::operator()(unsigned int j)
    {
        return array[outer_index + j];
    }
    ...
    template <class T>
    SIRE_INLINE_TEMPLATE void PairMatrix<T>::setOuterIndex(unsigned int i)
    {
        outer_index = n_inner * i;
    }

    template <class T>
    SIRE_INLINE_TEMPLATE const T &PairMatrix<T>::at(unsigned int i, unsigned int j) const
    {
        return PairMatrix::operator()(i, j);
    }

    template <class T>
    SIRE_INLINE_TEMPLATE void PairMatrix<T>::redimension(unsigned int i, unsigned int j)
    {
        n_outer = i;
        n_inner = j;
