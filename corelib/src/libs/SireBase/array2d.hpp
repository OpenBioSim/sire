...
namespace SireBase
{
    ...
    */
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Array2D<T>::Array2D() : Array2DBase()
    {
    }

    /** Construct a Array2D that holds nrow rows of ncolumn columns. */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Array2D<T>::Array2D(int nrows, int ncolumns) : Array2DBase(nrows, ncolumns)
    {
    ...
    /** Construct a Array2D that holds nrow rows of ncolumn columns. */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Array2D<T>::Array2D(int nrows, int ncolumns, const T &default_value)
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Array2D<T>::Array2D(const Array2D<T> &other) : Array2DBase(other), array(other.array)
    {
    }

    /** Destructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Array2D<T>::~Array2D()
    {
    }
    ...
    /** Copy assignment operator - fast as this class is implicitly
        shared */
    template <class T>
    SIRE_INLINE_TEMPLATE Array2D<T> &Array2D<T>::operator=(const Array2D<T> &other)
    {
    ...
    /** Comparison operator */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE bool Array2D<T>::operator==(const Array2D<T> &other) const
    {
    ...
    */
    template <class T>
    SIRE_INLINE_TEMPLATE const T &Array2D<T>::operator()(int i, int j) const
    {
    ...
    */
    template <class T>
    SIRE_INLINE_TEMPLATE T &Array2D<T>::operator()(int i, int j)
    {
        Array2DBase::assertValidIndex(i, j);
    ...
    */
    template <class T>
    SIRE_INLINE_TEMPLATE const T &Array2D<T>::at(int i, int j) const
    {
        return Array2D<T>::operator()(i, j);
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void Array2D<T>::set(int i, int j, const T &value)
    {
        this->operator()(i, j) = value;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const T &Array2D<T>::get(int i, int j) const
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void Array2D<T>::redimension(int nrows, int ncolumns)
    {
        if (nrows < 0)
            nrows = 0;
    ...
    template <class T>
    SIRE_INLINE_TEMPLATE const T *Array2D<T>::data() const
    {
        return array.data();
    }
    ...
    /** Return a raw pointer into the data of the array. Remember
        that the data is in row-major order */
    template <class T>
    SIRE_INLINE_TEMPLATE T *Array2D<T>::data()
    {
        return array.data();
    }
    ...
    /** Return a raw pointer into the data of the array. Remember
        that the data is in row-major order */
    template <class T>
    SIRE_INLINE_TEMPLATE const T *Array2D<T>::constData() const
    {
    ...
    template <class T>
    SIRE_INLINE_TEMPLATE const T *Array2D<T>::row(int i) const
    {
    ...
    /** Return a raw pointer to the first item in row 'i' */
    template <class T>
    SIRE_INLINE_TEMPLATE T *Array2D<T>::row(int i)
    {
        return array.data() + Array2DBase::map(i, 0);
    }
    ...
    /** Return a raw pointer to the first item in row 'i' */
    template <class T>
    SIRE_INLINE_TEMPLATE const T *Array2D<T>::constRow(int i) const
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Array2D<T> Array2D<T>::transpose() const
    {
        if (array.isEmpty())
            return Array2D<T>();
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void Array2D<T>::setAll(const T &value)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE QString Array2D<T>::toString() const
    {
        if (this->nRows() == 0)
            return "( )";
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireBase::Array2D<T> &array)
{
    SireStream::SharedDataStream sds(ds);

    sds << static_cast<const SireBase::Array2DBase &>(array) << array.array;
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireBase::Array2D<T> &array)
{
    SireStream::SharedDataStream sds(ds);

    sds >> static_cast<SireBase::Array2DBase &>(array) >> array.array;
