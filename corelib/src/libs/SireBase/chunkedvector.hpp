...
namespace SireBase
{
    ...
    */
    ...
    class ChunkedVector
    {
        ...
        friend const void *SireBase::detail::get_shared_container_pointer<>(const ChunkedVector<T, N> &);

    public:
        class iterator
        {
        public:
            iterator() : end_chunk(0), current_chunk(0), end_item(0), current_item(0)
            {
            }

            iterator(const iterator &other)
                : end_chunk(other.end_chunk), current_chunk(other.current_chunk), end_item(other.end_item),
        ...
        {
        public:
            const_iterator() : end_chunk(0), current_chunk(0), end_item(0), current_item(0)
            {
            }
    ...
    /** Empty constructor */
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N>::ChunkedVector()
    {
    }

    /** Construct a chunked vector of size 'size' */
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N>::ChunkedVector(int size)
    {
        if (size <= 0)
            return;
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N>::ChunkedVector(int size, const T &value)
    {
        if (size <= 0)
            return;
    ...
    /** Copy constructor */
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N>::ChunkedVector(const ChunkedVector<T, N> &other) : _chunks(other._chunks)
    {
    }
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N> &ChunkedVector<T, N>::operator=(const ChunkedVector<T, N> &other)
    {
    ...
    /** Comparison operator */
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedVector<T, N>::operator==(const ChunkedVector<T, N> &other) const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::assertValidIndex(int i) const
    {
        int c = this->count();
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE T &ChunkedVector<T, N>::operator[](int i)
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE const T &ChunkedVector<T, N>::operator[](int i) const
    {
        this->assertValidIndex(i);
    ...
    template <class T, int N>
    SIRE_INLINE_TEMPLATE const T &ChunkedVector<T, N>::at(int i) const
    {
        return this->_chunks[i / N][i % N];
    }
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE T ChunkedVector<T, N>::value(int i) const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE T ChunkedVector<T, N>::value(int i, const T &default_value) const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedVector<T, N>::nChunks() const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedVector<T, N>::nFullChunks() const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedVector<T, N>::nRemainder() const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::append(const T &value)
    {
        int nchunks = this->nChunks();
        int nfullchunks = this->nFullChunks();
    ...
    /** Remove the item at index 'i' from this vector - this can be
        slow (O[n]) */
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::remove(int i)
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::remove(int i, int count)
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedVector<T, N>::count() const
    {
        int nfullchunks = this->nFullChunks();
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE bool ChunkedVector<T, N>::isEmpty() const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedVector<T, N>::count(const T &value) const
    {
        int size = 0;
        int nchunks = this->_chunks.count();
        const QVector<T> *chunks_array = this->_chunks.constData();
    ...
    /** Return the number of items in this vector */
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedVector<T, N>::size() const
    {
        return this->count();
    }
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE int ChunkedVector<T, N>::capacity() const
    {
        int nfullchunks = this->nFullChunks();
        int nchunks = this->nChunks();
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::clear()
    {
        this->_chunks.clear();
    }
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::resize(int new_count)
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::squeeze()
    {
        int nchunks = this->nChunks();
        int nfullchunks = this->nFullChunks();
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE void ChunkedVector<T, N>::reserve(int count)
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE QVector<T> ChunkedVector<T, N>::toVector() const
    {
        int size = this->count();
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE QList<T> ChunkedVector<T, N>::toList() const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE std::vector<T> ChunkedVector<T, N>::toStdVector() const
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N> ChunkedVector<T, N>::fromVector(const QVector<T> &vector)
    {
        if (vector.isEmpty())
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N> ChunkedVector<T, N>::fromList(const QList<T> &list)
    {
    ...
    template <class T, int N>
    SIRE_OUTOFLINE_TEMPLATE ChunkedVector<T, N> ChunkedVector<T, N>::fromStdVector(const std::vector<T> &vector)
    {
    ...
    namespace detail
    {
        ...
        template <class T, int N>
        SIRE_OUTOFLINE_TEMPLATE const void *get_shared_container_pointer(const SireBase::ChunkedVector<T, N> &vector)
        {
            if (vector.count() == 0)
                return 0;
            else
...
template <class T, int N>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireBase::ChunkedVector<T, N> &vec)
{
    ...
    for (typename SireBase::ChunkedVector<T, N>::const_iterator it = vec.constBegin(); it != vec.constEnd(); ++it)
    {
        ds << *it;
    }
...
template <class T, int N>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireBase::ChunkedVector<T, N> &vec)
{
...
SIRE_OUTOFLINE_TEMPLATE SireStream::SharedDataStream &operator<<(SireStream::SharedDataStream &sds,
                                                                 const SireBase::ChunkedVector<T, N> &vector)
{
    sds.sharedSaveContainer<SireBase::ChunkedVector<T, N>, SireBase::detail::GetChunkedVectorPointer<T, N>>(vector);
...
template <class T, int N>
SIRE_OUTOFLINE_TEMPLATE SireStream::SharedDataStream &operator>>(SireStream::SharedDataStream &sds,
                                                                 SireBase::ChunkedVector<T, N> &vector)
{
    sds.sharedLoadContainer<SireBase::ChunkedVector<T, N>, SireBase::detail::GetChunkedVectorPointer<T, N>>(vector);
...
template class SireBase::ChunkedVector<double, 100>;
#endif
