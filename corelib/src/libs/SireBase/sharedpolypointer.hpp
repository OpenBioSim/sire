...
namespace SireBase
{
    ...
    */
    ...
    public:
        ...
        static void save(QDataStream &ds, const char *objname, const void *data);

        static void *read(QDataStream &ds, void *data, const char *objname);

    protected:
        static void throwInvalidCast(const QString &newtype, const QString &oldtype);
    };
    ...
    */
    ...
    /** Null constructor */
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer() : SharedPolyPointerBase(), d(0)
    {
    }
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(T *adata) : SharedPolyPointerBase(), d(adata)
    {
        if (d)
            d->ref.ref();
    }
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(const T &obj) : SharedPolyPointerBase()
    {
    ...
    /** Copy constructor */
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(const SharedPolyPointer<T> &other)
    ...
    /** Move constructor */
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(SharedPolyPointer<T> &&other)
        : SharedPolyPointerBase(), d(other.d)
    {
        other.d = 0;
    }
    ...
    */
    template <class T>
    template <class S>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(const SharedPolyPointer<S> &other) : d(0)
    {
        ...
        if (other_ptr)
        {
            ...
            if (d)
                ...
                throwInvalidCast(SharedPolyPointerHelper<S>::what(*other), SharedPolyPointerHelper<T>::typeName());
        }
    }
    ...
    */
    template <class T>
    template <class S>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(SharedPolyPointer<S> &&other) : d(0)
    {
        S *other_ptr = const_cast<S *>(other.constData());
    ...
    */
    template <class T>
    template <class S>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(S *data) : d(dynamic_cast<T *>(data))
    {
        if (data)
        {
            if (d)
                ...
                throwInvalidCast(SharedPolyPointerHelper<S>::what(*data), SharedPolyPointerHelper<T>::typeName());
        }
    }
    ...
    */
    template <class T>
    template <class S>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::SharedPolyPointer(const S &obj)
    {
        T *obj_ptr = dynamic_cast<T *>(const_cast<S *>(&obj));

        if (!obj_ptr)
            throwInvalidCast(SharedPolyPointerHelper<S>::what(obj), SharedPolyPointerHelper<T>::typeName());
    ...
    /** Destructor */
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::~SharedPolyPointer()
    {
        if (d && !d->ref.deref())
            delete d;
        d = 0;
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(int)
    {
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(T *ptr)
    {
        if (ptr != d)
        {
            if (ptr)
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(const T &obj)
    {
        if (d != &obj)
        {
            T *obj_ptr = const_cast<T *>(&obj);
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(const SharedPolyPointer<T> &other)
    {
        if (other.d != d)
        {
            if (other.d)
    ...
    */
    template <class T>
    template <class S>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(const SharedPolyPointer<S> &other)
    {
        S *other_ptr = const_cast<S *>(other.constData());
        ...
        if (other_ptr and (void *) d != (void *)other_ptr)
        {
            ...
            if (!obj_ptr)
                throwInvalidCast(SharedPolyPointerHelper<S>::what(*obj_ptr), SharedPolyPointerHelper<T>::typeName());
    ...
    */
    template <class T>
    template <class S>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(S *ptr)
    {
        if (ptr and ptr != d)
        {
            ...
            if (!obj_ptr)
                throwInvalidCast(SharedPolyPointerHelper<S>::what(*ptr), SharedPolyPointerHelper<T>::typeName());
    ...
    */
    template <class T>
    template <class S>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(const S &obj)
    {
        if (d != &obj)
        {
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T> &SharedPolyPointer<T>::operator=(SharedPolyPointer<T> &&other)
    {
        qSwap(d, other.d);
        return *this;
    }
    ...
    template <class T>
    Q_OUTOFLINE_TEMPLATE void SharedPolyPointer<T>::detach_helper()
    {
        T *x = d->clone();
        x->ref.ref();
        if (!d->ref.deref())
    ...
    template <class T>
    Q_INLINE_TEMPLATE void SharedPolyPointer<T>::detach()
    {
        if (d && d->ref.hasMultipleReferences())
            detach_helper();
    }

    /** Return whether or not this pointer is unique (there are no copies) */
    template <class T>
    SIRE_INLINE_TEMPLATE bool SharedPolyPointer<T>::unique() const
    {
    ...
    template <class T>
    Q_INLINE_TEMPLATE T &SharedPolyPointer<T>::operator*()
    {
        detach();
        return *d;
    }
    ...
    /** Dereference this pointer */
    template <class T>
    Q_INLINE_TEMPLATE const T &SharedPolyPointer<T>::operator*() const
    {
        return *d;
    }
    ...
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::operator T *()
    {
        detach();
        return d;
    }
    ...
    /** Cast back to a normal pointer */
    template <class T>
    Q_INLINE_TEMPLATE SharedPolyPointer<T>::operator const T *() const
    {
        return d;
    }
    ...
    template <class T>
    Q_INLINE_TEMPLATE T *SharedPolyPointer<T>::data()
    {
        detach();
        return d;
    }
    ...
    /** Return the pointer held by this shared pointer */
    template <class T>
    Q_INLINE_TEMPLATE const T *SharedPolyPointer<T>::data() const
    {
        return d;
    }
    ...
    /** Return the pointer held by this shared pointer */
    template <class T>
    Q_INLINE_TEMPLATE const T *SharedPolyPointer<T>::constData() const
    {
        return d;
    }
    ...
    template <class T>
    Q_INLINE_TEMPLATE bool SharedPolyPointer<T>::operator!() const
    {
        return !d;
    }
    ...
    /** Comparison operator */
    template <class T>
    Q_INLINE_TEMPLATE bool SharedPolyPointer<T>::operator==(const SharedPolyPointer<T> &other) const
    {
        return d == other.d;
    }
    ...
    /** Comparison operator */
    template <class T>
    Q_INLINE_TEMPLATE bool SharedPolyPointer<T>::operator==(const T *other_ptr) const
    {
        return d == other_ptr;
    }
...
*/
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireBase::SharedPolyPointer<T> &ptr)
{
    ...
    else
    {
        ...
        SireBase::SharedPolyPointerBase::save(ds, SireBase::SharedPolyPointerHelper<T>::what(*(ptr.d)), ptr.d);
    }
...
template <class T>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireBase::SharedPolyPointer<T> &ptr)
{
    SireStream::VersionID v =
        SireStream::readHeader(ds, SireBase::sharedpolypointer_magic, "SireBase::SharedPolyPointer");
    ...
    if (v == 1)
    {
        if (ptr.d)
        {
            ...
            ptr = static_cast<T *>(
                SireBase::SharedPolyPointerBase::read(ds, ptr.d, SireBase::SharedPolyPointerHelper<T>::what(*(ptr.d))));
        }
        else
        {
            ptr = static_cast<T *>(SireBase::SharedPolyPointerBase::read(ds, 0, 0));
        }
    }
    else
