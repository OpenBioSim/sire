...
namespace SireBase
{
    ...
    */
    ...
    {
        if (not ptr)
            throw SireError::nullptr_error(QObject::tr("Cannot cast a null pointer!"), CODELOC);

        const S *sptr = dynamic_cast<const S *>(ptr.constData());

        if (not sptr)
            throw SireError::invalid_cast(
                QObject::tr("Cannot cast a SharedPolyPointer of type \"%1\" to "
                            "type \"%2\".")
                    .arg(SharedPolyPointerHelper<T>::what(*ptr), SharedPolyPointerHelper<S>::typeName()),
                CODELOC);
