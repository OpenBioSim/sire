...
QDataStream &operator<<(QDataStream &ds, const CoulombProbe &cprobe)
{
    writeHeader(ds, r_cprobe, 1);

    ds << cprobe.chg.to(mod_electron) << static_cast<const Probe &>(cprobe);
...
CoulombProbe::CoulombProbe() : ConcreteProperty<CoulombProbe, Probe>(), chg(1 * mod_electron)
{
    reduced_chg = ::getCharge(chg);
}

/** Construct a probe with charge 'charge' */
CoulombProbe::CoulombProbe(Charge charge) : ConcreteProperty<CoulombProbe, Probe>(), chg(charge)
{
    reduced_chg = ::getCharge(chg);
}

/** Construct a probe with charge taken from 'cljprobe' */
CoulombProbe::CoulombProbe(const CLJProbe &cljprobe) : ConcreteProperty<CoulombProbe, Probe>(), chg(cljprobe.charge())
{
    reduced_chg = ::getCharge(chg);
}

/** Construct from the passed probe */
CoulombProbe::CoulombProbe(const Probe &probe) : ConcreteProperty<CoulombProbe, Probe>(), chg(0)
{
...
CoulombProbe::CoulombProbe(const CoulombProbe &other)
    : ConcreteProperty<CoulombProbe, Probe>(other), chg(other.chg), reduced_chg(other.reduced_chg)
{
}
...
LJProbe::LJProbe(const LJParameter &lj) : ConcreteProperty<LJProbe, Probe>(), ljparam(lj)
{
}

/** Construct to take the LJ probe from the passed CLJProbe */
LJProbe::LJProbe(const CLJProbe &cljprobe) : ConcreteProperty<LJProbe, Probe>(), ljparam(cljprobe.lj())
{
}

/** Construct from the passed probe */
LJProbe::LJProbe(const Probe &probe) : ConcreteProperty<LJProbe, Probe>()
{
...
/** Copy constructor */
LJProbe::LJProbe(const LJProbe &other) : ConcreteProperty<LJProbe, Probe>(other), ljparam(other.ljparam)
{
}
...
QDataStream &operator<<(QDataStream &ds, const CLJProbe &cljprobe)
{
    ...
    ds << cljprobe.chg.to(mod_electron) << cljprobe.ljparam << static_cast<const Probe &>(cljprobe);
...
CLJProbe::CLJProbe() : ConcreteProperty<CLJProbe, Probe>(), ljparam(ua_methane), chg(1 * mod_electron)
{
    reduced_chg = ::getCharge(chg);
}
...
CLJProbe::CLJProbe(SireUnits::Dimension::Charge charge)
    : ConcreteProperty<CLJProbe, Probe>(), ljparam(LJParameter::dummy()), chg(charge)
{
    reduced_chg = ::getCharge(chg);
}

/** Construct a probe that is just the passed LJ parameter (zero charge) */
CLJProbe::CLJProbe(const LJParameter &lj) : ConcreteProperty<CLJProbe, Probe>(), ljparam(lj), chg(0), reduced_chg(0)
{
}

/** Construct a probe with the passed charge and LJ parameter */
CLJProbe::CLJProbe(Charge charge, const LJParameter &lj) : ConcreteProperty<CLJProbe, Probe>(), ljparam(lj), chg(charge)
{
    reduced_chg = ::getCharge(chg);
}
...
CLJProbe::CLJProbe(const CoulombProbe &probe)
    : ConcreteProperty<CLJProbe, Probe>(), ljparam(LJParameter::dummy()), chg(probe.charge())
{
    reduced_chg = ::getCharge(chg);
}
...
CLJProbe::CLJProbe(const LJProbe &probe)
    : ConcreteProperty<CLJProbe, Probe>(), ljparam(probe.lj()), chg(0), reduced_chg(0)
{
}

/** Copy constructor */
CLJProbe::CLJProbe(const CLJProbe &other)
    : ConcreteProperty<CLJProbe, Probe>(other), ljparam(other.ljparam), chg(other.chg), reduced_chg(other.reduced_chg)
{
}
...
/** Construct from the passed probe */
CLJProbe::CLJProbe(const Probe &other) : ConcreteProperty<CLJProbe, Probe>(), chg(0), reduced_chg(0)
{
...
{
    return reduced_chg == other.reduced_chg and ljparam == other.ljparam and Probe::operator==(other);
}
