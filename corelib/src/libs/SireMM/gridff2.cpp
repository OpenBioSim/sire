...
QDataStream &operator<<(QDataStream &ds, const GridFF2 &gridff2)
{
    ...
    sds << gridff2.gridbox << gridff2.dimx << gridff2.dimy << gridff2.dimz << gridff2.gridpot << gridff2.buffer_size
        << gridff2.grid_spacing << gridff2.coul_cutoff << gridff2.lj_cutoff << gridff2.fixedatoms_coords
        << gridff2.fixedatoms_params << gridff2.closemols_coords << gridff2.closemols_params;

    sds << quint32(gridff2.oldnrgs.count());

    for (QHash<MolNum, CLJEnergy>::const_iterator it = gridff2.oldnrgs.constBegin(); it != gridff2.oldnrgs.constEnd();
         ++it)
    {
...
QDataStream &operator>>(QDataStream &ds, GridFF2 &gridff2)
{
    ...
    if (v == 1)
    {
        ...
        sds >> gridff2.gridbox >> gridff2.dimx >> gridff2.dimy >> gridff2.dimz >> gridff2.gridpot >>
            gridff2.buffer_size >> gridff2.grid_spacing >> gridff2.coul_cutoff >> gridff2.lj_cutoff >>
            gridff2.fixedatoms_coords >> fixedatoms_params >> gridff2.closemols_coords >> closemols_params;
...
/** Empty constructor */
GridFF2::GridFF2()
    : ConcreteProperty<GridFF2, InterGroupCLJFF>(), buffer_size(2.5), grid_spacing(1.0), coul_cutoff(50), lj_cutoff(7.5)
{
...
GridFF2::GridFF2(const QString &name)
    : ConcreteProperty<GridFF2, InterGroupCLJFF>(name), buffer_size(2.5), grid_spacing(1.0), coul_cutoff(50),
      lj_cutoff(7.5)
{
    this->setSwitchingFunction(NoCutoff());
}
...
GridFF2::GridFF2(const GridFF2 &other)
    : ConcreteProperty<GridFF2, InterGroupCLJFF>(other), gridbox(other.gridbox), buffer_size(other.buffer_size),
...
SIRE_ALWAYS_INLINE GridFF2::Vector4::Vector4(const Vector &v, double chg) : x(v.x()), y(v.y()), z(v.z()), q(chg)
{
}

void GridFF2::appendTo(QVector<GridFF2::Vector4> &coords_and_charges, const Vector *coords,
...
static void arrayIndexToGridIndex(int ipt, int dimx, int dimy, int dimz, int &ix, int &iy, int &iz)
{
    ix = ipt / (dimy * dimz);
    ipt -= ix * dimy * dimz;
...
static void getGridPoint(int ipt, const Vector &min, int dimx, int dimy, int dimz, double grid_spacing, MultiFloat &x,
                         MultiFloat &y, MultiFloat &z)
{
    int ix, iy, iz;
...
void GridFF2::addToGrid(const QVector<float> &vx, const QVector<float> &vy, const QVector<float> &vz,
                        const QVector<float> &vq)
{
    ...
    else if (useReactionField())
    {
        ...
        const MultiFloat k_rf((1.0 / pow_3(coul_cutoff)) *
                              ((reactionFieldDielectric() - 1) / (2 * reactionFieldDielectric() + 1)));
        const MultiFloat c_rf((1.0 / coul_cutoff) *
                              ((3 * reactionFieldDielectric()) / (2 * reactionFieldDielectric() + 1)));
...
void GridFF2::rebuildGrid()
{
    ...
    if (lj_cutoff > coul_cutoff)
    {
        ...
        throw SireError::invalid_state(QObject::tr("You cannot have a grid forcefield with a longer LJ cutoff (%1 A) "
    ...
    if (fixedatoms_coords.count() > 0)
    {
        for (int i = 0; i < fixedatoms_coords.count(); ++i)
        {
            ...
            QVector<Vector> coords = spce.getImagesWithin(fixedatoms_coords.constData()[i], grid_center, image_cutoff);
            ...
            for (int igroup = 0; igroup < ngroups; ++igroup)
            {
                ...
                for (int i = 0; i < coordgroup.count(); ++i)
                {
                    ...
                    QVector<Vector> coords =
                        spce.getImagesWithin(fixedatoms_coords.constData()[i], grid_center, image_cutoff);
...
GridFF2::CLJAtoms::CLJAtoms(const CoordGroup &coords, const GridFF2::CLJParameters::Array &params)
{
...
void GridFF2::calculateEnergy(const CLJAtoms &atoms0, const CLJAtoms &atoms1, double &return_cnrg, double &return_ljnrg)
{
...
void GridFF2::calculateEnergy(const CoordGroup &coords0, const GridFF2::CLJParameters::Array &params0,
...
void GridFF2::_pvt_added(quint32 groupid, const PartialMolecule &mol, const PropertyMap &map)
{
    this->mustNowRecalculateFromScratch();
...
void GridFF2::_pvt_changed(quint32 groupid, const SireMol::Molecule &molecule, bool auto_commit)
{
    InterGroupCLJFF::_pvt_changed(groupid, molecule, auto_commit);
}

/** Any changes to group 1 mean that the forcefield must be recalculated from scratch */
void GridFF2::_pvt_changed(quint32 groupid, const QList<SireMol::Molecule> &molecules, bool auto_commit)
{
