...
namespace SireMM
{
    namespace detail
    {
        template class IntraScaledParameters<CoulombNBPairs>;

        template class IntraScaledAtomicParameters<AtomicParameters3D<ChargeParameter>, IntraScaledParameters<CoulombNBPairs>>;
    } // namespace detail
} // namespace SireMM
...
namespace SireFF
{
    namespace detail
    {
        template class AtomicParameters3D<ChargeParameter>;

        template class FFMolecule3D<InterCoulombPotential>;

        template class FFMolecules3D<InterCoulombPotential>;

        template class ChangedMolecule<InterCoulombPotential::Molecule>;

        template class FFMolecule3D<IntraCoulombPotential>;

        template class FFMolecules3D<IntraCoulombPotential>;

        template class ChangedMolecule<IntraCoulombPotential::Molecule>;
    } // namespace detail
} // namespace SireFF

/** Streaming functions for ChargeParameter */
QDataStream &operator<<(QDataStream &ds, const SireMM::detail::ChargeParameter &chgparam)
{
...
static PackedArray2D<ChargeParameter> getChargeParameters(const PartialMolecule &molecule,
...
static const RegisterMetaType<CoulombPotential> r_coulpot(MAGIC_ONLY, NO_ROOT, "SireMM::CoulombPotential");

/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const CoulombPotential &coulpot)
{
...
QDataStream &operator>>(QDataStream &ds, CoulombPotential &coulpot)
{
    ...
    if (v == 1)
    {
        ...
        coulpot.switchfunc = coulpot.props.property("switchingFunction").asA<SwitchingFunction>();

        coulpot.use_electrostatic_shifting = coulpot.props.property("shiftElectrostatics").asABoolean();
    }
    else
...
CoulombPotential::CoulombPotential()
    : spce(Space::null()), switchfunc(SwitchingFunction::null()), use_electrostatic_shifting(false)
{
    // record the defaults
...
CoulombPotential::CoulombPotential(const CoulombPotential &other)
    : props(other.props), spce(other.spce), switchfunc(other.switchfunc),
...
/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const InterCoulombPotential &intercoul)
{
...
/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, InterCoulombPotential &intercoul)
{
...
/** Copy constructor */
InterCoulombPotential::InterCoulombPotential(const InterCoulombPotential &other) : CoulombPotential(other)
{
}
...
/** Copy assignment operator */
InterCoulombPotential &InterCoulombPotential::operator=(const InterCoulombPotential &other)
{
...
*/
InterCoulombPotential::Parameters InterCoulombPotential::getParameters(const PartialMolecule &molecule,
...
/** Update the parameters for the molecule going from 'old_molecule' to
    ...
    \throw SireError::incompatible_error
*/
InterCoulombPotential::Parameters InterCoulombPotential::updateParameters(
    const InterCoulombPotential::Parameters &old_params, const PartialMolecule &old_molecule,
    const PartialMolecule &new_molecule, const PropertyMap &map)
{
    ...
    bool new_chg = (old_molecule.version(chg_property) != new_molecule.version(chg_property));
    ...
    const PartialMolecule &new_molecule, const PropertyMap &old_map, const PropertyMap &new_map)
{
    ...
    bool changed_chg = (new_chg != old_chg) or (old_molecule.version(old_chg) != new_molecule.version(old_chg));

    if (changed_coords)
        new_params.setAtomicCoordinates(AtomicCoords3D(new_molecule, new_coords));

    if (changed_chg)
        new_params.setAtomicParameters(getChargeParameters(new_molecule, new_chg));
...
/** Return the InterCoulombPotential::Molecule representation of 'molecule',
    ...
    \throw SireError::incompatible_error
*/
InterCoulombPotential::Molecule InterCoulombPotential::parameterise(const PartialMolecule &molecule,
                                                                    const PropertyMap &map)
{
...
/** Convert the passed group of molecules into InterCoulombPotential::Molecules,
    ...
    \throw SireError::incompatible_error
*/
InterCoulombPotential::Molecules InterCoulombPotential::parameterise(const MoleculeGroup &molecules,
                                                                     const PropertyMap &map)
{
...
double InterCoulombPotential::totalCharge(const InterCoulombPotential::Parameters::Array &params) const
{
    int n = params.count();
...
void InterCoulombPotential::_pvt_calculateEnergy(const InterCoulombPotential::Molecule &mol0,
...
/** Add to the forces in 'forces0' the forces acting on 'mol0' caused
    by 'mol1' */
void InterCoulombPotential::_pvt_calculateCoulombForce(const InterCoulombPotential::Molecule &mol0,
                                                       const InterCoulombPotential::Molecule &mol1,
...
/** Add to the potentials in 'pots0' the coulomb potential acting on 'mol0' caused
    by 'mol1' */
void InterCoulombPotential::_pvt_calculateCoulombPotential(const InterCoulombPotential::Molecule &mol0,
                                                           const InterCoulombPotential::Molecule &mol1,
...
/** Add to the potentials in 'pots0' the coulomb potential on the passed
    grid caused by 'mol' */
void InterCoulombPotential::_pvt_calculateCoulombPotential(const InterCoulombPotential::Molecule &mol,
...
/** Calculate the coulomb field caused by the molecule 'mol' on the grid points in
    'fields' */
void InterCoulombPotential::_pvt_calculateCoulombField(const InterCoulombPotential::Molecule &mol,
...
void InterCoulombPotential::_pvt_calculateCoulombField(const InterCoulombPotential::Molecule &mol0,
                                                       const InterCoulombPotential::Molecule &mol1,
...
/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const IntraCoulombPotential &intracoul)
{
...
/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, IntraCoulombPotential &intracoul)
{
...
/** Copy constructor */
IntraCoulombPotential::IntraCoulombPotential(const IntraCoulombPotential &other) : CoulombPotential(other)
{
}
...
/** Return all of the parameters needed by this potential for
    ...
    \throw SireError::incompatible_error
*/
IntraCoulombPotential::Parameters IntraCoulombPotential::getParameters(const PartialMolecule &molecule,
                                                                       const PropertyMap &map)
{
    return Parameters(AtomicParameters3D<ChargeParameter>(molecule, map[parameters().coordinates()],
                                                          getChargeParameters(molecule, map[parameters().charge()])),
                      IntraScaledParameters<CoulombNBPairs>(molecule, map[parameters().intraScaleFactors()]));
}
...
/** Update the parameters for the molecule going from 'old_molecule' to
    ...
    \throw SireError::incompatible_error
*/
IntraCoulombPotential::Parameters IntraCoulombPotential::updateParameters(
    const IntraCoulombPotential::Parameters &old_params, const PartialMolecule &old_molecule,
    const PartialMolecule &new_molecule, const PropertyMap &map)
{
    if (old_molecule.selection() != new_molecule.selection())
        // the selection has changed - just get completely new parameters
    ...
    if (new_scl)
        new_params.setIntraScaleFactors(IntraScaledParameters<CoulombNBPairs>(new_molecule, scl_property));
    ...
    const PartialMolecule &new_molecule, const PropertyMap &old_map, const PropertyMap &new_map)
{
    ...
    if (changed_scl)
        new_params.setIntraScaleFactors(IntraScaledParameters<CoulombNBPairs>(new_molecule, new_scl));
...
/** Return the IntraCoulombPotential::Molecule representation of 'molecule',
    ...
    \throw SireError::incompatible_error
*/
IntraCoulombPotential::Molecule IntraCoulombPotential::parameterise(const PartialMolecule &molecule,
                                                                    const PropertyMap &map)
{
...
/** Concert the passed group of molecules into IntraCoulombPotential::Molecules,
    ...
    \throw SireError::incompatible_error
*/
IntraCoulombPotential::Molecules IntraCoulombPotential::parameterise(const MoleculeGroup &molecules,
                                                                     const PropertyMap &map)
{
...
/** Return the total charge of the parameters for the group in 'params' */
double IntraCoulombPotential::totalCharge(const IntraCoulombPotential::Parameters::Array &params) const
{
    int n = params.count();
    const Parameter *params_array = params.constData();
                                            ...
                                            const quint32 nats1, double &icnrg) const
{
    if (group_pairs.isEmpty())
    {
        ...
        for (quint32 i = 0; i < nats0; ++i)
        {
            ...
            for (quint32 j = 0; j < nats1; ++j)
            {
                icnrg += param0.reduced_charge * params1_array[j].reduced_charge / distmat[j];
            }
        }
    }
    else
        ...
        for (quint32 i = 0; i < nats0; ++i)
        {
            ...
            for (quint32 j = 0; j < nats1; ++j)
            {
                ...
                if (coulscl.coulomb() != 0)
                    icnrg += coulscl.coulomb() * param0.reduced_charge * params1_array[j].reduced_charge / distmat[j];
            }
        }
    }
}

void IntraCoulombPotential::calculateEnergy(const CoulombNBPairs::CGPairs &group_pairs, const QSet<Index> &atoms0,
                                            const QSet<Index> &atoms1, IntraCoulombPotential::EnergyWorkspace &distmat,
                                            const IntraCoulombPotential::Parameter *params0_array,
                                            const IntraCoulombPotential::Parameter *params1_array, const quint32 nats0,
                                            const quint32 nats1, double &icnrg) const
{
...
void IntraCoulombPotential::calculateEnergy(const IntraCoulombPotential::Molecule &mol,
                                            ...
                                            IntraCoulombPotential::EnergyWorkspace &distmat, double scale_energy) const
{
    ...
    for (quint32 igroup = 0; igroup < ngroups; ++igroup)
    {
        ...
        for (quint32 jgroup = igroup; jgroup < ngroups; ++jgroup)
        {
            ...
            const CoulombNBPairs::CGPairs &group_pairs = nbpairs(cgidx_igroup, cgidx_jgroup);
            ...
            calculateEnergy(group_pairs, distmat, params0_array, params1_array, nats0, nats1, icnrg);
...
void IntraCoulombPotential::calculateCoulombForce(const CoulombNBPairs::CGPairs &group_pairs, const CoordGroup &group0,
                                                  ...
                                                  const quint32 nats0, const quint32 nats1, const double shift_coul,
            ...
            for (quint32 i = 0; i < nats0; ++i)
            {
                ...
                for (quint32 j = 0; j < nats1; ++j)
                {
                    ...
                    if (coulscl.coulomb() != 0)
                    {
                        ...
                        {
                            // calculate the coulomb energy
                            const double cnrg = coulscl.coulomb() * scl_coul * q2 / distmat[j].length();
        ...
        else
        {
            ...
            for (quint32 i = 0; i < nats0; ++i)
            {
                ...
                for (quint32 j = 0; j < nats1; ++j)
                {
                    ...
                    double cscale = coulscl.coulomb() * params1_array[j].reduced_charge;
...
void IntraCoulombPotential::calculateCoulombForce(const CoulombNBPairs::CGPairs &group_pairs, const QSet<Index> &atoms0,
...
/** Calculate the coulomb forces between the atoms in the molecule 'mol'
    ...
    the passed workspace to perform the calculation */
void IntraCoulombPotential::calculateCoulombForce(const IntraCoulombPotential::Molecule &mol, MolForceTable &forces,
                                                  IntraCoulombPotential::ForceWorkspace &distmat,
                                                  double scale_force) const
{
...
/** Calculate the coulomb forces between the atoms in the molecule 'mol'
    and add these forces onto 'forces'. This uses
    the passed workspace to perform the calculation */
void IntraCoulombPotential::calculateForce(const IntraCoulombPotential::Molecule &mol, MolForceTable &forces,
                                           IntraCoulombPotential::ForceWorkspace &distmat, double scale_force) const
{
...
/** Calculate the coulomb force acting on the part of the molecule
    ...
    \throw SireError::incompatible_error
*/
void IntraCoulombPotential::calculateCoulombForce(const IntraCoulombPotential::Molecule &mol,
                                                  const IntraCoulombPotential::Molecule &rest_of_mol,
                                                  MolForceTable &forces, IntraCoulombPotential::ForceWorkspace &distmat,
                                                  double scale_force) const
{
...
/** Calculate the coulomb force acting on the part of the molecule
    ...
    \throw SireError::incompatible_error
*/
void IntraCoulombPotential::calculateForce(const IntraCoulombPotential::Molecule &mol,
...
void IntraCoulombPotential::calculateField(const IntraCoulombPotential::Molecule &mol, const CoulombProbe &probe,
                                           MolFieldTable &fields, IntraCoulombPotential::FieldWorkspace &workspace,
                                           double scale_field) const
{
    throw SireError::incomplete_code(QObject::tr("Calculating the intramolecular coulomb field has not "
...
void IntraCoulombPotential::calculateField(const IntraCoulombPotential::Molecule &mol,
                                           ...
                                           IntraCoulombPotential::FieldWorkspace &workspace, double scale_field) const
{
    throw SireError::incomplete_code(QObject::tr("Calculating the intramolecular coulomb field has not "
                                                 "yet been implemented."),
                                     CODELOC);
...
void IntraCoulombPotential::calculatePotential(const IntraCoulombPotential::Molecule &mol, const CoulombProbe &probe,
                                               MolPotentialTable &potentials,
                                               IntraCoulombPotential::PotentialWorkspace &workspace,
                                               double scale_potential) const
{
    throw SireError::incomplete_code(QObject::tr("Calculating the intramolecular coulomb potential has not "
                                                 "yet been implemented."),
                                     CODELOC);
}
...
void IntraCoulombPotential::calculatePotential(const IntraCoulombPotential::Molecule &mol,
                                               const IntraCoulombPotential::Molecule &rest_of_mol,
                                               const CoulombProbe &probe, MolPotentialTable &potentials,
                                               IntraCoulombPotential::PotentialWorkspace &workspace,
                                               double scale_potential) const
{
    throw SireError::incomplete_code(QObject::tr("Calculating the intramolecular coulomb potential has not "
...
void IntraCoulombPotential::calculateCoulombField(const IntraCoulombPotential::Molecule &mol, const CoulombProbe &probe,
...
void IntraCoulombPotential::calculateCoulombPotential(const IntraCoulombPotential::Molecule &mol,
                                                      const CoulombProbe &probe, GridPotentialTable &fields,
                                                      IntraCoulombPotential::PotentialWorkspace &workspace,
                                                      double scale_potential) const
{
    throw SireError::incomplete_code(QObject::tr("Calculating the intramolecular coulomb potential has not "
