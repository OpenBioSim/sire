...
namespace SireMM
{
    ...
    public:
        ...
        static QVector<CLJAtom> buildFrom(const MoleculeView &molecule, const PropertyMap &map = PropertyMap());
    ...
    */
    ...
    public:
        ...
        CLJAtoms(const QVector<Vector> &coordinates, const QVector<Charge> &charges, const QVector<LJParameter> &ljparams,
                 qint32 atomid = 1);

        CLJAtoms(const QVector<Vector> &coordinates, const QVector<Charge> &charges, const QVector<LJParameter> &ljparams,
                 const QVector<qint32> &ids);

        CLJAtoms(const MoleculeView &molecule, const PropertyMap &map = PropertyMap());

        CLJAtoms(const MoleculeView &molecule, ID_SOURCE id_source, const PropertyMap &map = PropertyMap());

        CLJAtoms(const Molecules &molecules, const PropertyMap &map = PropertyMap());

        CLJAtoms(const MoleculeGroup &molecules, const PropertyMap &map = PropertyMap());

        CLJAtoms(const Molecules &molecules, ID_SOURCE id_source, const PropertyMap &map = PropertyMap());

        CLJAtoms(const MoleculeGroup &molecules, ID_SOURCE id_source, const PropertyMap &map = PropertyMap());

        CLJAtoms(const CLJAtoms &other);

        ~CLJAtoms();

        void reconstruct(const MoleculeView &molecule, const PropertyMap &map = PropertyMap());

        void reconstruct(const MoleculeView &molecule, ID_SOURCE id_source, const PropertyMap &map = PropertyMap());
    ...
    private:
        void constructFrom(const MoleculeGroup &molecules, const ID_SOURCE id_source, const PropertyMap &map);
        void constructFrom(const Molecules &molecules, const ID_SOURCE id_source, const PropertyMap &map);
        void constructFrom(const MoleculeView &molecule, const ID_SOURCE id_source, const PropertyMap &map);
