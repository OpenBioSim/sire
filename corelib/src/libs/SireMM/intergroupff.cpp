...
namespace SireMM
{
    namespace detail
    {
        ...
        {
        public:
            InterGroupFFData() : RefCountData(), fixed_only(false), parallel_calc(true), repro_sum(false)
            {
            }

            InterGroupFFData(const InterGroupFFData &other)
                : RefCountData(), cljfuncs(other.cljfuncs), fixed_atoms(other.fixed_atoms), cljcomps(other.cljcomps),
                  props(other.props), fixed_only(other.fixed_only), parallel_calc(other.parallel_calc),
...
QDataStream &operator<<(QDataStream &ds, const InterGroupFF &groupff)
{
    ...
    sds << groupff.cljgroup[0] << groupff.cljgroup[1] << groupff.needs_accepting << groupff.d->cljfuncs
        << groupff.d->cljcomps << groupff.d->fixed_atoms << groupff.d->fixed_only << groupff.d->parallel_calc
        << groupff.d->repro_sum << static_cast<const G2FF &>(groupff);
...
QDataStream &operator>>(QDataStream &ds, InterGroupFF &groupff)
{
    ...
    if (v == 2)
    {
        ...
        sds >> groupff.cljgroup[0] >> groupff.cljgroup[1] >> groupff.needs_accepting >> groupff.d->cljfuncs >>
            groupff.d->cljcomps >> groupff.d->fixed_atoms >> groupff.d->fixed_only >> groupff.d->parallel_calc >>
            groupff.d->repro_sum >> static_cast<G2FF &>(groupff);
...
/** Constructor */
InterGroupFF::InterGroupFF() : ConcreteProperty<InterGroupFF, G2FF>(), needs_accepting(false)
{
...
InterGroupFF::InterGroupFF(const QString &name) : ConcreteProperty<InterGroupFF, G2FF>(), needs_accepting(false)
{
    cljgroup[0] = CLJGroup(CLJExtractor::EXTRACT_BY_CUTGROUP);
    cljgroup[1] = CLJGroup(CLJExtractor::EXTRACT_BY_CUTGROUP);
...
InterGroupFF::InterGroupFF(const InterGroupFF &other)
    : ConcreteProperty<InterGroupFF, G2FF>(other), d(other.d), needs_accepting(other.needs_accepting)
{
...
{
    if (d.constData()->cljfuncs.isEmpty())
        throw SireError::program_bug(QObject::tr("There should always be at least one CLJFunction in InterFF!"),
                                     CODELOC);
...
bool InterGroupFF::operator==(const InterGroupFF &other) const
{
    return (this == &other) or
           (G2FF::operator==(other) and d->fixed_atoms == other.d->fixed_atoms and d->cljfuncs == other.d->cljfuncs and
            d->cljcomps == other.d->cljcomps and cljgroup[0] == other.cljgroup[0] and
            cljgroup[1] == other.cljgroup[1] and needs_accepting == other.needs_accepting);
}
...
void InterGroupFF::recalculateEnergy()
{
    if (cljgroup[0].recalculatingFromScratch() or cljgroup[1].recalculatingFromScratch())
    {
        ...
        if (cljgroup[0].isEmpty() or (cljgroup[1].isEmpty() and d.constData()->fixed_atoms[0].isEmpty()))
        {
            // no atoms
        ...
        if (d.constData()->cljcomps.count() == 1)
        {
            ...
            if (not d.constData()->fixed_only)
            {
                if (d.constData()->parallel_calc)
                {
                    ...
                    nrgs = calc.calculate(cljFunction(), cljgroup[0].cljBoxes(), cljgroup[1].cljBoxes());
                }
                else
                {
                    nrgs = cljFunction().calculate(cljgroup[0].cljBoxes(), cljgroup[1].cljBoxes());
                }
            }
            ...
            if (not d.constData()->fixed_atoms[0].isEmpty())
            {
                ...
                tuple<double, double> grid_nrgs = d.constData()->fixed_atoms[0].calculate(cljgroup[0].cljBoxes());
        ...
        else
        {
            ...
            if (not d.constData()->fixed_only)
            {
                ...
                {
                    CLJCalculator calc(d->repro_sum);
                    nrgs = calc.calculate(d.constData()->cljfuncs, cljgroup[0].cljBoxes(), cljgroup[1].cljBoxes());
                }
                else
                {
                    nrgs = CLJFunction::multiCalculate(d.constData()->cljfuncs, cljgroup[0].cljBoxes(),
                                                       cljgroup[1].cljBoxes());
                }
            }
            else
            ...
            if (not d.constData()->fixed_atoms[0].isEmpty()) // if 0 is empty, they are all empty
            {
                ...
                for (int i = 0; i < d.constData()->fixed_atoms.count(); ++i)
                {
                    tuple<double, double> grid_nrgs = d.constData()->fixed_atoms[i].calculate(cljgroup[0].cljBoxes());
        ...
        if (d.constData()->cljfuncs.count() == 1)
        {
            ...
            if (not d.constData()->fixed_only)
            {
                ...
                if (d.constData()->parallel_calc)
                {
                    ...
                    delta_nrgs = calc.calculate(cljFunction(), changed_atoms, cljgroup[1].cljBoxes());
                }
                else
                {
            ...
            if (not d.constData()->fixed_atoms[0].isEmpty())
            {
                tuple<double, double> grid_deltas = d.constData()->fixed_atoms[0].calculate(changed_atoms);

                delta_nrgs.get<0>() += grid_deltas.get<0>();
                delta_nrgs.get<1>() += grid_deltas.get<1>();
            }

            d.constData()->cljcomps.changeEnergy(*this, MultiCLJEnergy(delta_nrgs.get<0>(), delta_nrgs.get<1>()));
        }
        else
        {
            ...
            if (not d.constData()->fixed_only)
            {
                ...
                {
                    CLJCalculator calc(d.constData()->repro_sum);
                    delta_nrgs = calc.calculate(d.constData()->cljfuncs, changed_atoms, cljgroup[1].cljBoxes());
                }
                else
                {
                    delta_nrgs =
                        CLJFunction::multiCalculate(d.constData()->cljfuncs, changed_atoms, cljgroup[1].cljBoxes());
                }
            }
            ...
            if (not d.constData()->fixed_atoms[0].isEmpty()) // if 0 is empty, they are all empty
            {
                for (int i = 0; i < d.constData()->fixed_atoms.count(); ++i)
                {
                    tuple<double, double> grid_deltas = d.constData()->fixed_atoms[i].calculate(changed_atoms);
    ...
    else if (cljgroup[1].needsAccepting())
    {
        ...
        if (d.constData()->cljfuncs.count() == 1)
        {
            ...
            if (not d.constData()->fixed_only)
            {
                ...
                {
                    CLJCalculator calc(d.constData()->repro_sum);
                    delta_nrgs = calc.calculate(cljFunction(), changed_atoms, cljgroup[0].cljBoxes());
                }
                else
                {
        ...
        else
        {
            ...
            if (not d.constData()->fixed_only)
            {
                ...
                {
                    CLJCalculator calc(d.constData()->repro_sum);
                    delta_nrgs = calc.calculate(d.constData()->cljfuncs, changed_atoms, cljgroup[0].cljBoxes());
                }
                else
                {
                    delta_nrgs =
                        CLJFunction::multiCalculate(d.constData()->cljfuncs, changed_atoms, cljgroup[0].cljBoxes());
                }
...
void InterGroupFF::_pvt_added(quint32 group_id, const SireMol::PartialMolecule &mol, const SireBase::PropertyMap &map)
{
    if (needs_accepting)
    {
        cljgroup[0].accept();
...
void InterGroupFF::_pvt_changed(quint32 group_id, const QList<SireMol::Molecule> &molecules, bool auto_update)
{
    if (needs_accepting)
    {
        cljgroup[0].accept();
