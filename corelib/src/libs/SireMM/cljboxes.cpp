...
CLJBox::CLJBox(const CLJBoxIndex &index, Length length) : RefCountData(), box_index(index), box_length(length.value())
{
}
...
CLJBox::CLJBox(const CLJBox &other)
    : RefCountData(), atms(other.atms), gaps(other.gaps), box_index(other.box_index), box_length(other.box_length)
{
}
...
CLJBox CLJBox::operator+(const CLJBox &other) const
{
    if (box_index != other.box_index or box_length != other.box_length)
        throw SireError::incompatible_error(
            QObject::tr("You cannot add together two boxes that are at different points in space! "
                        "Box0 = %1, dimensions %2, while Box1 = %3, dimensions %4.")
                .arg(index().toString())
                .arg(dimensions().toString())
                .arg(other.index().toString())
                .arg(other.dimensions().toString()),
            CODELOC);
...
{
    return d.constData() == other.d.constData() or *(d.constData()) == *(other.d.constData());
}
...
{
    return CLJBoxIndex(qMin(i(), other.i()), qMin(j(), other.j()), qMin(k(), other.k()), qMin(index(), other.index()));
}

/** Return the maximum box indicies of the two passed boxes */
CLJBoxIndex CLJBoxIndex::max(const CLJBoxIndex &other) const
{
    return CLJBoxIndex(qMax(i(), other.i()), qMax(j(), other.j()), qMax(k(), other.k()), qMax(index(), other.index()));
}
...
AABox CLJBoxIndex::box(Length box_length) const
{
    ...
    Vector origin(i() * length.x(), j() * length.y(), k() * length.z());
...
CLJBoxIndex CLJBoxIndex::createWithInverseBoxLength(float x, float y, float z, float inv_length)
{
    ...
    if (i < min16 or i > max16 or j < min16 or j > max16 or k < min16 or k > max16)
    {
        throw SireError::program_bug(
            QObject::tr("It is not possible to get the index of the box containing the point "
...
{
    return CLJBoxIndex::createWithInverseBoxLength(coords.x(), coords.y(), coords.z(), 1.0 / box_length.value());
}
...
CLJBoxDistance::CLJBoxDistance(quint32 box0, quint32 box1, float distance) : dist(distance), b0(box0), b1(box1)
{
}

/** Copy constructor */
CLJBoxDistance::CLJBoxDistance(const CLJBoxDistance &other) : dist(other.dist), b0(other.b0), b1(other.b1)
{
}
...
{
    return dist == other.dist and b0 == other.b0 and b1 == other.b1;
}
...
{
    return QObject::tr("CLJBoxDistance( %1->%2, %3 A )").arg(b0).arg(b1).arg(dist);
}
...
void CLJBoxes::constructFrom(const CLJAtoms &atoms0, const CLJAtoms &atoms1)
{
    ...
    if (atoms0.count() < 50 and atoms1.count() < 50)
    {
        ...
        for (int i = 0; i < atoms0.count(); ++i)
        {
            ...
            if (atom.ID() != 0)
            {
                CLJBoxIndex index = CLJBoxIndex::createWithInverseBoxLength(atom.coordinates(), inv_length);
    ...
    for (int i = 0; i < atoms0.count(); ++i)
    {
        ...
        if (atom.ID() != 0)
        {
            CLJBoxIndex cljindex = CLJBoxIndex::createWithInverseBoxLength(atom.coordinates(), inv_length);
    ...
    for (QHash<CLJBoxIndex, QList<CLJAtom>>::const_iterator it = boxed_atoms.constBegin(); it != boxed_atoms.constEnd();
         ++it)
    {
...
CLJBoxes CLJBoxes::operator+(const CLJBoxes &other) const
{
    ...
    {
        throw SireError::incompatible_error(
            QObject::tr("You cannot add together two CLJBoxes objects that have different "
                        "box lengths (%1 A vs. %2 A)")
                .arg(box_length)
                .arg(other.box_length),
            CODELOC);
    }
    ...
    for (ContainerMap::const_iterator it = other.box_to_idx.constBegin(); it != other.box_to_idx.constEnd(); ++it)
    {
...
CLJBoxes::CLJBoxes(const CLJAtoms &atoms, Length box_size) : box_length(box_size.value())
{
    if (box_length < 2)
        // don't be silly
        box_length = 2.0;
...
/** Box up the passed set of atoms into boxes of specified box size */
CLJBoxes::CLJBoxes(const CLJAtoms &atoms0, const CLJAtoms &atoms1, Length box_size) : box_length(box_size.value())
{
    if (box_length < 2)
        // don't be silly
        box_length = 2.0;
...
/** Copy constructor */
CLJBoxes::CLJBoxes(const CLJBoxes &other) : box_to_idx(other.box_to_idx), bxs(other.bxs), box_length(other.box_length)
{
}
...
{
    return QObject::tr("CLJBoxes( nAtoms() == %1, nOccupiedBoxes() == %2 )").arg(nAtoms()).arg(nOccupiedBoxes());
}
...
int CLJBoxes::nAtoms() const
{
    ...
    for (const_iterator it = bxs.constBegin(); it != bxs.constEnd(); ++it)
    {
        n += it->read().nAtoms();
    }
...
QVector<CLJBoxIndex> CLJBoxes::add(const CLJAtoms &atoms)
{
    ...
    // now build the CLJAtoms for each box
    for (QHash<CLJBoxIndex, tuple<QList<CLJAtom>, QList<int>>>::iterator it = boxed_atoms.begin();
         it != boxed_atoms.end(); ++it)
    {
        int idx = box_to_idx.value(it.key(), -1);
        ...
        else
        {
            ...
            for (int i = 0; i < atom_idxs.count(); ++i)
            {
                indicies[atom_idxs.at(i)] = CLJBoxIndex(it.key().i(), it.key().j(), it.key().k(), i);
            }
        }
    }
...
SIRE_ALWAYS_INLINE int getDelta2(const CLJBoxIndex &minbox0, const CLJBoxIndex &maxbox0, const CLJBoxIndex &box1)
{
...
float CLJBoxes::getDistance(const Space &space, const CLJBoxIndex &box0, const CLJBoxIndex &box1) const
{
...
float CLJBoxes::getDistance(const Space &space, const CLJBoxIndex &box0, const CLJBoxIndex &box1, quint32 nx,
                            quint32 ny, quint32 nz) const
{
    const Length l(box_length);
    ...
    return space.minimumDistance(AABox::from(min_box.minCoords(), max_box.maxCoords()), box1.box(l));
}
...
QVector<CLJBoxDistance> CLJBoxes::getDistances(const Space &space, const CLJBoxes &boxes, Length cutoff)
{
    // QElapsedTimer t;
    // t.start();
...
QVector<CLJBoxDistance> CLJBoxes::getDistances(const Space &space, const CLJBoxes &boxes0, const CLJBoxes &boxes1)
{
    QVector<CLJBoxDistance> dists;
    ...
    if (space.isCartesian() and (boxes0.box_length == boxes1.box_length) and not space.isPeriodic())
    {
...
QVector<CLJBoxDistance> CLJBoxes::getDistances(const Space &space, const CLJAtoms &atoms0, const CLJBoxes &boxes1,
                                               Length cutoff)
{
    // QElapsedTimer t;
    // t.start();
