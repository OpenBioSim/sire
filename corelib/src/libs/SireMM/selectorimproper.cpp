...
inline QSet<IDQuad> _to_int_set(const QList<ImproperID> &vals, const MoleculeInfoData &molinfo)
{
    QSet<IDQuad> s;
    ...
    {
        s.insert(IDQuad(molinfo.atomIdx(val[0]).value(), molinfo.atomIdx(val[1]).value(),
                        molinfo.atomIdx(val[2]).value(), molinfo.atomIdx(val[3]).value()));
    }

    return s;
}

inline QList<ImproperID> _from_int_set(const QSet<IDQuad> &vals)
{
    QVector<IDQuad> v;
    ...
    {
        l.append(ImproperID(AtomIdx(val.atom0), AtomIdx(val.atom1), AtomIdx(val.atom2), AtomIdx(val.atom3)));
    }

    return l;
}

QList<ImproperID> _get_impropers(const MoleculeData &moldata, const PropertyMap &map)
{
    ...
    if (moldata.hasProperty(map["improper"]))
    {
        ...
        for (const auto &func : funcs.potentials())
        {
            ...
            if (have_connectivity)
            {
                ...
                if (c.areConnected(atomidx0, atomidx1) and c.areConnected(atomidx2, atomidx1) and
                    c.areConnected(atomidx3, atomidx1))
                {
                    // this is the expected order :-)
                    impropers.append(ImproperID(atomidx0, atomidx1, atomidx2, atomidx3));
                }
                else if (c.areConnected(atomidx0, atomidx2) and c.areConnected(atomidx1, atomidx2) and
                         c.areConnected(atomidx3, atomidx2))
                {
                    // this is the reverse order
                    impropers.append(ImproperID(atomidx0, atomidx2, atomidx1, atomidx3));
                }
                else if (c.areConnected(atomidx0, atomidx1) and c.areConnected(atomidx0, atomidx2) and
                         c.areConnected(atomidx0, atomidx3))
                {
                    // the first atom is the central atom - strange
                    impropers.append(ImproperID(atomidx1, atomidx0, atomidx2, atomidx3));
                }
                else if (c.areConnected(atomidx3, atomidx0) and c.areConnected(atomidx3, atomidx1) and
                         c.areConnected(atomidx3, atomidx2))
                {
                    // the last atom is the central atom - strange
                    impropers.append(ImproperID(atomidx0, atomidx3, atomidx1, atomidx2));
                }
                else
                {
                    auto i = ImproperID(atomidx0, atomidx1, atomidx2, atomidx3);

                    qDebug() << i.toString() << "does not match the connectivity of the molecule.";
...
SelectorImproper::SelectorImproper(const MoleculeView &mol, const SireBase::PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>(mol)
{
    // do this so that the impropers are canonicalized and sorted
    auto impropers = _from_int_set(_to_int_set(_get_impropers(this->data(), map), this->data().info()));
    ...
    else
    {
        ...
        {
            if (s.selected(improper.atom0()) and s.selected(improper.atom1()) and s.selected(improper.atom2()) and
...
SelectorImproper::SelectorImproper(const MoleculeData &moldata, const SireBase::PropertyMap &map)
...
inline QSet<IDQuad> _filter(const QSet<IDQuad> &impropers, const QVector<quint32> &atoms, int position)
{
    QSet<IDQuad> result;
...
SelectorImproper::SelectorImproper(const MoleculeView &mol, const ImproperID &improper, const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>(mol)
{
    ...
    auto impropers = _to_int_set(_get_impropers(this->data(), map), this->data().info());
...
SelectorImproper::SelectorImproper(const MoleculeView &mol, const QList<ImproperID> &impropers,
    ...
    : ConcreteProperty<SelectorImproper, MoleculeView>(mol)
{
    ...
    {
        auto all_impropers = _to_int_set(_get_impropers(this->data(), map), this->data().info());
...
SelectorImproper::SelectorImproper(const MoleculeView &mol, const AtomID &atom, const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>(mol)
{
    ...
    imps = _from_int_set(impropers0 + impropers1 + impropers2 + impropers3);
}

SelectorImproper::SelectorImproper(const MoleculeView &mol, const AtomID &atom0, const AtomID &atom1,
    ...
    : ConcreteProperty<SelectorImproper, MoleculeView>(mol)
{
    ...
    imps = _from_int_set(impropers10 + impropers12 + impropers13 + impropers01 + impropers21 + impropers31);
}
    ...
    : ConcreteProperty<SelectorImproper, MoleculeView>(mol)
{
    ...
    imps = _from_int_set(impropers012 + impropers013 + impropers213 + impropers210 + impropers310 + impropers312);
}

SelectorImproper::SelectorImproper(const MoleculeView &mol, const AtomID &atom0, const AtomID &atom1,
                                   const AtomID &atom2, const AtomID &atom3, const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>()
{
    this->operator=(SelectorImproper(mol, ImproperID(atom0, atom1, atom2, atom3), map));
}

SelectorImproper::SelectorImproper(const MoleculeData &mol, const AtomID &atom, const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>()
{
    this->operator=(SelectorImproper(Molecule(mol), atom, map));
}

SelectorImproper::SelectorImproper(const MoleculeData &mol, const AtomID &atom0, const AtomID &atom1,
                                   const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>()
{
...
SelectorImproper::SelectorImproper(const MoleculeData &mol, const AtomID &atom0, const AtomID &atom1,
    ...
    : ConcreteProperty<SelectorImproper, MoleculeView>()
{
    this->operator=(SelectorImproper(Molecule(mol), atom0, atom1, atom2, atom3, map));
}

SelectorImproper::SelectorImproper(const Selector<Atom> &atoms, const PropertyMap &map)
...
SelectorImproper::SelectorImproper(const Selector<Atom> &atoms0, const Selector<Atom> &atoms1, const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>(atoms0)
{
    if (not atoms0.isSameMolecule(atoms1))
        throw SireError::incompatible_error(
            QObject::tr("You can only create an improper from atoms in the same molecule. "
    ...
    imps = _from_int_set(impropers01 + impropers21 + impropers31 + impropers10 + impropers12 + impropers13);
}

SelectorImproper::SelectorImproper(const Selector<Atom> &atoms0, const Selector<Atom> &atoms1,
                                   const Selector<Atom> &atoms2, const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>(atoms0)
{
...
SelectorImproper::SelectorImproper(const Selector<Atom> &atoms0, const Selector<Atom> &atoms1,
                                   const Selector<Atom> &atoms2, const Selector<Atom> &atoms3, const PropertyMap &map)
    : ConcreteProperty<SelectorImproper, MoleculeView>(atoms0)
{
    if (not(atoms0.isSameMolecule(atoms1) and atoms0.isSameMolecule(atoms2) and atoms0.isSameMolecule(atoms3)))
        throw SireError::incompatible_error(
            QObject::tr("You can only create an improper from atoms in the same molecule. "
...
SelectorImproper::SelectorImproper(const SelectorImproper &other)
    : ConcreteProperty<SelectorImproper, MoleculeView>(other), imps(other.imps)
{
}
...
SelectorImproper SelectorImproper::add(const Improper &improper) const
{
    ...
    {
        throw SireError::incompatible_error(QObject::tr("You cannot add Impropers from a different molecule (%1) to "
...
bool SelectorImproper::hasMetadata(const PropertyName &key, const PropertyName &metakey) const
{
    return false;
}
...
QList<PropertyPtr> SelectorImproper::property(const PropertyName &key, const Property &default_value) const
{
    QList<PropertyPtr> props;
