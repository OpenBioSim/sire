...
namespace SireMM
{
    ...
    SIRE_ALWAYS_INLINE Vector calcLJForce(const DistVector &r, const LJPair &ljpair)
    {
        ...
        else
        {
            ...
            return (4 * ljpair.epsilon() * (6 * sig_over_r7 - 12 * sig_over_r13)) * r.direction();
        }
    }
    ...
    SIRE_ALWAYS_INLINE Vector calcLJForce(const DistVector &r, const LJPair &ljpair, const double scl, const double dscl_dr)
    {
        if (r.length() == 0)
            return Vector(0);
        else
        {
            ...
            const double ljnrg = 4 * ljpair.epsilon() * (sig_over_r12 - sig_over_r6);

            return (scl * 4 * ljpair.epsilon() * (6.0 * sig_over_r7 - 12.0 * sig_over_r13)) * r.direction()
    ...
    SIRE_ALWAYS_INLINE __m128d calcLJEnergy(const double r0_2, const double r1_2, const LJPair &lj0pair,
                                            const LJPair &lj1pair)
    {
        __m128d sse_nrg;
        ...
        else
        {
            ...
            const __m128d sse_eps = _mm_set_pd(lj0pair.epsilon(), lj1pair.epsilon());
            ...
            const __m128d sse_sig_over_r12 = _mm_mul_pd(sse_sig_over_r6, sse_sig_over_r6);
