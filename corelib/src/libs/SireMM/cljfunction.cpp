...
void CLJFunction::extractDetailsFromRules(SireMM::CLJFunction::COMBINING_RULES rules)
{
    ...
    case GEOMETRIC:
        ...
        break;
    default:
        throw SireError::program_bug(QObject::tr("Cannot understand the passed combining rules (%1)").arg(rules),
                                     CODELOC);
    }
}

/** Construct, using vacuum boundary conditions, but specifying the combining rules */
CLJFunction::CLJFunction(COMBINING_RULES combining_rules) : Property(), use_arithmetic(true), use_box(false)
{
...
void CLJFunction::extractDetailsFromSpace()
{
    ...
    else if (spce.read().isA<Cartesian>())
    {
        ...
        throw SireError::unsupported(QObject::tr("CLJFunction-based forcefields currently only support using either "
                                                 "periodic (cubic) boundary conditions, or vacuum boundary conditions. "
                                                 "They are not compatible with the passed space \"%1\".")
                                         .arg(spce.read().toString()),
                                     CODELOC);
}

/** Construct, using arithmetic combining rules, but specifying the space */
CLJFunction::CLJFunction(const Space &space) : Property(), spce(space), use_arithmetic(true), use_box(false)
{
...
CLJFunction::CLJFunction(const CLJFunction &other)
    : Property(other), spce(other.spce), box_dimensions(other.box_dimensions), use_arithmetic(other.use_arithmetic),
...
{
    return use_arithmetic == other.use_arithmetic and spce == other.spce and Property::operator==(other);
}
...
CLJFunctionPtr CLJFunction::setProperty(const QString &name, const Property &value) const
{
    ...
    else if (name == "combiningRules")
    {
        ...
        if (typ.toLower() == "arithmetic")
            ...
            throw SireError::invalid_arg(QObject::tr("Cannot interpret combining rules from value \"%1\". Valid "
    ...
    {
        throw SireBase::missing_property(QObject::tr("There is no property with the name \"%1\" in function \"%2\". "
                                                     "Available property names are %3.")
                                             .arg(name)
                                             .arg(this->toString())
                                             .arg(Sire::toString(this->properties().propertyKeys())),
                                         CODELOC);
    }
...
void CLJFunction::calcBoxGrid(const CLJAtoms &atoms, const GridInfo &gridinfo, const Vector &box_dimensions,
...
/** Dummy function that needs to be overridden to support grid calculations */
void CLJFunction::calcVacGrid(const CLJAtoms &atoms, const GridInfo &gridinfo, const int start, const int end,
                              float *gridpot) const
{
    for (int i = start; i < end; ++i)
    {
...
namespace SireMM
{
    namespace detail
    {
        ...
        {
        public:
            CLJGridCalculator() : atoms(0), grid_info(0), cljfunc(0), gridpot(0), use_box(false)
            {
            }

            CLJGridCalculator(const CLJAtoms &_atoms, const GridInfo &_grid_info, const CLJFunction &_cljfunc, float *_gridpot)
                : atoms(&_atoms), grid_info(&_grid_info), cljfunc(&_cljfunc), gridpot(_gridpot), use_box(_cljfunc.use_box)
            {
            }
            ...
            void operator()(const tbb::blocked_range<int> &range) const
            {
                if (use_box)
                {
                    cljfunc->calcBoxGrid(*atoms, *grid_info, cljfunc->box_dimensions, range.begin(), range.end(),
                                         &(gridpot[range.begin()]));
                }
                else
                {
                    cljfunc->calcVacGrid(*atoms, *grid_info, range.begin(), range.end(), &(gridpot[range.begin()]));
                }
            }
...
double CLJFunction::calcVacCoulombEnergyAri(const CLJAtoms &atoms0, const CLJAtoms &atoms1, float min_distance) const
{
    double cnrg, ljnrg;
...
/** Calculate the coulomb energy between the atoms in 'atoms0' and the atoms in 'atoms1' */
double CLJFunction::calcVacCoulombEnergyGeo(const CLJAtoms &atoms0, const CLJAtoms &atoms1, float min_distance) const
{
    double cnrg, ljnrg;
...
double CLJFunction::calcVacLJEnergyAri(const CLJAtoms &atoms0, const CLJAtoms &atoms1, float min_distance) const
{
    double cnrg, ljnrg;
    calcVacEnergyAri(atoms0, atoms1, cnrg, ljnrg, min_distance);
    return ljnrg;
...
/** Calculate the LJ energy between the atoms in 'atoms0' and the atoms in 'atoms1' */
double CLJFunction::calcVacLJEnergyGeo(const CLJAtoms &atoms0, const CLJAtoms &atoms1, float min_distance) const
{
    double cnrg, ljnrg;
    calcVacEnergyGeo(atoms0, atoms1, cnrg, ljnrg, min_distance);
    return ljnrg;
...
/** Calculate the coulomb energy between all atoms in 'atoms' */
double CLJFunction::calcBoxCoulombEnergyAri(const CLJAtoms &atoms, const Vector &box_dimensions) const
{
    double cnrg, ljnrg;
...
/** Calculate the coulomb energy between all atoms in 'atoms' */
double CLJFunction::calcBoxCoulombEnergyGeo(const CLJAtoms &atoms, const Vector &box_dimensions) const
{
    double cnrg, ljnrg;
...
/** Calculate the LJ energy between all atoms in 'atoms' */
double CLJFunction::calcBoxLJEnergyAri(const CLJAtoms &atoms, const Vector &box_dimensions) const
{
    double cnrg, ljnrg;
    calcBoxEnergyAri(atoms, box_dimensions, cnrg, ljnrg);
    return ljnrg;
...
/** Calculate the LJ energy between the atoms in 'atoms0' and the atoms in 'atoms1' */
double CLJFunction::calcBoxLJEnergyAri(const CLJAtoms &atoms0, const CLJAtoms &atoms1, const Vector &box_dimensions,
...
/** Calculate the LJ energy between all atoms in 'atoms' */
double CLJFunction::calcBoxLJEnergyGeo(const CLJAtoms &atoms, const Vector &box_dimensions) const
{
    double cnrg, ljnrg;
    calcBoxEnergyGeo(atoms, box_dimensions, cnrg, ljnrg);
    return ljnrg;
...
/** Calculate the LJ energy between the atoms in 'atoms0' and the atoms in 'atoms1' */
double CLJFunction::calcBoxLJEnergyGeo(const CLJAtoms &atoms0, const CLJAtoms &atoms1, const Vector &box_dimensions,
                                       float min_distance) const
{
    double cnrg, ljnrg;
    calcBoxEnergyGeo(atoms0, atoms1, box_dimensions, cnrg, ljnrg, min_distance);
...
void CLJFunction::operator()(const CLJAtoms &atoms, double &cnrg, double &ljnrg) const
{
    if (atoms.isEmpty())
    {
        cnrg = 0;
...
/** Return the total energy between 'atoms0' and 'atoms1', returning the coulomb part in 'cnrg'
    and the LJ part in 'ljnrg' */
void CLJFunction::operator()(const CLJAtoms &atoms0, const CLJAtoms &atoms1, double &cnrg, double &ljnrg,
                             float min_distance) const
{
...
void CLJFunction::operator()(const CLJBoxes &atoms, double &cnrg, double &ljnrg) const
{
    ...
    if (this->hasCutoff())
    {
        ...
        for (CLJBoxes::const_iterator it0 = boxes.constBegin(); it0 != boxes.constEnd(); ++it0)
        {
            ...
            for (++it1; it1 != boxes.constEnd(); ++it1)
            {
                ...
                {
                    this->total(it0->read().atoms(), it1->read().atoms(), icnrg, iljnrg, mindist);
    ...
    else
    {
        ...
        for (CLJBoxes::const_iterator it0 = boxes.constBegin(); it0 != boxes.constEnd(); ++it0)
        {
            ...
            for (++it1; it1 != boxes.constEnd(); ++it1)
            {
                this->total(it0->read().atoms(), it1->read().atoms(), icnrg, iljnrg);
...
/** Return the total energy between 'atoms0' and 'atoms1', returning the coulomb part in 'cnrg'
    and the LJ part in 'ljnrg' */
void CLJFunction::operator()(const CLJBoxes &atoms0, const CLJBoxes &atoms1, double &cnrg, double &ljnrg) const
{
    cnrg = 0;
    ljnrg = 0;
    ...
    if (this->hasCutoff() and atoms0.length() == atoms1.length())
    {
        ...
        for (CLJBoxes::const_iterator it0 = boxes0.constBegin(); it0 != boxes0.constEnd(); ++it0)
        {
            const CLJBoxIndex &idx0 = it0->read().index();

            for (CLJBoxes::const_iterator it1 = boxes1.constBegin(); it1 != boxes1.constEnd(); ++it1)
            {
                double icnrg(0), iljnrg(0);
                const CLJBoxIndex &idx1 = it1->read().index();
                ...
                if (mindist < min_cutoff)
                {
                    this->operator()(it0->read().atoms(), it1->read().atoms(), icnrg, iljnrg, mindist);
                }
    ...
    else
    {
        ...
        for (CLJBoxes::const_iterator it0 = boxes0.constBegin(); it0 != boxes0.constEnd(); ++it0)
        {
            for (CLJBoxes::const_iterator it1 = boxes1.constBegin(); it1 != boxes1.constEnd(); ++it1)
            {
                ...
                this->operator()(it0->read().atoms(), it1->read().atoms(), icnrg, iljnrg);
...
/** Return the total energy between 'atoms0' and 'atoms1', returning the coulomb part in 'cnrg'
    and the LJ part in 'ljnrg' */
void CLJFunction::operator()(const CLJAtoms &atoms0, const CLJBoxes &atoms1, double &cnrg, double &ljnrg) const
{
    cnrg = 0;
    ljnrg = 0;
    ...
    else
    {
        ...
        for (CLJBoxes::const_iterator it1 = boxes1.constBegin(); it1 != boxes1.constEnd(); ++it1)
        {
            ...
            this->operator()(atoms0, it1->read().atoms(), icnrg, iljnrg);
...
/** Return the total energy between 'atoms0' and 'atoms1', returning the coulomb part in 'cnrg'
    and the LJ part in 'ljnrg' */
void CLJFunction::total(const CLJAtoms &atoms0, const CLJAtoms &atoms1, double &cnrg, double &ljnrg,
                        float min_distance) const
{
...
/** Return the total energy between 'atoms0' and 'atoms1', returning the coulomb part in 'cnrg'
    and the LJ part in 'ljnrg' */
void CLJFunction::total(const CLJBoxes &atoms0, const CLJBoxes &atoms1, double &cnrg, double &ljnrg) const
{
...
/** Return the total energy between 'atoms0' and 'atoms1', returning the coulomb part as the first
    element of the tuple and the LJ part as the second */
boost::tuple<double, double> CLJFunction::calculate(const CLJBoxes &atoms0, const CLJBoxes &atoms1) const
{
    double cnrg, ljnrg;
    this->operator()(atoms0, atoms1, cnrg, ljnrg);
    return boost::tuple<double, double>(cnrg, ljnrg);
...
/** Return the total energy between 'atoms0' and 'atoms1', returning the coulomb part as the first
    element of the tuple and the LJ part as the second */
boost::tuple<double, double> CLJFunction::calculate(const CLJAtoms &atoms0, const CLJBoxes &atoms1) const
{
    double cnrg, ljnrg;
    this->operator()(atoms0, atoms1, cnrg, ljnrg);
    return boost::tuple<double, double>(cnrg, ljnrg);
...
double CLJFunction::coulomb(const CLJAtoms &atoms0, const CLJAtoms &atoms1, float min_distance) const
{
    if (atoms0.isEmpty() or atoms1.isEmpty())
    {
        return 0;
...
double CLJFunction::lj(const CLJAtoms &atoms0, const CLJAtoms &atoms1, float min_distance) const
{
    if (atoms0.isEmpty() or atoms1.isEmpty())
    {
        return 0;
...
tuple<QVector<double>, QVector<double>> CLJFunction::multiCalculate(const QVector<CLJFunctionPtr> &funcs,
                                                                    ...
                                                                    float min_distance)
{
    ...
    for (int i = 0; i < funcs.count(); ++i)
    {
        tuple<double, double> nrgs = funcs.constData()[i].read().calculate(atoms0, atoms1, min_distance);
        cnrgs[i] = nrgs.get<0>();
        ljnrgs[i] = nrgs.get<1>();
    }
...
/** Copy constructor */
NullCLJFunction::NullCLJFunction(const NullCLJFunction &other) : ConcreteProperty<NullCLJFunction, CLJFunction>(other)
{
}
...
void NullCLJFunction::calcVacEnergyAri(const CLJAtoms &atoms, double &cnrg, double &ljnrg) const
{
    cnrg = 0;
    ljnrg = 0;
}

void NullCLJFunction::calcVacEnergyAri(const CLJAtoms &atoms0, const CLJAtoms &atoms1, double &cnrg, double &ljnrg,
                                       float min_distance) const
{
    cnrg = 0;
    ljnrg = 0;
...
void NullCLJFunction::calcVacEnergyGeo(const CLJAtoms &atoms, double &cnrg, double &ljnrg) const
{
    cnrg = 0;
    ljnrg = 0;
}

void NullCLJFunction::calcVacEnergyGeo(const CLJAtoms &atoms0, const CLJAtoms &atoms1, double &cnrg, double &ljnrg,
                                       float min_distance) const
{
    cnrg = 0;
    ljnrg = 0;
...
void NullCLJFunction::calcBoxEnergyAri(const CLJAtoms &atoms, const Vector &box, double &cnrg, double &ljnrg) const
{
    cnrg = 0;
    ljnrg = 0;
}

void NullCLJFunction::calcBoxEnergyAri(const CLJAtoms &atoms0, const CLJAtoms &atoms1, const Vector &box, double &cnrg,
...
void NullCLJFunction::calcBoxEnergyGeo(const CLJAtoms &atoms, const Vector &box, double &cnrg, double &ljnrg) const
{
    cnrg = 0;
    ljnrg = 0;
}

void NullCLJFunction::calcBoxEnergyGeo(const CLJAtoms &atoms0, const CLJAtoms &atoms1, const Vector &box, double &cnrg,
                                       double &ljnrg, float min_distance) const
{
    cnrg = 0;
    ljnrg = 0;
...
static const RegisterMetaType<CLJCutoffFunction> r_cutoff(MAGIC_ONLY, CLJCutoffFunction::typeName());

QDataStream &operator<<(QDataStream &ds, const CLJCutoffFunction &func)
{
    writeHeader(ds, r_cutoff, 1);

    ds << func.coul_cutoff << func.lj_cutoff << static_cast<const CLJFunction &>(func);
...
QDataStream &operator>>(QDataStream &ds, CLJCutoffFunction &func)
{
    ...
    if (v == 1)
    {
        ds >> func.coul_cutoff >> func.lj_cutoff >> static_cast<CLJFunction &>(func);
    }
    else
...
CLJCutoffFunction::CLJCutoffFunction() : CLJFunction(), coul_cutoff(10), lj_cutoff(10)
{
}
...
CLJCutoffFunction::CLJCutoffFunction(Length cutoff) : CLJFunction(), coul_cutoff(10), lj_cutoff(10)
{
    pvt_setCutoff(cutoff, cutoff);
}

/** Construct, specifying the coulomb and LJ cutoffs */
CLJCutoffFunction::CLJCutoffFunction(Length coulomb, Length lj) : CLJFunction(), coul_cutoff(10), lj_cutoff(10)
{
...
CLJCutoffFunction::CLJCutoffFunction(const Space &space, Length cutoff, COMBINING_RULES combining_rules)
    : CLJFunction(space, combining_rules), coul_cutoff(10), lj_cutoff(10)
{
    pvt_setCutoff(cutoff, cutoff);
}

CLJCutoffFunction::CLJCutoffFunction(const Space &space, Length coulomb, Length lj, COMBINING_RULES combining_rules)
    : CLJFunction(space, combining_rules), coul_cutoff(10), lj_cutoff(10)
{
    pvt_setCutoff(coulomb, lj);
}
...
{
    return coul_cutoff == other.coul_cutoff and lj_cutoff == other.lj_cutoff and CLJFunction::operator==(other);
}
...
Properties CLJCutoffFunction::properties() const
{
    ...
    props.setProperty("switchingFunction", HarmonicSwitchingFunction(Length(coul_cutoff), Length(lj_cutoff)));
...
{
    return (name == "switchingFunction") or (name == "coulombCutoff") or (name == "ljCutoff") or
           CLJFunction::containsProperty(name);
}
...
QDataStream &operator>>(QDataStream &ds, CLJIntraFunction &func)
{
    ...
    if (v == 1)
    {
        ...
        sds >> connectivity >> static_cast<CLJCutoffFunction &>(func);
...
CLJIntraFunction::CLJIntraFunction(Length coul_cutoff, Length lj_cutoff) : CLJCutoffFunction(coul_cutoff, lj_cutoff)
{
}

CLJIntraFunction::CLJIntraFunction(const Space &space, Length cutoff) : CLJCutoffFunction(space, cutoff)
{
}

CLJIntraFunction::CLJIntraFunction(const Space &space, Length coul_cutoff, Length lj_cutoff)
...
CLJIntraFunction::CLJIntraFunction(Length coul_cutoff, Length lj_cutoff, COMBINING_RULES combining_rules)
...
CLJIntraFunction::CLJIntraFunction(const Space &space, Length cutoff, COMBINING_RULES combining_rules)
...
bool CLJIntraFunction::isNotBonded(const QVector<MultiInt> &ids0, const QVector<MultiInt> &ids1) const
{
...
QDataStream &operator<<(QDataStream &ds, const CLJSoftFunction &func)
{
    ...
    ds << func.alpha_value << func.shift_delta << func.coulomb_power << static_cast<const CLJCutoffFunction &>(func);
...
QDataStream &operator>>(QDataStream &ds, CLJSoftFunction &func)
{
    ...
    if (v == 1)
    {
        ds >> func.alpha_value >> func.shift_delta >> func.coulomb_power >> static_cast<CLJCutoffFunction &>(func);
    }
    else
...
/** Constructor */
CLJSoftFunction::CLJSoftFunction() : CLJCutoffFunction(), alpha_value(0), shift_delta(0), coulomb_power(1)
{
}

/** Construct, specifying the cutoff */
CLJSoftFunction::CLJSoftFunction(Length cutoff) : CLJCutoffFunction(cutoff)
{
}

/** Construct, specifying the coulomb and LJ cutoffs */
CLJSoftFunction::CLJSoftFunction(Length coulomb, Length lj) : CLJCutoffFunction(coulomb, lj)
{
}

CLJSoftFunction::CLJSoftFunction(const Space &space, Length cutoff) : CLJCutoffFunction(space, cutoff)
{
}

CLJSoftFunction::CLJSoftFunction(const Space &space, Length coulomb, Length lj) : CLJCutoffFunction(space, coulomb, lj)
{
}
...
CLJSoftFunction::CLJSoftFunction(const Space &space, Length cutoff, COMBINING_RULES combining_rules)
    : CLJCutoffFunction(space, cutoff, combining_rules)
{
}

CLJSoftFunction::CLJSoftFunction(const Space &space, Length coulomb, Length lj, COMBINING_RULES combining_rules)
...
CLJSoftFunction::CLJSoftFunction(const CLJSoftFunction &other)
    : CLJCutoffFunction(other), alpha_value(other.alpha_value), shift_delta(other.shift_delta),
...
{
    return alpha_value == other.alpha_value and shift_delta == other.shift_delta and
           coulomb_power == other.coulomb_power and CLJCutoffFunction::operator==(other);
}
...
CLJFunctionPtr CLJSoftFunction::setProperty(const QString &name, const Property &value) const
{
    ...
    if (name == "alpha")
    {
        ret.edit().asA<CLJSoftFunction>().setAlpha(value.asADouble());
    }
    else if (name == "shiftDelta")
    {
        ret.edit().asA<CLJSoftFunction>().setShiftDelta(value.asADouble());
    }
    else if (name == "coulombPower")
    {
        ret.edit().asA<CLJSoftFunction>().setCoulombPower(value.asADouble());
    }
    else
    {
        ret = CLJCutoffFunction::setProperty(name, value);
...
{
    return (name == "alpha") or (name == "shiftDelta") or (name == "coulombPower") or
           CLJCutoffFunction::containsProperty(name);
}
    ...
    if (alpha > 1)
    {
        if (power != int(power))
            throw SireError::incompatible_error(
                QObject::tr("You cannot have a soft-core function where alpha > 1 and "
...
static const RegisterMetaType<CLJSoftIntraFunction> r_softintra(MAGIC_ONLY, CLJSoftIntraFunction::typeName());

QDataStream &operator<<(QDataStream &ds, const CLJSoftIntraFunction &func)
{
    writeHeader(ds, r_softintra, 1);

    ds << func.alpha_value << func.shift_delta << func.coulomb_power << static_cast<const CLJIntraFunction &>(func);
...
QDataStream &operator>>(QDataStream &ds, CLJSoftIntraFunction &func)
{
    ...
    if (v == 1)
    {
        ds >> func.alpha_value >> func.shift_delta >> func.coulomb_power >> static_cast<CLJIntraFunction &>(func);
    }
    else
...
/** Constructor */
CLJSoftIntraFunction::CLJSoftIntraFunction() : CLJIntraFunction(), alpha_value(0), shift_delta(0), coulomb_power(1)
{
}
...
CLJSoftIntraFunction::CLJSoftIntraFunction(const Space &space, Length cutoff) : CLJIntraFunction(space, cutoff)
{
}

CLJSoftIntraFunction::CLJSoftIntraFunction(const Space &space, Length coul_cutoff, Length lj_cutoff)
...
CLJSoftIntraFunction::CLJSoftIntraFunction(Length coul_cutoff, Length lj_cutoff, COMBINING_RULES combining_rules)
...
CLJSoftIntraFunction::CLJSoftIntraFunction(const Space &space, Length cutoff, COMBINING_RULES combining_rules)
...
CLJSoftIntraFunction::CLJSoftIntraFunction(const CLJSoftIntraFunction &other)
    : CLJIntraFunction(other), alpha_value(other.alpha_value), shift_delta(other.shift_delta),
      coulomb_power(other.coulomb_power)
{
}
...
{
    return alpha_value == other.alpha_value and shift_delta == other.shift_delta and
           coulomb_power == other.coulomb_power and CLJIntraFunction::operator==(other);
}
...
CLJFunctionPtr CLJSoftIntraFunction::setProperty(const QString &name, const Property &value) const
{
    ...
    if (name == "alpha")
    {
        ret.edit().asA<CLJSoftIntraFunction>().setAlpha(value.asADouble());
    }
    else if (name == "shiftDelta")
    {
        ret.edit().asA<CLJSoftIntraFunction>().setShiftDelta(value.asADouble());
    }
    else if (name == "coulombPower")
    {
        ret.edit().asA<CLJSoftIntraFunction>().setCoulombPower(value.asADouble());
    }
    else
    {
...
{
    return (name == "alpha") or (name == "shiftDelta") or (name == "coulombPower") or
           CLJIntraFunction::containsProperty(name);
}
