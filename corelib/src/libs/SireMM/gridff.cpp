...
QDataStream &operator<<(QDataStream &ds, const GridFF &gridff)
{
    ...
    sds << gridff.gridbox << gridff.dimx << gridff.dimy << gridff.dimz << gridff.gridpot << gridff.buffer_size
        << gridff.grid_spacing << gridff.coul_cutoff << gridff.lj_cutoff << gridff.fixedatoms_coords
        << gridff.fixedatoms_params << gridff.closemols_coords << gridff.closemols_params;

    sds << quint32(gridff.oldnrgs.count());

    for (QHash<MolNum, CLJEnergy>::const_iterator it = gridff.oldnrgs.constBegin(); it != gridff.oldnrgs.constEnd();
         ++it)
    {
...
QDataStream &operator>>(QDataStream &ds, GridFF &gridff)
{
    ...
    if (v == 5)
    {
        ...
        sds >> gridff.gridbox >> gridff.dimx >> gridff.dimy >> gridff.dimz >> gridff.gridpot >> gridff.buffer_size >>
            gridff.grid_spacing >> gridff.coul_cutoff >> gridff.lj_cutoff >> gridff.fixedatoms_coords >>
            fixedatoms_params >> gridff.closemols_coords >> closemols_params;
        ...
        LJParameterDB::lock();
        for (QHash<quint32, LJParameter>::const_iterator it = used_ljs.constBegin(); it != used_ljs.constEnd(); ++it)
        {
    ...
    else if (v == 4)
    {
        ...
        sds >> gridff.buffer_size >> gridff.grid_spacing >> gridff.coul_cutoff >> gridff.lj_cutoff >>
            gridff.fixedatoms_coords >> fixedatoms_params >> used_ljs;
...
/** Empty constructor */
GridFF::GridFF()
    : ConcreteProperty<GridFF, InterGroupCLJFF>(), buffer_size(2.5), grid_spacing(1.0), coul_cutoff(50), lj_cutoff(7.5)
{
...
GridFF::GridFF(const QString &name)
    : ConcreteProperty<GridFF, InterGroupCLJFF>(name), buffer_size(2.5), grid_spacing(1.0), coul_cutoff(50),
      lj_cutoff(7.5)
{
    this->setSwitchingFunction(NoCutoff());
}
...
GridFF::GridFF(const GridFF &other)
    : ConcreteProperty<GridFF, InterGroupCLJFF>(other), gridbox(other.gridbox), buffer_size(other.buffer_size),
      grid_spacing(other.grid_spacing), coul_cutoff(other.coul_cutoff), lj_cutoff(other.lj_cutoff), dimx(other.dimx),
      dimy(other.dimy), dimz(other.dimz), gridpot(other.gridpot), fixedatoms_coords(other.fixedatoms_coords),
      fixedatoms_params(other.fixedatoms_params), closemols_coords(other.closemols_coords),
      closemols_params(other.closemols_params), oldnrgs(other.oldnrgs)
{
    this->setSwitchingFunction(NoCutoff());
}
...
{
    return buffer_size == other.buffer_size and grid_spacing == other.grid_spacing and
           InterGroupCLJFF::operator==(other);
}
...
void GridFF::addFixedAtoms(const MoleculeView &fixed_atoms, const PropertyMap &map)
{
    ...
    const QVector<Vector> coords =
        fixed_atoms.molecule().property(coords_property).asA<AtomCoords>().toVector(fixed_atoms.selection());

    const QVector<SireUnits::Dimension::Charge> charges =
        fixed_atoms.molecule().property(chg_property).asA<AtomCharges>().toVector(fixed_atoms.selection());

    const QVector<LJParameter> ljs =
        fixed_atoms.molecule().property(lj_property).asA<AtomLJs>().toVector(fixed_atoms.selection());
...
void GridFF::addFixedAtoms(const SireMol::Molecules &fixed_atoms, const PropertyMap &map)
{
    ...
    for (SireMol::Molecules::const_iterator it = fixed_atoms.constBegin(); it != fixed_atoms.constEnd(); ++it)
    {
        SireMol::Molecule mol = it->molecule();
        AtomSelection selection = it->selection();

        const QVector<Vector> coords = mol.property(coords_property).asA<AtomCoords>().toVector(selection);

        const QVector<SireUnits::Dimension::Charge> charges =
            mol.property(chg_property).asA<AtomCharges>().toVector(selection);

        const QVector<LJParameter> ljs = mol.property(lj_property).asA<AtomLJs>().toVector(selection);
...
SIRE_ALWAYS_INLINE GridFF::Vector4::Vector4(const Vector &v, double chg) : x(v.x()), y(v.y()), z(v.z()), q(chg)
{
}

void GridFF::appendTo(QVector<GridFF::Vector4> &coords_and_charges, const Vector *coords,
                      const SireMM::detail::CLJParameter *params, int nats)
{
    if (nats > 0)
    {
...
{
    return QString("{ %1, %2 }").arg(*((const double *)&sseval)).arg(*(((const double *)&sseval) + 1));
}
#endif
...
void GridFF::addToGrid(const QVector<GridFF::Vector4> &coords_and_charges)
{
...
#ifdef SIRE_USE_SSE
        {
            ...
            for (int ipt = 0; ipt < (npts - 1); ipt += 2)
            {
                ...
                for (int iat = 0; iat < nats; ++iat)
                {
                    ...
                    const __m128d sse_r =
                        _mm_sqrt_pd(_mm_add_pd(_mm_mul_pd(sse_dx, sse_dx),
                                               _mm_add_pd(_mm_mul_pd(sse_dy, sse_dy), _mm_mul_pd(sse_dz, sse_dz))));
            ...
            if (remainder)
            {
                ...
                for (int iat = 0; iat < nats; ++iat)
                {
                    ...
                    total += in_cutoff * c.q * (one_over_r - one_over_Rc + one_over_Rc2 * (r - Rc));
                }
    ...
    else if (useReactionField())
    {
        ...
        const double k_rf = (1.0 / pow_3(Rc)) * ((rf_dielectric - 1) / (2 * rf_dielectric + 1));
        const double c_rf = (1.0 / Rc) * ((3 * rf_dielectric) / (2 * rf_dielectric + 1));
...
void GridFF::rebuildGrid()
{
    ...
    for (ChunkedVector<CLJMolecule>::const_iterator it = mols[0].moleculesByIndex().constBegin();
         it != mols[0].moleculesByIndex().constEnd(); ++it)
    {
    ...
    if (buffer_size > 0)
        gridbox =
            AABox::from(group0_box.minCoords() - Vector(buffer_size), group0_box.maxCoords() + Vector(buffer_size));
    ...
    Vector maxpoint =
        gridbox.minCoords() + Vector((dimx - 1) * grid_spacing, (dimy - 1) * grid_spacing, (dimz - 1) * grid_spacing);
    ...
    if (not cljmols.isEmpty())
    {
        ...
        for (ChunkedVector<CLJMolecule>::const_iterator it = cljmols.constBegin(); it != cljmols.constEnd(); ++it)
        {
            ...
            const CLJParameters::Array *params_array = cljmol.parameters().atomicParameters().constData();
...
void GridFF::calculateEnergy(const CoordGroup &coords0, const GridFF::CLJParameters::Array &params0, double &cnrg,
                             double &ljnrg)
{
    double icnrg = 0;
    double iljnrg = 0;
                ...
                for (int j = 0; j < nats1 - 1; j += 2)
                {
                    ...
                    for (int i = 0; i < nats0; ++i)
                    {
                        ...
                        // calculate the LJ energy
                        {
                            ...
                            const __m128d sse_eps = _mm_set_pd(ljpair0.epsilon(), ljpair1.epsilon());

                            // calculate (sigma/r)^6 and (sigma/r)^12
                            __m128d sse_sig_over_r2 = _mm_mul_pd(sse_sig, sse_one_over_r);
                            sse_sig_over_r2 = _mm_mul_pd(sse_sig_over_r2, sse_sig_over_r2);

                            __m128d sse_sig_over_r6 = _mm_mul_pd(sse_sig_over_r2, sse_sig_over_r2);

                            sse_sig_over_r6 = _mm_mul_pd(sse_sig_over_r6, sse_sig_over_r2);

                            __m128d sse_sig_over_r12 = _mm_mul_pd(sse_sig_over_r6, sse_sig_over_r6);

                            // calculate LJ energy (the factor of 4 is added later)
                            __m128d tmp = _mm_sub_pd(sse_sig_over_r12, sse_sig_over_r6);
    ...
    for (int i0 = 0; i0 < nats0; ++i0)
    {
        ...
        if (i_0 < 0 or i_0 >= int(dimx - 1) or j_0 < 0 or j_0 >= int(dimy - 1) or k_0 < 0 or k_0 >= int(dimz - 1))
        {
        ...
        else
        {
            ...
            //
            const Vector c000 =
                gridbox.minCoords() + Vector(i_0 * grid_spacing, j_0 * grid_spacing, k_0 * grid_spacing);
            ...
            double phi =
                (gridpot_array[i000] * (1 - R) * (1 - S) * (1 - T)) + (gridpot_array[i001] * (1 - R) * (1 - S) * (T)) +
                (gridpot_array[i010] * (1 - R) * (S) * (1 - T)) + (gridpot_array[i100] * (R) * (1 - S) * (1 - T)) +
                (gridpot_array[i011] * (1 - R) * (S) * (T)) + (gridpot_array[i101] * (R) * (1 - S) * (T)) +
                (gridpot_array[i110] * (R) * (S) * (1 - T)) + (gridpot_array[i111] * (R) * (S) * (T));
...
void GridFF::_pvt_added(quint32 groupid, const PartialMolecule &mol, const PropertyMap &map)
{
    this->mustNowRecalculateFromScratch();
...
/** Any changes to group 1 mean that the forcefield must be recalculated from scratch */
void GridFF::_pvt_changed(quint32 groupid, const QList<SireMol::Molecule> &molecules, bool auto_commit)
{
...
void GridFF::recalculateEnergy()
{
    ...
    else if (not changed_mols[1].isEmpty())
    {
        ...
        for (QHash<MolNum, ChangedMolecule>::const_iterator it = this->changed_mols[1].constBegin();
    ...
    if (must_recalculate)
    {
             ...
             it != mols[0].moleculesByIndex().constEnd(); ++it)
        {
            ...
            for (int igroup = 0; igroup < ngroups; ++igroup)
            {
                ...
                double icnrg, iljnrg;
                calculateEnergy(group, params_array[igroup], icnrg, iljnrg);
    ...
    else
    {
        ...
        for (QHash<MolNum, ChangedMolecule>::const_iterator it = this->changed_mols[0].constBegin();
             it != this->changed_mols[0].constEnd(); ++it)
        {
            ...
            else
            {
                ...
                double old_ljnrg(0);
                {
                    ...
                    const CLJParameters::Array *params_array = oldmol.parameters().atomicParameters().constData();
                ...
                double new_ljnrg(0);
                {
                    ...
                    const CLJParameters::Array *params_array = newmol.parameters().atomicParameters().constData();
                ...
                CLJEnergy new_nrg =
                    CLJEnergy(old_nrg.coulomb() + new_cnrg - old_cnrg, old_nrg.lj() + new_ljnrg - old_ljnrg);
