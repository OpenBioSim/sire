...
QDataStream &operator<<(QDataStream &ds, const DataPoint &dp)
{
    ...
    ds << dp._x << dp._y << dp._xminerr << dp._yminerr << dp._xmaxerr << dp._ymaxerr;
...
DataPoint::DataPoint() : _x(0), _y(0), _xminerr(0), _yminerr(0), _xmaxerr(0), _ymaxerr(0)
{
}

/** Construct the point (x,y) with no error */
DataPoint::DataPoint(double x, double y) : _x(x), _y(y), _xminerr(0), _yminerr(0), _xmaxerr(0), _ymaxerr(0)
{
}

/** Construct the point (x,y) with error (xerror,yerror) */
DataPoint::DataPoint(double x, double y, double xerror, double yerror)
    : _x(x), _y(y), _xminerr(std::abs(xerror)), _yminerr(std::abs(yerror)), _xmaxerr(std::abs(xerror)),
      _ymaxerr(std::abs(yerror))
{
}
...
DataPoint::DataPoint(double x, double y, double xminerror, double yminerror, double xmaxerror, double ymaxerror)
    : _x(x), _y(y), _xminerr(std::abs(xminerror)), _yminerr(std::abs(yminerror)), _xmaxerr(std::abs(xmaxerror)),
      _ymaxerr(std::abs(ymaxerror))
{
...
DataPoint::DataPoint(const DataPoint &other)
    : _x(other._x), _y(other._y), _xminerr(other._xminerr), _yminerr(other._yminerr), _xmaxerr(other._xmaxerr),
...
{
    return _x == other._x and _y == other._y and _xminerr == other._xminerr and _yminerr == other._yminerr and
...
{
    return x() + xError() >= other.x() - other.xError() and x() - xError() <= other.x() + other.xError() and

           y() + yError() >= other.y() - other.yError() and y() - yError() <= other.y() + other.yError();
}
...
{
    return x() + xMinError() >= other.x() - other.xMinError() and x() - xMinError() <= other.x() + other.xMinError() and

           y() + yMinError() >= other.y() - other.yMinError() and y() - yMinError() <= other.y() + other.yMinError();
}
...
{
    return x() + xMaxError() >= other.x() - other.xMaxError() and x() - xMaxError() <= other.x() + other.xMaxError() and

           y() + yMaxError() >= other.y() - other.yMaxError() and y() - yMaxError() <= other.y() + other.yMaxError();
}
...
/** Copy constructor */
PMF::PMF(const PMF &other) : ConcreteProperty<PMF, Property>(other), vals(other.vals)
{
}
...
QString PMF::toString() const
{
    if (vals.isEmpty())
        ...
        return QString("PMF( { deltaG() == %1, error() == %2 } )").arg(deltaG()).arg(error());
}
    ...
    for (int i = 0; i < lamvals.count() - 1; ++i)
    {
        if (lamvals[i] == lamvals[i + 1])
            throw SireError::invalid_arg(
                QObject::tr("You cannot have duplicate values of FEP windows. %1").arg(Sire::toString(lamvals)),
                CODELOC);
    }

    // check that all of the deltas match up with the supplied lambda values
    for (QMap<double, FreeEnergyAverage>::const_iterator it = fwds_deltas.constBegin(); it != fwds_deltas.constEnd();
         ++it)
    {
        if (not have_first)
        {
        ...
        {
            throw SireError::invalid_arg(QObject::tr("You cannot construct a set of FEP deltas using free energy "
        ...
        if (idx == -1)
            throw SireError::invalid_arg(
                QObject::tr("All of the FEP deltas must correspond to one of the FEP windows. "
        ...
        if (idx == lamvals.count())
            ...
            throw SireError::invalid_arg(
                QObject::tr("There should be no forwards delta (%1) for the last FEP window (%2).")
                    .arg(it.value().toString())
                    .arg(it.key()),
                CODELOC);
    }

    for (QMap<double, FreeEnergyAverage>::const_iterator it = bwds_deltas.constBegin(); it != bwds_deltas.constEnd();
         ++it)
    {
        if (not have_first)
        {
        ...
        if (idx == lamvals.count())
            ...
            throw SireError::invalid_arg(
                QObject::tr("There should be no backwards delta (%1) for the first FEP window (%2).")
                    .arg(it.value().toString())
                    .arg(it.key()),
                CODELOC);
    }
...
FEPDeltas::FEPDeltas(const QList<double> &windows, const QMap<double, FreeEnergyAverage> &deltas)
    : ConcreteProperty<FEPDeltas, Property>(), lamvals(windows), fwds_deltas(deltas)
{
...
FEPDeltas::FEPDeltas(const QList<double> &windows, const QMap<double, FreeEnergyAverage> &forwards_deltas,
                     const QMap<double, FreeEnergyAverage> &backwards_deltas)
    : ConcreteProperty<FEPDeltas, Property>(), lamvals(windows), fwds_deltas(forwards_deltas),
      bwds_deltas(backwards_deltas)
{
    std::sort(lamvals.begin(), lamvals.end());
    checkSane();
}
...
FEPDeltas::FEPDeltas(const FEPDeltas &other)
    : ConcreteProperty<FEPDeltas, Property>(other), lamvals(other.lamvals), fwds_deltas(other.fwds_deltas),
...
{
    return lamvals == other.lamvals and fwds_deltas == other.fwds_deltas and bwds_deltas == other.bwds_deltas;
}
    ...
    else
    {
        if (lamvals != other.lamvals)
            throw SireError::incompatible_error(
                QObject::tr("Cannot add together these two FEPDeltas as the lambda windows are different. "
                            "%1 vs. %2.")
                    .arg(Sire::toString(lamvals))
        ...
        if (temperature() != other.temperature())
            throw SireError::incompatible_error(
                QObject::tr("Cannot add together these two FEPDeltas as the temperature at which their "
        ...
        for (QMap<double, FreeEnergyAverage>::const_iterator it = other.fwds_deltas.constBegin();
             it != other.fwds_deltas.constEnd(); ++it)
        {
        ...
        for (QMap<double, FreeEnergyAverage>::const_iterator it = other.bwds_deltas.constBegin();
             it != other.bwds_deltas.constEnd(); ++it)
        {
...
PMF FEPDeltas::sum() const
{
    ...
    for (int i = 0; i < lamvals.count() - 1; ++i)
    {
        ...
        else
        {
            ...
            maxerr = minerr + fwds->histogram().standardError(90) + bwds->histogram().standardError(90);
        }
        ...
        points.append(DataPoint(lamvals.at(i + 1), total, 0, total_minerr, 0, total_maxerr));
    }
...
FEP::FEP(const QList<double> &windows, const QMap<double, FreeEnergyAverage> &forwards_deltas,
         const QMap<double, FreeEnergyAverage> &backwards_deltas)
    : ConcreteProperty<FEP, Property>()
{
...
FEP::FEP(const FEPDeltas &deltas) : ConcreteProperty<FEP, Property>()
{
    this->add(deltas);
}
...
void FEP::add(const QList<double> &windows, const QMap<double, FreeEnergyAverage> &deltas)
{
    this->add(FEPDeltas(windows, deltas));
}
...
void FEP::add(const QList<double> &windows, const QMap<double, FreeEnergyAverage> &forwards_deltas,
              const QMap<double, FreeEnergyAverage> &backwards_deltas)
{
    this->add(FEPDeltas(windows, forwards_deltas, backwards_deltas));
}
...
void FEP::set(int i, const QList<double> &windows, const QMap<double, FreeEnergyAverage> &deltas)
{
    set(i, FEPDeltas(windows, deltas));
}

/** Set the deltas for the ith iteration */
void FEP::set(int i, const QList<double> &windows, const QMap<double, FreeEnergyAverage> &forwards_deltas,
              const QMap<double, FreeEnergyAverage> &backwards_deltas)
{
