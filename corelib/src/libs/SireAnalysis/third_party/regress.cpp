...
unsigned int Regress::SetMatrix()
{
    vector<double> a;
    a.clear();
    a.resize((2 * degree + 1), 0.0);
    vector<double> b;
    b.clear();
    b.resize((degree + 1), 0.0);
    ...
    bool _print) const
{
    ...
    list<stREGRESS>::const_iterator b = sample.begin();
    b++;
    double area = 0.0;
    ...
    const string &_file, unsigned int _step) const
{
    ...
    if (ofs.bad())
    {
        cout << "file " << _file << "cannot be opened" << endl;
        return (false);
