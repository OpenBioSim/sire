...
namespace SireStream
{
    ...
    namespace detail
    {
        ...
        {
            const SharedDataHolderT<T> *this_ptr = dynamic_cast<const SharedDataHolderT<T> *>(this);
        ...
        */
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE const T &SharedDataRegistry::getSharedObject(quint32 id) const
        {
        ...
        template <class T, class GETPOINTER>
        SIRE_OUTOFLINE_TEMPLATE quint32 SharedDataRegistry::getID(const T &shared_object)
        {
        ...
        template <class T, class GETPOINTER>
        SIRE_OUTOFLINE_TEMPLATE void SharedDataRegistry::loadedSharedObject(quint32 id, const T &shared_object)
        {
            const void *ptr = GETPOINTER::value(shared_object);
        ...
        template <class T, class GETPOINTER>
        SIRE_OUTOFLINE_TEMPLATE bool SharedDataRegistry::contains(const T &shared_object) const
        {
            const void *ptr = GETPOINTER::value(shared_object);
    ...
    */
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::standardSave(const T &value)
    {
        this->ds << value;
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::standardLoad(T &value)
    {
        this->ds >> value;
    }
    ...
    template <class T, class GETPOINTER>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::sharedSave(const T &value)
    {
    ...
    template <class T, class GETPOINTER>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::sharedLoad(T &value, const T &default_value)
    {
        this->readVersion();
    ...
    /** This function is used to load a shared object from the stream */
    template <class T, class GETPOINTER>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::sharedLoad(T &value)
    {
    ...
    template <class T, class GETPOINTER>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::sharedLoadPointer(T &pointer)
    {
        this->readVersion();
    ...
    template <class T, class GETPOINTER>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::sharedSavePointer(const T &pointer)
    {
    ...
    template <class T, class GETPOINTER>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::sharedLoadContainer(T &container)
    {
        this->readVersion();
    ...
    template <class T, class GETPOINTER>
    SIRE_OUTOFLINE_TEMPLATE void SharedDataStream::sharedSaveContainer(const T &container)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const QSharedDataPointer<T> &objptr)
    {
        sds.sharedSavePointer<QSharedDataPointer<T>, detail::GetSharedDataPointer<QSharedDataPointer<T>, T>>(objptr);
        return sds;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, QSharedDataPointer<T> &objptr)
    {
        sds.sharedLoadPointer<QSharedDataPointer<T>, detail::GetSharedDataPointer<QSharedDataPointer<T>, T>>(objptr);

        return sds;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const SharedDataPointer<T> &objptr)
    {
        sds.sharedSavePointer<SharedDataPointer<T>, detail::GetSharedDataPointer<SharedDataPointer<T>, T>>(objptr);
    ...
    /** Specialisation of the deserialisation function for implicitly shared
        ...
        data is preserved. */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, SharedDataPointer<T> &objptr)
    {
        sds.sharedLoadPointer<SharedDataPointer<T>, detail::GetSharedDataPointer<SharedDataPointer<T>, T>>(objptr);

        return sds;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const SharedPolyPointer<T> &objptr)
    {
        sds.sharedSave<SharedPolyPointer<T>, detail::GetSharedPolyPointer<SharedPolyPointer<T>>>(objptr);

        return sds;
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, SharedPolyPointer<T> &objptr)
    {
        sds.sharedLoad<SharedPolyPointer<T>, detail::GetSharedPolyPointer<SharedPolyPointer<T>>>(objptr);
    ...
    namespace detail
    {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE SharedDataStream &sharedSaveContainer(SharedDataStream &sds, const T &container)
        {
        ...
        template <class T>
        SIRE_OUTOFLINE_TEMPLATE SharedDataStream &sharedLoadContainer(SharedDataStream &sds, T &container)
        {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const QList<T> &list)
    {
        return detail::sharedSaveContainer(sds, list);
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, QList<T> &list)
    {
        return detail::sharedLoadContainer(sds, list);
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const QVector<T> &vector)
    {
        return detail::sharedSaveContainer(sds, vector);
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, QVector<T> &vector)
    {
        return detail::sharedLoadContainer(sds, vector);
    }

    template <class Key, class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const QHash<Key, T> &hash)
    {
        return detail::sharedSaveContainer(sds, hash);
    }

    template <class Key, class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, QHash<Key, T> &hash)
    {
        return detail::sharedLoadContainer(sds, hash);
    }

    template <class Key, class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const QMap<Key, T> &map)
    {
        return detail::sharedSaveContainer(sds, map);
    }

    template <class Key, class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, QMap<Key, T> &map)
    {
        return detail::sharedLoadContainer(sds, map);
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator<<(SharedDataStream &sds, const T &value)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE SharedDataStream &operator>>(SharedDataStream &sds, T &value)
    {
