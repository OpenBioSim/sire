...
bool ArcCosh::operator==(const ExBase &other) const
{
    ...
    return other_cos != 0 and typeid(other).name() == typeid(*this).name() and
           this->argument() == other_cos->argument();
}
...
double ArcCosh::evaluate(const Values &values) const
{
    ...
    if (not val.isReal())
        throw SireMaths::domain_error(
            QObject::tr("arccosh(%1) is complex (%2): %3").arg(x().evaluate(values)).arg(val.toString(), toString()),
            CODELOC);
...
double ArcTanh::evaluate(const Values &values) const
{
    ...
    if (not val.isReal())
        throw SireMaths::domain_error(
            QObject::tr("arctanh(%1) is complex (%2): %3").arg(x().evaluate(values)).arg(val.toString(), toString()),
            CODELOC);
...
double ArcCsch::evaluate(const Values &values) const
{
    ...
    if (not val.isReal())
        throw SireMaths::domain_error(
            QObject::tr("arccsch(%1) is complex (%2): %3").arg(x().evaluate(values)).arg(val.toString(), toString()),
            CODELOC);
...
double ArcSech::evaluate(const Values &values) const
{
    ...
    if (not val.isReal())
        throw SireMaths::domain_error(
            QObject::tr("arcsech(%1) is complex (%2): %3").arg(x().evaluate(values)).arg(val.toString(), toString()),
            CODELOC);
...
double ArcCoth::evaluate(const Values &values) const
{
    ...
    if (not val.isReal())
        throw SireMaths::domain_error(
            QObject::tr("arccoth(%1) is complex (%2): %3").arg(x().evaluate(values)).arg(val.toString(), toString()),
            CODELOC);
