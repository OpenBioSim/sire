...
namespace SireFF
{
    ...
    */
    ...
    class Inter2B3DFF : public SireBase::ConcreteProperty<Inter2B3DFF<Potential>, Inter2BFF<Potential>>, public FF3D
    {
    public:
        Inter2B3DFF();
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B3DFF<Potential>::Inter2B3DFF()
        : SireBase::ConcreteProperty<Inter2B3DFF<Potential>, Inter2BFF<Potential>>(), FF3D()
    {
    }

    /** Construct, giving this forcefield a name */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B3DFF<Potential>::Inter2B3DFF(const QString &name)
        : SireBase::ConcreteProperty<Inter2B3DFF<Potential>, Inter2BFF<Potential>>(name), FF3D()
    {
    }

    /** Copy constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B3DFF<Potential>::Inter2B3DFF(const Inter2B3DFF<Potential> &other)
        : SireBase::ConcreteProperty<Inter2B3DFF<Potential>, Inter2BFF<Potential>>(other), FF3D(other)
    {
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B3DFF<Potential> &Inter2B3DFF<Potential>::operator=(const Inter2B3DFF<Potential> &other)
    {
    ...
    /** Comparison operator */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2B3DFF<Potential>::operator==(const Inter2B3DFF<Potential> &other) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2B3DFF<Potential>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Inter2B3DFF<Potential>>());
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2B3DFF<Potential>::what() const
    {
        return Inter2B3DFF<Potential>::typeName();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B3DFF<Potential> *Inter2B3DFF<Potential>::clone() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::packCoordinates()
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::recalculateEnergy()
    {
        int nmols = this->mols.count();
        const ChunkedVector<typename Potential::Molecule> mols_array = this->mols.moleculesByIndex();
        ...
        try
        {
            ...
            if (this->changed_mols.isEmpty())
            {
...

                {
                    ...
                    const ChunkedVector<SireVol::AABox> &aaboxes_array = this->mols.aaBoxesByIndex();
            ...
            else
            {
...

                {
                    ...
                    {
                        const typename Potential::ChangedMolecule &changed_mol = *(my_changed_mols.constBegin());
                        ...
                        for (int i = 0; i < my_nmols; ++i)
                        {
                            if (idx != i)
                            {
                                ...
                                {
                                    Potential::calculateEnergy(mol, changed_mol.oldParts(), my_old_nrg, old_workspace);
                                }

                                if (not spce.beyond(cutoff, new_box, aabox))
                                {
                                    Potential::calculateEnergy(mol, changed_mol.newParts(), my_new_nrg, new_workspace);
                                }
                            }
                        }
                    }
                    else
                    {
                        for (int i = 0; i < my_nmols; ++i)
                        {
                            ...
                            if (it == my_changed_mols.constEnd())
                            {
                                ...
                                for (typename QHash<MolNum, ChangedMolecule>::const_iterator it2 =
                                         my_changed_mols.constBegin();
                                     it2 != this->changed_mols.constEnd(); ++it2)
                                {
                                    if (not spce.beyond(cutoff, aabox0, it2->oldMolecule().aaBox()))
                                    {
                                        Potential::calculateEnergy(mol0, it2->oldParts(), my_old_nrg, old_workspace);
                                    }

                                    if (not spce.beyond(cutoff, aabox0, it2->newMolecule().aaBox()))
                                    {
                                        Potential::calculateEnergy(mol0, it2->newParts(), my_new_nrg, new_workspace);
                                    }
                                }
                            }
                            else if (this->changed_mols.count() > 1)
                            {
                                ...
                                if (this_changed_all)
                                {
                                    ...
                                    {
                                        Potential::calculateEnergy(it->oldMolecule(), it2->oldMolecule(), my_old_nrg,
                                                                   old_workspace);

                                        Potential::calculateEnergy(it->newMolecule(), it2->newMolecule(), my_new_nrg,
                                                                   new_workspace);
                                    }
                                }
                                else
                                {
                                    for (++it2; it2 != my_changed_mols.constEnd(); ++it2)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            // the first molecule...
                                            Potential::calculateEnergy(it->oldParts(), it2->oldMolecule(), my_old_nrg,
                                                                       old_workspace);

                                            Potential::calculateEnergy(it->newParts(), it2->newMolecule(), my_new_nrg,
                                                                       new_workspace);

                                            // now the change in energy associated with changing
                                            // the second molecule...
                                            Potential::calculateEnergy(it2->oldParts(), it->oldMolecule(), my_old_nrg,
                                                                       old_workspace);

                                            Potential::calculateEnergy(it2->newParts(), it->newMolecule(), my_new_nrg,
                                                                       new_workspace);

                                            // now remove double counted changed in mol1 with
                                            // change in mol2
                                            Potential::calculateEnergy(it->oldParts(), it2->oldParts(), my_old_nrg,
                                                                       old_workspace, -1);

                                            Potential::calculateEnergy(it->newParts(), it2->newParts(), my_new_nrg,
                                                                       new_workspace, -1);
                                        }
                                    }
                                }
                ...
                if (not this->removed_mols.isEmpty())
                {
                    ...
                    for (typename QSet<MolNum>::const_iterator it = this->removed_mols.constBegin();
                         it != this->removed_mols.constEnd(); ++it)
                    {
                        ...
                        for (++it2; it2 != this->removed_mols.constEnd(); ++it2)
                        {
                            ...
                            Potential::calculateEnergy(mol0.oldMolecule(), mol1.oldMolecule(), old_nrg, workspace);
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE SireUnits::Dimension::MolarEnergy Inter2B3DFF<Potential>::energy()
    {
        return Inter2BFF<Potential>::energy();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE SireUnits::Dimension::MolarEnergy Inter2B3DFF<Potential>::energy(const Symbol &component)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::energy(EnergyTable &energytable, double scale_energy)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::energy(EnergyTable &energytable, const Symbol &symbol,
                                                                double scale_energy)
    {
        if (scale_energy == 0)
            return;
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::force(ForceTable &forcetable, double scale_force)
    {
        if (scale_force == 0)
            return;
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::force(ForceTable &forcetable, const Symbol &symbol,
                                                               double scale_force)
    {
        if (scale_force == 0)
            return;
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::field(FieldTable &fieldtable, const Probe &prob,
                                                               double scale_field)
    {
        if (scale_field == 0)
            return;
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols; ++j)
                {
                    ...
                    Potential::calculateField(mol, probe, gridtable, workspace, scale_field);
                }
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::field(FieldTable &fieldtable, const Symbol &component,
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols; ++j)
                {
                    ...
                    Potential::calculateField(mol, probe, gridtable, component, this->components(), workspace, scale_field);
                }
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::potential(PotentialTable &potentialtable, const Probe &prob,
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols; ++j)
                {
                    ...
                    Potential::calculatePotential(mol, probe, gridtable, workspace, scale_potential);
                }
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::potential(PotentialTable &potentialtable, const Symbol &component,
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols; ++j)
                {
                    ...
                    Potential::calculatePotential(mol, probe, gridtable, component, this->components(), workspace,
                                                  scale_potential);
                }
            }
        }
    ...
    /** Calculate the fields acting at the points in the passed fieldtable
        ...
        in the field table, multiplied by the passed (optional) scaling factor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::field(FieldTable &fieldtable, double scale_field)
    {
    ...
    /** Calculate the potential acting at the points in the passed potentialtable
        ...
        in the potential table, multiplied by the passed (optional) scaling factor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B3DFF<Potential>::potential(PotentialTable &potentialtable, double scale_potential)
    {
        typename Potential::Probe default_probe;
    ...
    {
        typename Potential::Probe default_probe;
        Inter2B3DFF<Potential>::potential(potentialtable, component, default_probe, scale_potential);
    }
