...
namespace SireFF
{
    ...
    namespace detail
    {
        SIREFF_EXPORT void throwIntra2B2GFFIncompatibeScaleFactorsError(const QString &molname, MolNum molnum, quint64 v0,
                                                                        quint64 v1);
    }
    ...
    */
    template <class Potential>
    class Intra2B2GFF : public SireBase::ConcreteProperty<Intra2B2GFF<Potential>, G2FF>, public Potential
    {
    ...
    protected:
        ...
        void _pvt_added(quint32 groupid, const PartialMolecule &mol, const PropertyMap &map);
        ...
        bool _pvt_wouldChangeProperties(quint32 groupid, MolNum molnum, const PropertyMap &map) const;
    ...
    /** Constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2GFF<Potential>::Intra2B2GFF()
        : SireBase::ConcreteProperty<Intra2B2GFF<Potential>, G2FF>(), Potential()
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2GFF<Potential>::Intra2B2GFF(const QString &ffname)
        : SireBase::ConcreteProperty<Intra2B2GFF<Potential>, G2FF>(), Potential()
    {
        FF::setName(ffname);
    }
    ...
    /** Copy constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2GFF<Potential>::Intra2B2GFF(const Intra2B2GFF<Potential> &other)
        : SireBase::ConcreteProperty<Intra2B2GFF<Potential>, G2FF>(other), Potential(other),
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2GFF<Potential> &Intra2B2GFF<Potential>::operator=(const Intra2B2GFF<Potential> &other)
    {
        if (this != &other)
        {
            G2FF::operator=(other);
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2B2GFF<Potential>::operator==(const Intra2B2GFF<Potential> &other) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Intra2B2GFF<Potential>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Intra2B2GFF<Potential>>());
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Intra2B2GFF<Potential>::what() const
    {
        return Intra2B2GFF<Potential>::typeName();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2GFF<Potential> *Intra2B2GFF<Potential>::clone() const
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::assertSameIntraScaleFactors(MolNum molnum) const
    {
        if (mols[0].contains(molnum) and mols[1].contains(molnum))
        {
            const typename Potential::Molecule &mol0 = mols[0].moleculesByIndex()[mols[0].indexOf(molnum)];

            const typename Potential::Molecule &mol1 = mols[1].moleculesByIndex()[mols[1].indexOf(molnum)];

            if (mol0.parameters().intraScaleFactors() != mol1.parameters().intraScaleFactors())
            {
                SireFF::detail::throwIntra2B2GFFIncompatibeScaleFactorsError(
                    mol0.molecule().name(), mol0.molecule().number(), mol0.molecule().version(), mol1.molecule().version());
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const typename Potential::Components &Intra2B2GFF<Potential>::components() const
    {
        return ffcomponents;
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::_pvt_updateName()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2B2GFF<Potential>::setProperty(const QString &name, const Property &value)
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Property &Intra2B2GFF<Potential>::property(const QString &name) const
    {
    ...
    /** Return whether or not this forcefield contains a property
        called 'name' */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2B2GFF<Potential>::containsProperty(const QString &name) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Properties &Intra2B2GFF<Potential>::properties() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::mustNowRecalculateFromScratch()
    {
    ...
    /** Return whether or not we need to record the changes to this
        ...
        from scratch) */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2B2GFF<Potential>::recordingChanges() const
    {
        return not(G2FF::isDirty() and changed_mols[0].isEmpty() and changed_mols[1].isEmpty());
    }

    /** Record the change described in 'change' */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::recordChange(quint32 groupid,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const FFComponent &Intra2B2GFF<Potential>::_pvt_components() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::changedPotential()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::_pvt_added(quint32 groupid, const PartialMolecule &molecule,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::_pvt_removed(quint32 groupid, const PartialMolecule &molecule)
    {
        if (this->recordingChanges())
        {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::_pvt_changed(quint32 groupid, const SireMol::Molecule &molecule,
                                                                      bool auto_commit)
    {
        Molecules old_state = mols[groupid];
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::_pvt_changed(quint32 groupid,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::_pvt_removedAll(quint32 groupid)
    {
        mols[groupid].clear();
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2B2GFF<Potential>::_pvt_wouldChangeProperties(quint32 groupid, MolNum molnum,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2GFF<Potential>::recalculateEnergy()
    {
        int nmols0 = mols[0].count();
        const ChunkedVector<typename Potential::Molecule> &mols0_array = mols[0].moleculesByIndex();

        int nmols1 = mols[1].count();
        const ChunkedVector<typename Potential::Molecule> &mols1_array = mols[1].moleculesByIndex();

        if (changed_mols[0].count() == nmols0 or changed_mols[1].count() == nmols1)
        {
        ...
        try
        {
            ...
            if (changed_mols[0].isEmpty() and changed_mols[1].isEmpty())
            {
                ...
                if (nmols0 <= nmols1)
                {
                    for (int i = 0; i < nmols0; ++i)
                    {
                        ...
                        const typename Potential::Molecule &mol1 = mols1_array[mols[1].indexOf(mol0.number())];
                ...
                else
                {
                    for (int j = 0; j < nmols1; ++j)
                    {
                        ...
                        const typename Potential::Molecule &mol0 = mols0_array[mols[0].indexOf(mol1.number())];
            ...
            else
            {
                ...
                {
                    for (typename QHash<MolNum, ChangedMolecule>::const_iterator it = changed_mols[1].constBegin();
                         it != changed_mols[1].constEnd(); ++it)
                    {
                        ...
                        const typename Potential::Molecule &mol0 = mols0_array[mols[0].indexOf(molnum)];

                        Potential::calculateEnergy(it->oldParts(), mol0, old_nrg, old_workspace);

                        Potential::calculateEnergy(it->newParts(), mol0, new_nrg, new_workspace);
                    }
                }
                else if (changed_mols[1].isEmpty())
                {
                    for (typename QHash<MolNum, ChangedMolecule>::const_iterator it = changed_mols[0].constBegin();
                         it != changed_mols[0].constEnd(); ++it)
                    {
                        MolNum molnum = it.key();
                        ...
                        const typename Potential::Molecule &mol1 = mols1_array[mols[1].indexOf(molnum)];

                        Potential::calculateEnergy(it->oldParts(), mol1, old_nrg, old_workspace);

                        Potential::calculateEnergy(it->newParts(), mol1, new_nrg, new_workspace);
                    }
                }
                else
                {
                         ...
                         it != changed_mols[0].constEnd(); ++it)
                    {
                        ...
                        typename QHash<MolNum, ChangedMolecule>::const_iterator it2 = changed_mols[1].constFind(molnum);

                        if (it2 != changed_mols[1].constEnd())
                        {
                            // both changed!
                            Potential::calculateEnergy(it->oldMolecule(), it2->oldMolecule(), old_nrg, old_workspace);

                            Potential::calculateEnergy(it->newMolecule(), it2->newMolecule(), new_nrg, new_workspace);
                        }
                         ...
                         it != changed_mols[1].constEnd(); ++it)
                    {
                        ...
                        if (not changed_mols[0].contains(molnum) and mols[0].contains(molnum))
                        {
                            const typename Potential::Molecule &mol0 = mols0_array[mols[0].indexOf(molnum)];

                            Potential::calculateEnergy(mol0, it->oldParts(), old_nrg, old_workspace);

                            Potential::calculateEnergy(mol0, it->newParts(), new_nrg, new_workspace);
                        }
                    }
                }
...
namespace SireFF
{
    namespace detail
    {
        ...
        template <class Potential>
        const RegisterMetaType<SireFF::Intra2B2GFF<Potential>> Intra2B2GRMT<Potential>::r_intra2b2gff;
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireFF::Intra2B2GFF<Potential> &intra2b2gff)
{
    SireStream::writeHeader(ds, SireFF::detail::Intra2B2GRMT<Potential>::r_intra2b2gff, 1);

    SireStream::SharedDataStream sds(ds);

    sds << intra2b2gff.mols[0] << intra2b2gff.mols[1] << intra2b2gff.changed_mols[0] << intra2b2gff.changed_mols[1]
        << static_cast<const Potential &>(intra2b2gff) << static_cast<const SireFF::G2FF &>(intra2b2gff);
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireFF::Intra2B2GFF<Potential> &intra2b2gff)
{
    SireStream::VersionID v = SireStream::readHeader(ds, SireFF::detail::Intra2B2GRMT<Potential>::r_intra2b2gff);

    if (v == 1)
    {
        SireStream::SharedDataStream sds(ds);

        sds >> intra2b2gff.mols[0] >> intra2b2gff.mols[1] >> intra2b2gff.changed_mols[0] >>
            intra2b2gff.changed_mols[1] >> static_cast<Potential &>(intra2b2gff) >>
        ...
        throw SireStream::version_error(v, "1", SireFF::detail::Intra2B2GRMT<Potential>::r_intra2b2gff, CODELOC);
