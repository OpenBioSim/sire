...
namespace SireFF
{
    ...
    */
    template <class Potential>
    class Inter2B2GFF : public SireBase::ConcreteProperty<Inter2B2GFF<Potential>, G2FF>, public Potential
    {
...
#if !defined(SIRE_SKIP_INLINE_FUNCTIONS) && !defined(InterCLJFF_hpp__pyplusplus_wrapper) && \
    !defined(InterCLJFFBase_hpp__pyplusplus_wrapper) &&                                     \
    !defined(CLJPotentialInterface_InterCLJPotential__hpp__pyplusplus_wrapper) &&           \
    !defined(InterGroupCLJFFBase_hpp__pyplusplus_wrapper) && !defined(InterGroupCLJFF_hpp__pyplusplus_wrapper)
    /** Constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2GFF<Potential>::Inter2B2GFF()
        : SireBase::ConcreteProperty<Inter2B2GFF<Potential>, G2FF>(), Potential()
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2GFF<Potential>::Inter2B2GFF(const QString &ffname)
        : SireBase::ConcreteProperty<Inter2B2GFF<Potential>, G2FF>(), Potential()
    {
        FF::setName(ffname);
    }
    ...
    /** Copy constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2GFF<Potential>::Inter2B2GFF(const Inter2B2GFF<Potential> &other)
        : SireBase::ConcreteProperty<Inter2B2GFF<Potential>, G2FF>(other), Potential(other),
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2GFF<Potential> &Inter2B2GFF<Potential>::operator=(const Inter2B2GFF<Potential> &other)
    {
        if (this != &other)
        {
            G2FF::operator=(other);
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2B2GFF<Potential>::operator==(const Inter2B2GFF<Potential> &other) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2B2GFF<Potential>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Inter2B2GFF<Potential>>());
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2B2GFF<Potential>::what() const
    {
        return Inter2B2GFF<Potential>::typeName();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2GFF<Potential> *Inter2B2GFF<Potential>::clone() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const typename Potential::Components &Inter2B2GFF<Potential>::components() const
    {
        return ffcomponents;
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::_pvt_updateName()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2B2GFF<Potential>::setProperty(const QString &name, const Property &value)
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Property &Inter2B2GFF<Potential>::property(const QString &name) const
    {
    ...
    /** Return whether or not this forcefield contains a property
        called 'name' */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2B2GFF<Potential>::containsProperty(const QString &name) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Properties &Inter2B2GFF<Potential>::properties() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::mustNowRecalculateFromScratch()
    {
    ...
    /** Return whether or not we need to record the changes to this
        ...
        from scratch) */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2B2GFF<Potential>::recordingChanges() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::recordChange(quint32 groupid,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const FFComponent &Inter2B2GFF<Potential>::_pvt_components() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::changedPotential()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::_pvt_added(quint32 groupid, const PartialMolecule &molecule,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::_pvt_removed(quint32 groupid, const PartialMolecule &molecule)
    {
        if (this->recordingChanges())
        {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::_pvt_changed(quint32 groupid, const SireMol::Molecule &molecule,
                                                                      bool auto_commit)
    {
        if (this->recordingChanges())
        {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::_pvt_changed(quint32 groupid,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::_pvt_removedAll(quint32 groupid)
    {
        mols[groupid].clear();
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2B2GFF<Potential>::_pvt_wouldChangeProperties(quint32 groupid, MolNum molnum,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2GFF<Potential>::recalculateEnergy()
    {
        int nmols0 = mols[0].count();
        const SireBase::ChunkedVector<typename Potential::Molecule> &mols0_array = mols[0].moleculesByIndex();

        int nmols1 = mols[1].count();
        const SireBase::ChunkedVector<typename Potential::Molecule> &mols1_array = mols[1].moleculesByIndex();
        ...
        try
        {
            ...
            else
            {
                ...
                if (changed_mols[0].count() == 1 and changed_mols[1].isEmpty())
                {
                    ...
                    const typename Potential::ChangedMolecule &changed_mol = *(changed_mols[0].constBegin());

                    for (int i = 0; i < nmols1; ++i)
                    {
                        const typename Potential::Molecule &mol = mols1_array[i];

                        Potential::calculateEnergy(mol, changed_mol.oldParts(), old_nrg, old_workspace);

                        Potential::calculateEnergy(mol, changed_mol.newParts(), new_nrg, new_workspace);
                    }
                }
                else if (changed_mols[0].isEmpty() and changed_mols[1].count() == 1)
                {
                    // only need to calculate the energy of changed_mol0 with changed_mols1
                    const typename Potential::ChangedMolecule &changed_mol = *(changed_mols[1].constBegin());
                ...
                else
                {
                    ...
                    if (not changed_mols[0].isEmpty())
                    {
                        for (int i = 0; i < nmols1; ++i)
                        {
                            ...
                            {
                                Potential::calculateEnergy(mol1, it->oldParts(), old_nrg, old_workspace);

                                Potential::calculateEnergy(mol1, it->newParts(), new_nrg, new_workspace);
                            }
                        }
                    }
                    ...
                    {
                        for (typename QHash<MolNum, ChangedMolecule>::const_iterator it0 = changed_mols[0].constBegin();
                             it0 != changed_mols[0].constEnd(); ++it0)
                        {
                            bool changed_all_0 = it0->changedAll();

                            for (typename QHash<MolNum, ChangedMolecule>::const_iterator it1 = changed_mols[1].constBegin();
                                 it1 != changed_mols[1].constEnd(); ++it1)
                            {
                                if (changed_all_0 or it1->changedAll())
                                {
                                    // need to do the whole molecule calculation
                                    Potential::calculateEnergy(it0->oldMolecule(), it1->oldMolecule(), old_nrg,
                                                               old_workspace);

                                    Potential::calculateEnergy(it0->newMolecule(), it1->newMolecule(), new_nrg,
                                                               new_workspace);
                                }
                                else
                                {
                                    ...
                                    Potential::calculateEnergy(it0->oldParts(), it1->oldMolecule(), old_nrg, old_workspace);

                                    Potential::calculateEnergy(it0->newParts(), it1->newMolecule(), new_nrg, new_workspace);

                                    Potential::calculateEnergy(it1->oldParts(), it0->oldMolecule(), old_nrg, old_workspace);

                                    Potential::calculateEnergy(it1->newParts(), it0->newMolecule(), new_nrg, new_workspace);

                                    // now we need to subtract the double counted changed
                                    // parts of mol0 with changed parts of mol1
                                    Potential::calculateEnergy(it0->oldParts(), it1->oldParts(), old_nrg, old_workspace,
                                                               -1);

                                    Potential::calculateEnergy(it0->newParts(), it1->newParts(), new_nrg, new_workspace,
                                                               -1);
                                }
                            }
                        }
...
namespace SireFF
{
    namespace detail
    {
        ...
        template <class Potential>
        const RegisterMetaType<SireFF::Inter2B2GFF<Potential>> Inter2B2GRMT<Potential>::r_inter2b2gff;
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireFF::Inter2B2GFF<Potential> &inter2b2gff)
{
    SireStream::writeHeader(ds, SireFF::detail::Inter2B2GRMT<Potential>::r_inter2b2gff, 1);

    SireStream::SharedDataStream sds(ds);

    sds << inter2b2gff.mols[0] << inter2b2gff.mols[1] << inter2b2gff.changed_mols[0] << inter2b2gff.changed_mols[1]
        << static_cast<const Potential &>(inter2b2gff) << static_cast<const SireFF::G2FF &>(inter2b2gff);
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireFF::Inter2B2GFF<Potential> &inter2b2gff)
{
    SireStream::VersionID v = SireStream::readHeader(ds, SireFF::detail::Inter2B2GRMT<Potential>::r_inter2b2gff);

    if (v == 1)
    {
        SireStream::SharedDataStream sds(ds);

        sds >> inter2b2gff.mols[0] >> inter2b2gff.mols[1] >> inter2b2gff.changed_mols[0] >>
            inter2b2gff.changed_mols[1] >> static_cast<Potential &>(inter2b2gff) >>
        ...
        throw SireStream::version_error(v, "1", SireFF::detail::Inter2B2GRMT<Potential>::r_inter2b2gff, CODELOC);
