...
namespace SireFF
{
    ...
    */
    ...
    class Inter2B2G3DFF : public SireBase::ConcreteProperty<Inter2B2G3DFF<Potential>, Inter2B2GFF<Potential>>, public FF3D
    {
    public:
        Inter2B2G3DFF();
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2G3DFF<Potential>::Inter2B2G3DFF()
        : SireBase::ConcreteProperty<Inter2B2G3DFF<Potential>, Inter2B2GFF<Potential>>(), FF3D()
    {
    }

    /** Construct, giving this forcefield a name */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2G3DFF<Potential>::Inter2B2G3DFF(const QString &name)
        : SireBase::ConcreteProperty<Inter2B2G3DFF<Potential>, Inter2B2GFF<Potential>>(name), FF3D()
    {
    }

    /** Copy constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2G3DFF<Potential>::Inter2B2G3DFF(const Inter2B2G3DFF<Potential> &other)
        : SireBase::ConcreteProperty<Inter2B2G3DFF<Potential>, Inter2B2GFF<Potential>>(other), FF3D(other)
    {
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2G3DFF<Potential> &Inter2B2G3DFF<Potential>::operator=(
        const Inter2B2G3DFF<Potential> &other)
    {
    ...
    /** Comparison operator */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2B2G3DFF<Potential>::operator==(const Inter2B2G3DFF<Potential> &other) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2B2G3DFF<Potential>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Inter2B2G3DFF<Potential>>());
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2B2G3DFF<Potential>::what() const
    {
        return Inter2B2G3DFF<Potential>::typeName();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2B2G3DFF<Potential> *Inter2B2G3DFF<Potential>::clone() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::recalculateEnergy()
    {
        int nmols0 = this->mols[0].count();
        const SireBase::ChunkedVector<typename Potential::Molecule> &mols0_array = this->mols[0].moleculesByIndex();

        int nmols1 = this->mols[1].count();
        const SireBase::ChunkedVector<typename Potential::Molecule> &mols1_array = this->mols[1].moleculesByIndex();
        ...
        try
        {
            ...
            const SireBase::ChunkedVector<SireVol::AABox> &aaboxes0_array = this->mols[0].aaBoxesByIndex();

            const SireBase::ChunkedVector<SireVol::AABox> &aaboxes1_array = this->mols[1].aaBoxesByIndex();
            ...
            else
            {
                ...
                if (this->changed_mols[0].count() == 1 and this->changed_mols[1].isEmpty())
                {
                    ...
                    const typename Potential::ChangedMolecule &changed_mol = *(this->changed_mols[0].constBegin());
                ...
                {
                    // only need to calculate the energy of changed_mol0 with changed_mols1
                    const typename Potential::ChangedMolecule &changed_mol = *(this->changed_mols[1].constBegin());
                    ...
                    if (not this->changed_mols[1].isEmpty())
                    {
                        for (int i = 0; i < nmols0; ++i)
                        {
                            ...
                            for (typename QHash<MolNum, ChangedMolecule>::const_iterator it =
                                 ...
                                 it != this->changed_mols[1].constEnd(); ++it)
                            {
                    ...
                    if (not this->changed_mols[0].isEmpty())
                    {
                        for (int i = 0; i < nmols1; ++i)
                        {
                            ...
                            for (typename QHash<MolNum, ChangedMolecule>::const_iterator it =
                                 ...
                                 it != this->changed_mols[0].constEnd(); ++it)
                            {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE SireUnits::Dimension::MolarEnergy Inter2B2G3DFF<Potential>::energy()
    {
        return Inter2B2GFF<Potential>::energy();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE SireUnits::Dimension::MolarEnergy Inter2B2G3DFF<Potential>::energy(const Symbol &component)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::energy(EnergyTable &energytable, double scale_energy)
    {
        throw SireError::incomplete_code(QObject::tr("Inter2B2G3DFF does not yet support energy calculations!"), CODELOC);
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::energy(EnergyTable &energytable, const Symbol &symbol,
                                                                  double scale_energy)
    {
        if (scale_energy == 0)
            return;
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::force(ForceTable &forcetable, double scale_force)
    {
        if (scale_force == 0)
            return;
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::force(ForceTable &forcetable, const Symbol &symbol,
                                                                 double scale_force)
    {
        if (scale_force == 0)
            return;
        ...
        for (int i = 0; i < nforcemols; ++i)
        {
            ...
            if (this->mols[0].contains(molnum))
            {
                ...
                for (int j = 0; j < nmols1; ++j)
                {
                    ...
                    Potential::calculateForce(mol0, mol1, moltable, symbol, this->components(), workspace, scale_force);
                }
            }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::field(FieldTable &fieldtable, const Probe &prob,
                                                                 double scale_field)
    {
        if (scale_field == 0)
            return;
            ...
            for (int i = 0; i < nfieldmols; ++i)
            {
                ...
                if (this->mols[0].contains(molnum))
                {
                    ...
                    for (int j = 0; j < nmols1; ++j)
                    {
                        ...
                        Potential::calculateField(mol0, mol1, probe, moltable, workspace, scale_field);
                    }
                }
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols0; ++j)
                {
                    ...
                    Potential::calculateField(mol0, probe, gridtable, workspace, scale_field);
                }
                ...
                for (int j = 0; j < nmols1; ++j)
                {
                    ...
                    Potential::calculateField(mol1, probe, gridtable, workspace, scale_field);
                }
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::field(FieldTable &fieldtable, const Symbol &component,
            ...
            for (int i = 0; i < nfieldmols; ++i)
            {
                ...
                if (this->mols[0].contains(molnum))
                {
                    ...
                    for (int j = 0; j < nmols1; ++j)
                    {
                        ...
                        Potential::calculateField(mol0, mol1, probe, moltable, component, this->components(), workspace,
                                                  scale_field);
                    }
                }
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols0; ++j)
                {
                    ...
                    Potential::calculateField(mol0, probe, gridtable, component, this->components(), workspace,
                                              scale_field);
                }
                ...
                for (int j = 0; j < nmols1; ++j)
                {
                    ...
                    Potential::calculateField(mol1, probe, gridtable, component, this->components(), workspace,
                                              scale_field);
                }
            }
        }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::potential(PotentialTable &potentialtable, const Probe &prob,
            ...
            for (int i = 0; i < npotmols; ++i)
            {
                ...
                if (this->mols[0].contains(molnum))
                {
                    ...
                    for (int j = 0; j < nmols1; ++j)
                    {
                        ...
                        Potential::calculatePotential(mol0, mol1, probe, moltable, workspace, scale_potential);
                    }
                }
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols0; ++j)
                {
                    ...
                    Potential::calculatePotential(mol0, probe, gridtable, workspace, scale_potential);
                }
                ...
                for (int j = 0; j < nmols1; ++j)
                {
                    ...
                    Potential::calculatePotential(mol1, probe, gridtable, workspace, scale_potential);
                }
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::potential(PotentialTable &potentialtable,
            ...
            for (int i = 0; i < npotmols; ++i)
            {
                ...
                if (this->mols[0].contains(molnum))
                {
                    ...
                    for (int j = 0; j < nmols1; ++j)
                    {
                        ...
                        Potential::calculatePotential(mol0, mol1, probe, moltable, component, this->components(), workspace,
                                                      scale_potential);
                    }
                }
        ...
        if (ngrids > 0)
        {
            ...
            for (int i = 0; i < ngrids; ++i)
            {
                ...
                for (int j = 0; j < nmols0; ++j)
                {
                    ...
                    Potential::calculatePotential(mol0, probe, gridtable, component, this->components(), workspace,
                                                  scale_potential);
                }
                ...
                for (int j = 0; j < nmols1; ++j)
                {
                    ...
                    Potential::calculatePotential(mol1, probe, gridtable, component, this->components(), workspace,
                                                  scale_potential);
                }
            }
        }
    ...
    /** Calculate the fields acting at the points in the passed fieldtable
        ...
        in the field table, multiplied by the passed (optional) scaling factor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::field(FieldTable &fieldtable, double scale_field)
    {
    ...
    {
        typename Potential::Probe default_probe;
        Inter2B2G3DFF<Potential>::field(fieldtable, component, default_probe, scale_field);
    }
    ...
    /** Calculate the potential acting at the points in the passed potentialtable
        ...
        in the potential table, multiplied by the passed (optional) scaling factor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2B2G3DFF<Potential>::potential(PotentialTable &potentialtable, double scale_potential)
    {
        typename Potential::Probe default_probe;
    ...
    {
        typename Potential::Probe default_probe;
        Inter2B2G3DFF<Potential>::potential(potentialtable, component, default_probe, scale_potential);
    }
