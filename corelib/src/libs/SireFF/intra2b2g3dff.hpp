...
namespace SireFF
{
    ...
    */
    ...
    class Intra2B2G3DFF : public SireBase::ConcreteProperty<Intra2B2G3DFF<Potential>, Intra2B2GFF<Potential>>, public FF3D
    {
    public:
        Intra2B2G3DFF();
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2G3DFF<Potential>::Intra2B2G3DFF()
        : SireBase::ConcreteProperty<Intra2B2G3DFF<Potential>, Intra2B2GFF<Potential>>(), FF3D()
    {
    }

    /** Construct, giving this forcefield a name */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2G3DFF<Potential>::Intra2B2G3DFF(const QString &name)
        : SireBase::ConcreteProperty<Intra2B2G3DFF<Potential>, Intra2B2GFF<Potential>>(name), FF3D()
    {
    }

    /** Copy constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2G3DFF<Potential>::Intra2B2G3DFF(const Intra2B2G3DFF<Potential> &other)
        : SireBase::ConcreteProperty<Intra2B2G3DFF<Potential>, Intra2B2GFF<Potential>>(other), FF3D(other)
    {
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2G3DFF<Potential> &Intra2B2G3DFF<Potential>::operator=(
        const Intra2B2G3DFF<Potential> &other)
    {
    ...
    /** Comparison operator */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2B2G3DFF<Potential>::operator==(const Intra2B2G3DFF<Potential> &other) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Intra2B2G3DFF<Potential>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Intra2B2G3DFF<Potential>>());
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Intra2B2G3DFF<Potential>::what() const
    {
        return Intra2B2G3DFF<Potential>::typeName();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2B2G3DFF<Potential> *Intra2B2G3DFF<Potential>::clone() const
    {
        return new Intra2B2G3DFF<Potential>(*this);
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE SireUnits::Dimension::MolarEnergy Intra2B2G3DFF<Potential>::energy()
    {
        return Intra2B2GFF<Potential>::energy();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE SireUnits::Dimension::MolarEnergy Intra2B2G3DFF<Potential>::energy(const Symbol &component)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::energy(EnergyTable &energytable, double scale_energy)
    {
        throw SireError::incomplete_code(QObject::tr("Intra2B2G3DFF does not yet support energy calculations!"), CODELOC);
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::energy(EnergyTable &energytable, const Symbol &symbol,
                                                                  double scale_energy)
    {
        throw SireError::incomplete_code(QObject::tr("Intra2B2G3DFF does not yet support energy calculations!"), CODELOC);
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::force(ForceTable &forcetable, double scale_force)
    {
        if (scale_force == 0)
            return;
        ...
        const ChunkedVector<typename Potential::Molecule> &mols0_array = this->mols[0].moleculesByIndex();
        const ChunkedVector<typename Potential::Molecule> &mols1_array = this->mols[1].moleculesByIndex();
        ...
        for (int i = 0; i < nforcemols; ++i)
        {
            ...
            if (this->mols[0].contains(molnum) and this->mols[1].contains(molnum))
            {
                ...
                Potential::calculateForce(mol1, mol0, moltable, workspace, scale_force);
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::force(ForceTable &forcetable, const Symbol &symbol,
                                                                 double scale_force)
    {
        if (scale_force == 0)
            return;
        ...
        for (int i = 0; i < nforcemols; ++i)
        {
            ...
            if (this->mols[0].contains(molnum) and this->mols[1].contains(molnum))
            {
                ...
                // now add on the forces on mols[1] caused by mols[0]
                Potential::calculateForce(mol1, mol0, moltable, symbol, this->components(), workspace, scale_force);
            }
        }
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::field(FieldTable &fieldtable, const Probe &prob,
                                                                 double scale_field)
    {
        if (scale_field == 0)
            return;
        ...
        for (int i = 0; i < nfieldmols; ++i)
        {
            ...
            if (this->mols[0].contains(molnum) and this->mols[1].contains(molnum))
            {
                ...
                // now add on the forces on mols[1] caused by mols[0]
                Potential::calculateField(mol1, mol0, probe, moltable, workspace, scale_field);
            }
        }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::field(FieldTable &fieldtable, const Symbol &component,
                                                                 const Probe &prob, double scale_field)
    {
        ...
        for (int i = 0; i < nfieldmols; ++i)
        {
            ...
            if (this->mols[0].contains(molnum) and this->mols[1].contains(molnum))
            {
                ...
                // now add on the forces on mols[1] caused by mols[0]
                Potential::calculateField(mol1, mol0, probe, moltable, component, this->components(), workspace,
                                          scale_field);
            }
        }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::potential(PotentialTable &potentialtable, const Probe &prob,
                                                                     double scale_potential)
    {
        ...
        for (int i = 0; i < npotentialmols; ++i)
        {
            ...
            if (this->mols[0].contains(molnum) and this->mols[1].contains(molnum))
            {
                ...
                Potential::calculatePotential(mol1, mol0, probe, moltable, workspace, scale_potential);
            }
        }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::potential(PotentialTable &potentialtable,
                                                                     ...
                                                                     double scale_potential)
    {
        ...
        for (int i = 0; i < npotentialmols; ++i)
        {
            ...
            if (this->mols[0].contains(molnum) and this->mols[1].contains(molnum))
            {
                ...
                // now add on the potential on mols[1] caused by mols[0]
                Potential::calculatePotential(mol1, mol0, probe, moltable, component, this->components(), workspace,
                                              scale_potential);
            }
        }
    ...
    /** Calculate the fields acting at the points in the passed fieldtable
        ...
        in the field table, multiplied by the passed (optional) scaling factor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::field(FieldTable &fieldtable, double scale_field)
    {
    ...
    /** Calculate the potential acting at the points in the passed potentialtable
        ...
        in the potential table, multiplied by the passed (optional) scaling factor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2B2G3DFF<Potential>::potential(PotentialTable &potentialtable, double scale_potential)
    {
        typename Potential::Probe default_probe;
    ...
    {
        typename Potential::Probe default_probe;
        Intra2B2G3DFF<Potential>::potential(potentialtable, component, default_probe, scale_potential);
    }
