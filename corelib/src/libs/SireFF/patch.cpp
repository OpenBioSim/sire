...
QDataStream &operator<<(QDataStream &ds, const Patch &patch)
{
    ...
    sds << patch.coords << patch.params << patch.idx_to_beadid << patch.beadid_to_idx
        << static_cast<const Property &>(patch);
...
Patch::Patch(const Patch &other)
    : ConcreteProperty<Patch, Property>(other), coords(other.coords), params(other.params),
      idx_to_beadid(other.idx_to_beadid), beadid_to_idx(other.beadid_to_idx), aabox(other.aabox)
{
}
...
bool Patch::operator==(const Patch &other) const
{
    return this == &other or
           (coords == other.coords and params == other.params and idx_to_beadid == other.idx_to_beadid and
            beadid_to_idx == other.beadid_to_idx and Property::operator==(other));
}
...
int Patch::getLocation(quint32 beadid) const
{
    ...
    if (idx == -1)
        throw SireError::invalid_key(QObject::tr("There is no bead with ID %1 in this patch. Available beads "
                                                 "are %1.")
                                         .arg(beadid)
                                         .arg(Sire::toString(beadid_to_idx.keys())),
                                     CODELOC);
...
QString Patch::toString() const
{
    ...
    return QObject::tr("Patch( aaBox() == %1, nBeads() == %2 )").arg(this->aaBox().toString()).arg(this->nBeads());
}
...
FFBead Patch::add(quint32 beadid, const CoordGroup &groupcoords, const FFParameters &groupparams)
{
...
/** Internal function used to add the array of beads with the passed
    array of beadids */
QHash<quint32, FFBead> Patch::add(const QVarLengthArray<quint32> &beadids, const CoordGroupArray &groupcoords,
                                  const FFParametersArray &groupparams)
{
    if (beadids.count() != groupcoords.count() or beadids.count() != groupparams.count())
    {
        throw SireError::incompatible_error(QObject::tr("You must ensure that the number of CoordGroups (%1) "
...
int Patch::getBeadIdx(quint32 beadid) const
{
    ...
    if (idx == -1)
        throw SireError::program_bug(
            QObject::tr("There is a bug as there is no bead with ID %1 in this Patch!").arg(beadid), CODELOC);
...
FFBeadChange Patch::update(quint32 beadid, const CoordGroup &new_coords)
{
    ...
    if (old_coords != new_coords)
    {
        ...
        delta = FFBeadChange(FFBead(old_coords, beadparams), FFBead(new_coords, beadparams));
...
/** Update the bead with ID 'beadid' with the new set of passed coordinates
    ...
    \throw SireError::incompatible_error
*/
FFBeadChange Patch::update(quint32 beadid, const CoordGroup &new_coords, const FFParameters &new_params)
{
    int idx = getBeadIdx(beadid);
...
/** Update the specified beads with the specified coords
    ...
    \throw SireError::incompatible_error
*/
QHash<quint32, FFBeadChange> Patch::update(const QVarLengthArray<quint32> &beadids, const CoordGroupArray &new_coords)
{
    if (beadids.count() != new_coords.count())
        throw SireError::invalid_arg(QObject::tr("You must pass in the same number of CoordGroups (%1) as there are "
    ...
    {
        FFBeadChange delta = this->update(beadids.constData()[0], new_coords.constData()[0]);
        ...
        for (int i = 0; i < beadids.count(); ++i)
        {
            ...
            if (coords.at(idx) != new_coords.constData()[i])
            {
                ...
                deltas.insert(beadids.constData()[i],
                              FFBeadChange(FFBead(coords.at(idx), beadparams), FFBead(new_coords.at(i), beadparams)));
            }
        }
...
/** Update the specified beads with the specified forcefield parameters
    ...
    \throw SireError::incompatible_error
*/
QHash<quint32, FFBeadChange> Patch::update(const QVarLengthArray<quint32> &beadids, const FFParametersArray &new_params)
{
    if (beadids.count() != new_params.count())
        throw SireError::invalid_arg(QObject::tr("You must pass in the same number of parameters (%1) as there are "
                                                 "beads specified in the array (%2).")
    ...
    else
    {
        ...
        for (int i = 0; i < beadids.count(); ++i)
        {
            ...
            deltas.insert(beadids.constData()[i], FFBeadChange(FFBead(coords.at(idx), params.read().at(idx)),
...
{
    if (beadids.count() != new_coords.count() or beadids.count() != new_params.count())
    {
        throw SireError::incompatible_error(QObject::tr("You must ensure that the number of CoordGroups (%1) "
                                                        "is equal to the number of parameter groups (%2) and "
                                                        "that is equal to the number of specified beads (%3).")
    ...
    else
    {
        ...
        for (int i = 0; i < beadids.count(); ++i)
        {
            ...
            deltas.insert(beadids.constData()[i],
                          FFBeadChange(FFBead(coords.at(idx), old_params), FFBead(new_coords.at(i), new_params.at(i))));
        }
...
QHash<quint32, FFBead> Patch::remove(const QVarLengthArray<quint32> &beadids)
{
    ...
    {
        int idx = getBeadIdx(beadids.constData()[i]);
        old_beads.insert(beadids.constData()[i], FFBead(coords.at(idx), params.read().at(idx)));
