...
namespace SireFF
{
    ...
    */
    class SIREFF_EXPORT G2FF : public FF
    {
        ...
        friend SIREFF_EXPORT QDataStream & ::operator<<(QDataStream &, const G2FF &);
        friend SIREFF_EXPORT QDataStream & ::operator>>(QDataStream &, G2FF &);
    ...
    protected:
        ...
        ////
        virtual void _pvt_added(quint32 groupid, const PartialMolecule &mol, const PropertyMap &map) = 0;

        virtual void _pvt_removed(quint32 groupid, const PartialMolecule &mol) = 0;

        virtual void _pvt_changed(quint32 groupid, const Molecule &molecule, bool auto_commit) = 0;

        virtual void _pvt_changed(quint32 groupid, const QList<Molecule> &molecules, bool auto_commit) = 0;

        virtual void _pvt_removedAll(quint32 groupid) = 0;

        virtual bool _pvt_wouldChangeProperties(quint32 groupid, MolNum molnum, const PropertyMap &map) const = 0;

        ////
        //// Virtual functions that must be changed if this
        //// forcefield allows overlapping atoms (most don't!)
        ////
        virtual void _pvt_added(quint32 groupid, const ViewsOfMol &mol, const PropertyMap &map);

        virtual void _pvt_removed(quint32 groupid, const ViewsOfMol &mol);

        virtual void _pvt_removedAll(quint32 groupid, const PartialMolecule &mol);

        virtual void _pvt_removedAll(quint32 groupid, const ViewsOfMol &mol);
    ...
    private:
        ...
        void assertNoOverlap(const MoleculeGroup &molgroup, const MoleculeView &molview) const;

        void assertNoOverlap(const MoleculeGroup &molgroup, const Molecules &molecules) const;
