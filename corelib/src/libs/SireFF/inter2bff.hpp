...
namespace SireFF
{
    ...
    */
    template <class Potential>
    class Inter2BFF : public SireBase::ConcreteProperty<Inter2BFF<Potential>, G1FF>, public Potential
    {
    ...
    /** Constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2BFF<Potential>::Inter2BFF()
        : SireBase::ConcreteProperty<Inter2BFF<Potential>, G1FF>(), Potential()
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2BFF<Potential>::Inter2BFF(const QString &ffname)
        : SireBase::ConcreteProperty<Inter2BFF<Potential>, G1FF>(), Potential()
    {
        FF::setName(ffname);
    }
    ...
    /** Copy constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2BFF<Potential>::Inter2BFF(const Inter2BFF<Potential> &other)
        : SireBase::ConcreteProperty<Inter2BFF<Potential>, G1FF>(other), Potential(other), mols(other.mols),
          changed_mols(other.changed_mols), removed_mols(other.removed_mols), ffcomponents(other.ffcomponents)
    {
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2BFF<Potential> &Inter2BFF<Potential>::operator=(const Inter2BFF<Potential> &other)
    {
        if (this != &other)
        {
            G1FF::operator=(other);
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2BFF<Potential>::operator==(const Inter2BFF<Potential> &other) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2BFF<Potential>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Inter2BFF<Potential>>());
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Inter2BFF<Potential>::what() const
    {
        return Inter2BFF<Potential>::typeName();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Inter2BFF<Potential> *Inter2BFF<Potential>::clone() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const typename Potential::Components &Inter2BFF<Potential>::components() const
    {
        return ffcomponents;
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::_pvt_updateName()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2BFF<Potential>::setProperty(const QString &name, const Property &value)
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Property &Inter2BFF<Potential>::property(const QString &name) const
    {
    ...
    /** Return whether or not this forcefield contains a property
        called 'name' */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2BFF<Potential>::containsProperty(const QString &name) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Properties &Inter2BFF<Potential>::properties() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::mustNowRecalculateFromScratch()
    {
    ...
    /** Return whether or not we need to record the changes to this
        ...
        from scratch) */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2BFF<Potential>::recordingChanges() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::recordChange(const typename Potential::ChangedMolecule &change)
    {
        if (change.isEmpty())
            return;
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::changedPotential()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::_pvt_added(const PartialMolecule &molecule, const PropertyMap &map)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::_pvt_removed(const PartialMolecule &molecule)
    {
        if (this->recordingChanges())
        {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::_pvt_changed(const SireMol::Molecule &molecule, bool auto_commit)
    {
        if (this->recordingChanges())
        {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::_pvt_changed(const QList<SireMol::Molecule> &molecules,
                                                                    bool auto_commit)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::_pvt_removedAll()
    {
        mols.clear();
        this->mustNowRecalculateFromScratch();
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Inter2BFF<Potential>::_pvt_wouldChangeProperties(MolNum molnum,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Inter2BFF<Potential>::recalculateEnergy()
    {
        int nmols = mols.count();
        const ChunkedVector<typename Potential::Molecule> &mols_array = mols.moleculesByIndex();
        ...
        try
        {
            ...
            else
            {
                ...
                for (int i = 0; i < nmols; ++i)
                {
                    ...
                    typename QHash<MolNum, ChangedMolecule>::const_iterator it = changed_mols.constFind(mol0.number());
                    ...
                    if (it == changed_mols.constEnd())
                    {
                        ...
                        for (typename QHash<MolNum, ChangedMolecule>::const_iterator it2 = changed_mols.constBegin();
                             it2 != changed_mols.constEnd(); ++it2)
                        {
                            Potential::calculateEnergy(mol0, it2->oldParts(), old_nrg, old_workspace);

                            Potential::calculateEnergy(mol0, it2->newParts(), new_nrg, new_workspace);
                        }
                    }
                        ...
                        else
                        {
                            for (++it2; it2 != changed_mols.constEnd(); ++it2)
                            {
                                if (it2->changedAll())
                                {
                                    ...
                                    // interaction
                                    Potential::calculateEnergy(it->oldMolecule(), it2->oldMolecule(), old_nrg,
                                                               old_workspace);

                                    Potential::calculateEnergy(it->newMolecule(), it2->newMolecule(), new_nrg,
                                                               new_workspace);
                                }
                                else
                                {
                                    ...
                                    // the first molecule...
                                    Potential::calculateEnergy(it->oldParts(), it2->oldMolecule(), old_nrg, old_workspace);

                                    Potential::calculateEnergy(it->newParts(), it2->newMolecule(), new_nrg, new_workspace);

                                    // now the change in energy associated with changing
                                    // the second molecule...
                                    Potential::calculateEnergy(it2->oldParts(), it->oldMolecule(), old_nrg, old_workspace);

                                    Potential::calculateEnergy(it2->newParts(), it->newMolecule(), new_nrg, new_workspace);

                                    // now remove double counted changed in mol1 with
                                    // change in mol2
                                    Potential::calculateEnergy(it->oldParts(), it2->oldParts(), old_nrg, old_workspace, -1);

                                    Potential::calculateEnergy(it->newParts(), it2->newParts(), new_nrg, new_workspace, -1);
                                }
                            }
                        }
                    }
                ...
                for (typename QSet<MolNum>::const_iterator it = removed_mols.constBegin(); it != removed_mols.constEnd();
                     ++it)
                {
                    ...
                    for (++it2; it2 != removed_mols.constEnd(); ++it2)
                    {
                        ...
                        Potential::calculateEnergy(mol0.oldMolecule(), mol1.oldMolecule(), old_nrg, old_workspace);
...
namespace SireFF
{
    namespace detail
    {
        ...
        template <class Potential>
        const RegisterMetaType<SireFF::Inter2BFF<Potential>> Inter2BRMT<Potential>::r_inter2bff;
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireFF::Inter2BFF<Potential> &inter2bff)
{
    ...
    sds << inter2bff.mols << inter2bff.changed_mols << inter2bff.removed_mols
        << static_cast<const Potential &>(inter2bff) << static_cast<const SireFF::G1FF &>(inter2bff);
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireFF::Inter2BFF<Potential> &inter2bff)
{
    SireStream::VersionID v = SireStream::readHeader(ds, SireFF::detail::Inter2BRMT<Potential>::r_inter2bff);
    ...
    if (v == 1)
    {
        ...
        sds >> inter2bff.mols >> inter2bff.changed_mols >> inter2bff.removed_mols >>
            static_cast<Potential &>(inter2bff) >> static_cast<SireFF::G1FF &>(inter2bff);
        ...
        throw SireStream::version_error(v, "1", SireFF::detail::Inter2BRMT<Potential>::r_inter2bff, CODELOC);
