...
namespace SireFF
{
    ...
    */
    template <class Potential>
    class Intra2BFF : public SireBase::ConcreteProperty<Intra2BFF<Potential>, G1FF>, public Potential
    {
    ...
    /** Constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2BFF<Potential>::Intra2BFF()
        : SireBase::ConcreteProperty<Intra2BFF<Potential>, G1FF>(), Potential()
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2BFF<Potential>::Intra2BFF(const QString &ffname)
        : SireBase::ConcreteProperty<Intra2BFF<Potential>, G1FF>(), Potential()
    {
        FF::setName(ffname);
    }
    ...
    /** Copy constructor */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2BFF<Potential>::Intra2BFF(const Intra2BFF<Potential> &other)
        : SireBase::ConcreteProperty<Intra2BFF<Potential>, G1FF>(other), Potential(other), mols(other.mols),
          changed_mols(other.changed_mols), ffcomponents(other.ffcomponents)
    {
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2BFF<Potential> &Intra2BFF<Potential>::operator=(const Intra2BFF<Potential> &other)
    {
        if (this != &other)
        {
            G1FF::operator=(other);
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2BFF<Potential>::operator==(const Intra2BFF<Potential> &other) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Intra2BFF<Potential>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Intra2BFF<Potential>>());
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const char *Intra2BFF<Potential>::what() const
    {
        return Intra2BFF<Potential>::typeName();
    }

    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE Intra2BFF<Potential> *Intra2BFF<Potential>::clone() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const typename Potential::Components &Intra2BFF<Potential>::components() const
    {
        return ffcomponents;
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::_pvt_updateName()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2BFF<Potential>::setProperty(const QString &name, const Property &value)
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Property &Intra2BFF<Potential>::property(const QString &name) const
    {
    ...
    /** Return whether or not this forcefield contains a property
        called 'name' */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2BFF<Potential>::containsProperty(const QString &name) const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const Properties &Intra2BFF<Potential>::properties() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::mustNowRecalculateFromScratch()
    {
    ...
    /** Return whether or not we need to record the changes to this
        ...
        from scratch) */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2BFF<Potential>::recordingChanges() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::recordChange(const typename Potential::ChangedMolecule &change)
    {
        if (change.isEmpty())
            return;
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE const FFComponent &Intra2BFF<Potential>::_pvt_components() const
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::changedPotential()
    {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::_pvt_added(const PartialMolecule &molecule, const PropertyMap &map)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::_pvt_removed(const PartialMolecule &molecule)
    {
        if (this->recordingChanges())
        {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::_pvt_changed(const SireMol::Molecule &molecule, bool auto_commit)
    {
        if (this->recordingChanges())
        {
    ...
    */
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::_pvt_changed(const QList<SireMol::Molecule> &molecules,
                                                                    bool auto_commit)
    {
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::_pvt_removedAll()
    {
        mols.clear();
        this->mustNowRecalculateFromScratch();
    }
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE bool Intra2BFF<Potential>::_pvt_wouldChangeProperties(MolNum molnum,
    ...
    template <class Potential>
    SIRE_OUTOFLINE_TEMPLATE void Intra2BFF<Potential>::recalculateEnergy()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                for (typename QHash<MolNum, ChangedMolecule>::const_iterator it = changed_mols.constBegin();
                     it != changed_mols.constEnd(); ++it)
                {
                    if (it->changedAll())
                    {
                        // calculate the change in energy of this whole molecule
                        Potential::calculateEnergy(it->oldMolecule(), old_nrg, old_workspace);

                        Potential::calculateEnergy(it->newMolecule(), new_nrg, new_workspace);
                    }
                    else
                    {
                        ...
                        Potential::calculateEnergy(it->oldParts(), it->oldMolecule(), old_nrg, old_workspace);

                        Potential::calculateEnergy(it->newParts(), new_nrg, new_workspace);

                        Potential::calculateEnergy(it->newParts(), it->newMolecule(), new_nrg, new_workspace);
                    }
                }
...
namespace SireFF
{
    namespace detail
    {
        ...
        template <class Potential>
        const RegisterMetaType<SireFF::Intra2BFF<Potential>> Intra2BRMT<Potential>::r_intra2bff;
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireFF::Intra2BFF<Potential> &intra2bff)
{
    SireStream::writeHeader(ds, SireFF::detail::Intra2BRMT<Potential>::r_intra2bff, 1);

    SireStream::SharedDataStream sds(ds);

    sds << intra2bff.mols << intra2bff.changed_mols << static_cast<const Potential &>(intra2bff)
...
template <class Potential>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireFF::Intra2BFF<Potential> &intra2bff)
{
    SireStream::VersionID v = SireStream::readHeader(ds, SireFF::detail::Intra2BRMT<Potential>::r_intra2bff);

    if (v == 1)
    {
        SireStream::SharedDataStream sds(ds);

        sds >> intra2bff.mols >> intra2bff.changed_mols >> static_cast<Potential &>(intra2bff) >>
        ...
        throw SireStream::version_error(v, "1", SireFF::detail::Intra2BRMT<Potential>::r_intra2bff, CODELOC);
