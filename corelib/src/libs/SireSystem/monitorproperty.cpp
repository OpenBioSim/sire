...
QDataStream &operator<<(QDataStream &ds, const MonitorProperty &monprop)
{
...
QDataStream &operator>>(QDataStream &ds, MonitorProperty &monprop)
{
    ...
    else if (v == 1)
    {
        ...
        case MonitorProperty::MOLECULE_PROPERTY:
            ...
            for (QHash<MolNum, QVector<PropertyPtr>>::const_iterator it = molprops.constBegin();
                 it != molprops.constEnd(); ++it)
            {
                mon.molprops.insert(it.key(), ChunkedVector<PropertyPtr, 2048>::fromVector(*it));
            }
...
/** Null constructor */
MonitorProperty::MonitorProperty() : ConcreteProperty<MonitorProperty, SystemMonitor>(), what_is_monitored(IS_NULL)
{
}

/** Construct to monitor the system property 'property' */
MonitorProperty::MonitorProperty(const QString &property)
    : ConcreteProperty<MonitorProperty, SystemMonitor>(), prop(property), what_is_monitored(SYSTEM_PROPERTY)
{
}
...
MonitorProperty::MonitorProperty(const QString &property, const MoleculeGroup &molgroup)
    : ConcreteProperty<MonitorProperty, SystemMonitor>(), prop(property), mgid(molgroup.number()),
...
MonitorProperty::MonitorProperty(const QString &property, const MGID &id)
    : ConcreteProperty<MonitorProperty, SystemMonitor>(), prop(property), mgid(id), what_is_monitored(MOLECULE_PROPERTY)
{
}
...
MonitorProperty::MonitorProperty(const QString &property, const FF &forcefield)
    : ConcreteProperty<MonitorProperty, SystemMonitor>(), prop(property), ffid(forcefield.name()),
...
MonitorProperty::MonitorProperty(const QString &property, const FFID &id)
    : ConcreteProperty<MonitorProperty, SystemMonitor>(), prop(property), ffid(id),
      what_is_monitored(FORCEFIELD_PROPERTY)
{
}
...
MonitorProperty::MonitorProperty(const MonitorProperty &other)
    : ConcreteProperty<MonitorProperty, SystemMonitor>(other), prop(other.prop), mgid(other.mgid), ffid(other.ffid),
      what_is_monitored(other.what_is_monitored), props(other.props), molprops(other.molprops)
{
}
...
bool MonitorProperty::operator==(const MonitorProperty &other) const
{
    return this == &other or
           (prop == other.prop and what_is_monitored == other.what_is_monitored and ffid == other.ffid and
            mgid == other.mgid and props == other.props and molprops == other.molprops);
}
...
QString MonitorProperty::toString() const
{
    ...
    case SYSTEM_PROPERTY:
        return QObject::tr("MonitorProperty( system property \"%1\" )").arg(prop);

    case FORCEFIELD_PROPERTY:
        return QObject::tr("MonitorProperty( forcefield (%1) property \"%2\" )").arg(ffid.toString(), prop);

    case MOLECULE_PROPERTY:
        return QObject::tr("MonitorProperty( molecule group (%1) property \"%2\" )").arg(mgid.toString(), prop);
    }
...
const MGID &MonitorProperty::mgID() const
{
    if (not monitoringMoleculeProperty())
        throw SireError::incompatible_error(QObject::tr("This MonitorProperty (%1) is not monitoring the property "
...
QVector<PropertyPtr> MonitorProperty::properties(MolNum molnum) const
{
    ...
    QHash<MolNum, ChunkedVector<PropertyPtr, 2048>>::const_iterator it = molprops.constFind(molnum);

    if (it == molprops.constEnd())
        throw SireMol::missing_molecule(QObject::tr("No properties were monitored for the molecule with number %1. "
...
void MonitorProperty::writeToDisk(const QString &filename)
{
    ...
    {
        for (QHash<MolNum, ChunkedVector<PropertyPtr, 2048>>::const_iterator it = molprops.constBegin();
             it != molprops.constEnd(); ++it)
        {
            ts << QObject::tr("\n ---- Molecule %1 ----\n").arg(it.key());
