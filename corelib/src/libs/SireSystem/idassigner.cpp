...
QDataStream &operator<<(QDataStream &ds, const IDAssigner &idassigner)
{
    ...
    sds << idassigner.molgroup << idassigner.identity_points << idassigner.spce << idassigner.map;
...
void IDAssigner::validatePoints(const QVector<PointPtr> &points) const
{
    ...
    {
        if (not(it->read().isIntraMoleculePoint() or it->read().isExtraMoleculePoint()))
        {
            throw SireError::incompatible_error(QObject::tr("An IDAssigner can only be used with "
...
/** Constructor */
IDAssigner::IDAssigner() : ConcreteProperty<IDAssigner, Property>(), distances_changed(false)
{
}
...
IDAssigner::IDAssigner(const QVector<PointPtr> &points, const MoleculeGroup &group, const Space &space,
                       const PropertyMap &property_map)
    : ConcreteProperty<IDAssigner, Property>(), molgroup(group), identity_points(points), spce(space),
      map(property_map), distances_changed(false)
{
    ...
    if (not identity_points.isEmpty())
    {
        ...
        {
            points_with_mols.append(CloseMols(identity_points.at(i), molgroup, spce, npoints, map));
        }
...
/** Construct to find the identity of the molecules from
    ...
    this creates a list of n molecules, where the ith molecule
    is matched to the ith point */
IDAssigner::IDAssigner(const QVector<PointPtr> &points, const MoleculeGroup &group, const PropertyMap &property_map)
...
/** Constructor */
IDAssigner::IDAssigner(const PointRef &point, const MoleculeGroup &group, const PropertyMap &property_map)
    : ConcreteProperty<IDAssigner, Property>()
{
    QVector<PointPtr> points;
...
/** Constructor */
IDAssigner::IDAssigner(const PointRef &point, const MoleculeGroup &group, const Space &space,
                       const PropertyMap &property_map)
    : ConcreteProperty<IDAssigner, Property>()
{
    QVector<PointPtr> points;
...
IDAssigner::IDAssigner(const IDAssigner &other)
    : ConcreteProperty<IDAssigner, Property>(other), molgroup(other.molgroup), identity_points(other.identity_points),
      spce(other.spce), map(other.map), points_with_mols(other.points_with_mols), mol_to_molnum(other.mol_to_molnum),
      point_distances(other.point_distances), point_to_mol(other.point_to_mol),
...
{
    return molgroup == other.molgroup and identity_points == other.identity_points and map == other.map;
}
...
QString IDAssigner::toString() const
{
    ...
    for (QVector<PointPtr>::const_iterator it = identity_points.constBegin(); it != identity_points.constEnd(); ++it)
    {
...
QVector<PartialMolecule> IDAssigner::identifiedMolecules() const
{
    ...
    if (mol_to_molnum.count() < n_to_match)
        throw SireError::program_bug(QObject::tr("WEIRD: Number of matched molecules is greater than the number of "
                                                 "identified molecules? %1, %2, %3")
                                         .arg(n_to_match)
                                         .arg(mol_to_molnum.count())
                                         .arg(Sire::toString(mol_to_molnum)),
                                     CODELOC);

    if (point_to_mol.count() < n_to_match)
        throw SireError::program_bug(
            QObject::tr("WEIRD: Number of point_to_mol matched molecules is less than the number of "
                        "identified molecules? %1, %2, %3")
                .arg(n_to_match)
                .arg(point_to_mol.count())
                .arg(Sire::toString(point_to_mol)),
            CODELOC);
