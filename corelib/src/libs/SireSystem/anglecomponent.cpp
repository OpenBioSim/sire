...
QDataStream &operator<<(QDataStream &ds, const AngleComponent &angcomp)
{
    writeHeader(ds, r_angcomp, 1);

    SharedDataStream sds(ds);

    sds << angcomp.p0 << angcomp.p1 << angcomp.p2 << static_cast<const GeometryComponent &>(angcomp);
...
QDataStream &operator>>(QDataStream &ds, AngleComponent &angcomp)
{
    ...
    if (v == 1)
    {
        ...
        sds >> angcomp.p0 >> angcomp.p1 >> angcomp.p2 >> static_cast<GeometryComponent &>(angcomp);

        angcomp.intra_molecule_points = Point::areIntraMoleculePoints(angcomp.p0, angcomp.p1) and
                                        Point::areIntraMoleculePoints(angcomp.p0, angcomp.p2);
    }
    else
...
/** Null constructor */
AngleComponent::AngleComponent() : ConcreteProperty<AngleComponent, GeometryComponent>(), intra_molecule_points(true)
{
}
...
AngleComponent::AngleComponent(const Symbol &constrained_symbol, const PointRef &point0, const PointRef &point1,
                               const PointRef &point2, const PropertyMap &map)
    : ConcreteProperty<AngleComponent, GeometryComponent>(constrained_symbol, AngleComponent::theta(), map), p0(point0),
      p1(point1), p2(point2)
{
    intra_molecule_points = Point::areIntraMoleculePoints(p0, p1) and Point::areIntraMoleculePoints(p0, p2);
}
...
AngleComponent::AngleComponent(const Symbol &constrained_symbol, const PointRef &point0, const PointRef &point1,
                               const PointRef &point2, const Expression &geometry_expression, const PropertyMap &map)
    : ConcreteProperty<AngleComponent, GeometryComponent>(constrained_symbol, geometry_expression, map), p0(point0),
      p1(point1), p2(point2)
{
    intra_molecule_points = Point::areIntraMoleculePoints(p0, p1) and Point::areIntraMoleculePoints(p0, p2);
}
...
AngleComponent::AngleComponent(const AngleComponent &other)
    : ConcreteProperty<AngleComponent, GeometryComponent>(other), p0(other.p0), p1(other.p1), p2(other.p2),
...
bool AngleComponent::operator==(const AngleComponent &other) const
{
    return this == &other or
           (p0 == other.p0 and p1 == other.p1 and p2 == other.p2 and GeometryComponent::operator==(other));
}
...
Triangle AngleComponent::getTriangle() const
{
    ...
    {
        return Triangle(p0.read().point(), p1.read().point(), p2.read().point());
    }
    else
    {
        return Triangle(space().getMinimumImage(p0.read().point(), p1.read().point()), p1.read().point(),
