...
QDataStream &operator<<(QDataStream &ds, const DihedralComponent &dihcomp)
{
    writeHeader(ds, r_dihcomp, 1);

    SharedDataStream sds(ds);

    sds << dihcomp.p0 << dihcomp.p1 << dihcomp.p2 << dihcomp.p3 << static_cast<const GeometryComponent &>(dihcomp);
...
QDataStream &operator>>(QDataStream &ds, DihedralComponent &dihcomp)
{
    ...
    if (v == 1)
    {
        ...
        sds >> dihcomp.p0 >> dihcomp.p1 >> dihcomp.p2 >> dihcomp.p3 >> static_cast<GeometryComponent &>(dihcomp);

        dihcomp.intra_molecule_points = Point::areIntraMoleculePoints(dihcomp.p0, dihcomp.p1) and
                                        Point::areIntraMoleculePoints(dihcomp.p0, dihcomp.p2) and
                                        Point::areIntraMoleculePoints(dihcomp.p0, dihcomp.p3);
    }
    else
...
DihedralComponent::DihedralComponent()
    : ConcreteProperty<DihedralComponent, GeometryComponent>(), intra_molecule_points(true)
{
}
...
DihedralComponent::DihedralComponent(const Symbol &constrained_symbol, const PointRef &point0, const PointRef &point1,
                                     const PointRef &point2, const PointRef &point3, const PropertyMap &map)
    : ConcreteProperty<DihedralComponent, GeometryComponent>(constrained_symbol, DihedralComponent::phi(), map),
      p0(point0), p1(point1), p2(point2), p3(point3)
{
    intra_molecule_points = Point::areIntraMoleculePoints(p0, p1) and Point::areIntraMoleculePoints(p0, p2) and
...
DihedralComponent::DihedralComponent(const Symbol &constrained_symbol, const PointRef &point0, const PointRef &point1,
    ...
    : ConcreteProperty<DihedralComponent, GeometryComponent>(constrained_symbol, geometry_expression, map), p0(point0),
...
DihedralComponent::DihedralComponent(const DihedralComponent &other)
    : ConcreteProperty<DihedralComponent, GeometryComponent>(other), p0(other.p0), p1(other.p1), p2(other.p2),
      p3(other.p3), intra_molecule_points(other.intra_molecule_points)
{
}
...
Torsion DihedralComponent::getTorsion() const
{
    ...
    {
        return Torsion(p0.read().point(), p1.read().point(), p2.read().point(), p3.read().point());
    }
    else
    {
        return Torsion(space().getMinimumImage(p0.read().point(), p2.read().point()),
                       space().getMinimumImage(p1.read().point(), p2.read().point()), p2.read().point(),
