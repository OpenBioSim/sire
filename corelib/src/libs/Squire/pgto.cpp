...
P_GTO::P_GTO(double alpha, double scale) : ConcreteProperty<P_GTO, GTO>(alpha, scale)
{
}
...
QString P_GTO::toString() const
{
    if (GTO::scale() == 1)
        ...
        return QObject::tr("P (alpha = %1, scale = %2)").arg(GTO::alpha()).arg(GTO::scale());
}
...
/** Constructor */
PS_GTO::PS_GTO() : ConcreteProperty<PS_GTO, GTOPair>(), norm_scl(0)
{
}
...
/** Construct combining orbital 'a' at position 'A' with orbital 'b' at
    position 'B' */
PS_GTO::PS_GTO(const Vector &A, const P_GTO &a, const Vector &B, const S_GTO &b)
...
/** Construct combining orbital 'a' at position 'A' with orbital 'b' at
    position 'B' */
PS_GTO::PS_GTO(const Vector &A, const S_GTO &a, const Vector &B, const P_GTO &b)
...
PS_GTO::PS_GTO(const PS_GTO &other)
    : ConcreteProperty<PS_GTO, GTOPair>(other), p_minus_a(other.p_minus_a), norm_scl(other.norm_scl)
{
}
...
{
    return GTOPair::operator==(other) and p_minus_a == other.p_minus_a and norm_scl == other.norm_scl;
}
...
QDataStream &operator<<(QDataStream &ds, const PP_GTO &pp)
{
    ...
    ds << pp.p_minus_a << pp.p_minus_b << pp.norm_scl << static_cast<const GTOPair &>(pp);
...
QDataStream &operator>>(QDataStream &ds, PP_GTO &pp)
{
    ...
    if (v == 1)
    {
        ds >> pp.p_minus_a >> pp.p_minus_b >> pp.norm_scl >> static_cast<GTOPair &>(pp);
    }
    else
...
/** Construct combining orbital 'a' at position 'A' with orbital 'b' at
    position 'B' */
PP_GTO::PP_GTO(const Vector &A, const P_GTO &a, const Vector &B, const P_GTO &b)
...
PP_GTO::PP_GTO(const PP_GTO &other)
    : ConcreteProperty<PP_GTO, GTOPair>(other), p_minus_a(other.p_minus_a), p_minus_b(other.p_minus_b),
      norm_scl(other.norm_scl)
{
}
...
{
    return GTOPair::operator==(other) and p_minus_a == other.p_minus_a and p_minus_b == other.p_minus_b and
           norm_scl == other.norm_scl;
}
...
namespace Squire
{
    ...
    Vector kinetic_integral(const PS_GTO &ps)
    {
        ...
        return (ps.scale() * ps.ss() * ps.xi() * (5 - 2 * ps.R2() * ps.xi())) * ps.P_minus_A();
    }
    ...
    Vector potential_integral(const PointCharge &C, const PS_GTO &ps, int m)
    {
        QVector<PointCharge> Cs(1, C);
    ...
    Vector potential_integral(const QVector<PointDipole> &C, const PS_GTO &ps)
    {
    ...
    Vector potential_integral(const QVector<PointDipole> &C, const PS_GTO &ps, int m)
    {
        throw SireError::incomplete_code("Not implemented", CODELOC);
        return Vector();
    }
    ...
    Vector potential_integral(const PointDipole &C, const PS_GTO &ps, int m)
    {
        QVector<PointDipole> Cs(1, C);
        return potential_integral(Cs, ps, m);
    }
    ...
    Matrix kinetic_integral(const PP_GTO &pp)
    {
        ...
        const double extra = prefac0 * pp.zeta() * (-5 + 2 * pp.R2() * pp.xi());
    ...
    Matrix potential_integral(const QVector<PointCharge> &C, const PP_GTO &P)
    {
        ...
        for (int ic = 0; ic < nc; ++ic)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                for (int j = i + 1; j < 3; ++j)
                {
                    const double val = q_prefac * (boys[0] * pa[i] * pb[j] + boys[2] * pc[i] * pc[j] -
            ...
            for (int i = 0; i < 3; ++i)
            {
                m[mat.offset(i, i)] = q_prefac * (boys[0] * pa[i] * pb[i] + boys[2] * pc[i] * pc[i] -
    ...
    static Vector my_electron_integral(const PS_GTO &P, const SS_GTO &Q, const double boys[2])
    {
    ...
    static Matrix my_electron_integral(const PS_GTO &P, const PS_GTO &Q, const double boys[3])
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int k = 0; k < 3; ++k)
            {
                const double val = prefac * (boys[0] * pa[i] * qc[k] + boys[2] * wp[i] * wq[k] +
    ...
    static Matrix my_electron_integral(const PP_GTO &P, const SS_GTO &Q, const double boys[3])
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                const double val = prefac * (boys[0] * pa[i] * pb[j] + boys[2] * wp[i] * wp[j] +
        ...
        const double extra = 0.5 * prefac * one_over_zeta * (boys[0] - boys[1] * rho * one_over_zeta);
    ...
    static void my_electron_integral(const PP_GTO &P, const PS_GTO &Q, const double boys[4], Array2D<Vector> &mat)
    {
        ...
        const Vector extra_ij =
            inv_zeta * (boys[0] * Q.Q_minus_C() + boys[1] * (W_minus_Q - (rho / P.zeta()) * Q.Q_minus_C()) -
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                ...
                for (int k = 0; k < 3; ++k)
                {
                    vdata[k] = boys[3] * wp[i] * wp[j] * wq[k] + boys[0] * pa[i] * pb[j] * qc[k] +
                               boys[1] * (pb[j] * qc[k] * wp[i] + pa[i] * qc[k] * wp[j] + pa[i] * pb[j] * wq[k]) +
                               boys[2] * (qc[k] * wp[i] * wp[j] + pb[j] * wp[i] * wq[k] + pa[i] * wp[j] * wq[k]);
                }
    ...
    void electron_integral(const PP_GTO &P, const PS_GTO &Q, Array2D<Vector> &matrix)
    {
        double boys[4];
    ...
    void electron_integral(const PS_GTO &P, const PP_GTO &Q, Array2D<Vector> &matrix)
    {
    ...
    /** Calculate the electron repulsion integral (pp|ps) into the passed matrix */
    void electron_integral(const PP_GTO &P, const PS_GTO &Q, int m, Array2D<Vector> &matrix)
    {
        double boys[4];
    ...
    /** Calculate the electron repulsion integral (ps|pp) into the passed matrix */
    void electron_integral(const PS_GTO &P, const PP_GTO &Q, int m, Array2D<Vector> &matrix)
    {
    ...
    Array2D<Vector> electron_integral(const PP_GTO &P, const PS_GTO &Q, int m)
    {
        Array2D<Vector> matrix(3, 3);
    ...
    Array2D<Vector> electron_integral(const PS_GTO &P, const PP_GTO &Q, int m)
    {
        return electron_integral(Q, P, m);
    }

    static void my_electron_integral(const PP_GTO &P, const PP_GTO &Q, const double boys[5], Array2D<Matrix> &mat)
    {
        ...
        for (int k = 0; k < 3; ++k)
        {
            for (int l = 0; l < 3; ++l)
            {
                m_ij[delta_ij.offset(k, l)] =
                    prefac_over_2zeta * (boys[0] * qc[k] * qd[l] - boys[3] * rho_over_zeta * wq[k] * wq[l] +
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 3; ++j)
            {
                m_kl[delta_kl.offset(i, j)] =
                    prefac_over_2eta * (boys[0] * pa[i] * pb[j] - boys[3] * rho_over_eta * wp[i] * wp[j] +
        ...
        for (int j = 0; j < 3; ++j)
        {
            for (int l = 0; l < 3; ++l)
            {
                m_ik[delta_ik.offset(j, l)] =
                    prefac_over_2e_pl_z *
                    (boys[1] * pb[j] * qd[l] + boys[2] * (qd[l] * wp[j] + pb[j] * wq[l]) + boys[3] * wp[j] * wq[l]);
            }
        }
        ...
        for (int j = 0; j < 3; ++j)
        {
            for (int k = 0; k < 3; ++k)
            {
                m_il[delta_il.offset(j, k)] =
                    prefac_over_2e_pl_z *
                    (boys[1] * pb[j] * qc[k] + boys[2] * (qc[k] * wp[j] + pb[j] * wq[k]) + boys[3] * wp[j] * wq[k]);
            }
        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int l = 0; l < 3; ++l)
            {
                m_jk[delta_jk.offset(i, l)] =
                    prefac_over_2e_pl_z *
                    (boys[1] * pa[i] * qd[l] + boys[2] * (qd[l] * wp[i] + pa[i] * wq[l]) + boys[3] * wp[i] * wq[l]);
            }
        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int k = 0; k < 3; ++k)
            {
                m_jl[delta_jl.offset(i, k)] =
                    prefac_over_2e_pl_z *
                    (boys[1] * pa[i] * qc[k] + boys[2] * (qc[k] * wp[i] + pa[i] * wq[k]) + boys[3] * wp[i] * wq[k]);
            }
        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            m_ij[delta_ij.offset(i, i)] +=
                prefac_over_4ez * (boys[0] - boys[1] * (rho_over_eta + rho_over_zeta) + boys[2] * rho * rho_over_ez);
        }
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                for (int k = 0; k < 3; ++k)
                {
                    for (int l = 0; l < 3; ++l)
                    {
                        ...
                        const double val =
                            prefac * (boys[4] * wp[i] * wp[j] * wq[k] * wq[l] +
                                      boys[3] * (qd[l] * wp[i] * wp[j] * wq[k] + qc[k] * wp[i] * wp[j] * wq[l] +
                                                 pb[j] * wp[i] * wq[k] * wq[l] + pa[i] * wp[j] * wq[k] * wq[l]) +
                                      boys[0] * pa[i] * pb[j] * qc[k] * qd[l] +
                                      boys[2] * (qc[k] * qd[l] * wp[i] * wp[j] + pb[j] * qd[l] * wp[i] * wq[k] +
                                                 pa[i] * qd[l] * wp[j] * wq[k] + pb[j] * qc[k] * wp[i] * wq[l] +
                                                 pa[i] * qc[k] * wp[j] * wq[l] + pa[i] * pb[j] * wq[k] * wq[l]) +
                                      boys[1] * (pb[j] * qc[k] * qd[l] * wp[i] + pa[i] * qc[k] * qd[l] * wp[j] +
                                                 pa[i] * pb[j] * qd[l] * wq[k] + pa[i] * pb[j] * qc[k] * wq[l]));
    ...
    void electron_integral(const PP_GTO &P, const PP_GTO &Q, Array2D<Matrix> &matrix)
    {
        double boys[5];
    ...
    void electron_integral(const PP_GTO &P, const PP_GTO &Q, int m, Array2D<Matrix> &matrix)
    {
        double boys[5];
    ...
    Array2D<Matrix> electron_integral(const PP_GTO &P, const PP_GTO &Q, int m)
    {
        Array2D<Matrix> matrix(3, 3);
        electron_integral(P, Q, matrix);
        return matrix;
...
void PS_GTOs::_pvt_create(const QVector<S_GTO> &s_gtos, const QVector<Vector> &s_centers, const QVector<P_GTO> &p_gtos,
                          const QVector<Vector> &p_centers)
{
    if (s_gtos.count() != s_centers.count())
        throw SireError::invalid_arg(QObject::tr("You must pass in the same number of centers (%1) as "
    ...
    for (int i = 0; i < np; ++i)
    {
        ...
        for (int j = 0; j < ns; ++j)
        {
            *orbs_array = PS_GTO(p_center, p_gto, s_centers_array[j], s_gtos_array[j]);
...
/** Construct for the set of s orbitals s_gtos located at centers
    ...
    p_centers */
PS_GTOs::PS_GTOs(const QVector<S_GTO> &s_gtos, const QVector<Vector> &s_centers, const QVector<P_GTO> &p_gtos,
                 const QVector<Vector> &p_centers)
{
...
/** Construct for the set of s orbitals s_gtos located at centers
    s_centers, and p orbitals p_gtos located at centers
    p_centers */
PS_GTOs::PS_GTOs(const QVector<P_GTO> &p_gtos, const QVector<Vector> &p_centers, const QVector<S_GTO> &s_gtos,
...
/** Copy constructor */
PS_GTOs::PS_GTOs(const PS_GTOs &other) : orbs(other.orbs)
{
}
...
/** Construct for the set of p orbitals p_gtos located at centers
    p_centers */
PP_GTOs::PP_GTOs(const QVector<P_GTO> &p_gtos, const QVector<Vector> &p_centers)
{
    if (p_gtos.count() != p_centers.count())
        throw SireError::invalid_arg(QObject::tr("You must pass in the same number of centers (%1) as "
                                                 "there are P orbitals (%2).")
...
/** Copy constructor */
PP_GTOs::PP_GTOs(const PP_GTOs &other) : orbs(other.orbs)
{
}
