...
namespace Squire
{
    ...
    */
    ...
    protected:
        ...
        QString chargeCommandFile(const Molecule &molecule, const PropertyMap &map) const;

    private:
        QString createCommandFile(QString cmd_template, const QList<QPair<Vector, SireMol::Element>> &atoms) const;

        QString createCommandFile(QString cmd_template, const QMPotential::Molecules &molecules) const;

        QString writeShellFile(const SireBase::TempDir &tempdir) const;

        QStringList runMopac(const QString &cmdfile) const;

        QString _pvt_chargeCommandFile(const Molecule &molecule, const PropertyMap &map, QHash<int, int> *atom_idx) const;
