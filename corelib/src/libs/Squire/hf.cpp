...
void HF::add(const Vector &center, const Orbital &orbital)
{
    ...
    else if (orbital.isA<P_GTO>())
    {
        ...
        throw SireError::unsupported(QObject::tr("The HF program does not support orbitals of type %1 (%2)")
                                         .arg(orbital.toString())
                                         .arg(orbital.what()),
                                     CODELOC);
}
...
static NMatrix make_core_fock_matrix(const Array2D<SS_GTO> &orbitals, const QVector<PointCharge> &charges)
{
    const int norbitals = orbitals.nRows();
    ...
    for (int i = 0; i < norbitals; ++i)
    {
        for (int j = 0; j < norbitals; ++j)
        {
            for (int k = 0; k < norbitals; ++k)
            {
                ...
                {
                    qDebug() << i + 1 << j + 1 << k + 1 << l + 1 << electron_integral(orbitals(i, j), orbitals(k, l));
                }
            }
        }
    }
    ...
    for (int i = 0; i < norbitals; ++i)
    {
        for (int j = 0; j < norbitals; ++j)
        {
            ...
            for (int k = 0; k < norbitals; ++k)
            {
                for (int l = 0; l < norbitals; ++l)
                {
                    v += P(k, l) * (electron_integral(orbitals(i, j), orbitals(k, l)) -
...
void HF::solve()
{
    ...
    for (int i = 0; i < norbitals; ++i)
    {
        for (int j = 0; j < norbitals; ++j)
        {
            ss_orbitals(i, j) = SS_GTO(s_centers.at(i), s_orbs.at(i), s_centers.at(j), s_orbs.at(j));
        }
    }
