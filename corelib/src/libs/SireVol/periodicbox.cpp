...
{
    writeHeader(ds, r_box, 2) << box.boxlength << static_cast<const Cartesian &>(box);
...
QDataStream &operator>>(QDataStream &ds, PeriodicBox &box)
{
    ...
    else if (v == 1)
    {
        ...
        ds >> mincoords >> maxcoords >> static_cast<Cartesian &>(box);
...
const Vector max_boxlength(std::pow(0.9 * std::numeric_limits<double>::max(), 1.0 / 3.0));
...
{
    boxlength = Vector(std::abs(dimensions.x()), std::abs(dimensions.y()), std::abs(dimensions.z()));
    ...
    {
        throw SireError::invalid_arg(QObject::tr("Cannot set the box size of dimension %1 as "
                                                 "at least one side is equal to zero.")
                                         .arg(boxlength.toString()),
                                     CODELOC);
    }
...
PeriodicBox::PeriodicBox(const Vector &dimensions) : ConcreteProperty<PeriodicBox, Cartesian>()
{
    this->setDimensions(dimensions);
}

/** Construct a PeriodicBox volume that goes from min to max */
PeriodicBox::PeriodicBox(const Vector &minval, const Vector &maxval) : ConcreteProperty<PeriodicBox, Cartesian>()
{
...
PeriodicBox::PeriodicBox(const PeriodicBox &other)
    : ConcreteProperty<PeriodicBox, Cartesian>(other), boxlength(other.boxlength), halflength(other.halflength),
...
{
    return QObject::tr("PeriodicBox( %1 )").arg(this->dimensions().toString());
}
...
SireUnits::Dimension::Length PeriodicBox::maximumCutoff() const
{
    return SireUnits::Dimension::Length(
        std::min(0.5 * boxlength.x(), std::min(0.5 * boxlength.y(), 0.5 * boxlength.z())));
}
...
/** Populate the matrix 'mat' with the distances between all of the
    ...
    CoordGroups. */
double PeriodicBox::calcDist(const CoordGroup &group0, const CoordGroup &group1, DistMatrix &mat) const
{
...
#ifdef SIRE_USE_SSE
    {
        ...
        for (int i = 0; i < n0; ++i)
        {
            ...
            mindist = qMin(*((const double *)&sse_mindist), *(((const double *)&sse_mindist) + 1));
...
/** Populate the matrix 'mat' with the distances between all of the
    ...
    distance. */
double PeriodicBox::calcDist(const CoordGroup &group, const Vector &point, DistMatrix &mat) const
{
    double mindist(std::numeric_limits<double>::max());
...
/** Populate the matrix 'mat' with the distances squared between all of the
    atoms of the passed CoordGroup to the passed point. Return the shortest
    distance. */
double PeriodicBox::calcDist2(const CoordGroup &group, const Vector &point, DistMatrix &mat) const
{
...
#ifdef SIRE_USE_SSE
    {
        ...
        mindist2 = qMin(*((const double *)&sse_mindist2), *(((const double *)&sse_mindist2) + 1));
...
/** Populate the matrix 'mat' with the distances^2 between all of the
    ...
    two CoordGroups. */
double PeriodicBox::calcDist2(const CoordGroup &group0, const CoordGroup &group1, DistMatrix &mat) const
{
    double mindist2(std::numeric_limits<double>::max());
...
double PeriodicBox::calcInvDist(const CoordGroup &group0, const CoordGroup &group1, DistMatrix &mat) const
{
    double maxinvdist(0);
    double tmpdist;
...
/** Populate the matrix 'mat' with the inverse distances^2 between all of the
    atoms of the two CoordGroups. Return the shortest distance between the two CoordGroups. */
double PeriodicBox::calcInvDist2(const CoordGroup &group0, const CoordGroup &group1, DistMatrix &mat) const
{
...
DistVector PeriodicBox::calcDistVector(const Vector &point0, const Vector &point1) const
{
    Vector wrapdelta = this->wrapDelta(point0, point1);
...
double PeriodicBox::calcDistVectors(const CoordGroup &group0, const CoordGroup &group1, DistVectorMatrix &mat) const
{
    double mindist(std::numeric_limits<double>::max());
...
double PeriodicBox::calcDistVectors(const CoordGroup &group, const Vector &point, DistVectorMatrix &mat) const
{
    double mindist(std::numeric_limits<double>::max());
...
Angle PeriodicBox::calcAngle(const Vector &point0, const Vector &point1, const Vector &point2) const
{
...
/** Calculate the torsion angle between the passed four points. This should
    ...
    degrees */
Angle PeriodicBox::calcDihedral(const Vector &point0, const Vector &point1, const Vector &point2,
...
bool PeriodicBox::beyond(double dist, const CoordGroup &group0, const CoordGroup &group1) const
{
...
/** Return the minimum distance between the points in 'group0' and 'group1'.
    ...
    used) */
double PeriodicBox::minimumDistance(const CoordGroup &group0, const CoordGroup &group1) const
{
    double mindist2(std::numeric_limits<double>::max());
...
CoordGroup PeriodicBox::getMinimumImage(const CoordGroup &group, const Vector &point) const
{
...
/** Private function used to get the minimum image of all of the
    groups in 'groups' */
CoordGroupArray PeriodicBox::_pvt_getMinimumImage(const CoordGroupArray &groups, const Vector &point) const
{
...
CoordGroupArray PeriodicBox::getMinimumImage(const CoordGroupArray &groups, const Vector &point,
...
QVector<Vector> PeriodicBox::getImagesWithin(const Vector &point, const Vector &center, double dist) const
{
        ...
        for (int i = -nlayers_x; i <= nlayers_x; ++i)
        {
            for (int j = -nlayers_y; j <= nlayers_y; ++j)
            {
                for (int k = -nlayers_z; k <= nlayers_z; ++k)
                {
                    ...
                    Vector delta(i * boxlength.x(), j * boxlength.y(), k * boxlength.z());
...
QList<tuple<double, CoordGroup>> PeriodicBox::getCopiesWithin(const CoordGroup &group, const CoordGroup &center,
                                                              double dist) const
{
...
Vector PeriodicBox::getRandomPoint(const Vector &center, const RanGenerator &generator) const
{
    return Vector(generator.rand(-halflength[0], halflength[0]), generator.rand(-halflength[1], halflength[1]),
