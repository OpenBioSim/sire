...
QDataStream &operator<<(QDataStream &ds, const GridIndex &idx)
{
...
QString GridIndex::toString() const
{
    if (isNull())
        ...
        return QObject::tr("GridIndex( %1, %2, %3 )").arg(_i).arg(_j).arg(_k);
}
...
QDataStream &operator<<(QDataStream &ds, const GridInfo &info)
{
    ...
    ds << info.grid_origin << info.grid_spacing << info.dimx << info.dimy << info.dimz;
...
/** Constructor */
GridInfo::GridInfo() : grid_origin(-0.5), grid_spacing(1.0), inv_grid_spacing(1.0), dimx(0), dimy(0), dimz(0)
{
}

/** Construct a grid of specified dimensions and spacing */
GridInfo::GridInfo(const AABox &dimensions, Length spacing) : grid_spacing(spacing.value()), dimx(0), dimy(0), dimz(0)
{
    if (grid_spacing <= 0)
    {
        throw SireError::invalid_arg(QObject::tr("You cannot construct a grid with zero or negative grid spacing! "
                                                 "(%1 A)")
                                         .arg(grid_spacing),
                                     CODELOC);
    }
...
GridInfo::GridInfo(const GridInfo &other)
    : grid_origin(other.grid_origin), grid_spacing(other.grid_spacing), inv_grid_spacing(other.inv_grid_spacing),
...
{
    return grid_origin == other.grid_origin and grid_spacing == other.grid_spacing and dimx == other.dimx and
           dimy == other.dimy and dimz == other.dimz;
}
...
void GridInfo::pointToGridCorners(const Vector &point, QVector<int> &indicies) const
{
    ...
    if (r < 0 or r >= (dimx - 1) or s < 0 or s >= (dimy - 1) or t < 0 or t >= (dimz - 1))
    {
        for (int i = 0; i < 8; ++i)
        {
            ia[i] = -1;
...
void GridInfo::pointToGridCorners(const Vector &point, QVector<int> &indicies, QVector<float> &weights) const
{
    indicies.resize(8);
    weights.resize(8);
...
/** Return array indicies of the eight grid points that are on the corners of the
    ...
    that are in the box */
int GridInfo::pointToGridCorners(const MultiFloat &x, const MultiFloat &y, const MultiFloat &z,
                                 QVector<MultiInt> &indicies) const
{
    ...
    {
        if (r[i] < 0 or r[i] >= (dimx - 1) or s[i] < 0 or s[i] >= (dimy - 1) or t[i] < 0 or t[i] >= (dimz - 1))
        {
            for (int j = 0; j < 8; ++j)
            {
                ia[j].set(i, -1);
...
AABox GridInfo::box(const GridIndex &idx) const
{
    ...
    {
        return AABox(grid_origin + Vector((0.5 + idx.i()) * grid_spacing, (0.5 + idx.j()) * grid_spacing,
...
Vector GridInfo::point(const GridIndex &idx) const
{
    ...
    {
        return Vector(grid_origin + Vector(idx.i() * grid_spacing, idx.j() * grid_spacing, idx.k() * grid_spacing));
    }
}
...
QVector<float> GridInfo::redimension(const QVector<float> &vals, const GridInfo &new_grid) const
{
    if (vals.count() != this->nPoints())
        throw SireError::incompatible_error(
            QObject::tr("Cannot redimension the passed values as the number of grid points (%1) "
