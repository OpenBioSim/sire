...
namespace SireVol
{
    ...
    */
    ...
    public:
        ...
        void pointToGridCorners(const Vector &point, QVector<int> &indicies, QVector<float> &weights) const;

        int pointToGridCorners(const MultiFloat &x, const MultiFloat &y, const MultiFloat &z,
                               QVector<MultiInt> &indicies) const;
        int pointToGridCorners(const MultiFloat &x, const MultiFloat &y, const MultiFloat &z, QVector<MultiInt> &indicies,
    ...
    {
        return AABox::from(grid_origin, grid_origin + Vector((dimx - 1) * grid_spacing, (dimy - 1) * grid_spacing,
                                                             (dimz - 1) * grid_spacing));
    }
