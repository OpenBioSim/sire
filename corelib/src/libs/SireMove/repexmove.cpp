...
QDataStream &operator>>(QDataStream &ds, RepExSubMove &repexsubmove)
{
    ...
    if (v == 1 or v == 2)
    {
        ...
        sds >> new_submove.partner_properties >> new_submove.have_new_vals;

        if (new_submove.have_new_vals)
        {
            double new_volume_i, new_energy_i, new_volume_j, new_energy_j;

            sds >> new_volume_i >> new_energy_i >> new_volume_j >> new_energy_j;
...
/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const RepExMove &repexmove)
{
...
/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, RepExMove &repexmove)
{
    ...
    else if (v == 1)
    {
        ...
        sds >> repexmove.rangenerator >> nmoves >> repexmove.naccept >> repexmove.nreject;
...
void RepExMove::performMove(Nodes &nodes, Replicas &replicas, bool record_stats)
{
    ...
    for (int i = 0; i < nreplicas; ++i)
    {
        submoves[i] = subsims[i].result().subMoves().asA<SameSupraSubMoves>()[0].asA<RepExSubMove>();
