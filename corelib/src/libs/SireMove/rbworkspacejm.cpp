                       ...
                       QPair<qint32, QVector<qint32>> &beading, qint32 &nbeads)
{
    ...
    if (moldata.hasProperty(beading_property))
    {
        ...
        if (mol.selectedAll())
        {
            const AtomBeads &beadprop = moldata.property(beading_property).asA<AtomBeads>();
...
//         MolNum molnum = molgroup.molNumAt(i);
//
//	qDebug() << " Mol " << i << " MolNum " << molnum.toString() << " beading.first is " << beading.first << "
// beading.second is " << beading.second;
//
...
static void calculateEnergies(const ViewsOfMol &mol, const MolEnergyTable &energies,
                              const QVector<Vector> &atom_int_coords, const QVector<qint32> &beading,
...
bool RBWorkspaceJM::calculateForces(const Symbol &nrg_component)
{
    ...
    for (int i = 0; i < nmols; ++i)
    {
        ...
        ::calculateEnergies(molgroup[molnum], energytable.getTable(molnum), int_coords_array[i], beading.second,
                            bead_energies_array + beading.first);
...
RBWorkspaceJM::RBWorkspaceJM(const PropertyMap &map) : ConcreteProperty<RBWorkspaceJM, IntegratorWorkspaceJM>(map)
{
}
...
RBWorkspaceJM::RBWorkspaceJM(const RBWorkspaceJM &other)
    : ConcreteProperty<RBWorkspaceJM, IntegratorWorkspaceJM>(other), atom_int_coords(other.atom_int_coords),
      ...
      bead_inertia(other.bead_inertia), bead_energies(other.bead_energies), vel_generator(other.vel_generator)
{
}
...
bool RBWorkspaceJM::operator==(const RBWorkspaceJM &other) const
{
    return this == &other or
           (IntegratorWorkspaceJM::operator==(other) and atom_int_coords == other.atom_int_coords and
            ...
            bead_energies == other.bead_energies and vel_generator == other.vel_generator);
