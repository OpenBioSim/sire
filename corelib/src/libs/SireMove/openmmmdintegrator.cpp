...
QDataStream &operator<<(QDataStream &ds, const OpenMMMDIntegrator &velver)
{
    ...
    sds << velver.frequent_save_velocities << velver.molgroup << velver.Integrator_type << velver.friction
        ...
        << velver.reinetialise_context << velver.is_periodic << static_cast<const Integrator &>(velver);
...
QDataStream &operator>>(QDataStream &ds, OpenMMMDIntegrator &velver)
{
    ...
    if (v == 2)
    {
        ...
        sds >> velver.frequent_save_velocities >> velver.molgroup >> velver.Integrator_type >> velver.friction >>
            ...
            velver.timeskip >> velver.reinetialise_context >> velver.is_periodic >> static_cast<Integrator &>(velver);
    ...
    else if (v == 1)
    {
        ...
        sds >> velver.frequent_save_velocities >> velver.molgroup >> velver.Integrator_type >> velver.friction >>
            ...
            velver.platform_type >> velver.Restraint_flag >> velver.CMMremoval_frequency >> velver.buffer_frequency >>
            velver.device_index >> velver.LJ_dispersion >> velver.precision >> velver.integration_tol >>
            velver.timeskip >> velver.reinetialise_context >> static_cast<Integrator &>(velver);
...
OpenMMMDIntegrator::OpenMMMDIntegrator(bool frequent_save)
    : ConcreteProperty<OpenMMMDIntegrator, Integrator>(), frequent_save_velocities(frequent_save),
...
OpenMMMDIntegrator::OpenMMMDIntegrator(const OpenMMMDIntegrator &other)
    : ConcreteProperty<OpenMMMDIntegrator, Integrator>(other), frequent_save_velocities(other.frequent_save_velocities),
      molgroup(other.molgroup), openmm_system(other.openmm_system), openmm_context(other.openmm_context),
      ...
      device_index(other.device_index), LJ_dispersion(other.LJ_dispersion), precision(other.precision),
      reinetialise_context(other.reinetialise_context), integration_tol(other.integration_tol),
      timeskip(other.timeskip), is_periodic(other.is_periodic)
{
}
...
bool OpenMMMDIntegrator::operator==(const OpenMMMDIntegrator &other) const
{
    ...
    return frequent_save_velocities == other.frequent_save_velocities and
           ...
           buffer_frequency == other.buffer_frequency and device_index == other.device_index and
           precision == other.precision and Integrator_type == other.Integrator_type and friction == other.friction and
           integration_tol == other.integration_tol and timeskip == other.timeskip and
           reinetialise_context == other.reinetialise_context and is_periodic == other.is_periodic and
           Integrator::operator==(other);
}
...
void OpenMMMDIntegrator::initialise()
{
    ...
    if (moleculegroup.isEmpty())
    {
        throw SireError::program_bug(
            QObject::tr("Cannot initialise OpenMMMDIntegrator because molgroup has not been defined"), CODELOC);
    }
    ...
    if (CutoffType == "nocutoff")
        ...
        throw SireError::program_bug(QObject::tr("The CutOff method has not been specified. Possible choices: "
                                                 "nocutoff, cutoffnonperiodic, cutoffperiodic,ewal,PME"),
                                     CODELOC);
    ...
    if (ConstraintType == "none")
        ...
        throw SireError::program_bug(
            QObject::tr(
                "The Constraints method has not been specified. Possible choices: none, hbonds, allbonds, hangles"),
            CODELOC);
    ...
    system_openmm->setDefaultPeriodicBoxVectors(OpenMM::Vec3(4, 0, 0), OpenMM::Vec3(0, 5, 0), OpenMM::Vec3(0, 0, 6));
    ...
    if (Andersen_flag == true)
    {
        ...
        {
            throw SireError::program_bug(QObject::tr("The Andersen thermostat can only be used with the leapfrogverlet "
                                                     "or variableleapfrogverlet integrators"),
                                         CODELOC);
        }
    ...
    for (int i = 0; i < nmols; i++)
    {
        ...
        for (int j = 0; j < dihedrals_ff.length(); j++)
        {
            ...
            for (int k = 0; k < dihedral_params.length(); k = k + 3)
            {
                ...
                /*cout << "Dihedral between atom global index " << idx0 << " and " << idx1 << " and " << idx2 << " and "
                << idx3<<"\n"; cout << "Amplitude_dih = " << v << " periodicity " << periodicity << " phase " <<
                phase<<"\n";*/
                // cout << "Dihedral local" << dihedral_ff.toString() << " v " << v << " periodicity " << periodicity <<
                // " phase " << phase; cout << "\n";
            }
        } // end of dihedrals
        ...
        for (int j = 0; j < impropers_ff.length(); j++)
        {
            ...
            for (int k = 0; k < improper_params.length(); k = k + 3)
            {
                ...
                bondTorsion_openmm->addTorsion(idx0, idx1, idx2, idx3, periodicity, phase, v * OpenMM::KJPerKcal);
                /*cout << "Improper Dihedral between atom global index " << idx0 << " and " << idx1 << " and " << idx2
                << " and " << idx3<<"\n"; cout << "Amplitude_impr = " << v << " periodicity " << periodicity << " phase
                " << phase <<"\n";*/
                // cout << "\n";
            }
        } // end of impropers
...
void OpenMMMDIntegrator::createContext(IntegratorWorkspace &workspace, SireUnits::Dimension::Time timestep)
{
    bool Debug = false;
    ...
    if (not this->isSystemInitialised)
    {
        ...
        throw SireError::program_bug(
            QObject::tr("OpenMMMDintegrator should have been initialised before calling integrate."), CODELOC);
    }
...
MolarEnergy OpenMMMDIntegrator::getKineticEnergy()
{
    ...
    {
        throw SireError::program_bug(
            QObject::tr("System was not initialised! Initialise first before requesting energies!"), CODELOC);
    }
    else
    {
...
System OpenMMMDIntegrator::equilibrateSystem(System &system, SireUnits::Dimension::Time equib_time_step,
                                             int equib_steps)
{
    bool Debug = false;
    ...
    if (Debug)
    {
        ...
        cout << "Box dimensions are: " << sp.dimensions()[0] << " " << sp.dimensions()[1] << " " << sp.dimensions()[2]
             << endl;
    }
    workspace.edit().setSystem(system);
                                   ...
                                   SireUnits::Dimension::Time timestep, int nmoves, bool record_stats)
{
    ...
    if (coord_freq > 0)
    {
        ...
        {
            throw SireError::program_bug(
                QObject::tr("You are requesting to buffer %1 frames, which is above the hardcoded limit of %2.")
                    .arg(nframes, MAXFRAMES),
                CODELOC);
        }
    }
    else
...
void OpenMMMDIntegrator::updateBoxDimensions(OpenMM::State &state_openmm, QVector<QVector<Vector>> &buffered_dimensions,
