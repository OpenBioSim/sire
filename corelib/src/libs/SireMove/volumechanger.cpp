...
static const RegisterMetaType<VolumeChanger> r_volchanger(MAGIC_ONLY, VolumeChanger::typeName());
...
QDataStream &operator<<(QDataStream &ds, const VolumeChanger &volchanger)
{
    ...
    sds << volchanger.rangen << volchanger.mgid << static_cast<const Property &>(volchanger);
...
QDataStream &operator>>(QDataStream &ds, VolumeChanger &volchanger)
{
    ...
    if (v == 1)
    {
        ...
        sds >> volchanger.rangen >> volchanger.mgid >> static_cast<Property &>(volchanger);
    }
    else
...
VolumeChanger::VolumeChanger(const MoleculeGroup &molgroup) : Property(), mgid(molgroup.number())
{
}

/** Copy constructor */
VolumeChanger::VolumeChanger(const VolumeChanger &other) : Property(other), mgid(other.mgid)
{
}
...
*/
int VolumeChanger::changeVolume(System &system, const Volume &delta, const PropertyMap &map) const
{
...
/** Change the volume of the passed system 'system' by a random
    ...
    \throw SireError::incompatible_error
*/
int VolumeChanger::randomChangeVolume(System &system, const Volume &maxvolchange, double &new_bias, double &old_bias,
                                      const PropertyMap &map) const
{
    new_bias = 1;
    old_bias = 1;

    return this->changeVolume(system, Volume(rangen.rand(-maxvolchange, maxvolchange)), map);
}
...
/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const NullVolumeChanger &nullvolchanger)
{
...
/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, NullVolumeChanger &nullvolchanger)
{
...
/** Constructor */
NullVolumeChanger::NullVolumeChanger() : ConcreteProperty<NullVolumeChanger, VolumeChanger>()
{
}
...
/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const ScaleVolumeFromCenter &scalevol)
{
    writeHeader(ds, r_scalevol, 1);

    SharedDataStream sds(ds);

    sds << scalevol.scale_point << static_cast<const VolumeChanger &>(scalevol);

    return ds;
}

/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, ScaleVolumeFromCenter &scalevol)
{
    ...
    if (v == 1)
    {
        ...
        sds >> scalevol.scale_point >> static_cast<VolumeChanger &>(scalevol);
    }
    else
...
/** Null constructor */
ScaleVolumeFromCenter::ScaleVolumeFromCenter() : ConcreteProperty<ScaleVolumeFromCenter, VolumeChanger>()
{
}
...
ScaleVolumeFromCenter::ScaleVolumeFromCenter(const MGID &mgid)
    : ConcreteProperty<ScaleVolumeFromCenter, VolumeChanger>(mgid), scale_point(VectorPoint(Vector(0)))
{
}
...
ScaleVolumeFromCenter::ScaleVolumeFromCenter(const MoleculeGroup &molgroup)
    : ConcreteProperty<ScaleVolumeFromCenter, VolumeChanger>(molgroup), scale_point(VectorPoint(Vector(0)))
{
}
...
ScaleVolumeFromCenter::ScaleVolumeFromCenter(const MGID &mgid, const PointRef &point)
    : ConcreteProperty<ScaleVolumeFromCenter, VolumeChanger>(mgid), scale_point(point)
{
}

/** Construct to scale the molecules in the group 'molgroup',
    scaling the molecules from the point 'point' */
ScaleVolumeFromCenter::ScaleVolumeFromCenter(const MoleculeGroup &molgroup, const PointRef &point)
    : ConcreteProperty<ScaleVolumeFromCenter, VolumeChanger>(molgroup), scale_point(point)
{
}

/** Copy constructor */
ScaleVolumeFromCenter::ScaleVolumeFromCenter(const ScaleVolumeFromCenter &other)
    : ConcreteProperty<ScaleVolumeFromCenter, VolumeChanger>(other), scale_point(other.scale_point)
{
}
...
/** Copy assignment operator */
ScaleVolumeFromCenter &ScaleVolumeFromCenter::operator=(const ScaleVolumeFromCenter &other)
{
    VolumeChanger::operator=(other);
...
{
    return VolumeChanger::operator==(other) and scale_point == other.scale_point;
}

/** Comparison operator */
bool ScaleVolumeFromCenter::operator!=(const ScaleVolumeFromCenter &other) const
{
    return VolumeChanger::operator!=(other) or scale_point != other.scale_point;
}
...
/** Set the volume of the system 'system' to 'volume', using the
    ...
    \throw SireError::incompatible_error
*/
int ScaleVolumeFromCenter::setVolume(System &system, const Volume &volume, const PropertyMap &map) const
{
    ...
    if (old_volume <= Volume(0))
        throw SireError::invalid_state(QObject::tr("Cannot set the volume to %1 A^3 as the volume of the space "
                                                   "%2 (%3 A^3) is less than or equal to zero!")
                                           .arg(volume.to(angstrom3))
                                           .arg(old_space.toString())
                                           .arg(old_volume.to(angstrom3)),
                                       CODELOC);
...
// amount, which is the cube-root of the ratio of the volume change)
    ...
    for (Molecules::const_iterator it = old_molecules.constBegin(); it != old_molecules.constEnd(); ++it)
    {
