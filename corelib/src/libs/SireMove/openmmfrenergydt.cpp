...
OpenMMFrEnergyDT::OpenMMFrEnergyDT(const MoleculeGroup &molecule_group, const MoleculeGroup &solute_group,
                                   bool frequent_save)
    : ConcreteProperty<OpenMMFrEnergyDT, Integrator>(), frequent_save_velocities(frequent_save),
...
OpenMMFrEnergyDT::OpenMMFrEnergyDT(const OpenMMFrEnergyDT &other)
    : ConcreteProperty<OpenMMFrEnergyDT, Integrator>(other), frequent_save_velocities(other.frequent_save_velocities),
      ...
      device_index(other.device_index), Alchemical_value(other.Alchemical_value), coulomb_power(other.coulomb_power),
      shift_delta(other.shift_delta), delta_alchemical(other.delta_alchemical), buffer_coords(other.buffer_coords),
...
void OpenMMFrEnergyDT::initialise()
{
    ...
    if (moleculegroup.isEmpty())
    {
        throw SireError::program_bug(
            QObject::tr("Cannot initialise OpenMMFrEnergyDT because molgroup has not been defined"), CODELOC);
    }
    ...
    {
        throw SireError::program_bug(
            QObject::tr("Cannot initialise OpenMMFrEnergyDT because solute group has not been defined"), CODELOC);
    }
    ...
    if (ConstraintType == "none")
        ...
        throw SireError::program_bug(
            QObject::tr(
                "The Constraints method has not been specified. Possible choises: none, hbonds, allbonds, hangles"),
            CODELOC);
    ...
    if (flag_cutoff == NOCUTOFF)
    {
        ...
        custom_softcore_solute_solvent =
            new OpenMM::CustomNonbondedForce("(10.0*Hls+100.0*Hcs)*ZeroOne;"
        ...
        custom_solute_solute_solvent_solvent =
            new OpenMM::CustomNonbondedForce("(Hl+Hc)*ZeroOne;"
    ...
    else
    {
        ...
        custom_softcore_solute_solvent = new OpenMM::CustomNonbondedForce(
            "(10.0*Hls+100.0*Hcs)*ZeroOne;"
            "ZeroOne=issolute1*(1-issolute2)+issolute2*(1-issolute1);"
...
void OpenMMFrEnergyDT::integrate(IntegratorWorkspace &workspace, const Symbol &nrg_component,
                                 SireUnits::Dimension::Time timestep, int nmoves, bool record_stats)
{
    ...
    if (not this->isInitialised)
    {
        ...
        throw SireError::program_bug(
            QObject::tr("OpenMMFrEnergyDT should have been initialised before calling integrate."), CODELOC);
    }
    ...
    if (nmoves < energy_frequency)
        throw SireError::program_bug(
            QObject::tr(
                "You are requesting to save energy every  %1 steps, which is above the total number of %2 steps.")
    ...
    if (Debug)
    {
        ...
        qDebug() << "Start - Lambda = " << lam_val
                 << " Potential energy lambda  = " << state_openmm.getPotentialEnergy() * OpenMM::KcalPerKJ
                 << " [A + A^2] kcal"
                 << "\n";
    ...
    while (sample_count < n_samples)
    {
        ...
        if (Debug)
            qDebug() << "Lambda = " << lam_val
                     << " Potential energy lambda MD = " << potential_energy_lambda * OpenMM::KcalPerKJ << " kcal/mol"
                     << "\n";
        ...
        if ((Alchemical_value + delta_alchemical) > 2.0)
        {
            ...
            if (Debug)
            {
                ...
                qDebug() << "Lambda - delta = " << lam_val
                         << " Potential energy minus  = " << potential_energy_lambda_minus_delta * OpenMM::KcalPerKJ
                         << " kcal/mol"
                         << "\n";
            }
        }
        else if ((Alchemical_value - delta_alchemical) < 0.0)
        {
            ...
            {
                qDebug() << "Lambda + delta = " << lam_val
                         << " Potential energy plus  = " << potential_energy_lambda_plus_delta * OpenMM::KcalPerKJ
                         << " kcal/mol"
                         << "\n";
        ...
        {
            throw SireError::program_bug(QObject::tr("******************* Not a Number. Error! *******************"),
                                         CODELOC);
        }
    ...
    if (MCBarostat_flag == true)
    {
        ...
        else
        {
            ...
            {
                const QString buffered_space = "buffered_space_" + QString::number(k);
                Vector dims(buffered_dimensions[k][0].x(), buffered_dimensions[k][1].y(),
