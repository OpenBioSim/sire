...
static const RegisterMetaType<HMCVelGen> r_hmcvelgen(MAGIC_ONLY, HMCVelGen::typeName());

QDataStream &operator<<(QDataStream &ds, const HMCVelGen &hmcvelgen)
{
    writeHeader(ds, r_hmcvelgen, 1);

    ds << hmcvelgen.rangen << hmcvelgen.temp << static_cast<const VelocityGenerator &>(hmcvelgen);
...
QDataStream &operator>>(QDataStream &ds, HMCVelGen &hmcvelgen)
{
    ...
    if (v == 1)
    {
        ds >> hmcvelgen.rangen >> hmcvelgen.temp >> static_cast<VelocityGenerator &>(hmcvelgen);
    }
    else
...
/** Copy constructor */
HMCVelGen::HMCVelGen(const HMCVelGen &other) : VelocityGenerator(other), rangen(other.rangen), temp(other.temp)
{
}
...
/** Copy constructor */
HMCGenerator::HMCGenerator(const HMCGenerator &other) : ConcreteProperty<HMCGenerator, HMCVelGen>(other)
{
}
...
AtomVelocities HMCGenerator::generate(const MoleculeView &molview, const PropertyMap &map) const
{
    ...
    for (int i = 0; i < sz; ++i)
    {
        ...
        Vector norm_rand(rand().randNorm(0, 1), rand().randNorm(0, 1), rand().randNorm(0, 1));
...
HybridMC::HybridMC(const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(map), velgen(HMCGenerator()), nsteps(100)
{
...
HybridMC::HybridMC(const MoleculeGroup &molgroup, const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(molgroup, map), velgen(HMCGenerator()), nsteps(100)
{
    BOOST_ASSERT(md.ensemble() == Ensemble::NVE());
    BOOST_ASSERT(md.integrator().isTimeReversible());
...
void HybridMC::assertValidIntegrator(const Integrator &integrator) const
{
    if (integrator.ensemble() != Ensemble::NVE())
        throw SireError::incompatible_error(QObject::tr("You can only use an integrator that samples from the "
                                                ...
                                                .arg(integrator.ensemble().toString()),
                                            CODELOC);

    if (not integrator.isTimeReversible())
        throw SireError::incompatible_error(QObject::tr("You can only use a time-reversible integrator with "
...
HybridMC::HybridMC(const MoleculeGroup &molgroup, const Integrator &integrator, const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(molgroup, integrator, map), velgen(HMCGenerator()), nsteps(100)
{
...
HybridMC::HybridMC(const MoleculeGroup &molgroup, Time timestep, const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(molgroup, timestep, map), velgen(HMCGenerator()), nsteps(100)
{
    BOOST_ASSERT(md.ensemble() == Ensemble::NVE());
    BOOST_ASSERT(md.integrator().isTimeReversible());
...
HybridMC::HybridMC(const MoleculeGroup &molgroup, const Integrator &integrator, Time timestep, const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(molgroup, integrator, timestep, map), velgen(HMCGenerator()),
      nsteps(100)
{
    assertValidIntegrator(integrator);
    MonteCarlo::setEnsemble(Ensemble::NVT(25 * celsius));
...
HybridMC::HybridMC(const MoleculeGroup &molgroup, int n, const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(molgroup, map), velgen(HMCGenerator()), nsteps(0)
{
    if (n > 0)
        nsteps = n;
...
HybridMC::HybridMC(const MoleculeGroup &molgroup, const Integrator &integrator, int n, const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(molgroup, integrator, map), velgen(HMCGenerator()), nsteps(0)
{
    if (n > 0)
        nsteps = n;
...
HybridMC::HybridMC(const MoleculeGroup &molgroup, Time timestep, int n, const PropertyMap &map)
    : ConcreteProperty<HybridMC, MonteCarlo>(map), md(molgroup, timestep, map), velgen(HMCGenerator()), nsteps(0)
{
    if (n > 0)
        nsteps = n;
...
/** Construct to perform hybrid MC moves on the passed molecule group,
    ...
    running 'nmoves' MD moves per MC move */
HybridMC::HybridMC(const MoleculeGroup &molgroup, const Integrator &integrator, Time timestep, int n,
...
HybridMC::HybridMC(const HybridMC &other)
    : ConcreteProperty<HybridMC, MonteCarlo>(other), md(other.md), velgen(other.velgen), nsteps(other.nsteps)
{
}
...
bool HybridMC::operator==(const HybridMC &other) const
{
    return this == &other or
           (md == other.md and nsteps == other.nsteps and velgen == other.velgen and MonteCarlo::operator==(other));
}
...
void HybridMC::move(System &system, int nmoves, bool record_stats)
{
    ...
    try
    {
        ...
        for (int i = 0; i < nmoves; ++i)
        {
            ...
            qDebug() << old_nrg << md.kineticEnergy().value() << (old_nrg + md.kineticEnergy().value());
            ...
            qDebug() << new_nrg << md.kineticEnergy().value() << (new_nrg + md.kineticEnergy().value());
