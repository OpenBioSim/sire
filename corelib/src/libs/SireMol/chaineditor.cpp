...
QDataStream &operator<<(QDataStream &ds, const ChainEditor &chaineditor)
{
...
QDataStream &operator>>(QDataStream &ds, ChainEditor &chaineditor)
{
...
ChainEditor::ChainEditor(const Chain &chain) : ConcreteProperty<ChainEditor, Editor<ChainEditor, Chain>>(chain)
{
}

/** Copy constructor */
ChainEditor::ChainEditor(const ChainEditor &other) : ConcreteProperty<ChainEditor, Editor<ChainEditor, Chain>>(other)
{
}
...
/** Transfer all residues that match the ID 'resid' in this Chain
    ...
    \throw SireError::invalid_index
*/
ChainStructureEditor ChainEditor::transfer(const ResID &resid, const ChainID &cgid) const
{
    ChainStructureEditor editor(*this);
...
/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const ChainStructureEditor &chaineditor)
{
    writeHeader(ds, r_cgstructeditor, 1);

    ds << chaineditor.uid << static_cast<const StructureEditor &>(chaineditor);

    return ds;
}

/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, ChainStructureEditor &chaineditor)
{
    VersionID v = readHeader(ds, r_cgstructeditor);

    if (v == 1)
    {
        ds >> chaineditor.uid >> static_cast<StructureEditor &>(chaineditor);
    }
    else
...
/** Null constructor */
ChainStructureEditor::ChainStructureEditor() : StructureEditor(), uid(0)
{
    this->operator=(StructureEditor::addChain());
}

/** Construct to edit a copy of the Chain 'chain' */
ChainStructureEditor::ChainStructureEditor(const Chain &chain) : StructureEditor(chain.data())
{
...
/** Construct to edit the Chain at index 'cgidx' in the molecule
    ...
    \throw SireError::invalid_index
*/
ChainStructureEditor::ChainStructureEditor(const StructureEditor &data, ChainIdx cgidx) : StructureEditor(data)
{
    uid = this->getUID(cgidx);
}

/** Copy constructor */
ChainStructureEditor::ChainStructureEditor(const ChainStructureEditor &other) : StructureEditor(other), uid(other.uid)
{
}
...
{
    return QObject::tr("StructureEditor{ Chain( %1 ) }").arg(this->name());
}
...
/** Transfer all residues that match the ID 'resid' in this Chain
    ...
    \throw SireError::invalid_index
*/
ChainStructureEditor &ChainStructureEditor::transfer(const ResID &resid, const ChainID &chainid)
{
    this->reparentResidue(this->getUID(this->index() + resid), this->chainIdx(chainid));
...
/** Transfer the ith residue of this Chain into the Chain identified
    ...
    \throw SireError::invalid_index
*/
ChainStructureEditor &ChainStructureEditor::transfer(int i, const ChainID &chainid)
{
