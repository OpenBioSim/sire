...
public:
    findmcs_callback(const G0 &mg0, const G1 &mg1, int nats, int *msize, QList<QHash<int, int>> *matches,
                     QElapsedTimer *tt, QElapsedTimer *td, qint64 maxtime, qint64 *update, bool *timedout,
                     const QSet<int> &user_verts, bool is_user_map, bool verbose)
        : g0(mg0), g1(mg1), max_nats(nats), max_size(msize), best_matches(matches), t_total(tt), t_delta(td),
          last_update(update), max_time_ns(maxtime), timed_out(timedout), user_matched_verts(user_verts),
          is_user_map(is_user_map), verbose(verbose)
    {
        *max_size = 0;
    }

    template <typename Map01, typename Map10>
    bool operator()(Map01 map01, Map10 map10, typename graph_traits<G0>::vertices_size_type subgraph_size)
    {
...
QVector<Element> getElements(const Molecule &mol, const PropertyMap &map)
{
    ...
    try
    {
        if (mol.hasProperty(map["mass"]))
        {
            ...
            for (int i = 0; i < mol.nAtoms(); ++i)
            {
                elements[i] = Element::elementWithMass(m.at(mol.data().info().cgAtomIdx(AtomIdx(i))));
            }
    ...
    throw SireBase::missing_property(
        QObject::tr("Need to work out the element of each atom, so need an element or mass property."), CODELOC);
...
QHash<AtomIdx, AtomIdx> pvt_findMCS(const MoleculeView &mol, const MoleculeView &other, const AtomMatcher &matcher,
                                    const Time &timeout, bool match_light_atoms, const PropertyMap &map0,
                                    const PropertyMap &map1, bool is_pre_match, bool check_reverse_if_timeout,
                                    int min_heavy_protons, bool verbose)
{
    ...
    if (not matcher.isNull())
    {
        ...
        for (QHash<AtomIdx, AtomIdx>::const_iterator it = user_map01.constBegin(); it != user_map01.constEnd(); ++it)
        {
    ...
    findmcs_callback<Graph, Graph> func(g0, g1, qMin(nats0 - nskip0, nats1 - nskip1), &max_size, &best_matches,
                                        &t_total, &t_delta, max_time_ns, &last_update, &timed_out, user_matched_verts,
                                        is_user_map, verbose);
    ...
    if (not best_matches.isEmpty())
    {
        ...
        for (QHash<int, int>::const_iterator it = best_match.constBegin(); it != best_match.constEnd(); ++it)
        {
    ...
    if (timed_out)
    {
        if (check_reverse_if_timeout)
        {
            ...
            QHash<AtomIdx, AtomIdx> rmap =
                pvt_findMCS(other, mol, AtomMatchInverter(matcher), timeout, match_light_atoms, map1, map0,
                            is_pre_match, false, min_heavy_protons, verbose);
            ...
            if (rmap.count() > map.count())
            {
                ...
                for (QHash<AtomIdx, AtomIdx>::const_iterator it = rmap.constBegin(); it != rmap.constEnd(); ++it)
                {
                                           ...
                                           int min_heavy_protons, bool verbose) const
{
    if (match_light_atoms and min_heavy_protons > 1)
    {
        ...
        QHash<AtomIdx, AtomIdx> pre_match =
            pvt_findMCS(*this, other, matcher, timeout, false, map0, map1, true, true, min_heavy_protons, verbose);

        // now use the pre-match to speed up the full match
        return pvt_findMCS(*this, other, AtomResultMatcher(pre_match), timeout, true, map0, map1, false, true,
                           min_heavy_protons, verbose);
    }
    else
        return pvt_findMCS(*this, other, matcher, timeout, false, map0, map1, false, true, min_heavy_protons, verbose);
}

QVector<QHash<AtomIdx, AtomIdx>> pvt_findMCSmatches(const MoleculeView &mol, const MoleculeView &other,
                                                    const AtomMatcher &matcher, const Time &timeout,
                                                    bool match_light_atoms, const PropertyMap &map0,
                                                    const PropertyMap &map1, bool is_pre_match,
                                                    bool check_reverse_if_timeout, int min_heavy_protons, bool verbose)
{
    // first, see if the user has specified any match
    QHash<AtomIdx, AtomIdx> user_map01;
    QHash<AtomIdx, AtomIdx> user_map10;
    ...
    QVector<QHash<AtomIdx, AtomIdx>> maps;
    ...
    if (not best_matches.isEmpty())
    {
        for (auto &match : best_matches)
        {
            ...
            for (QHash<int, int>::const_iterator it = match.constBegin(); it != match.constEnd(); ++it)
            {
                map.insert(idx_to_atomidx0[it.key()], idx_to_atomidx1[it.value()]);
            }
    ...
    if (timed_out)
    {
        if (check_reverse_if_timeout)
        {
            ...
            if (verbose)
                qDebug() << "Initial match timed out, so trying the reverse match...";
            QVector<QHash<AtomIdx, AtomIdx>> rmaps =
                pvt_findMCSmatches(other, mol, AtomMatchInverter(matcher), timeout, match_light_atoms, map1, map0,
                                   is_pre_match, false, min_heavy_protons, verbose);
            ...
            if (rmaps.count() > 0)
            {
                if ((maps.count() == 0) or (rmaps[0].count() > maps[0].count()))
                {
                    ...
                    for (auto &match : rmaps)
                    {
                        ...
                        for (QHash<AtomIdx, AtomIdx>::const_iterator it = match.constBegin(); it != match.constEnd();
                             ++it)
                        {
                            map.insert(it.value(), it.key());
                        }
    ...
    if (maps.isEmpty() and best_matches.isEmpty())
    {
        if (verbose)
            qDebug() << "FOUND NO MATCHES?";
        // return the original map that constrained this search
        maps.append(user_map01);
    }
                                                           ...
                                                           int min_heavy_protons, bool verbose) const
{
    if (match_light_atoms and min_heavy_protons > 1)
    {
        ...
        // now use the pre-match to speed up the full match
        return pvt_findMCSmatches(*this, other, AtomResultMatcher(pre_match), timeout, true, map0, map1, false, true,
                                  min_heavy_protons, verbose);
    }
    else
