...
namespace SireMol
{
    ...
    */
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE QList<T> MolInfo::intersection(const QList<T> &list0, const QList<T> &list1)
    {
        ...
        if (list0.count() <= list1.count())
        {
            ...
            for (typename QList<T>::const_iterator it = list0.constBegin(); it != list0.constEnd(); ++it)
            {
                if (set1.contains(*it))
        ...
        else
        {
            ...
            for (typename QList<T>::const_iterator it = list1.constBegin(); it != list1.constEnd(); ++it)
            {
                if (set0.contains(*it))
                    intersection_list.append(*it);
            }
    ...
    namespace detail
    {
        ...
        template <class T>
        QList<typename T::Index> getAll(const MolInfo &molinfo, const AtomSelection &selected_atoms);
