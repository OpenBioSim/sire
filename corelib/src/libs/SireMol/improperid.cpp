...
QDataStream &operator<<(QDataStream &ds, const ImproperID &improperid)
{
    writeHeader(ds, r_improperid, 1);

    SharedDataStream sds(ds);

    sds << improperid.atm0 << improperid.atm1 << improperid.atm2 << improperid.atm3;

    return ds;
}

/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, ImproperID &improperid)
{
...
/** Construct a improper between the two specified atoms. The order
    ...
    molecules */
ImproperID::ImproperID(const AtomID &atom0, const AtomID &atom1, const AtomID &atom2, const AtomID &atom3)
...
ImproperID::ImproperID(const ImproperID &other)
    : ID(other), atm0(other.atm0), atm1(other.atm1), atm2(other.atm2), atm3(other.atm3)
{
}
...
{
    return atm0 == other.atm0 and atm1 == other.atm1 and atm2 == other.atm2 and atm3 == other.atm3;
}

/** Comparison operator - the order is important */
bool ImproperID::operator!=(const ImproperID &other) const
{
    return atm0 != other.atm0 or atm1 != other.atm1 or atm2 != other.atm2 or atm3 != other.atm3;
}
...
{
    return ((atm0.hash() * atm1.hash()) << 16) | ((atm2.hash() * atm3.hash()) & 0x0000FFFF);
}
...
{
    return atm0.isNull() and atm1.isNull() and atm2.isNull() and atm3.isNull();
}
...
*/
tuple<AtomIdx, AtomIdx, AtomIdx, AtomIdx> ImproperID::map(const MoleculeInfoData &molinfo) const
{
    return tuple<AtomIdx, AtomIdx, AtomIdx, AtomIdx>(molinfo.atomIdx(atm0), molinfo.atomIdx(atm1),
...
/** Return the indicies of the four atoms of this improper, between the
    ...
    \throw SireError::invalid_index
*/
tuple<AtomIdx, AtomIdx, AtomIdx, AtomIdx> ImproperID::map(const MoleculeInfoData &mol0info,
                                                          const MoleculeInfoData &mol1info,
                                                          const MoleculeInfoData &mol2info,
                                                          const MoleculeInfoData &mol3info) const
{
    return tuple<AtomIdx, AtomIdx, AtomIdx, AtomIdx>(mol0info.atomIdx(atm0), mol1info.atomIdx(atm1),
...
/** Return the geometric torsion formed by the four atoms
    ...
    \throw SireError::invalid_index
*/
Torsion ImproperID::torsion(const MoleculeData &moldata, const PropertyMap &map) const
{
    const AtomCoords &coords = moldata.property(map["coordinates"]).asA<AtomCoords>();

    return Torsion(coords.at(moldata.info().cgAtomIdx(atm0)), coords.at(moldata.info().cgAtomIdx(atm1)),
                   coords.at(moldata.info().cgAtomIdx(atm2)), coords.at(moldata.info().cgAtomIdx(atm3)));
}
...
/** Return the geometric torsion formed by the four atoms,
    ...
    \throw SireError::invalid_index
*/
Torsion ImproperID::torsion(const MoleculeData &mol0data, const PropertyMap &map0, const MoleculeData &mol1data,
                            ...
                            const MoleculeData &mol3data, const PropertyMap &map3) const
{
    ...
    const AtomCoords &coords3 = mol3data.property(map3["coordinates"]).asA<AtomCoords>();

    return Torsion(coords0.at(mol0data.info().cgAtomIdx(atm0)), coords1.at(mol1data.info().cgAtomIdx(atm1)),
                   coords2.at(mol2data.info().cgAtomIdx(atm2)), coords3.at(mol3data.info().cgAtomIdx(atm3)));
}
...
/** Return the geometric torsion formed by the four atoms,
    ...
    \throw SireError::invalid_index
*/
Torsion ImproperID::torsion(const MoleculeData &mol0data, const MoleculeData &mol1data, const MoleculeData &mol2data,
                            const MoleculeData &mol3data, const PropertyMap &map) const
{
    return this->torsion(mol0data, map, mol1data, map, mol2data, map, mol3data, map);
}
...
/** Return the size of this improper in the molecule whose data
    ...
    \throw SireError::invalid_index
*/
Angle ImproperID::size(const MoleculeData &moldata, const PropertyMap &map) const
{
...
/** Return the size of the improper between atom0() in the
    ...
    \throw SireError::invalid_index
*/
Angle ImproperID::size(const MoleculeData &mol0data, const PropertyMap &map0, const MoleculeData &mol1data,
                       const PropertyMap &map1, const MoleculeData &mol2data, const PropertyMap &map2,
                       const MoleculeData &mol3data, const PropertyMap &map3) const
{
    return this->torsion(mol0data, map0, mol1data, map1, mol2data, map2, mol3data, map3).improperAngle();
}
...
/** Return the size of the improper between atom0() in the
    ...
    \throw SireError::invalid_index
*/
Angle ImproperID::size(const MoleculeData &mol0data, const MoleculeData &mol1data, const MoleculeData &mol2data,
                       const MoleculeData &mol3data, const PropertyMap &map) const
{
    return this->torsion(mol0data, mol1data, mol2data, mol3data, map).improperAngle();
}
