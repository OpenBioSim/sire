...
namespace SireMol
{
    ...
    */
    ...
    /** Null constructor */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP>::AtomsIn() : AtomID(), strt(0), end(-1)
    {
    }

    /** Construct to get all of the atoms in the passed group */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP>::AtomsIn(const GROUP &id) : AtomID(), groupid(id), strt(0), end(-1)
    {
    }

    /** Construct for a specified atom */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP>::AtomsIn(const GROUP &id, qint32 i) : AtomID(), groupid(id), strt(i), end(i)
    {
    }

    /** Construct for a range of atoms */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP>::AtomsIn(const GROUP &id, qint32 i, qint32 j)
    ...
    /** Copy constructor */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP>::AtomsIn(const AtomsIn<GROUP> &other)
        : AtomID(other), groupid(other.groupid), strt(other.strt), end(other.end)
    {
    }

    /** Destructor */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP>::~AtomsIn()
    {
    }

    /** Return a string representation of this ID */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE QString AtomsIn<GROUP>::toString() const
    {
        if (strt == 0 and end == -1)
            ...
            return QString("(%1).atoms(%2,%3)").arg(groupid.toString()).arg(strt).arg(end);
    }

    /** Copy assignment operator */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP> &AtomsIn<GROUP>::operator=(const AtomsIn<GROUP> &other)
    {
        if (this != &other)
        {
    ...
    /** Comparison operator */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE bool AtomsIn<GROUP>::operator==(const AtomsIn<GROUP> &other) const
    {
        return strt == other.strt and end == other.end and groupid == other.groupid;
    }

    /** Comparison operator */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE bool AtomsIn<GROUP>::operator==(const SireID::ID &other) const
    {
        return SireID::ID::compare<AtomsIn<GROUP>>(*this, other);
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE const char *AtomsIn<GROUP>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<AtomsIn<GROUP>>());
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE const char *AtomsIn<GROUP>::what() const
    {
        return AtomsIn<GROUP>::typeName();
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE AtomsIn<GROUP> *AtomsIn<GROUP>::clone() const
    {
    ...
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE bool AtomsIn<GROUP>::isNull() const
    {
        return groupid.isNull() and strt == 0 and end == -1;
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE uint AtomsIn<GROUP>::hash() const
    {
    ...
    */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE QList<AtomIdx> AtomsIn<GROUP>::map(const MolInfo &molinfo) const
    {
...
template <class GROUP>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireMol::AtomsIn<GROUP> &atoms)
{
...
template <class GROUP>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireMol::AtomsIn<GROUP> &atoms)
{
    ds >> atoms.groupid >> atoms.strt >> atoms.end;
    return ds;
}
