...
namespace SireMol
{
    ...
    */
    ...
    public:
        ...
        Mover<T> &rotate(const Quaternion &quat, const Vector &point, const PropertyMap &map = PropertyMap());

        Mover<T> &rotate(const Matrix &rotmat, const Vector &point, const PropertyMap &map = PropertyMap());

        Mover<T> &transform(const Transform &transform, const PropertyMap &map = PropertyMap());

        Mover<T> &change(const BondID &bond, SireUnits::Dimension::Length delta, const PropertyMap &map = PropertyMap());

        Mover<T> &change(const AngleID &angle, SireUnits::Dimension::Angle delta, const PropertyMap &map = PropertyMap());

        Mover<T> &change(const DihedralID &dihedral, SireUnits::Dimension::Angle delta,
                         const PropertyMap &map = PropertyMap());

        Mover<T> &change(const BondID &bond, SireUnits::Dimension::Angle delta, const PropertyMap &map = PropertyMap());

        Mover<T> &change(const ImproperID &improper, SireUnits::Dimension::Angle delta,
                         const PropertyMap &map = PropertyMap());

        Mover<T> &set(const BondID &bond, SireUnits::Dimension::Length value, const PropertyMap &map = PropertyMap());

        Mover<T> &set(const AngleID &angle, SireUnits::Dimension::Angle value, const PropertyMap &map = PropertyMap());
        ...
        Mover<T> &align(const MoleculeView &other, const PropertyMap &map = PropertyMap());

        Mover<T> &align(const MoleculeView &other, const PropertyMap &map0, const PropertyMap &map1);

        Mover<T> &align(const MoleculeView &other, const AtomMatcher &matcher, const PropertyMap &map = PropertyMap());

        Mover<T> &align(const MoleculeView &other, const AtomMatcher &matcher, const PropertyMap &map0,
    ...
    /** Null constructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T>::Mover() : SireBase::ConcreteProperty<Mover<T>, T>(), MoverBase()
    {
    }
    ...
    /** Construct a mover that can move all of the atoms
        in the view 'view' */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T>::Mover(const T &view)
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T>::Mover(const T &view, const AtomSelection &movable_atoms)
    ...
    /** Copy constructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T>::Mover(const Mover<T> &other)
    ...
    /** Destructor */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T>::~Mover()
    {
    }

    /** Copy assignment operator */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::operator=(const Mover<T> &other)
    {
        T::operator=(other);
    ...
    /** Copy assignment operator */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::operator=(const T &other)
    {
        T::operator=(other);
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE const char *Mover<T>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<Mover<T>>());
    }

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> *Mover<T>::clone() const
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE T Mover<T>::commit() const
    {
        return *this;
    }
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE QString Mover<T>::toString() const
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::mapInto(const AxisSet &axes, const PropertyMap &map)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::changeFrame(const AxisSet &from_frame, const AxisSet &to_frame,
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::translate(const Vector &delta, const PropertyMap &map)
    {
    ...
    /** Rotate the movable atoms using the quaternion 'quat' about the
        ...
        properties */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::rotate(const Quaternion &quat, const Vector &point, const PropertyMap &map)
    {
    ...
    /** Rotate the movable atoms using the rotation matrix 'rotmat' about
        ...
        properties */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::rotate(const Matrix &rotmat, const Vector &point, const PropertyMap &map)
    {
    ...
    /** Transform the movable atoms using the transformation 't',
        ...
        properties */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::transform(const Transform &t, const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::change(const BondID &bond, SireUnits::Dimension::Length delta,
                                                       const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::change(const AngleID &angle, SireUnits::Dimension::Angle delta,
                                                       const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::change(const DihedralID &dihedral, SireUnits::Dimension::Angle delta,
                                                       const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::change(const BondID &bond, SireUnits::Dimension::Angle delta,
                                                       const PropertyMap &map)
    {
        MoverBase::change(*(this->d), bond, delta, map);
        return *this;
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::change(const ImproperID &improper, SireUnits::Dimension::Angle delta,
                                                       const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::set(const BondID &bond, SireUnits::Dimension::Length value,
                                                    const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::set(const AngleID &angle, SireUnits::Dimension::Angle value,
                                                    const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::set(const DihedralID &dihedral, SireUnits::Dimension::Angle value,
                                                    const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::setAll(const DihedralID &dihedral, SireUnits::Dimension::Angle value,
                                                       const PropertyMap &map)
    {
    ...
    */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::set(const ImproperID &improper, SireUnits::Dimension::Angle value,
                                                    const PropertyMap &map)
    {
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::align(const MoleculeView &other, const AtomMatcher &matcher,
                                                      const PropertyMap &map0, const PropertyMap &map1)
    {
        MoverBase::transform(*(this->d), SireMol::getAlignment(other, map1, *this, map0, AtomMatchInverter(matcher)), map0);
    ...
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::align(const MoleculeView &other, const PropertyMap &map)
    {
        MoverBase::transform(*(this->d), SireMol::getAlignment(other, *this, map), map);
    ...
    /** Align this molecule view against 'other' using the supplied property
        ...
        then supply an AtomMatcher */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::align(const MoleculeView &other, const PropertyMap &map0,
                                                      const PropertyMap &map1)
    {
        MoverBase::transform(*(this->d), SireMol::getAlignment(other, map1, *this, map0), map0);
    ...
    SIRE_OUTOFLINE_TEMPLATE Mover<T> &Mover<T>::align(const MoleculeView &other, const AtomMatcher &matcher,
                                                      const PropertyMap &map)
    {
        MoverBase::transform(*(this->d), SireMol::getAlignment(other, *this, AtomMatchInverter(matcher), map), map);
