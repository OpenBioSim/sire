...
namespace SireMol
{
    ...
    */
    ...
    /** Null constructor */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP>::ResIn() : ResID(), strt(0), end(-1)
    {
    }

    /** Construct to get all of the residues in the passed group */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP>::ResIn(const GROUP &id) : ResID(), groupid(id), strt(0), end(-1)
    {
    }

    /** Construct for a specified residue */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP>::ResIn(const GROUP &id, qint32 i) : ResID(), groupid(id), strt(i), end(i)
    {
    }

    /** Construct for a range of residues */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP>::ResIn(const GROUP &id, qint32 i, qint32 j) : ResID(), groupid(id), strt(i), end(j)
    {
    }

    /** Copy constructor */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP>::ResIn(const ResIn<GROUP> &other)
        : ResID(other), groupid(other.groupid), strt(other.strt), end(other.end)
    {
    }

    /** Destructor */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP>::~ResIn()
    {
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE const char *ResIn<GROUP>::typeName()
    {
        return QMetaType::typeName(qMetaTypeId<ResIn<GROUP>>());
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE const char *ResIn<GROUP>::what() const
    {
        return ResIn<GROUP>::typeName();
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP> *ResIn<GROUP>::clone() const
    {
        return new ResIn<GROUP>(*this);
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE bool ResIn<GROUP>::operator!=(const ResIn<GROUP> &other) const
    {
        return not this->operator==(other);
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE bool ResIn<GROUP>::operator!=(const SireID::ID &other) const
    {
        return not this->operator==(other);
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE bool ResIn<GROUP>::isNull() const
    {
        return groupid.isNull() and strt == 0 and end == -1;
    }

    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE uint ResIn<GROUP>::hash() const
    {
    ...
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE QString ResIn<GROUP>::toString() const
    {
        if (strt == 0 and end == -1)
            ...
            return QString("(%1).residues(%2,%3)").arg(groupid.toString()).arg(strt).arg(end);
    }

    /** Copy assignment operator */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE ResIn<GROUP> &ResIn<GROUP>::operator=(const ResIn<GROUP> &other)
    {
        if (this != &other)
        {
    ...
    */
    template <class GROUP>
    SIRE_OUTOFLINE_TEMPLATE QList<ResIdx> ResIn<GROUP>::map(const MolInfo &molinfo) const
    {
...
#ifndef SIRE_SKIP_INLINE_FUNCTIONS
template <class GROUP>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator<<(QDataStream &ds, const SireMol::ResIn<GROUP> &res)
{
...
template <class GROUP>
SIRE_OUTOFLINE_TEMPLATE QDataStream &operator>>(QDataStream &ds, SireMol::ResIn<GROUP> &res)
{
    ds >> res.groupid >> res.strt >> res.end;
    return ds;
}
