...
namespace SireMol
{
    ...
    /**
    This template function is used to calculate the primary, aligned axes of a collection of atoms. The template type 'T'
    must possess functions that allow index based axes to the atoms in the collection.

    The reason that this is a template function is so that I don't have to write separate code to do this for both an
    AtomSet and an AtomArray (and perhaps any other atom collection that may be written!)

    @author Christopher Woods
    */

    template <class T>
    SIRE_OUTOFLINE_TEMPLATE AxisSet SIREMOL_EXPORT calculateAxes(const T &atoms)
    {
        if (atoms.count() == 0)
            return AxisSet();
    ...
    /** This function calculates the center of mass and the principal inertia tensor for 'atoms'. The template class 'T'
     * must have the same characteristics as that used by the 'calculateAxes' function. */
    template <class T>
    SIRE_OUTOFLINE_TEMPLATE AxisSet SIREMOL_EXPORT calculateInertiaAxes(const T &atoms)
    {
        if (atoms.count() == 0)
            return AxisSet();
        else if (atoms.count() == 1)
    ...
    SIRE_OUTOFLINE_TEMPLATE AxisSet SIREMOL_EXPORT calculateAlignmentAxes(const S &moveatoms, const T &refatoms)
    {
