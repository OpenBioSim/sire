...
void MolID::processMatches(QList<MolNum> &matches, const Molecules &mols) const
{
    if (matches.isEmpty())
        throw SireMol::missing_molecule(
            QObject::tr("There were no molecules that matched the ID \"%1\".").arg(this->toString()), CODELOC);
}
...
QList<MolNum> MolNum::map(const Molecules &molecules) const
{
    if (not molecules.contains(*this))
        throw SireMol::missing_molecule(
            QObject::tr("There is no molecule with number %1 in the set of molecules.").arg(this->toString()), CODELOC);
...
QList<MolNum> MolName::map(const Molecules &molecules) const
{
    ...
    if (molnums.isEmpty())
        throw SireMol::missing_molecule(
            QObject::tr("There is no molecule with name \"%1\" in the set of molecules.").arg(_name), CODELOC);
...
namespace SireID
{
    ...
    {
        for (QList<MolIdentifier>::const_iterator it = new_ids.constBegin(); it != new_ids.constEnd(); ++it)
        {
            this->add(it->base());
        }
    }
    ...
    uint IDAndSet<MolID>::hash() const
    {
        ...
        for (QSet<MolIdentifier>::const_iterator it = ids.constBegin(); it != ids.constEnd(); ++it)
        {
            h += it->hash();
        }
    ...
    QList<MolNum> IDAndSet<MolID>::_pvt_map(const T &group) const
    {
        ...
        if (molnums.isEmpty())
            throw SireMol::missing_molecule(QObject::tr("No molecule matches the ID \"%1\".").arg(this->toString()),
                                            CODELOC);
...
namespace SireID
{
    ...
    QList<MolNum> IDOrSet<MolID>::process(QList<MolNum> molnums) const
    {
        ...
        if (molnums.isEmpty())
            throw SireMol::missing_molecule(
                QObject::tr("There are no molecules that match the ID \"%1\".").arg(this->toString()), CODELOC);
