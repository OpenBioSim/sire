...
namespace SireIO
{
    ...
    System setAmberWater(const System &system, const QString &model, const PropertyMap &map)
    {
        ...
        {
            throw SireError::incompatible_error(QObject::tr("Unsupported AMBER water model '%1'").arg(model), CODELOC);
        }
        ...
        for (int i = 0; i < system.nMolecules(); ++i)
        {
            ...
            if (isWater(molecule))
            {
                molecule = _pvt_setAmberWater(molecule, template_molecule, _model, has_virtual, map);
            }
    ...
    SelectResult setAmberWater(const SelectResult &molecules, const QString &model, const PropertyMap &map)
    {
        ...
        for (const auto &molview : waters.views())
        {
            ...
            water = _pvt_setAmberWater(water, template_molecule, _model, has_virtual, map);
    ...
    SelectResult setGromacsWater(const SelectResult &molecules, const QString &model, const PropertyMap &map)
    {
        ...
        for (const auto &molview : waters.views())
        {
            ...
            // Apply the new template.
            water = _pvt_setGromacsWater(water, template_molecule, _model, has_virtual, map);
    ...
    System renumberConstituents(const System &system, unsigned mol_offset)
    {
        ...
        {
            auto molecule = system.molecule(MolIdx(i)).molecule();
            molecule = pvt_renumberConstituents(molecule, num_residues, num_atoms);
    ...
    System updateAndPreserveOrder(const System &system, const Molecule &molecule, unsigned index)
    {
        // Create a new system object.
        System new_system;
    ...
    System repartitionHydrogenMass(const System &system, const double factor, const unsigned water, const PropertyMap &map)
    {
        // Create a new system object.
        System new_system;
        ...
        for (int i = 0; i < system.nMolecules(); ++i)
        {
            ...
            if (molecule.hasProperty("is_perturbable"))
            {
                ...
                molecule = repartitionHydrogenMass(molecule, factor, water, pmap);
    ...
    Molecule repartitionHydrogenMass(Molecule &molecule, const double factor, const unsigned water, const PropertyMap &map)
    {
        // Skip water molecules.
        if (water == 0 and isWater(molecule, map))
        {
        ...
        if (not molecule.hasProperty(elem_prop))
        {
            throw SireError::incompatible_error(
                QObject::tr("The molecule doesn't have a \"%1\" property!").arg(elem_prop.toString()), CODELOC);
        }

        if (not molecule.hasProperty(mass_prop))
        {
            throw SireError::incompatible_error(
                QObject::tr("The molecule doesn't have a \"%1\" property!").arg(mass_prop.toString()), CODELOC);
        }
        ...
        else
        {
            ...
            {
                throw SireError::incompatible_error(
                    QObject::tr("The molecule doesn't have a \"%1\" property!").arg(conn_prop.toString()), CODELOC);
            }
    ...
    boost::tuple<System, QHash<MolIdx, MolIdx>> updateCoordinatesAndVelocities(
        const System &system0, const System &system1, const QHash<MolIdx, MolIdx> &molecule_mapping, const bool is_lambda1,
        const PropertyMap &map0, const PropertyMap &map1)
    {
        ...
        if (molecule_mapping.isEmpty())
        {
                ...
                for (int j = 0; j < system0.nMolecules(); ++j)
                {
                    ...
                    if (not matched_mols.contains(j))
                    {
                        ...
                        if (atom.number() == num)
                        {
                            ...
                            {
                                molecule0 =
                                    molecule0.edit().setProperty(prop_c, molecule1.property(prop_c1)).molecule().commit();
                            ...
                            {
                                throw SireError::incompatible_error(
                                    QObject::tr("Couldn't update coordinates for molecule index '%1'").arg(i), CODELOC);
                            }
                            ...
                            if (molecule1.hasProperty(prop_v1))
                            {
                                try
                                {
                                    molecule0 = molecule0.edit()
                                                    .setProperty(prop_v0, molecule1.property(prop_v1))
                                                    .molecule()
                                                    .commit();
                                }
                                catch (...)
                                {
                                    throw SireError::incompatible_error(
                                        QObject::tr("Couldn't update velocities for molecule index '%1'").arg(i), CODELOC);
                                }
                            }
            ...
            {
                throw SireError::incompatible_error(QObject::tr("Only matched '%1' molecules, expected '%2'")
                                                        .arg(matched_mols.count())
                                                        .arg(system0.nMolecules()),
                                                    CODELOC);
            }
        }
        ...
        else
        {
            ...
            for (const auto &molIdx0 : molecule_mapping.keys())
            {
                ...
                try
                {
                    molecule0 = molecule0.edit().setProperty(prop_c, molecule1.property(prop_c1)).molecule().commit();
                }
                catch (...)
                {
                    throw SireError::incompatible_error(
                        QObject::tr("Couldn't update coordinates for molecule index '%1'").arg(molIdx0), CODELOC);
                }
                ...
                if (molecule1.hasProperty(prop_v1))
                {
                    try
                    {
                        molecule0 = molecule0.edit().setProperty(prop_v0, molecule1.property(prop_v1)).molecule().commit();
                    }
                    catch (...)
                    {
                        throw SireError::incompatible_error(
                            QObject::tr("Couldn't update velocities for molecule index '%1'").arg(molIdx0), CODELOC);
                    }
                }
    ...
    boost::tuple<System, QHash<MolIdx, MolIdx>> updateCoordinatesAndVelocities(
        const System &system_original, const System &system_renumbered, const System &system_updated,
        const QHash<MolIdx, MolIdx> &molecule_mapping, const bool is_lambda1, const PropertyMap &map0,
        const PropertyMap &map1)
    {
        // Update coordinates and velocities for the molecules in 'system_original'
        // using the molecules in 'system_updated', which may not be in the same
        // order. To do so, we make use of 'system_renumbered', in which atom and
        // residue numbers are unique, and in ascending ordering. This allows us
        // to uniquely map atoms from 'system_updated' to 'system_renumbered' by
        // index, with coordinates/velocities from 'system_renumbered' then copied
        // to the matchin atom in 'system_original'.
        ...
        // Create a local copy of the reference system.
        System new_system = system_original;

        // Create a local copy of the molecule mapping.
        QHash<MolIdx, MolIdx> local_mapping(molecule_mapping);

        // Extract the molNums for both systems.
        const auto molNums0 = system_original.molNums();
        const auto molNums1 = system_updated.molNums();
        ...
        if (molecule_mapping.isEmpty())
        {
            // Loop over all molecules in the original system in MolIdx order.
            for (int i = 0; i < system_original.nMolecules(); ++i)
            {
                // Extract the molecule from the original and renumbered systems.
                auto molecule_original = system_original.molecule(MolIdx(i)).molecule();
                auto molecule_renumbered = system_renumbered.molecule(MolIdx(i)).molecule();

                // Get the number of the first atom in the molecule.
                const auto num = molecule_renumbered.atom(AtomIdx(0)).number();

                // Loop over all molecules in the updated system in MolIdx order.
                for (int j = 0; j < system_updated.nMolecules(); ++j)
                {
                    // Make sure this molecule hasn't already been matched.
                    if (not matched_mols.contains(j))
                    {
                        // Extract the molecule.
                        const auto molecule_updated = system_updated.molecule(MolIdx(j)).molecule();

                        // Extract the first atom from the updated molecule.
                        const auto atom = molecule_updated.atom(AtomIdx(0));

                        // The atom numbers match.
                        if (atom.number() == num)
                        {
                            // Extract the molecule.
                            const auto molNum1 = molecule_updated.number();
                            const auto molecule_updated = system_updated[molNum1].molecule();

                            // Initialise the coordinates property.
                            auto prop_c = prop_c0;

                            // Check whether the molecule is perturbable.
                            if (molecule_original.hasProperty("is_perturbable"))
                            {
                                if (is_lambda1)
                                    QString prop_c = "coordinates1";
                                else
                            ...
                            try
                            {
                                molecule_original = molecule_original.edit()
                                                        .setProperty(prop_c, molecule_updated.property(prop_c1))
                                                        .molecule()
                                                        .commit();
                            ...
                            if (molecule_updated.hasProperty(prop_v1))
                            {
                                ...
                                {
                                    molecule_original = molecule_original.edit()
                                                            .setProperty(prop_v0, molecule_updated.property(prop_v1))
                                                            .molecule()
                                                            .commit();
                                }
                                catch (...)
                            ...
                            // Update the molecule in the original system.
                            new_system.update(molecule_original);
            ...
            // Make sure that we've mapped all of the molecules.
            if (matched_mols.count() != system_original.nMolecules())
            {
                throw SireError::incompatible_error(QObject::tr("Only matched '%1' molecules, expected '%2'")
                                                        .arg(matched_mols.count())
                                                        .arg(system_original.nMolecules()),
                                                    CODELOC);
            }
        }
        ...
        else
        {
            ...
            for (const auto &molIdx0 : molecule_mapping.keys())
            {
                ...
                // Extract the molecules from each system.
                auto molecule0 = system_original[molIdx0].molecule();
                const auto molecule1 = system_updated[molIdx1].molecule();
