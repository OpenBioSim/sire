...
#include "mmcif.hpp" // for string_to_int
...
namespace gemmi
{
    ...
    namespace
    {
        ...
        std::vector<Helix> read_helices(cif::Block &block)
        {
            ...
            for (const auto row : block.find("_struct_conf.", {"conf_type_id",                              // 0
        ...
        std::vector<Sheet> read_sheets(cif::Block &block)
        {
            ...
            for (const auto row : block.find("_struct_sheet_range.", {"sheet_id", "id",                            // 0-1
            ...
            for (const auto row : block.find("_struct_sheet_order.", {"sheet_id", "range_id_2", "sense"}))
            ...
            for (const auto row : block.find("_pdbx_struct_sheet_hbond.", {"sheet_id", "range_id_2",
        ...
        void read_connectivity(cif::Block &block, Structure &st)
        {
            ...
            for (const auto row : block.find("_struct_conn.", {"id", "conn_type_id",                                   // 0-1
        ...
        std::vector<Assembly> read_assemblies(cif::Block &block)
        {
            ...
            for (const auto row : block.find("_pdbx_struct_assembly.", {"id", "details", "method_details",
        ...
        void read_sifts_unp(cif::Block &block, Structure &st)
        {
            ...
            cif::Table table = block.find("_pdbx_sifts_xref_db.", {"entity_id", "asym_id", "seq_id_ordinal", "seq_id", "observed",
