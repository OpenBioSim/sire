...
namespace SireIO
{
    ...
    */
    ...
    public:
        enum
        {
            PROTEIN = 1, // a ProtoMS protein molecule
            SOLUTE = 2,  // a ProtoMS solute molecule
            SOLVENT = 3
        }; // a ProtoMS solvent molecule
        ...
        QString parameterisationCommandFile(const Molecule &molecule, int type) const;

        Molecule parameterise(const Molecule &molecule, int type, const PropertyMap &map = PropertyMap());

        Molecules parameterise(const Molecules &molecules, int type, const PropertyMap &map = PropertyMap());

    private:
        QString writeShellFile(const SireBase::TempDir &tempdir, const QString &cmdfile) const;
        QString writeCommandFile(const SireBase::TempDir &tempdir, const Molecule &molecule, int type) const;

        void processZMatrixLine(const QStringList &words, const Molecule &mol, int type, ZMatrix &zmatrix,
        ...
        void processZMatrixPertLine(const QStringList &words, const Molecule &mol, int type,
                                    ...
                                    const PropertyMap &pert_map, detail::ProtoMSWorkspace &workspace) const;

        void processAtomLine(const QStringList &words, MolEditor &editmol, int type, const QString &charge_property,
                             const QString &lj_property, detail::ProtoMSWorkspace &workspace) const;

        void processAtomPertLine(const QStringList &words, MolEditor &mol, int type, const QString &initial_charge_property,
                                 const QString &final_charge_property, const QString &initial_lj_property,
                                 const QString &final_lj_property, detail::ProtoMSWorkspace &workspace) const;

        void processBondLine(const QStringList &words, const Molecule &molecule, int type,
                             SireMM::TwoAtomFunctions &bondfuncs, detail::ProtoMSWorkspace &workspace) const;

        SireMol::PerturbationPtr processBondPertLine(const QStringList &words, const Molecule &molecule, int type,
                                                     const PropertyName &bond_property,
                                                     detail::ProtoMSWorkspace &workspace) const;

        void processConnectLine(const QStringList &words, const Molecule &molecule, int type,
                                SireMol::ConnectivityEditor &connectivity, detail::ProtoMSWorkspace &workspace) const;

        void processAngleLine(const QStringList &words, const Molecule &molecule, int type,
                              SireMM::ThreeAtomFunctions &anglefuncs, detail::ProtoMSWorkspace &workspace) const;

        SireMol::PerturbationPtr processAnglePertLine(const QStringList &words, const Molecule &molecule, int type,
                                                      const PropertyName &angle_property,
                                                      detail::ProtoMSWorkspace &workspace) const;

        QString processDihedralLine(QTextStream &ts, const QStringList &words, const Molecule &molecule, int type,
                                    SireMM::FourAtomFunctions &dihedralfuncs, const PropertyName &dihedral_property,
                                    QList<SireMol::PerturbationPtr> &perturbations,
                                    detail::ProtoMSWorkspace &workspace) const;

        void processBondDeltaLine(const QStringList &words, const Molecule &molecule, int type, ZMatrix &zmatrix,
                                  detail::ProtoMSWorkspace &workspace) const;

        void processAngleDeltaLine(const QStringList &words, const Molecule &molecule, int type, ZMatrix &zmatrix,
                                   detail::ProtoMSWorkspace &workspace) const;

        void processDihedralDeltaLine(const QStringList &words, const Molecule &molecule, int type, ZMatrix &zmatrix,
                                      detail::ProtoMSWorkspace &workspace) const;

        void processUBLine(const QStringList &words, const Molecule &molecule, int type, SireMM::TwoAtomFunctions &ubfuncs,
                           detail::ProtoMSWorkspace &workspace) const;

        SireMol::PerturbationPtr processUBPertLine(const QStringList &words, const Molecule &molecule, int type,
                                                   const PropertyName &ub_property,
                                                   detail::ProtoMSWorkspace &workspace) const;

        void processNBLine(const QStringList &words, const Molecule &molecule, int type, SireMM::CLJNBPairs &nbpairs,
                           detail::ProtoMSWorkspace &workspace) const;

        Molecule runProtoMS(const Molecule &molecule, int type, const PropertyMap &map) const;
