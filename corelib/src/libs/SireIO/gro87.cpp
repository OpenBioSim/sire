    ...
    while (true)
    {
        ...
        auto parse_atoms = [&](const QString &line, int iatm, bool *has_vels, QStringList &errors)
        {
            ...
            else
            {
                ...
                if (atmnam != atmnams_data[iatm] or atmnum != atmnums_data[iatm] or resnam != resnams_data[iatm] or
                    resnum != resnums_data[iatm])
                {
                    errors.append(QObject::tr("Disagreement in the ID of atom %1 "
                                      ...
                                      .arg(iatm)
                                      .arg(iframe)
                                      .arg(atmnams_data[iatm])
                                      .arg(atmnums_data[iframe])
                                      .arg(resnams_data[iatm])
        ...
        // v1(x) v2(y) v3(z) v1(y) v1(z) v2(x) v2(z) v3(x) v3(y) (the last six values can be omitted)
        {
            ...
            else
            {
                parse_warnings.append(
                    QObject::tr("Cannot read the periodic box information "
...
int Gro87::findAtom(const MoleculeInfoData &molinfo, int atmidx, int hint, bool *ids_match) const
{
    ...
    auto is_match = [&](int i)
    {
        ...
        else if (same_atmname)
        {
            ...
            partial_matches.insert((10 * same_resname) + (5 * same_atmnum) + (2 * same_resnum) + (1 * (i == hint)), i);
        }
...
void Gro87::finaliseSystem(System &system, const PropertyMap &map) const
{
    ...
    else if ((not box_v1.isEmpty()) and space_property.hasSource())
    {
        ...
        if (box_v1.at(0).manhattanLength() != x or box_v2.at(0).manhattanLength() != y or
            box_v3.at(0).manhattanLength() != z)
        {
            // This is a triclinic space.
            system.setProperty(space_property.source(),
                               SireVol::TriclinicBox(box_v1.at(0), box_v2.at(0), box_v3.at(0)));
        }
        else if (x + y + z > 0)
        {
...
void Gro87::addToSystem(System &system, const PropertyMap &map) const
{
    ...
    if (natoms != this->nAtoms())
        throw SireError::incompatible_error(
            QObject::tr("Incompatibility between the files, as this .gro file contains data "
    ...
    auto add_moldata = [&](int i)
    {
        ...
        {
            throw SireError::incompatible_error(
                QObject::tr("Incompatibility between the files, could not match all atoms to the "
                            "corresponding topology file. Check atom and residue names!"),
                CODELOC);
        }
