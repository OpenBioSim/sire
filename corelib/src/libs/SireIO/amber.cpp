...
public:
    ...
    FortranFormat(int r, const QString &t, int s, int d) : repeat(r), type(t), size(s), decimal(d)
    {
    }
...
enum
{
    UNKNOWN = 0,         // a flag that is not known, e.g. in newer format top files
...
enum
{
    IPTRES = 0, // final residue that is considered part of the solute,
...
static void processFormatLine(const QStringList &words, FortranFormat &format)
{
    ...
    else
    {
        ...
        throw SireError::program_bug(
            QObject::tr("The format line '%1' does not contain a supported format type").arg(tmp), CODELOC);
    }
...
static void processTitleLine(const QString &line, const FortranFormat &format, QString &title)
{
...
static void processIntegerLine(const QString &line, const FortranFormat &format, QList<int> &intarray)
{
    // FORMAT(10i8) reads up to 10 times 8 characters and treat them as an integer
    if (format.type != "I")
        throw SireError::program_bug(QObject::tr("Format '%1' is not supported, should be I").arg(format.type),
                                     CODELOC);
...
static void processStringLine(const QString &line, const FortranFormat &format, QStringList &stringarray)
{
    // FORMAT(20a4)
    if (format.type != "a")
        throw SireError::program_bug(QObject::tr("Format '%1' is not supported, should be a").arg(format.type),
                                     CODELOC);
...
static void processDoubleLine(const QString &line, const FortranFormat &format, QList<double> &doublearray)
{
    // FORMAT(5E16.8)
    if (format.type != "E")
        throw SireError::program_bug(QObject::tr("Format '%1' is not supported, should be E").arg(format.type),
                                     CODELOC);
...
static void setAtomParameters(AtomEditor &editatom, MolEditor &editmol, const QList<double> &crdCoords,
                              const PropertyName &coords_property, const QList<int> &element,
                              const PropertyName &element_property, const QList<double> &charge,
                              const PropertyName &charge_property, const QList<double> &mass,
                              const PropertyName &mass_property, const QList<int> &atom_type_index,
                              const QList<int> &nb_parm_index, const QList<double> &lj_a_coeff,
                              const QList<double> &lj_b_coeff, const PropertyName &lj_property,
                              const QStringList &amber_type, const PropertyName &ambertype_property,
                              const QList<int> &pointers)
{
    ...
    //  set the charges
    SireUnits::Dimension::Charge chg = (charge[atomNumber - 1] / AMBERCHARGECONV) * mod_electron;
...
static int setConnectivity(MolEditor &editmol, int pointer, const QList<int> &bondsArray,
                           ConnectivityEditor &connectivity, const PropertyName &connectivity_property,
                           int last_idx = -1)
{
...
static int setBonds(MolEditor &editmol, int pointer, const QList<int> &bondsArray,
                    const QList<double> &bond_force_constant, const QList<double> &bond_equil_value,
                    TwoAtomFunctions &bondfuncs, const PropertyName &bond_property, AmberParameters &amberparams,
                    const PropertyName &amberparameters_property, int last_idx = -1)
{
    int atomStart = editmol.atoms()(0).number().value();
    int atomEnd = editmol.atoms()(-1).number().value();
...
static int setAngles(MolEditor &editmol, int pointer, const QList<int> &anglesArray,
                     const QList<double> &ang_force_constant, const QList<double> &ang_equil_value,
                     ThreeAtomFunctions &anglefuncs, const PropertyName &angle_property, AmberParameters &amberparams,
                     const PropertyName &amberparameters_property, int last_idx = -1)
{
...
static int setDihedrals(MolEditor &editmol, int pointer, const QList<int> &dihedralsArray,
                        const QList<double> &dih_force_constant, const QList<double> &dih_periodicity,
                        const QList<double> &dih_phase, const QList<double> &sceefactor,
                        const QList<double> &scnbfactor, FourAtomFunctions &dihedralfuncs,
                        const PropertyName &dihedral_property, FourAtomFunctions &improperfuncs,
                        const PropertyName &improper_property, QHash<AtomNum, QList<AtomNum>> &atoms14,
                        QHash<AtomNum, QHash<AtomNum, double>> &atoms14sclee,
                        QHash<AtomNum, QHash<AtomNum, double>> &atoms14sclnb, double coul_14scl, double lj_14scl,
                        AmberParameters &amberparams, const PropertyName &amberparameters_property, int last_idx)
{
    // QSet<AtomNum> moleculeAtomNumbers = _pvt_selectAtomsbyNumber(editmol);
    int atomStart = editmol.atoms()(0).number().value();
    int atomEnd = editmol.atoms()(-1).number().value();
    ...
    for (int i = start_idx; i < pointer; ++i)
    {
        ...
        else
        {
            ...
            // JM Feb 13. Maybe better to create improper/dihedral functions with null terms in case they are changed by
            // a perturbation?
    ...
    for (QHash<DofID, Expression>::const_iterator it = dihedral_hash.constBegin(); it != dihedral_hash.constEnd(); ++it)
    {
        dihedralfuncs.set(it.key().atom0(), it.key().atom1(), it.key().atom2(), it.key().atom3(), it.value());
    }

    for (QHash<DofID, Expression>::const_iterator it = improper_hash.constBegin(); it != improper_hash.constEnd(); ++it)
    {
        improperfuncs.set(it.key().atom0(), it.key().atom1(), it.key().atom2(), it.key().atom3(), it.value());
    }
...
static void setNonBondedPairs(MolEditor &editmol, int pointer, const QList<int> &num_excluded_atoms,
                              const QList<int> &exc_atom_list, CLJNBPairs &nbpairs, const PropertyName &nb_property,
...
//                              double coul_14scl, double lj_14scl)
{
    ...
    for (int i = 0; i < natoms; i++)
    {
        ...
        // skip itself
        nbpairs.set(atom0.cgAtomIdx(), atom0.cgAtomIdx(), CLJScaleFactor(0.0, 0.0));
        ...
        for (int j = 0; j < nexcluded; j++)
        {
            ...
            if (atoms14[atom0.number()].contains(atom1.number()))
            {
                ...
                {
                    throw SireError::program_bug(
                        QObject::tr("It should not happen that atoms14sclee does not have an entry for atom0"),
                        CODELOC);
                }
                ...
                {
                    throw SireError::program_bug(
                        QObject::tr("It should not happen that atoms14sclee does not have an entry for [atom0]]atom1]"),
                        CODELOC);
                }
                ...
                {
                    throw SireError::program_bug(
                        QObject::tr("It should not happen that atoms14sclnb does not have an entry for atom0"),
                        CODELOC);
                }
                ...
                {
                    throw SireError::program_bug(
                        QObject::tr("It should not happen that atoms14sclnb does not have an entry for [atom0]]atom1]"),
                        CODELOC);
                }
            ...
            nbpairs.set(atom0.cgAtomIdx(), atom1.cgAtomIdx(), CLJScaleFactor(cscl, ljscl));
...
static void walk(int atom, const QMultiHash<int, int> &atoms12, int totalMolecules, QHash<int, int> &atIsInMol,
...
static void calcNumberMolecules(int &totalMolecules, QList<int> &atoms_per_mol, const QList<int> &bond_inc_h,
                                const QList<int> &bonds_exc_h, int natoms, int nbondsh, int nbondsa)
{
...
/** Copy constructor */
Amber::Amber(const Amber &other) : coul_14scl(other.coul_14scl), lj_14scl(other.lj_14scl)
{
}
...
tuple<MoleculeGroup, SpacePtr> Amber::readCrdTop(const QString &crdfile, const QString &topfile,
                                                 QString flag_cutting) const
{
    /**
    ...
    */
    ...
    while (not line.isNull())
    {
        ...
        if (line.startsWith("%"))
        {
            ...
            else
            {
                qDebug() << "ERROR" << line;
                throw SireError::program_bug(
                    QObject::tr("Does not know what to do with a '%1' statement").arg(words[0]), CODELOC);
            }
        }
        else
        {
            ...
            default:
            {
                ...
                qDebug() << currentFlag;
                qDebug() << line;
                throw SireError::program_bug(QObject::tr("Serious problem with the value of the variable "
                                                         "currentFlag, '%1'")
                                                 .arg(currentFlag),
                                             CODELOC);
                break;
    ...
    if (flag_cutting == "perresidue")
        ...
        throw SireError::program_bug(
            QObject::tr("The Cutting method has not been correctly specified. Possible choises: perresidue, peratom"),
            CODELOC);
    ...
    if (pointers[NATOM] != crd_atoms)
        throw SireError::incompatible_error(
            QObject::tr("The number of atoms in the crd file (%1) does not equal the number "
    ...
    else
    {
        ...
        calcNumberMolecules(total_molecules, atoms_per_mol, bond_inc_h, bonds_exc_h, pointers[NATOM], pointers[NBONH],
                            pointers[MBONA]);
    }
    ...
    for (int i = 0; i < total_molecules; i++)
    {
        ...
        for (int i = 0; i < natoms; ++i)
        {
            ...
            setAtomParameters(editatom, editmol, crd_coords, coords_property, element, element_property, charge,
                              charge_property, mass, mass_property, atom_type_index, nb_parm_index, lj_a_coeff,
                              lj_b_coeff, lj_property, amber_type, ambertype_property, pointers);
        }
        ...
        if (natoms > 1)
        {
            ...
            idx_inc_h =
                setConnectivity(editmol, pointers[NBONH], bond_inc_h, connectivity, connectivity_property, idx_inc_h);

            idx_exc_h =
                setConnectivity(editmol, pointers[MBONA], bonds_exc_h, connectivity, connectivity_property, idx_exc_h);

            // Next all the forcefield terms
            // qDebug() << " Setting up bonds ";
            idx_bnd_inc_h = setBonds(editmol, pointers[NBONH], bond_inc_h, bond_force_constant, bond_equil_value,
                                     bondfuncs, bond_property, amberparams, amberparameters_property, idx_bnd_inc_h);

            idx_bnd_exc_h = setBonds(editmol, pointers[MBONA], bonds_exc_h, bond_force_constant, bond_equil_value,
                                     bondfuncs, bond_property, amberparams, amberparameters_property, idx_bnd_exc_h);
        }

        if (natoms > 2)
        {
            // qDebug() << " Setting up angles ";
            idx_ang_inc_h = setAngles(editmol, pointers[NTHETH], angs_inc_h, ang_force_constant, ang_equil_value,
                                      anglefuncs, angle_property, amberparams, amberparameters_property, idx_ang_inc_h);

            idx_ang_exc_h = setAngles(editmol, pointers[MTHETA], angs_exc_h, ang_force_constant, ang_equil_value,
                                      anglefuncs, angle_property, amberparams, amberparameters_property, idx_ang_exc_h);
        }

        if (natoms > 3)
        {
            // qDebug() << " Setting up dihedrals ";
            idx_dih_inc_h = setDihedrals(editmol, pointers[NPHIH], dihs_inc_h, dih_force_constant, dih_periodicity,
                                         dih_phase, sceefactor, scnbfactor, dihedralfuncs, dihedral_property,
                                         improperfuncs, improper_property, atoms14, atoms14sclee, atoms14sclnb,
                                         coul_14scl, lj_14scl, amberparams, amberparameters_property, idx_dih_inc_h);

            idx_dih_exc_h = setDihedrals(editmol, pointers[MPHIA], dihs_exc_h, dih_force_constant, dih_periodicity,
                                         dih_phase, sceefactor, scnbfactor, dihedralfuncs, dihedral_property,
                                         improperfuncs, improper_property, atoms14, atoms14sclee, atoms14sclnb,
                                         coul_14scl, lj_14scl, amberparams, amberparameters_property, idx_dih_exc_h);
        }

        setNonBondedPairs(editmol, pointers[NEXT], num_excluded_atoms, exc_atom_list, nbpairs, nb_property, atoms14,
                          atoms14sclee, atoms14sclnb);
                     ...
                     const PropertyMap &map) const
{
    ...
    if (space.isA<PeriodicBox>())
    {
        ...
        fout << boxsize.x() << boxsize.y() << boxsize.z() << 90.0 << 90.0 << 90.0;
    ...
    if (nwritten_atoms > 0)
    {
        qDebug() << "WARNING: Number of written atoms is" << nwritten_atoms << "less than the number in the Molecules ("
                 << total_nats << ")";
    }
