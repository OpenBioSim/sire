...
AmberFormat::AmberFormat() : flag_type(SireIO::AmberPrm::UNKNOWN), num_values(0), field_width(0), point_width(0)
{
}

AmberFormat::AmberFormat(SireIO::AmberPrm::FLAG_TYPE flag, int num, int field, int point)
...
AmberFormat::AmberFormat(const QString &line)
{
    ...
    if (not m.hasMatch())
    {
        throw SireIO::parse_error(QObject::tr("Could not extract the format from the line '%1'. "
...
QString AmberFormat::toString() const
{
    ...
    case AmberPrm::STRING:
        return QString("AmberFormat( %1 x string[width = %2] )").arg(num_values).arg(field_width);
    case AmberPrm::INTEGER:
        return QString("AmberFormat( %1 x integer[width = %2] )").arg(num_values).arg(field_width);
    case AmberPrm::FLOAT:
...
namespace SireIO
{
    namespace detail
    {
        ...
        QStringList writeIntData(const QVector<qint64> &data, AmberFormat format, QStringList *errors, bool include_header)
        {
            QStringList lines;
            ...
            for (int i = 0; i < data.count(); ++i)
            {
                ...
                if (sval.length() > format.width())
                {
                    ...
                    if (errors)
                        errors->append(QObject::tr("Could not write the integer at index %1, value '%2' into "
                                   ...
                                   char float_format)
        {
            ...
            for (int i = 0; i < data.count(); ++i)
            {
                ...
                if (sval.length() > format.width())
                {
                    // we couldn't fit this number into the specified width!
                    if (errors)
                        errors->append(QObject::tr("Could not write the float at index %1, value '%2' into "
                                                   "the specified format %3.")
                                           .arg(i)
                                           .arg(value)
                                           .arg(format.toString()));
        ...
        /** Internal function used to write out an array of strings using the passed AmberFormat.
            ...
            is true, and will append any errors encountered during writing to 'errors'
            if this is non-zero. This returns the text lines as a QStringList */
        QStringList writeStringData(const QVector<QString> &data, AmberFormat format, QStringList *errors, bool include_header)
        {
            QStringList lines;
            ...
            for (int i = 0; i < data.count(); ++i)
            {
                ...
                {
                    if (errors)
                        errors->append(QObject::tr("Could not fully write the string data at index %1, value '%2', "
