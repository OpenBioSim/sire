...
Cube::Cube(SireUnits::Dimension::MolarEnergy c) : cutoff(convertTo(c.value(), kcal_per_mol))
{
}
...
{
    if (table.nGrids() != 1)
        throw SireError::incompatible_error(QObject::tr("Sire can only write Cube data files for potential tables "
                                                        "that contain a single Grid."),
                                            CODELOC);

    const Grid &grid = table.gridData()[0].grid();

    if (not grid.isA<RegularGrid>())
        throw SireError::incompatible_error(QObject::tr("Sire can only write Cube data files when using a RegularGrid. "
...
static void writeHeader(QTextStream &ts, int nats, const Grid &grid)
{
    ...
    ts << convertTo(grid.minCoords().x(), bohr_radii) << convertTo(grid.minCoords().y(), bohr_radii)
       << convertTo(grid.minCoords().z(), bohr_radii) << "\n";

    Vector basis = convertTo(rgrid.gridSpacing().value(), bohr_radii) * rgrid.basis().column0();
    ts.setFieldWidth(4);
    ts << rgrid.dimX();
    ts.setFieldWidth(12);
...
void Cube::write(const PotentialTable &table, const QString &filename, const PropertyMap &map) const
{
...
template <class T>
static void writeTable(QTextStream &ts, const MolPotentialTable &moltable, const T &molgroup, const PropertyMap &map,
                       double cutoff)
{
    ...
    else
    {
        ...
        const AtomCoords &coords = mol.property(map["coordinates"]).asA<AtomCoords>();

        const AtomElements &elements = mol.property(map["element"]).asA<AtomElements>();
        ...
        if (moltable.selectedAll())
        {
            for (CGIdx i(0); i < moltable.nCutGroups(); ++i)
            {
                ...
                for (int j = 0; j < coords.nAtoms(i); ++j)
                {
                    ...
                    ts << SireUnits::convertTo(nrg[j], kcal_per_mol) << SireUnits::convertTo(c[j].x(), bohr_radii)
                       << SireUnits::convertTo(c[j].y(), bohr_radii) << SireUnits::convertTo(c[j].z(), bohr_radii)
                       << "\n";
                }
            }
        }
...
static void writeGrid(QTextStream &ts, const GridPotentialTable &gridtable, double cutoff)
{
...
void Cube::write(const PotentialTable &table, const MoleculeGroup &molgroup, const QString &filename,
...
void Cube::write(const PotentialTable &table, const MolGroupsBase &molgroups, const QString &filename,
                 const PropertyMap &map) const
{
    assertValidGrid(table);
