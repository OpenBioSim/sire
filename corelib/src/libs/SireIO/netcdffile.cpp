...
static nc_type string_to_nc_type(const QString &typ)
{
    if (typ == "NC_BYTE")
        return NC_BYTE;
    else if (typ == "NC_UBYTE")
        return NC_UBYTE;
    else if (typ == "NC_CHAR")
        return NC_CHAR;
    else if (typ == "NC_SHORT")
        return NC_SHORT;
    else if (typ == "NC_USHORT")
        return NC_USHORT;
    else if (typ == "NC_INT")
        return NC_INT;
    else if (typ == "NC_UINT")
        return NC_UINT;
    else if (typ == "NC_INT64")
        return NC_INT64;
    else if (typ == "NC_UINT64")
        return NC_UINT64;
    else if (typ == "NC_FLOAT")
        return NC_FLOAT;
    else if (typ == "NC_DOUBLE")
        return NC_DOUBLE;
    else if (typ == "NC_STRING")
        return NC_STRING;
    else
    {
        throw SireError::io_error(QObject::tr("Unrecognised NetCDF type - %1").arg(typ), CODELOC);
...
static nc_type qvariant_to_nc_type(const QVariant &typ)
{
    ...
    {
        throw SireError::io_error(QObject::tr("Unable to convert the QVariant type %1 to an NC_TYPE").arg(name),
                                  CODELOC);
    }
...
{
    throw SireError::io_error(QObject::tr("Unable to convert the QVariant type %1 to an NC_TYPE").arg(typ.typeName()),
                              CODELOC);
    return 0;
}
#endif
...
// ensure there is enough space in the data
    ...
    if (nvals > (memdata.count() / nc_type_to_size(nc_type)))
    {
        throw SireError::invalid_arg(
            QObject::tr("You cannot read %1 values, as the array has only allocated space (%2) "
                        "for %3 values of type %4.")
                .arg(nvals)
                .arg(memdata.count())
                .arg(memdata.count() / nc_type_to_size(nc_type))
                .arg(nc_type_to_string(nc_type)),
            CODELOC);
    }
#endif
                               ...
                               QList<int> att_ts, QList<QVariant> att_vs)
{
    ...
    {
        throw SireError::invalid_arg(QObject::tr("You cannot construct a NetCDFDataInfo with a negative ID number (%1) "
                                                 "or negative xtype (%2).")
                                         .arg(idnum)
                                         .arg(xtyp),
                                     CODELOC);
...
NetCDFDataInfo::NetCDFDataInfo(int idnum, QString name, const QString &xtyp, QStringList dim_names,
                               QList<int> dim_sizes, QStringList att_names, QList<int> att_types,
                               QList<QVariant> att_values)
    : NetCDFDataInfo(idnum, name, string_to_nc_type(xtyp), dim_names, dim_sizes, att_names, att_types, att_values)
{
}
...
QString NetCDFDataInfo::toString() const
{
    ...
    else if (dim_names.isEmpty())
    {
        ...
        {
            return QObject::tr("NetCDFDataInfo( %1 = %2[%3]() )").arg(idnum).arg(nme).arg(this->type());
        }
        else
        {
            QStringList atts;
...
void NetCDFDataInfo::assertNValuesEquals(int n) const
{
    if (n != this->nValues())
    {
        throw SireError::incompatible_error(
            QObject::tr("Cannot store %1 items in this piece of NetCDF data, as it is dimensioned "
                        "to store %2 values. %3")
                .arg(n)
                .arg(this->nValues())
                .arg(this->toString()),
...
/** Copy constructor */
NetCDFData::NetCDFData(const NetCDFData &other) : NetCDFDataInfo(other), memdata(other.memdata)
{
}
...
static void assert_no_netcdf_error(int errnum)
{
    ...
    throw SireError::io_error(QObject::tr("NetCDF experienced an error: %1 (%2)").arg(err).arg(errnum), CODELOC);
#else
    throw SireError::io_error(QObject::tr("NetCDF experienced an error as it is not compiled and supported with "
                                          "this version of sire."),
                              CODELOC);
#endif
}
    ...
    if (hndl != -1)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                vars.insert(var_name, NetCDFDataInfo(i, var_name, var_type, dim_names, dim_sizes, att_names, att_types,
                                                     att_values));
    ...
    if (hndl != -1)
    {
        ...
        // dimensions, variables and attributes
        {
            ...
            for (const auto &dim : dims)
            {
                ...
                {
                    throw SireError::io_error(QObject::tr("The length of the name of the dimension '%1' (%2) cannot "
                ...
                call_netcdf_function([&]()
                                     { return nc_def_dim(hndl, c_dim.constData(), dimensions[dim], &idp); });
        ...
        for (const auto &variable : variables)
        {
            ...
            {
                throw SireError::io_error(QObject::tr("The length of the name of the variable '%1' (%2) cannot "
                                                      "be greater than NC_MAX_NAME, which is %2.")
                                              .arg(variable)
                                              .arg(c_var.length())
                                              .arg(NC_MAX_NAME),
            ...
            call_netcdf_function(
                [&]()
                { return nc_def_var(hndl, c_var.constData(), xtyp, ndims, dim_ids.constData(), &idp); });
        ...
        for (const auto &global_attribute : global_attributes)
        {
            ...
            call_netcdf_function([&]()
                                 { return nc_put_att_text(hndl, NC_GLOBAL, c_name.constData(), c_att.count(), c_att.constData()); });
        }
        ...
        for (const auto &variable : variables)
        {
            ...
            if (id < 0)
            {
                throw SireError::program_bug(
                    QObject::tr("How can the variable '%1' have a negative ID? %2").arg(variable).arg(id), CODELOC);
            }
            ...
            for (const auto &attribute : vardata.attributeNames())
            {
                ...
                case NC_CHAR:
                {
                    ...
                    call_netcdf_function(
                        [&]()
                        { return nc_put_att(hndl, id, c_attname.constData(), xtyp, len, c_att.constData()); });
                    break;
                }
                case NC_DOUBLE:
                {
                    ...
                    call_netcdf_function(
                        [&]()
                        { return nc_put_att(hndl, id, c_attname.constData(), xtyp, len, &val); });
                    break;
                }
                case NC_FLOAT:
                {
                ...
                default:
                    throw SireError::unsupported(QObject::tr("Not yet able to write a NetCDF attribute of type '%1' "
                                                             "for variable %2. (%3)")
                                                     .arg(att_type)
                                                     .arg(variable)
                                                     .arg(xtyp),
...
QHash<QString, int> NetCDFFile::_lkr_getDimensions() const
{
    ...
    if (hndl != -1)
    {
        int ndims;
        call_netcdf_function([&]()
                             { return nc_inq_ndims(hndl, &ndims); });
        ...
        try
        {
            for (int i = 0; i < ndims; ++i)
            {
                size_t dim_len;
                call_netcdf_function([&]()
                                     { return nc_inq_dim(hndl, i, dim_name, &dim_len); });
...
QString NetCDFFile::_lkr_getStringAttribute(const QString &name) const
{
    ...
    throw SireError::invalid_key(
        QObject::tr("There is not string attribute called '%1' in the NetCDF file '%2'").arg(name).arg(fname), CODELOC);
