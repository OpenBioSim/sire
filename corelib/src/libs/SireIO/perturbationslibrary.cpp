...
QDataStream &operator<<(QDataStream &ds, const PerturbationsTemplate &pertstemplate)
{
    writeHeader(ds, r_pertstemplate, 1);

    SharedDataStream sds(ds);

    sds << pertstemplate.name << pertstemplate.initcharges << pertstemplate.finalcharges << pertstemplate.initLJs
        << pertstemplate.finalLJs << pertstemplate.initatypes << pertstemplate.finalatypes << pertstemplate.initbondsk
        << pertstemplate.initbondsr << pertstemplate.finalbondsk << pertstemplate.finalbondsr
        << pertstemplate.initanglesk << pertstemplate.initanglest << pertstemplate.finalanglesk
        << pertstemplate.finalanglest
        //<< pertstemplate.initdihpotential << pertstemplate.finaldihpotential
        << pertstemplate.initdihparams
        << pertstemplate.finaldihparams
...
QDataStream &operator>>(QDataStream &ds, PerturbationsTemplate &pertstemplate)
{
    ...
    if (v == 1)
    {
        ...
        sds >> pertstemplate.name >> pertstemplate.initcharges >> pertstemplate.finalcharges >> pertstemplate.initLJs >>
            pertstemplate.finalLJs >> pertstemplate.initatypes >> pertstemplate.finalatypes >>
            pertstemplate.initbondsk >> pertstemplate.initbondsr >> pertstemplate.finalbondsk >>
            pertstemplate.finalbondsr >> pertstemplate.initanglesk >> pertstemplate.initanglest >>
...
PerturbationsTemplate::PerturbationsTemplate(const QString &name) : name(name)
{
}

/** Copy constructor */
PerturbationsTemplate::PerturbationsTemplate(const PerturbationsTemplate &other)
    : name(other.name), initcharges(other.initcharges), finalcharges(other.finalcharges), initLJs(other.initLJs),
      finalLJs(other.finalLJs), initatypes(other.initatypes), finalatypes(other.finalatypes),
      initbondsk(other.initbondsk), initbondsr(other.initbondsr), finalbondsk(other.finalbondsk),
      finalbondsr(other.finalbondsr), initanglesk(other.initanglesk), initanglest(other.initanglest),
      finalanglesk(other.finalanglesk), finalanglest(other.finalanglest),
      // initdihpotential(other.initdihpotential),
      // finaldihpotential(other.finaldihpotential),
      initdihparams(other.initdihparams), finaldihparams(other.finaldihparams),
      // initimppotential(other.initimppotential),
      // finalimppotential(other.finalimppotential)
      initimpparams(other.initimpparams), finalimpparams(other.finalimpparams)
{
}
...
bool PerturbationsTemplate::operator==(const PerturbationsTemplate &other) const
{
    return this == &other or
           (name == other.name and initcharges == other.initcharges and finalcharges == other.finalcharges and
            initLJs == other.initLJs and finalLJs == other.finalLJs and initatypes == other.initatypes and
            finalatypes == other.finalatypes and initbondsk == other.initbondsk and initbondsr == other.initbondsr and
...
static int processVersionLine(QString &line)
{
    ...
    if (not ok)
        throw SireError::program_bug(QObject::tr("Unexpected error while trying to read the version line "
                                                 "of the perturbations template file"),
                                     CODELOC);
...
QDataStream &operator<<(QDataStream &ds, const PerturbationsLibrary &pertslibrary)
{
...
QDataStream &operator>>(QDataStream &ds, PerturbationsLibrary &pertslibrary)
{
...
/** Copy constructor */
PerturbationsLibrary::PerturbationsLibrary(const PerturbationsLibrary &other) : templates(other.templates)
{
}
...
PerturbationsLibrary &PerturbationsLibrary::operator+=(const PerturbationsLibrary &other)
{
    ...
    {
        for (QHash<QString, PerturbationsTemplate>::const_iterator it = other.templates.constBegin();
             it != other.templates.constEnd(); ++it)
        {
...
const PerturbationsTemplate &PerturbationsLibrary::getTemplate(const QString &key)
{
    ...
    if (it == templates.constEnd())
        throw SireError::invalid_key(QObject::tr("Cannot find the template with key \"%1\". Available templates "
                                                 "are %2.")
                                         .arg(key, Sire::toString(templates.keys())),
                                     CODELOC);
...
void PerturbationsLibrary::setTemplate(const QString &key, const PerturbationsTemplate &tmplate)
{
...
Molecule PerturbationsLibrary::applyTemplate(const Molecule &molecule) const
{
    ...
    if (not templates.contains(molname))
        throw SireError::invalid_key(QObject::tr("There is no perturbations template for the molecule with name %1"
                                         ...
                                         .arg(molname, Sire::toString(templates.keys())),
                                     CODELOC);
    ...
    for (int i = 0; i < bonds.size(); i++)
    {
        ...
        TwoAtomPerturbation bondpert =
            TwoAtomPerturbation(atom0, atom1, base, initial_forms, final_forms, PropertyMap("parameters", "bond"));
    ...
    for (int i = 0; i < angles.size(); i++)
    {
        ...
        ThreeAtomPerturbation anglepert = ThreeAtomPerturbation(atom0, atom1, atom2, base, initial_forms, final_forms,
    ...
    for (int i = 0; i < dihedrals.size(); i++)
    {
        ...
        FourAtomPerturbation dihedralpert = FourAtomPerturbation(atom0, atom1, atom2, atom3, dihedralfunc_i,
                                                                 dihedralfunc_f, PropertyMap("parameters", "dihedral"));
    ...
    for (int i = 0; i < impropers.size(); i++)
    {
        ...
        FourAtomPerturbation improperpert = FourAtomPerturbation(atom0, atom1, atom2, atom3, improperfunc_i,
                                                                 improperfunc_f, PropertyMap("parameters", "improper"));
