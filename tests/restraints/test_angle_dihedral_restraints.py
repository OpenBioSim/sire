...
def test_angle_restraint_custom_ktheta(ala_mols):
    ...
    mols = ala_mols.clone()
    restraints = sr.restraints.angle(mols=mols, atoms=[0, 1, 2], ktheta="10 kcal mol-1 rad-2")
    assert restraints.num_restraints() == 1
    assert restraints[0].atoms() == [0, 1, 2]
...
def test_angle_restraint_custom_ktheta_and_theta0(ala_mols):
    ...
    mols = ala_mols.clone()
    restraints = sr.restraints.angle(mols=mols, atoms=[0, 1, 2], ktheta="10 kcal mol-1 rad-2", theta0="2 rad")
    assert restraints.num_restraints() == 1
    assert restraints[0].atoms() == [0, 1, 2]
    assert restraints[0].ktheta().value() == 10.0
...
def test_dihedral_restraint_custom_kphi(ala_mols):
    ...
    mols = ala_mols.clone()
    restraints = sr.restraints.dihedral(mols=mols, atoms=[0, 1, 2, 3], kphi="10 kcal mol-1 rad-2")
    assert restraints.num_restraints() == 1
    assert restraints[0].atoms() == [0, 1, 2, 3]
...
def test_dihedral_restraint_custom_kphi_and_phi0(ala_mols):
    ...
    mols = ala_mols.clone()
    restraints = sr.restraints.dihedral(mols=mols, atoms=[0, 1, 2, 3], kphi="10 kcal mol-1 rad-2", phi0="2 rad")
    assert restraints.num_restraints() == 1
    assert restraints[0].atoms() == [0, 1, 2, 3]
    assert restraints[0].kphi().value() == 10.0
